...
public class DirichletTest {
  ...
  public void testLogPdf() {
    ...
    assertEquals(Math.log(18.0 / 25.0),
        new Dirichlet(toDenseVec(2, 3, 1)).logPdf(toDenseVec(3.0 / 10.0, 2.0 / 10.0, 5.0 / 10.0)), 1e-13);
    assertEquals(Math.log(42.0 / 25.0),
        new Dirichlet(toDenseVec(2, 3, 1)).logPdf(toDenseVec(7.0 / 10.0, 2.0 / 10.0, 1.0 / 10.0)), 1e-13);
    assertEquals(Math.log(96.0 / 25.0),
        new Dirichlet(toDenseVec(2, 3, 1)).logPdf(toDenseVec(4.0 / 10.0, 4.0 / 10.0, 2.0 / 10.0)), 1e-13);

    // If it dosnt sum to 1, its not a possible value
    assertEquals(-Double.MAX_VALUE,
        new Dirichlet(toDenseVec(2, 3, 1)).logPdf(toDenseVec(5.0 / 10.0, 4.0 / 10.0, 2.0 / 10.0)), 1e-13);
    assertEquals(-Double.MAX_VALUE,
        new Dirichlet(toDenseVec(2, 3, 1)).logPdf(toDenseVec(1.0 / 10.0, 4.0 / 10.0, 2.0 / 10.0)), 1e-13);
    assertEquals(-Double.MAX_VALUE,
        new Dirichlet(toDenseVec(2, 3, 1)).logPdf(toDenseVec(-4.0 / 10.0, 4.0 / 10.0, 2.0 / 10.0)), 1e-13);
    assertEquals(-Double.MAX_VALUE,
        new Dirichlet(toDenseVec(2, 3, 1)).logPdf(toDenseVec(-4.0 / 10.0, 4.0 / 10.0, 10.0 / 10.0)), 1e-13);
  }
  ...
  public void testPdf() {
    ...
    assertEquals(18.0 / 25.0, new Dirichlet(toDenseVec(2, 3, 1)).pdf(toDenseVec(3.0 / 10.0, 2.0 / 10.0, 5.0 / 10.0)),
        1e-13);
    assertEquals(42.0 / 25.0, new Dirichlet(toDenseVec(2, 3, 1)).pdf(toDenseVec(7.0 / 10.0, 2.0 / 10.0, 1.0 / 10.0)),
        1e-13);
    assertEquals(96.0 / 25.0, new Dirichlet(toDenseVec(2, 3, 1)).pdf(toDenseVec(4.0 / 10.0, 4.0 / 10.0, 2.0 / 10.0)),
        1e-13);
