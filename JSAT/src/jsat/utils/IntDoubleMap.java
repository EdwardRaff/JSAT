...
/**
 * A hash map for storing the primitive types of integers (as keys) to doubles (as table). The implementation is based
 * on Algorithm D (Open addressing with double hashing) from Knuth's TAOCP page 528.
 *
 * @author Edward Raff
 */
public final class IntDoubleMap extends AbstractMap<Integer, Double> {
   ...
   *
   * @param key
   * @return the old value stored for this key, or {@link Double#NaN} if the key was not present in the map
   */
  ...
  /**
   * Gets the index of the given key. Based on that {@link #status} variable, the index is either the location to insert
   * OR the location of the key.
   *
   * This method returns 2 integer table in the long. The lower 32 bits are the index that either contains the key, or
   * is the first empty index.
   *
   * The upper 32 bits is the index of the first position marked as {@link #DELETED} either {@link Integer#MIN_VALUE} if
   * no position was marked as DELETED while searching.
   *
   * @param key they key to search for
   * @return the mixed long containing the index of the first DELETED position and the position that the key is in or
   * the first EMPTY position found
   */
