  ...
  static {
    ...
    try {
      if (isWindows()) {
        String output = null;
        try {
          // On windows, the comand line tool WMIC is used, see
          ...
          /*
           * Will print out the total L2 Cache for each CPU, and the number of
           * cores - something like this (2 CPUs) L2CacheSize NumberOfCores 1024
           * 4 1024 4
           */
        ...
        output = output.replaceAll("L2CacheSize\\s+NumberOfCores", "").trim();// Remove
                                                                              // header
        if (output.indexOf("\n") > 0) {// Multi line is bad!
          output = output.substring(0, output.indexOf("\n")).trim();// Get first
                                                                    // line
        }
        final String[] vals = output.split("\\s+");// Seperate into 2 seperate
                                                   // numbers, first is total L2
                                                   // cahce, 2nd is # CPU cores
        sizeToUse = Integer.valueOf(vals[0]) / Integer.valueOf(vals[1]) * 1024; // the
                                                                                // value
                                                                                // is
                                                                                // in
                                                                                // KB,
        ...
        try {
          ...
          while ((line = br.readLine()) != null) {
            if (line.contains("l1icachesize") && output == null) {// We just
                                                                  // need one
                                                                  // line that
                                                                  // says "cache
                                                                  // size"
      ...
      // make sure we at least set the default by avoiding any possible weird
      // exception
    }
    // TODO is there a good way to approximate this?
    if (sizeToUse == 0) {// we couldn't set it for some reason? 256KB seems to
                         // be a good default (modern P4 to i7s use this size,
                         // Anthalon 64 used this as the min size too)
      sizeToUse = 256 * 1024;
    } else if (sizeToUse < 128 * 1024) {// A weird value? 128KB would be very
                                        // small for an L2 - the P2 had more
                                        // than that!
      sizeToUse = 128 * 1024;
    }
