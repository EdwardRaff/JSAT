...
public class ClosedHashingUtil {
  ...
  /**
   * This value indicates that that status of an open addressing space is EMPTY,
   * meaning any value can be stored in it
   */
  ...
  /**
   * This value indicates that the status of an open addressing space is
   * DELETED, meaning i should not stop a search for a value - but the code is
   * free to overwrite the values at this location and change the status to
   * {@link #OCCUPIED}. <br>
   * This can not be set to {@link #EMPTY} unless we can guarantee that no value
   * is stored in a index chain that stops at this location.
   */
  public static final byte DELETED = OCCUPIED + 1;

  /**
   * This store the value {@link Integer#MIN_VALUE} in the upper 32 bits of a
   * long, so that the lower 32 bits can store any regular integer. This allows
   * a fast way to return 2 integers from one method in the form of a long. The
   * two values can then be recovered from the upper and lower 32 bits of the
   * long. This is meant to be used as a nonsense default value, indicating that
   * no information is present in the upper 32 bits.
   */
