...
public class StringUtils {
  ...
  // Float/Double to String follows algo laid out here
  ...
  private enum States {
    ...
    LEADING_ZEROS_MANTISSA, /**
                             * reading leading zeros in the fractional part of
                             * mantissa.
                             */
    LEADING_ZEROS_FRAC, /**
  ...
  public static double parseDouble(final CharSequence s, final int start, final int end) {
    ...
    long mantissa = 0;
    /**
     * Mantissa can only be incremented 18 times, then any more will overflow
     * (2^63-1 ~= 9.2* 10^18
     */
    ...
    while (pos < end)// run the state machine
    {
      ...
      case LEADING_ZEROS_MANTISSA:
        if (c == '0') {
          ...
          state = States.MANTISSA_FRAC_PART;// this is where that case is
                                            // handeled
        } else {
          throw new NumberFormatException();
        }
