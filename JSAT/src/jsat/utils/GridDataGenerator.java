...
/**
 * This is a utility to generate data in a grid fashion. Each data point will
 * belong to a different class. By default, the data is generated such that the
 * classes are trivially separable. <br>
 * <br>
 * All data points belong to a specific axis, and have noises added to them. For
 * example, a 1 dimensional grid with 2 classes would have data points of the
 * form 0+noise, and 1+noise. So long as the noise is less than 0.5, the data
...
public class GridDataGenerator {
  ...
  /**
   * Creates a new grid data generator for a 2 x 5 with uniform noise in the
   * range [-1/4, 1/4]
   */
   ...
   *
   * @param noiseSource
   *          the distribution that describes the noise that will be added to
   *          each data point. If no noise is used, each data point would lie
   *          exactly on its local axis.
   * @param dimensions
   *          an array describing how many groups there will be. The length of
   *          the array dictates the number of dimensions in the data set, each
   *          value describes how many axis of that dimensions to use. The total
   *          number of classes is the product of these values.
   *
   * @throws ArithmeticException
   *           if one of the dimension values is not a positive value, or a zero
   *           number of dimensions is given
   */
   ...
   *          exactly on its local axis.
   * @param rand
   *          the source of randomness that the noise will use
   * @param dimensions
   *          an array describing how many groups there will be. The length of
   *          the array dictates the number of dimensions in the data set, each
   *          value describes how many axis of that dimensions to use. The total
   ...
   *
   * @param curClass
   *          used as a pointer to an integer so that we dont have to add class
   *          tracking logic
   * @param curDim
   *          the current dimension to split on. If we are at the last
   *          dimension, we add data points instead.
   * @param samples
   *          the number of samples to take for each class
   * @param dataPoints
   *          the location to put the data points in
   * @param dim
   *          the array specifying the current point we are working from.
   */
  private void addSamples(final int[] curClass, final int curDim, final int samples, final List<DataPoint> dataPoints,
      final int[] dim) {
   ...
   *
   * @param samples
   *          the number of sample data points to create for each class in the
   *          data set.
