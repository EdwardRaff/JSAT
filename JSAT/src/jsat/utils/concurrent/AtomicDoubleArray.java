...
/**
 * Provides a double array that can have individual values updated atomically. It is backed by and mimics the
 * implementation of {@link AtomicLongArray}. As such, the methods have the same documentation
 *
 * @author Edward Raff
 */
public class AtomicDoubleArray implements Serializable {
  ...
  /**
   * Creates a new AtomixDouble Array that is of the same length as the input array. The values will be copied into the
   * new object
   *
  ...
  /**
   * Atomically sets the element at position {@code i} to the given updated value if the current value {@code ==} the
   * expected value.
   *
   * @param i the index
   ...
   * and does not provide ordering guarantees, so is only rarely an appropriate alternative to {@code compareAndSet}.
   *
   * @param i the index
   * @param expected the expected value
   * @param update the new value
