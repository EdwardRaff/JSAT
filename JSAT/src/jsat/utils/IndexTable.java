 ...
 * elements, the index table creates an array of index values, and sorts the
 * indices based on the values they point to. The IndexTable can then be used to
 * find the index of the i'th sorted element in the array. <br>
 * <br>
...
public class IndexTable implements Serializable {
  ...
  /**
   * We use an array of Integer objects instead of integers because we need the
   * arrays.sort function that accepts comparators.
   */
  ...
  /**
   * Creates a new index table based on the given array. The array will not be
   * altered.
   *
   * @param array
   *          the array to create an index table for.
   */
  ...
  /**
   * Creates a new index table based on the given list. The list will not be
   * altered.
   *
   * @param list
   *          the list to create an index table for
   */
  ...
  /**
   * Creates a new index table based on the given list and comparator. The list
   * will not be altered.
   *
   * @param list
   *          the list of points to obtain a sorted IndexTable for
   * @param comparator
   *          the comparator to determined the sorted order
   */
   ...
   *           if the length of the target array is not the same as the index
   *           table
   */
   ...
   *
   * @throws RuntimeException
   *           if the length of the target List is not the same as the index
   *           table
   */
   ...
   * ordering. It will be modified, and may be expanded using the
   * {@link List#add(java.lang.Object) add} method if it does not contain
   * sufficient space. Extra size in the tmp list will be ignored. After this
   * method is called, {@code tmp} will contain the same ordering that was in
   * {@code target} <br>
   * <br>
   ...
   *
   * @param target
   *          the list to sort, that should be the same size as the previously
   *          sorted list.
   * @param tmp
   *          the temp list that may be of any size
   */
  ...
  /**
   * Given the index <tt>i</tt> into what would be the sorted array, the index
   * in the unsorted original array is returned. <br>
   * If the original array was a double array, <i>double[] vals</i>, then the
   * sorted order can be printed with <br>
   * 
   * <pre>
   * <code>
   ...
   *
   * @param i
   *          the index of the i'th sorted value
   * @return the index in the original list that would be in the i'th position
   */
  public int index(final int i) {
    if (i >= prevSize || i < 0) {
      throw new IndexOutOfBoundsException(
          "The size of the previously sorted array/list is " + prevSize + " so index " + i + " is not valid");
    }
    return index.get(i);
  }
   ...
   * meaning the original input would be returned in its original order instead
   * of sorted order.
   */
  public void reset() {
   ...
   *
   * @param array
   *          the input to get sorted order of
   */
  ...
  /**
   * Adjusts this index table to contain the reverse sorted index order for the
   * given array
   *
   * @param array
   *          the input to get sorted order of
