...
/**
 * The index table provides a way of accessing the sorted view of an array or list, without ever sorting the elements of
 * said list. Given an array of elements, the index table creates an array of index values, and sorts the indices based
 * on the values they point to. The IndexTable can then be used to find the index of the i'th sorted element in the
 * array. <br>
 * <br>
 * The IndexTable can be sorted multiple times by calling the {@link #sort(java.util.List, java.util.Comparator) }
 * methods. This can be called on inputs of varying size, and the internal order will be expanded when necessary.
 *
 * @author Edward Raff
 */
public class IndexTable implements Serializable {
  ...
  /**
   * We use an array of Integer objects instead of integers because we need the arrays.sort function that accepts
   * comparators.
   */
  ...
  /**
   * Creates a new index table based on the given list and comparator. The list will not be altered.
   *
  ...
  /**
   * Resets the index table so that the returned indices are in linear order, meaning the original input would be
   * returned in its original order instead of sorted order.
   */
  public void reset() {
  ...
  /**
   * Given the index <tt>i</tt> into what would be the sorted array, the index in the unsorted original array is
   * returned. <br>
  ...
  /**
   * Applies this index table to the specified target, putting {@code target} into the same ordering as this IndexTable.
   *
  ...
  /**
   * Applies this index table to the specified target, putting {@code target} into the same ordering as this IndexTable.
   * It will use the provided {@code tmp} space to store the original values in target in the same ordering. It will be
   * modified, and may be expanded using the {@link
   * List#add(java.lang.Object) add} method if it does not contain sufficient space. Extra size in the tmp list will be
   * ignored. After this method is called, {@code tmp} will contain the same ordering that was in {@code target} <br>
   * <br>
   * This method is provided as a means to reducing memory use when multiple lists need to be sorted.
   *
   * @param target the list to sort, that should be the same size as the previously sorted list.
