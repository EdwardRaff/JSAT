...
public class ListUtils {
  ...
  /**
   * This method takes a list and breaks it into <tt>count</tt> lists backed by the original list, with elements being
   * equally spaced among the lists. The lists will be returned in order of the consecutive values they represent in the
   * source list.
   * <br><br><b>NOTE</b>: Because the implementation uses {@link List#subList(int, int) }, changes to the returned lists
   * will be reflected in the source list.
   *
  ...
  /**
   * Collects all future values in a collection into a list, and returns said list. This method will block until all
   * future objects are collected.
   *
  ...
  /**
   * Adds values into the given collection using integer in the specified range and step size. If the <tt>start</tt>
   * value is greater or equal to the <tt>to</tt> value, nothing will be added to the collection.
   *
  ...
  /**
   * Obtains a random sample without replacement from a source list and places it in the destination list. This is done
   ...
   * @param dest the list to store the random samples in. The list does not need to be empty for the sampling to work
   * correctly
   * @param samples the number of samples to select from the source
   * @param rand the source of randomness for the sampling
   * @throws IllegalArgumentException if the sample size is not positive or l arger than the source population.
   */
   ...
   * This is done without modifying the source collection. <br>
   * This random sampling is oblivious to failures to add to a collection that may occur, such as if the collection is a
   * {@link Set}
   *
   * @param <T> the list content type involved
   * @param source the source of values to randomly sample from
   * @param dest the collection to store the random samples in. It does not need to be empty for the sampling to work
   * correctly
   * @param samples the number of samples to select from the source
   * @param rand the source of randomness for the sampling
   * @throws IllegalArgumentException if the sample size is not positive or l arger than the source population.
