...
public class ListUtils {
  ...
  /**
   * Adds values into the given collection using integer in the specified range
   * and step size. If the <tt>start</tt> value is greater or equal to the
   * <tt>to</tt> value, nothing will be added to the collection.
   *
   * @param c
   *          the collection to add to
   * @param start
   *          the first value to add, inclusive
   * @param to
   *          the last value to add, exclusive
   * @param step
   *          the step size.
   * @throws RuntimeException
   *           if the step size is zero or negative.
   */
   ...
   *
   * @param <T>
   *          the type of future object
   * @param futures
   *          the collection of future objects
   * @return a list containing the object from the future.
   * @throws ExecutionException
   * @throws InterruptedException
   */
  public static <T> List<T> collectFutures(final Collection<Future<T>> futures)
   ...
   *
   * @param <T>
   *          the type the lists hold
   * @param left
   *          the left portion of the merged view
   * @param right
   *          the right portion of the merged view
   ...
   * places it in the destination collection. This is done without modifying the
   ...
   *
   * @param <T>
   *          the list content type involved
   * @param source
   *          the source of values to randomly sample from
   * @param dest
   *          the collection to store the random samples in. It does not need to
   *          be empty for the sampling to work correctly
   * @param samples
   *          the number of samples to select from the source
   * @param rand
   *          the source of randomness for the sampling
   * @throws IllegalArgumentException
   *           if the sample size is not positive or l arger than the source
   *           population.
   */
  public static <T> void randomSample(final Collection<T> source, final Collection<T> dest, int samples,
      final Random rand) {
   ...
   *          the source of values to randomly sample from
   * @param dest
   *          the list to store the random samples in. The list does not need to
   *          be empty for the sampling to work correctly
   * @param samples
   *          the number of samples to select from the source
   * @throws IllegalArgumentException
   ...
   * the original list, with elements being equally spaced among the lists. The
   * lists will be returned in order of the consecutive values they represent in
   * the source list. <br>
   * <br>
   * <b>NOTE</b>: Because the implementation uses
   * {@link List#subList(int, int) }, changes to the returned lists will be
   * reflected in the source list.
   *
   * @param <T>
   *          the type contained in the list
   * @param source
   *          the source list that will be used to back the <tt>count</tt> lists
   * @param count
   *          the number of lists to partition the source into.
