...
/**
 * This class represents a priority queue specifically designed to contain integer keys, and uses less memory then a
 * {@link PriorityQueue} filled with integers. This queue can optionally support <i>log(n)</i> removal of key values at
...
public class IntPriorityQueue extends AbstractQueue<Integer> implements Serializable {
  ...
  public static enum Mode {
    /**
     * Sets the priority queue to use the standard queue implementation with the standard complexity for the following
     * operations <br>
     * <br>
    ...
    STANDARD,
    /**
     * Sets the priority queue to use a backing hashtable to map values to their position in the heap. This changes the
     * complexity to <br>
     * <br>
     * O(1) time for {@link #peek() } and {@link #contains(java.lang.Object) } <br>
     * O(log n) time for {@link #poll() }, {@link #add(java.lang.Object) }, and {@link #remove(java.lang.Object) } <br>
     * And calling {@link Iterator#remove() } is O(log n), but the iteration is in no particular order<br>
     */
    HASH,
    /**
     * Sets the priority queue to use a backing array that indexes into the heap. This provides the fastest version if
     * fast {@link #remove(java.lang.Object) } and {@link #contains(java.lang.Object) }
     * are needed. However, it must create an array as large as the largest int value put into the queue, and negative
     * values will not be accepted. This method is best when the size of the queue is known to be bounded and all values
     * are in the range [0, N].<br>
     * <br>
     * O(1) time for {@link #peek() } and {@link #contains(java.lang.Object) } <br>
     * O(log n) time for {@link #poll() }, {@link #add(java.lang.Object) }, and {@link #remove(java.lang.Object) } <br>
     * And calling {@link Iterator#remove() } is O(log n), but the iteration is in no particular order<br>
   ...
   *
   * @param initialSize the initial storage size of the queue
   * @param fastValueRemove the mode that whether or not, and how, fast arbitrary object removal from the queue will be
   * done.
   */
  ...
  /**
   * Heapify up from the given index in the heap and make sure everything is correct. Stops when the child value is in
   * correct order with its parent.
   *
