...
public class IntPriorityQueue extends AbstractQueue<Integer>implements Serializable {
  ...
  public static enum Mode {
    /**
     * Sets the priority queue to use the standard queue implementation with the
     * standard complexity for the following operations <br>
     * <br>
     * O(1) time for {@link #peek() } <br>
     * O(log n) time for {@link #poll() }, {@link #add(java.lang.Object) } <br>
     * O(n) time for {@link #remove(java.lang.Object) } and
     * {@link #contains(java.lang.Object) }<br>
     * And calling {@link Iterator#remove() } is O(log n), but the iteration is
     * in no particular order<br>
     *
     */
    STANDARD, /**
               * Sets the priority queue to use a backing hashtable to map
               * values to their position in the heap. This changes the
               * complexity to <br>
               * <br>
               * O(1) time for {@link #peek() } and
               * {@link #contains(java.lang.Object) } <br>
               * O(log n) time for {@link #poll() },
               * {@link #add(java.lang.Object) }, and
               * {@link #remove(java.lang.Object) } <br>
               * And calling {@link Iterator#remove() } is O(log n), but the
               * iteration is in no particular order<br>
               */
    HASH, /**
           * Sets the priority queue to use a backing array that indexes into
           * the heap. This provides the fastest version if fast
           * {@link #remove(java.lang.Object) } and
           * {@link #contains(java.lang.Object) } are needed. However, it must
           * create an array as large as the largest int value put into the
           * queue, and negative values will not be accepted. This method is
           * best when the size of the queue is known to be bounded and all
           * values are in the range [0, N].<br>
           * <br>
           * O(1) time for {@link #peek() } and
           * {@link #contains(java.lang.Object) } <br>
   ...
   * @param comparator
   *          the comparator to determine the order of elements in the queue
   * @param fastValueRemove
   *          the mode that whether or not, and how, fast arbitrary object
   *          removal from the queue will be done.
   */
  ...
  private void heapDown(int i) {
    ...
    while (childIsSmallerAndValid(i, iL) || childIsSmallerAndValid(i, iR)) {
      // we are larger then one of ours children, so swap with the smallest of
      // the two
   ...
   *
   * @param i
   *          the index in the heap to start checking from.
   */
   ...
   *
   * @param e
   *          the value to store the index of
   * @param i
   ...
   *
   * @param i
   *          the valid heap node index to remove from the heap
