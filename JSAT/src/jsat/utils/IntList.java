...
/**
 * Provides a modifiable implementation of a List using a int array. This provides considerable memory efficency
 * improvements over using an {@link ArrayList} to store integers. Null is not allowed into the list.
 *
 * @author Edward Raff
 */
public class IntList extends AbstractList<Integer> implements Serializable {
  ...
  /**
   * Creates and returns a view of the given int array that requires only a small object allocation. Changes to the list
   * will be reflected in the array up to a point. If the modification would require increasing the capacity of the
   ...
   * @return an IntList backed by the given array, unless modified to the point of requiring the allocation of a new
   * array
   */
