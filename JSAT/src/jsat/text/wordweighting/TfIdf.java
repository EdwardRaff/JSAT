...
public class TfIdf extends WordWeighting {
  ...
  public enum TermFrequencyWeight {
    /**
     * BOOLEAN only takes into account whether or not the word is present in the document. <br>
     * 1.0 if the count is non zero.
     */
    BOOLEAN,
    /**
     * LOG returns a term weighting in [1, infinity) based on the log value of the term frequency<br>
     * 1 + log(count)
     */
    LOG,
    /**
     * DOC_NORMALIZED returns a term weighting in [0, 1] by normalizing the frequency by the most common word in the
     * document. <br>
  ...
  /**
   * Creates a new TF-IDF document weighting scheme that uses {@link TermFrequencyWeight#LOG LOG} weighting for term
   * frequency.
   */
  public TfIdf() {
   ...
   *
   * @param tfWeighting the weighting method to use for the term frequency (tf) component
   */
