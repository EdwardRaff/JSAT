...
public abstract class WordWeighting extends IndexFunction {
  ...
  /**
   * The implementation may want to pre compute come values based on the vector
   * it is about to be applied to. This should be called in place of
   * {@link Vec#applyIndexFunction(jsat.math.IndexFunction) } . The vector
   * should be in a bag-of-words form where each index value indicates how many
   * times the word for that index occurred in the document represented by the
   * vector.
   *
   * @param vec
   ...
   *
   * @param allDocuments
   *          the list of all vectors that make up the set of documents. The
   *          word vectors should be unmodified, containing the value of how
   *          many times a word appeared in the document for each index.
   * @param df
   *          a list mapping each integer index of a word to how many times that
   *          word occurred in total
   */
