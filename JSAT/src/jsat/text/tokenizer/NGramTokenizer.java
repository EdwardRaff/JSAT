...
/**
 * This tokenizer creates n-grams, which are sequences of tokens combined into their own larger token. For example, "the
 * dog barked" could be a 3-gram. If all sub n-grams are also being generated, the returned set would contain the
 * 1-grams "the", "dog", and "barked", the 2-grams "the dog" and "dog barked", and the aforementioned 3-gram. For this
 * to work, this tokenizer assumes the base tokenizer returns tokens in the order they were seen. <br>
 * Note that n-grams can significantly increase the number of unique tokens, and n-grams are inherently rarer than the
 * 1-grams they are generated from.
 *
 * @author Edward Raff
 */
public class NGramTokenizer implements Tokenizer {
   ...
   *
   * @param n the length of the ngrams. While it should be greater than 1, 1 is still a valid input.
   * @param base the base tokenizer to create n-grams from
   * @param allSubN {@code true} to generate all sub n-grams, {@code false} to only return the n-grams specified
   */
