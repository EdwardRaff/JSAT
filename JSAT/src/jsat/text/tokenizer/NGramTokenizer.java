...
public class NGramTokenizer implements Tokenizer {
   ...
   *
   * @param n
   *          the length of the ngrams. While it should be greater than 1, 1 is
   *          still a valid input.
   * @param base
   *          the base tokenizer to create n-grams from
   * @param allSubN
   *          {@code true} to generate all sub n-grams, {@code false} to only
   *          return the n-grams specified
   */
  ...
  public void tokenize(final String input, final StringBuilder workSpace, final List<String> storageSpace) {
    ...
    for (int i = 1; i < origSize; i++)// slide from left to right on the 1-grams
    {
      ...
      for (int gramSize = allSubN ? 2 : n; gramSize <= n; gramSize++) {
        ...
        int j = i - (gramSize - 1);
        if (j < 0) {// means we are going past what we have, and we would be
                    // adding duplicates
          continue;
        }
        for (; j < i; j++) {
