...
public interface Tokenizer extends Serializable {
   ...
   * objects or interned strings. If a token is returned that is backed by the
   * original document, memory may get leaked by processes consuming the token.
   *
   * @param input
   ...
   *          the string to tokenize
   * @param workSpace
   *          an already allocated (but empty) string builder than can be used
   *          as a temporary work space.
   * @param storageSpace
   *          an already allocated (but empty) list to place the tokens into
   */
