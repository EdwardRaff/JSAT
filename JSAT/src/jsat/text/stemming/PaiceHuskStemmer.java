...
public class PaiceHuskStemmer extends Stemmer {
  ...
  static private class Rule {
    ...
    public Rule(final String ending, final int toRemove, final String newEnding, final boolean virgin,
     ...
     * returned. An object comparison can then be done to check if the string
     * was modified. <br>
    ...
    public String apply(final String input) {
      if (input.endsWith(ending)) {
        ...
        if (toRemove == 0) {// Proctected word, return a new string explicitly
                            // to be super sure
   ...
   */
  private static final Rule[] ARules = new Rule[] { new Rule("ia", 2, "", true, true), // ai*2.
      new Rule("a", 1, "", true, true), // a*1.
  };

  private static final Rule[] BRules = new Rule[] { new Rule("bb", 1, "", false, true), // bb1.
  };

  private static final Rule[] CRules = new Rule[] { new Rule("ytic", 3, "s", false, true), // city3s.
      new Rule("ic", 2, "", false, false), // ci2>
      new Rule("nc", 1, "t", false, false), // cn1t>
  };

  private static final Rule[] DRules = new Rule[] { new Rule("dd", 1, "", false, true), // dd1.
  ...
  private static final Rule[] ERules = new Rule[] { new Rule("e", 1, "", false, false), // e1>
  };

  private static final Rule[] FRules = new Rule[] { new Rule("lief", 1, "v", false, true), // feil1v.
      new Rule("if", 2, "", false, true), // fi2>
  };

  private static final Rule[] GRules = new Rule[] { new Rule("ing", 3, "", false, false), // gni3>
      new Rule("iag", 3, "y", false, true), // gai3y.
      new Rule("ag", 2, "", false, false), // ga2>
      new Rule("gg", 1, "", false, true), // gg1.
  };

  private static final Rule[] HRules = new Rule[] { new Rule("th", 2, "", true, true), // ht*2.
      new Rule("guish", 5, "ct", false, true), // hsiug5ct.
      new Rule("ish", 3, "", false, false), // hsi3>
  };

  private static final Rule[] IRules = new Rule[] { new Rule("i", 1, "", true, true), // i*1.
      new Rule("i", 1, "y", false, false), // i1y>
  };

  private static final Rule[] JRules = new Rule[] { new Rule("ij", 1, "d", false, true), // ji1d.
  ...
  private static final Rule[] LRules = new Rule[] { new Rule("ifiabl", 6, "", false, true), // lbaifi6.
  ...
  private static final Rule[] MRules = new Rule[] { new Rule("ium", 3, "", false, true), // mui3.
      new Rule("mu", 2, "", true, true), // mu*2.
      new Rule("ism", 3, "", false, false), // msi3>
      new Rule("mm", 1, "", false, true), // mm1.
  };

  private static final Rule[] NRules = new Rule[] { new Rule("sion", 4, "j", false, false), // nois4j>
  ...
  private static final Rule[] PRules = new Rule[] { new Rule("ship", 4, "", false, false), // pihs4>
      new Rule("pp", 1, "", false, true), // pp1.
  };

  private static final Rule[] RRules = new Rule[] { new Rule("er", 2, "", false, false), // re2>
  ...
  private static final Rule[] SRules = new Rule[] { new Rule("ies", 3, "y", false, false), // sei3y>
  ...
  private static final Rule[] TRules = new Rule[] { new Rule("plicat", 4, "y", false, true), // tacilp4y.
  ...
  private static final Rule[] URules = new Rule[] { new Rule("iqu", 3, "", false, true), // uqi3.
      new Rule("ogu", 1, "", false, true), // ugo1.
  };

  private static final Rule[] VRules = new Rule[] { new Rule("siv", 3, "j", false, false), // vis3j>
      new Rule("iev", 0, "", false, true), // vie0.
      new Rule("iv", 2, "", false, false), // vi2>
  };

  private static final Rule[] YRules = new Rule[] { new Rule("bly", 1, "", false, false), // ylb1>
  ...
  private static final Rule[] ZRules = new Rule[] { new Rule("iz", 2, "", false, false), // zi2>
   ...
   * The rules for the
   */
  private static final Rule[][] rules = new Rule[][] { ARules, BRules, CRules, DRules, ERules, FRules, GRules, HRules,
      IRules, JRules, NoRules, LRules, MRules, NRules, NoRules, PRules, NoRules, RRules, SRules, TRules, URules, VRules,
      NoRules, NoRules, YRules, ZRules };
  ...
  public String stem(String word) {
    ...
    do {
      ...
      for (final Rule rule : rules[ruleIndex]) {
        ...
        if (test == null ? word != null : !test.equals(word)) // Rule was
                                                              // applied, is it
                                                              // acceptable?
        {
          word = test;
