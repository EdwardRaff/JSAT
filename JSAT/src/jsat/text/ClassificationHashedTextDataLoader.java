 ...
 * {@code addOriginalDocument} was called. <br>
...
public abstract class ClassificationHashedTextDataLoader extends HashedTextDataLoader {
  ...
  public ClassificationHashedTextDataLoader(final int dimensionSize, final Tokenizer tokenizer,
      final WordWeighting weighting) {
  ...
  public ClassificationDataSet getDataSet() {
    ...
    final ClassificationDataSet cds = new ClassificationDataSet(vectors.get(0).length(), new CategoricalData[] {},
        labelInfo);
  ...
  /**
   * The classification label data stored in {@link #labelInfo} must be set if
   * the text loader is to return a classification data set. As such, this
   * abstract class exists to force the user to set it, in this way they can not
   * forget. <br>
