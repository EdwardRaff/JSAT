...
public class BasicTextVectorCreator implements TextVectorCreator {
   ...
   * @param wordIndex
   *          the map of each known word to its index, the size of the map
   *          indicating the maximum (exclusive) index
   * @param weighting
   *          the weighting process to apply to each loaded document.
   */
  public BasicTextVectorCreator(final Tokenizer tokenizer, final Map<String, Integer> wordIndex,
  ...
  public Vec newText(final String input, final StringBuilder workSpace, final List<String> storageSpace) {
    ...
    for (final String word : storageSpace) {
      if (wordIndex.containsKey(word)) // Could also call retainAll on words
                                       // before looping. Worth while to
                                       // investigate
      {
