 ...
 * for learning a topic model from a set of documents. This implementation is based on Stochastic Variational Inference
 * and is meant for large collections (more than 100,000 data points) and can learn in an online fashion. <br>
 * <br>
 * For LDA it is common to set {@link #setAlpha(double) &alpha;} =
 * {@link #setEta(double) &eta;} = 1/K, where K is the number of topics to be learned. Note that &eta; is not a learning
 * rate parameter, as the symbol is usually used. <br>
 *
 * For this algorithm, some potential parameter combinations (by column) for
 * {@link #setMiniBatchSize(int) batch size}, {@link #setKappa(double) &kappa;}, and
 * {@link #setTau0(double) &tau;<sub>0</sub>} are:<br>
 * <table>
 * <caption></caption>
 * <tr>
 * <td>batch size</td>
 ...
 * See:<br>
 * <ul>
 * <li>Blei, D. M., Ng, A. Y.,&amp;Jordan, M. I. (2003). <i>Latent Dirichlet Allocation</i>. Journal of Machine Learning
 * Research, 3(4-5), 993–1022. doi:10.1162/jmlr.2003.3.4-5.993</li>
 * <li>Hoffman, M., Blei, D.,&amp;Bach, F. (2010). <i>Online Learning for Latent Dirichlet Allocation</i>. In Advances
 * in Neural Information Processing Systems (pp. 856–864). Retrieved from
 * <a href="http://videolectures.net/site/normal_dl/tag=83534/nips2010_1291.pdf">
 * here</a></li>
 * <li>Hoffman, M. D., Blei, D. M., Wang, C.,&amp;Paisley, J. (2013).
 * <i>Stochastic Variational Inference</i>. The Journal of Machine Learning Research, 14(1), 1303–1347.</li>
...
public class OnlineLDAsvi implements Parameterized {
  ...
  /**
   * Creates a new Online LDA learner. The number of {@link #setK(int) topics}, expected number of
   * {@link #setD(int) documents}, and the {@link #setVocabSize(int) vocabulary size} must be set before it can be used.
   */
  public OnlineLDAsvi() {
    K = D = W = -1;
  }
   ...
   *
   * We need one gamma for each document, and each will have a value for all K topics.
   */
  ...
  /**
   * Holds the temp vector used to store the exponentiated expectation of {@link #logThetaLocal}
   */
  ...
  /**
   * Returns the approximate number of documents that will be observed, or {@code -1} if <i>this</i> object is not ready
   * to learn
   *
  ...
  /**
   * Sets the prior for the on weight vector theta. 1/{@link #setK(int) K} is a common choice.
   *
  ...
  /**
   * A learning rate constant to control the influence of early iterations on the solution. Larger values reduce the
   * influence of earlier iterations, smaller values increase the weight of earlier iterations.
   *
   ...
   *
   * @return the number of training iterations over the data set that will be used
   */
  ...
  /**
   * The "forgetfulness" factor in the learning rate. Larger values increase the rate at which old information is
   * "forgotten"
   *
  ...
  /**
   * Returns the topic vector for a given topic. The vector should not be altered, and is scaled so that the sum of all
   * term weights sums to one.
   *
  ...
  /**
   * From the 2013 paper, see expectations in figure 5 on page 1323, and equation (27) on page 1325 See also equation 6
   * in the 2010 paper. 2013 paper figure 5 seems to be a typo
   *
  ...
  /**
   * Gets a sample from the exponential distribution. Implemented to be fast at the cost of accuracy
   *
   * @param lambdaInv the inverse of the lambda value that parameterizes the exponential distribution
  ...
  /**
   * Updates the Beta vectors associated with the {@link #gammaLocal gamma} topic distributions so that they can be used
   * to update against the given batch of documents. Once updated, the Betas are the only items needed to perform
   * updates from the given batch, and the gamma values can be updated as the updates are computed.
   *
  ...
  /**
   * Prepares gamma and the associated theta expectations are initialized so that the iterative updates to them can
   * begin.
   *
  ...
  /**
   * Performs the main iteration to determine the topic distribution of the given document against the current model
   * parameters. The non zero values of phi will be stored in {@code indexMap} and {@code phiCols}
   *
   * @param doc the document to get the topic assignments for
   * @param indexMap the array of integers to store the non zero document indices in
   * @param phiCols the array to store the normalized non zero values of phi in, where each value corresponds to the
   * associated index in {@code indexMap}
