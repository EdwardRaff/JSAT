...
/**
 * This class provides a framework for loading classification datasets made of text documents as vectors. This extension
 * uses {@link #addOriginalDocument(java.lang.String, int) } instead so that the original documents have a class label
 * associated with them. {@link #getDataSet() } then returns a classification data set, where the class label for each
 * data point is the label provided when
 * <tt>addOriginalDocument</tt> was called.
 * <br>
 * New vectors created with {@link #newText(java.lang.String) } are inherently not part of the original data set, so do
 * not need or receive a class label.
 *
 * @author Edward Raff
 */
public abstract class ClassificationTextDataLoader extends TextDataLoader {
  ...
  /**
   * The list of the true class labels for the data that was loaded before {@link #finishAdding() } was called.
   */
   ...
   * @param weighting the weighting scheme to apply to each vector in the collection
   */
  ...
  /**
   * The classification label data stored in {@link #labelInfo} must be set if the text loader is to return a
   * classification data set. As such, this abstract class exists to force the user to set it, in this way they can not
   * forget. <br>
   * This will be called in {@link #getDataSet() } just before {@link #initialLoad() } is called.
   */
