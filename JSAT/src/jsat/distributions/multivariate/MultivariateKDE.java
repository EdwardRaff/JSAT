...
/**
 * There are several methods of generalizing the {@link KernelDensityEstimator}
 * to the multivariate case. This class provides a contract for implementations
 * that provide a generalization of the KDE.
 *
 * @author Edward Raff
 */
abstract public class MultivariateKDE extends MultivariateDistributionSkeleton {
   ...
   * density of the query point <tt>x</tt>. Each vector is paired with its
   * integer index from the original constructing list vectors, and a double
   ...
   * density of the query point <tt>x</tt>. Each vector is paired with its
   * integer index from the original constructing list vectors, and a double
   * indicating its distance from the query point divided by the bandwidth of
   * the point.
   *
   * @param x
   *          the query point
  ...
  /**
   * A caller may want to increase or decrease the bandwidth after training has
   * been completed to get smoother model, or decrease it to observe behavior.
   * This method will scaled the bandwidth of each data point by the given
   * factor
   *
   * @param scale
   *          the value to scale the bandwidth used
   */
