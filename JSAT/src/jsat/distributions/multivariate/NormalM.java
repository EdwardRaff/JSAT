...
public class NormalM extends MultivariateDistributionSkeleton {
   ...
   * /  __\             2
   * \2 ||/   (|Sigma|)
   * </pre> where k is the dimension, Sigma is the covariance matrix, and || denotes the determinant. <br>
   ...
   * </pre>
   *
   * This can then be added to the log of the x dependent part, which, when exponentiated, gives the correct result of
   ...
   * <tt>mean</tt> should be of length <i>n</i> and <tt>covariance</tt> should be an <i>n</i> by <i>n</i> matrix. It is
   * also a requirement that the matrix be symmetric positive definite.
   *
   ...
   * @throws ArithmeticException if the covariance matrix is not square, does not agree with the mean, or is not
   * positive definite. An exception may not be throw for all bad matrices.
   */
