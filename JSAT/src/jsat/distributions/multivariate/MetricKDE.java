...
/**
 * MetricKDE is a generalization of the {@link KernelDensityEstimator} to the multivariate case. A
 * {@link KernelFunction} is used to weight the contribution of each data point, and a {@link DistanceMetric } is used
 * to effectively alter the shape of the kernel. The MetricKDE uses one bandwidth parameter, which can be estimated
 * using a nearest neighbor approach, or tuned by hand. The bandwidth of the MetricKDE can not be estimated en the same
 * way as the univariate case.
 *
 * @author Edward Raff
 */
public class MetricKDE extends MultivariateKDE implements Parameterized {
  ...
  /**
   * When estimating the bandwidth, the distances of the k'th nearest neighbors are used to perform the estimate. The
   * default value of this k is {@value #DEFAULT_K}
   */
  public static final int DEFAULT_K = 3;

  /**
   * When estimating the bandwidth, the distances of the k'th nearest neighbors are used to perform the estimate. The
   * default number of standard deviations from the mean to add to the bandwidth estimate is {@value #DEFAULT_STND_DEV}
   */
  public static final double DEFAULT_STND_DEV = 2.0;

  /**
   * When estimating the bandwidth, the distances of the k'th nearest neighbors are used to perform the estimate. The
   * weight of each neighbor is controlled by the kernel function.
   */
  ...
  /**
   * Sets the bandwidth used to estimate the density of the underlying distribution. Too small a bandwidth results in
   * high variance, while too large causes high bias.
   *
  ...
  /**
   * When estimating the bandwidth, the mean of the k'th nearest neighbors to each data point is used. This value
   * controls the default value of k used when it is not specified.
   *
   * @param defaultK
   */
  ...
  /**
   * When estimating the bandwidth, the mean of the neighbor distances is used, and a multiple of the standard
   * deviations is added. This controls the multiplier value used when the bandwidth is not specified. Zero and negative
   * multipliers are allowed, but a negative multiplier may result in the fitting failing.
   *
  ...
  /**
   * Sets the KDE to model the density of the given data set by estimating the bandwidth by using the <tt>k</tt> nearest
   * neighbors of each data point.
   *
   * @param dataSet the data set to model the density of
  ...
  /**
   * Sets the KDE to model the density of the given data set by estimating the bandwidth by using the <tt>k</tt> nearest
   * neighbors of each data data point. <br>
   * The bandwidth estimate is calculate as the mean of the distances of the k'th nearest neighbor plus
   * <tt>stndDevs</tt> standard deviations added to the mean.
   *
   * @param dataSet the data set to model the density of
   * @param k the number of neighbors to use to estimate the bandwidth
