...
public class ProductKDE extends MultivariateKDE {
  ...
  public ProductKDE clone() {
    ...
    if (sortedDimVals != null) {
      clone.sortedDimVals = new double[sortedDimVals.length][];
      for (int i = 0; i < sortedDimVals.length; i++) {
        clone.sortedDimVals[i] = Arrays.copyOf(sortedDimVals[i], sortedDimVals[i].length);
      }
    }
    if (sortedIndexVals != null) {
      clone.sortedIndexVals = new int[sortedIndexVals.length][];
      for (int i = 0; i < sortedIndexVals.length; i++) {
        clone.sortedIndexVals[i] = Arrays.copyOf(sortedIndexVals[i], sortedIndexVals[i].length);
      }
    }
    if (bandwidth != null) {
      clone.bandwidth = Arrays.copyOf(bandwidth, bandwidth.length);
    }
    if (originalVecs != null) {
      clone.originalVecs = new ArrayList<Vec>(originalVecs);
    }
    return clone;
  }
  ...
  public List<VecPaired<VecPaired<Vec, Integer>, Double>> getNearby(final Vec x) {
    ...
    final List<VecPaired<VecPaired<Vec, Integer>, Double>> results = new ArrayList<VecPaired<VecPaired<Vec, Integer>, Double>>(
        validIndecies.size());

    for (final int i : validIndecies) {
      final Vec v = originalVecs.get(i);
      results
          .add(new VecPaired<VecPaired<Vec, Integer>, Double>(new VecPaired<Vec, Integer>(v, i), exp(logProd.get(i))));
    }
    return results;
  }
  ...
  public List<VecPaired<VecPaired<Vec, Integer>, Double>> getNearbyRaw(final Vec x) {
    // Not entirly sure how I'm going to fix this... but this isnt technically
    // right
   ...
   *          the query vector
   * @param validIndecies
   *          the empty set that will be altered to contain the indices of
   *          vectors that had a non zero contribution to the density
   * @param logProd
   *          an empty sparce vector that will be modified to contain the log of
   *          the product of the kernels for each data point. Some indices that
   *          have zero contribution to the density will have non zero values.
   *          <tt>validIndecies</tt> should be used to access the correct
   *          indices.
  ...
  private double queryWork(final Vec x, final Set<Integer> validIndecies, final SparseVector logProd) {
    ...
    for (int i = 0; i < sortedDimVals.length; i++) {
      ...
      // Only values within a certain range will have an effect on the result,
      // so we will skip to that range!
      int from = Arrays.binarySearch(X, xi - h * k.cutOff());
      int to = Arrays.binarySearch(X, xi + h * k.cutOff());
      // Mostly likely the exact value of x is not in the list, so it retursn
      // the inseration points
