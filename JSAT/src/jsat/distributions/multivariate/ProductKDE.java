...
/**
 * The Product Kernel Density Estimator is a generalization of the {@link KernelDensityEstimator} to the multivariate
 * case. This is done by using a kernel and bandwidth for each dimension, such that the bandwidth for each dimension can
 * be determined using the same methods as the univariate KDE. This can simplify the difficulty in bandwidth selection
...
public class ProductKDE extends MultivariateKDE {
   ...
   * @param validIndecies the empty set that will be altered to contain the indices of vectors that had a non zero
   * contribution to the density
   * @param logProd an empty sparce vector that will be modified to contain the log of the product of the kernels for
   * each data point. Some indices that have zero contribution to the density will have non zero values.
   * <tt>validIndecies</tt> should be used to access the correct indices.
