 ...
 * {@link Dirichlet} distribution, and occurs when all alphas have the same
 * value.
 *
 * @author Edward Raff
 */
public class SymmetricDirichlet extends MultivariateDistributionSkeleton {
   ...
   *
   * @param alpha
   *          the positive alpha value for the distribution
   * @param dim
   *          the dimension of the distribution.
   * @throws ArithmeticException
   *           if a non positive alpha or dimension value is given
   */
  ...
  public double logPdf(final Vec x) {
    if (x.length() != dim) {
      throw new ArithmeticException(
          dim + " variable distribution can not awnser a " + x.length() + " dimension variable");
    }
    double logVal = 0;
   ...
   *
   * @param alpha
   *          the positive value for the distribution
   * @throws ArithmeticException
   *           if the value given is not a positive value
   */
  public void setAlpha(final double alpha) throws ArithmeticException {
    if (alpha <= 0 || Double.isNaN(alpha) || Double.isInfinite(alpha)) {
      throw new ArithmeticException(
          "Symmetric Dirichlet Distribution parameters must be positive, " + alpha + " is invalid");
    }
    this.alpha = alpha;
  }
  ...
  public <V extends Vec> boolean setUsingData(final List<V> dataSet) {
    ...
    alpha = optimize.optimize(1e-10, 100, logLike, guesses).get(0);
    return true;
  }
