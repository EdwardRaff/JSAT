 ...
 * values.<br>
 * <br>
 * The {@link #cdf(double) } method will behave by {@link Math#floor(double)
 * rounding down} and then calling the integer {@link #cdf(int) } counterpart.
 *
 * @author Edward Raff
 */
abstract public class DiscreteDistribution extends Distribution {
  ...
  public double invCdf(final double p) {
    // two special case checks, as they can cause a failure to get a positive
    // and negative value on the ends, which means we can't do a search for the
    // root
    ...
    // stewpwise nature fo discrete can cause problems for search, so we will
    // use a smoothed cdf to pass in
  ...
  protected double invCdf(final double p, final Function cdf) {
    ...
    // we can't use the max/min b/c we might overflow on some of the
    // computations, so lets tone it down a little
