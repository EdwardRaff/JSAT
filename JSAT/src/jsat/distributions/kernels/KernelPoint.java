...
/**
 * The Kernel Point represents a kernelized weight vector by a linear combination of vectors transformed through a
 * {@link KernelTrick kernel fuctiion}. This implementation allows the selection of multiple different budget
 * maintenance strategies <br>
 * <br>
 * See {@link KernelRLS} and {@link Projectron} for methods and papers based on the same ideas used to create this
 * class. <br>
 * Credit goes to Davis King of the <a href="http://dlib.net/ml.html">dlib library</a> for the idea of this type of
 * class. <br>
 * <br>
 * Changing the null {@link #setBudgetStrategy(jsat.distributions.kernels.KernelPoint.BudgetStrategy)
...
public class KernelPoint {
  ...
  public enum BudgetStrategy {
    /**
     * The budget is maintained by projecting the incoming vector onto the set of current vectors. If the error in the
     * projection is less than {@link #setErrorTolerance(double) } the projection is used, and the input is added to the
     * support vector set if the error was too large. <br>
     * Once the maximum budget size is reached, the projection is used regardless of the error of the projection. <br>
     * <br>
     * The time complexity of each update is <i>O(B<sup>2</sup>)</i> and uses <i>O(B<sup>2</sup>)</i> memory.
     *
     */
    PROJECTION,
    /**
     * The budget is maintained by merging two support vectors to minimize the error in the squared norm. The merged
     * support vector is not a member of the training set. <b>This method is only valid for the {@link RBFKernel} </b>.
     * Using any other kernel may cause invalid results<br>
     * <br>
     * See:<br>
     * <ul>
     * <li>Wang, Z., Crammer, K.,&amp;Vucetic, S. (2012). <i>Breaking the Curse of Kernelization : Budgeted Stochastic
     * Gradient Descent for Large-Scale SVM Training</i>. The Journal of Machine Learning Research, 13(1),
     * 3103–3131.</li>
     * <li>Wang, Z., Crammer, K.,&amp;Vucetic, S. (2010). <i>Multi-class pegasos on a budget</i>. In 27th International
     * Conference on Machine Learning (pp. 1143–1150). Retrieved from
     * <a href="http://www.ist.temple.edu/~vucetic/documents/wang10icml.pdf">
     * here</a></li>
     * </ul>
     * <br>
     * The time complexity of each update is <i>O(B)</i> and uses <i>O(B)</i> memory.
     */
    MERGE_RBF,
    /**
     * The budget is maintained by refusing to add new data points once the budget is reached. <br>
     * <br>
     * The time complexity of each update is <i>O(B)</i> and uses <i>O(B)</i> memory.
     */
    STOP,
    /**
     * The budget is maintained by randomly dropping a previous support vector. <br>
     * <br>
     * The time complexity of each update is <i>O(B)</i> and uses <i>O(B)</i> memory.
     */
    RANDOM,
  ...
  /**
   * Creates a new Kernel Point, which is a point in the kernel space represented by an accumulation of vectors and uses
   * the {@link BudgetStrategy#PROJECTION} strategy with an unbounded maximum budget
   *
   * @param k the kernel to use
   * @param errorTolerance the maximum error in [0, 1] allowed for projecting a vector instead of adding it to the basis
   * set
   */
  ...
  /**
   * Sets the error tolerance used for projection maintenance strategies such as {@link BudgetStrategy#PROJECTION}
   *
  ...
  /**
   * Returns the error tolerance that is used depending on the {@link BudgetStrategy} in use
   *
  ...
  /**
   * Computes the dot product between the kernel point this object represents and the given input vector in the kernel
   * space.
   *
   ...
   *
   * @param x the input vector to work with
   * @return the Euclidean distance between this point and {@code x} in the kernel space
   */
...
//            if (kernelAccel != null)
   ...
   * <br>. THIS METHOD IS A BOTTLE NECK, so it has some optimization hacks<br>
  ...
  /**
   * Returns the list of the raw vectors being used by the kernel points. Altering this vectors will alter the same
   * vectors used by the KernelPoint and will cause inconsistent results.<br>
   * <br>
