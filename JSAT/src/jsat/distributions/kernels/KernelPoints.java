...
/**
 * This class represents a list of {@link KernelPoint} objects. This is done to avoid excessive memory duplication that
 * can occur when multiple KernelPoints are in use at the same time.
 *
 * @author Edward Raff
 */
public class KernelPoints {
   ...
   *
   * @param k the kernel trick to use in which to represent a vector in the kernel space
   * @param points the initial number of kernel points to store in this set
   * @param errorTolerance the maximum error allowed for projecting a vector instead of adding it to the basis set
   */
   ...
   * @param points the initial number of kernel points to store in this set
   * @param errorTolerance the maximum error allowed for projecting a vector instead of adding it to the basis set
   * @param mergeGrams whether or not to merge the gram matrices of each KernelPoint.
   */
  ...
  public KernelPoints(KernelPoints toCopy) {
    ...
    {
      for (KernelPoint point : toCopy.points) {
        this.points.add(new KernelPoint(k, errorTolerance));
      }
    } else {
  ...
  /**
   * Sets the method used for maintaining the budget of support vectors. This method must be called <i>before</i> any
   * vectors are added to the KernelPoint. <br>
   * <br>
  ...
  /**
   * Returns the error tolerance that is used depending on the {@link KernelPoint.BudgetStrategy} in use
   *
  ...
  /**
   * Sets the maximum budget for support vectors to allow. Setting to {@link Integer#MAX_VALUE} is essentially an
   * unbounded number of support vectors. Increasing the budget after adding the first vector is always allowed, but it
   * may not be possible to reduce the number of current support vectors is above the desired budget.
   *
  ...
  /**
   * Computes the dot product between the {@code k}'th KernelPoint and the given vector in the kernel space.
   *
   * @param k the index of the KernelPoint in this set to contribute to the dot product
   * @param x the vector to contribute to the dot product
   * @param qi the query information for the vector, or {@code null} only if the kernel in use does not support
   * acceleration.
   * @return the dot product between the {@code k}'th KernelPoint and the given vector
   */
  ...
  /**
   * Computes the dot product between each KernelPoint in this set and the given vector in the kernel space. The results
   * are equivalent to an array and setting each value using {@link #dot(int, jsat.linear.Vec, java.util.List) } <br>
   * This method should be faster than computing the dot products individual since it avoids redundant kernel
   * computations
   *
   * @param x the vector to contribute to the dot product
   * @param qi the query information for the vector, or {@code null} only if the kernel in use does not support
   * acceleration.
   * @return an array where the <i>i'th</i> index contains the dot product of the <i>i'th</i> KernelPoint and the given
   * vector
   */
  ...
  /**
   * Computes the dot product between the {@code k}'th KernelPoint and the given KernelPoint
   *
   * @param k the index of the KernelPoint in this set to contribute to the dot product
   * @param x the other KernelPoint to contribute to the dot product
   * @return the dot product between the {@code k}'th KernelPoint and the given KernelPoint
   */
  ...
  /**
   * Computes the dot product between the {@code k}'th KernelPoint and the {@code j}'th KernelPoint in the given set of
   * points.
   *
   * @param k the index of the KernelPoint in this set to contribute to the dot product
   * @param X the other set of KernelPoints
   * @param j the index of the KernelPoint in the given set to contribute to the dot product
   * @return the dot product between the {@code k}'th KernelPoint and the {@code j}'th KernelPoint in the given set
   */
  ...
  /**
   * Computes the Euclidean distance in the kernel space between the {@code k}'th KernelPoint and the given vector
   *
   * @param k the index of the KernelPoint in this set to contribute to the dot product
   * @param x the point to get the Euclidean distance to
   * @param qi the query information for the vector, or {@code null} only if the kernel in use does not support
   * acceleration.
   * @return the Euclidean distance between the {@code k}'th KernelPoint and {@code x} in the kernel space
   */
  ...
  /**
   * Computes the Euclidean distance in the kernel space between the {@code k}'th KernelPoint and the given KernelPoint
   *
   * @param k the index of the KernelPoint in this set to contribute to the dot product
  ...
  /**
   * Computes the Euclidean distance in the kernel space between the {@code k}'th KernelPoint and the {@code j}'th
   * KernelPoint in the given set
   *
   * @param k the index of the KernelPoint in this set to contribute to the dot product
   * @param X the other set of kernel points to obtain the target KernelPoint
   * @param j the index of the KernelPoint in the given set to contribute to the dot product
   * @return the Euclidean distance between the {@code k}'th KernelPoint and the {@code j}'th KernelPoint in the other
   * set
   */
   ...
   *
   * @param x_t the vector to add to the kernel point
   * @param cs the array with the constant multiplies. Each non zero in {@code cs} is a constant to update one of the
   * vectors by. The vector updated is the one corresponding to the index of the non zero value
   * @param qi the query information for the vector, or {@code null} only if the kernel in use does not support
   * acceleration.
   */
  ...
  /**
   * Adds a new Kernel Point to the internal list this object represents. The new Kernel Point will be equivalent to
   * creating a new KernelPoint directly.
   */
  ...
  /**
   * Returns the number of basis vectors in use. If a vector has been added to more than one Kernel Point it may get
   * double counted (or more), so the value returned may not be reasonable in that case.
   *
  ...
  /**
   * Returns a list of the raw vectors being used by the kernel points. Altering this vectors will alter the same
   * vectors used by these objects and will cause inconsistent results.
   *
