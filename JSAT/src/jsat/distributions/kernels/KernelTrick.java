...
/**
 * The KernelTrick is a method can can be used to alter an algorithm to do its calculations in a projected feature
 * space, without explicitly forming the features. If an algorithm uses only dot products, the Kernel trick can be used
 * in place of these dot products, and computes the inner product in a different feature space.
 * <br><br>
 * All KerenlTrick objects are {@link Parameterized parameterized} so that the values of the kernel can be exposed by
 * the algorithm that makes use of these parameters. To avoid conflicts in parameter names, the parameters of a
...
public interface KernelTrick extends Parameterized, Cloneable, Serializable {
  ...
  //Cache related
  /**
   * Indicates if this kernel supports building an acceleration cache using the {@link #getAccelerationCache(List) } and
   * associated cache accelerated methods. By default this method will return {@code false}. If {@code true}, then a
   * cache can be obtained from this matrix and used in conjunction with {@link #eval(int, Vec, List, List, List) }
   * and {@link #eval(int, int, List, List) } to perform kernel products.
   *
   * @return {@code true} if cache acceleration is supported for this kernel, {@code false} otherwise.
   */
  public boolean supportsAcceleration();

  /**
   * Creates a new list cache values from a given list of training set vectors. If this kernel does not support
   * acceleration, {@code null} will be returned.
   *
   ...
   * kernel computations are going to be done against points in the original set for a point that is outside the
   * original space.
   * <br><br>
  ...
  /**
   * Appends the new cache values for the given vector to the list of cache values. This method is present for online
   * style kernel learning algorithms, where the set of vectors is not known in advance. When a vector is added to the
   * set of kernel vectors, its cache values can be added using this method. <br><br>
   * The results of calling this sequentially on a lit of vectors starting with an empty double list is equivalent to
   * getting the results from calling {@link #getAccelerationCache(java.util.List) }
   * <br><br>
  ...
  /**
   * Computes the kernel product between one vector in the original list of vectors with that of another vector not from
   * the original list, but had information generated by {@link #getQueryInfo(jsat.linear.Vec) }.
   * <br> If the cache input is {@code null}, then {@link #eval(jsat.linear.Vec, jsat.linear.Vec) } will be called
   * directly.
   *
  ...
  /**
   * Produces the correct kernel evaluation given the training set and the cache generated by {@link #getAccelerationCache(List)
   * }. The training vectors should be in the same order.
   *
   * @param a the index of the first training vector
   * @param b the index of the second training vector
   * @param trainingSet the list of training set vectors
   * @param cache the double list of cache values generated by this kernel for the given training set
   * @return the same kernel evaluation result as {@link #eval(jsat.linear.Vec, jsat.linear.Vec) }
   */
   ...
   * where <i>x</i> are the final set of vectors, and <i>&alpha;</i> the associated scalar multipliers
   *
