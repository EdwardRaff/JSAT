...
public class DistributionSearch {
  ...
  /**
   * Searches the distributions that are known for a possible fit, and returns what appears to be the best fit. If no
   * suitable fit can be found, a {@link KernelDensityEstimator} is fit to the data.
   *
   * @param v all the values from a sample
   * @param KDECutOff the cut off value used for using the KDE. Should be in the range (0, 1). Values less than zero
   * means the KDE will never be used, and greater then 1 means the KDE will always be used.
   * @return the distribution that provides the best fit to the data that this method could find.
   */
  ...
  /**
   * Searches the distributions that are given for a possible fit, and returns what appears to be the best fit. If no
   * suitable fit can be found, a {@link KernelDensityEstimator} is fit to the data.
   *
   * @param v all the values from a sample
   * @param KDECutOff the cut off value used for using the KDE. Should be in the range (0, 1). Values less than zero
