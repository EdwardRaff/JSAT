...
/**
 * This is the base class for representing a data set. A data set contains multiple samples, each of which should have
 * the same number of attributes. Conceptually, each {@link DataPoint} represents a row in the data set, and the
 * attributes form the columns.
 *
 * @author Edward Raff
 */
public abstract class DataSet<Type extends DataSet> {
  ...
  /**
   * The list, in order, of the names of the numeric variables. This should be filled with default values on
   * construction, that can then be changed later.
   */
  protected List<String> numericalVariableNames;

  /**
   * This cache is used to hold a reference to the column vectors that are returned. It is often the case that the
   * column could be requested multiple times, especially if someone is doing a grid search, and there is no need to do
   * the work over again. If the GC is low on memory it can still collect our cache since we use soft references<br>
   * <br>
  ...
  /**
   * Sets the unique name associated with the <tt>i</tt>'th numeric attribute. All strings will be converted to lower
   * case first.
   *
  ...
  /**
   * Applies the given transformation to all points in this data set, replacing each data point with the new value. No
   * mutation of the data points will occur
   *
  ...
  /**
   * Applies the given transformation to all points in this data set in parallel, replacing each data point with the new
   * value. No mutation of the data points will occur.
   *
   * @param dt the transformation to apply
   * @param ex the threadpool to provide threads from. May be {@code null} to perform operations in serial
   */
  ...
  /**
   * Applies the given transformation to all points in this data set. If the transform supports mutating the original
   * data points, this will be applied if {@code mutableTransform} is set to {@code true}
   *
   * @param dt the transformation to apply
   * @param mutate {@code true} to mutableTransform the original data points, {@code false} to ignore the ability to
   * mutableTransform and replace the original data points.
   */
  ...
  /**
   * Applies the given transformation to all points in this data set in parallel. If the transform supports mutating the
   * original data points, this will be applied if {@code mutableTransform} is set to {@code true}
   *
   * @param dt the transformation to apply
   * @param mutate {@code true} to mutableTransform the original data points, {@code false} to ignore the ability to
   * mutableTransform and replace the original
   * @param ex the threadpool to provide threads from. May be {@code null} to perform operations in serial
   */
  ...
  /**
   * Returns the <tt>i</tt>'th data point in this set. The order will never chance so long as no data points are added
  ...
  /**
   * Replaces an already existing data point with the one given. Any values associated with the data point, but not
   * apart of it, will remain intact.
   *
  ...
  /**
   * Returns summary statistics computed in an online fashion for each numeric variable. This returns all summary
   * statistics, but can be less numerically stable and uses more memory.
   *
   * @param useWeights {@code true} to return the weighted statistics, unweighted otherwise.
  ...
  /**
   * Returns an {@link OnLineStatistics } object that is built by observing what proportion of each data point contains
   * non zero numerical values. A mean of 1 indicates all values were fully dense, and a mean of 0 indicates all values
   * were completely sparse (all zeros).
   *
  ...
  /**
   * Computes the weighted mean and variance for each column of feature values. This has less overhead than 
     * {@link #getOnlineColumnStats(boolean) } but returns less information.
   *
   * @return an array of the vectors containing the mean and variance for each column.
   */
  ...
  /**
   * Returns an iterator that will iterate over all data points in the set. The behavior is not defined if one attempts
   * to modify the data set while being iterated.
   *
  ...
  /**
   * Returns the array containing the categorical data information for this data set. Changes to this will be reflected
   * in the data set.
   *
   ...
   * @param splits any array, where the length is the number of datasets to create and the value of in each index is the
   * fraction of samples that should be placed into that dataset. The sum of values must be less than or equal to 1.0
  ...
  /**
   * Creates <tt>folds</tt> data sets that contain data from this data set. The data points in each set will be random.
   * These are meant for cross validation
   *
  ...
  /**
   * Creates a list containing the same DataPoints in this set. They are soft copies, in the same order as this data
   * set. However, altering this list will have no effect on DataSet. Altering the DataPoints in the list will effect
   * the DataPoints in this DataSet.
   *
  ...
  /**
   * The data set can be seen as a NxM matrix, were each row is a data point, and each column the values for a
   * particular variable. This method grabs all the numerical values for a 'column' and returns it as one vector. <br>
  ...
  /**
   * Creates an array of column vectors for every numeric variable in this data set. The index of the array corresponds
   * to the numeric feature index. This method is faster and more efficient than calling {@link #getNumericColumn(int) }
   * when multiple columns are needed. <br>
   * <br>
   * Note, that the columns returned by this method may be cached and re used by the DataSet itself. If you need to
   * alter the columns you should create your own copy of these vectors. If you know that you will be the only person
   * getting a column vector from this data set, then you may safely alter the columns without mutating the data points
   * themselves. However, future callers may or may not receive the same vector objects.
   *
   ...
   * when multiple columns are needed. <br>
   * <br>
   * A set of columns to skip can be provided in order to save memory if one does not need all the columns. <br>
   * <br>
   * Note, that the columns returned by this method may be cached and re used by the DataSet itself. If you need to
   * alter the columns you should create your own copy of these vectors. If you know that you will be the only person
   * getting a column vector from this data set, then you may safely alter the columns without mutating the data points
   * themselves. However, future callers may or may not receive the same vector objects.
   *
   * @param skipColumns if a column's index is in this set, a {@code null} will be returned in the array at the column's
   * index instead of a vector
   *
   * @return an array of the column vectors
   */
  ...
  /**
   * Creates a matrix from the data set, where each row represent a data point, and each column is one of the numeric
  ...
  /**
   * Creates a matrix backed by the data set, where each row is a data point from the dataset, and each column is one of
   * the numeric examples from the data set. <br>
   * Any modifications to this matrix will be reflected in the dataset. <br>
   * This method has the advantage over {@link #getDataMatrix() } in that it does not use any additional memory and it
   * maintains any sparsity information.
   *
  ...
  /**
   * Returns a new version of this data set that is of the same type, and contains a different list pointing to the same
   * data points.
   *
  ...
  /**
   * Returns a new version of this data set that is of the same type, and contains a different listing pointing to
   * shallow data point copies. Because the data point object contains the weight itself, the weight is not shared -
   * while the vector and array information is. This allows altering the weights of the data points while preserving the
   * original weights. <br>
   * Altering the list or weights of the returned data set will not be reflected in the original. Altering the feature
   * values will.
   *
  ...
  /**
   * Returns statistics on the sparsity of the vectors in this data set. Vectors that are not considered sparse will be
   * treated as completely dense, even if zero values exist in the data.
   *
