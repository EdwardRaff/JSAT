...
public class NumericalToHistogram implements DataTransform {
  ...
  static public class NumericalToHistogramTransformFactory extends DataTransformFactoryParm {
    ...
    /**
     * Sets the number of numeric bins to use. {@link Integer#MAX_VALUE} is used
     * as a special value to indicate that the square root of the number of data
     * points should be used as the number of bins.
     *
     * @param n
     *          the number of bins to use, or {@link Integer#MAX_VALUE} to use
     *          sqrt(n) bins.
     */
  ...
  private final int n;
  // First index is the vector index, 2nd index is the min value then the
  // increment value
   ...
   *
   * @param dataSet
   *          the data set to create the transform from
   */
  ...
  public NumericalToHistogram(final DataSet dataSet, final int n) {
    ...
    System.arraycopy(dataSet.getCategories(), 0, newDataArray, dataSet.getNumNumericalVars(),
        dataSet.getNumCategoricalVars());
  }
  ...
  private NumericalToHistogram(final NumericalToHistogram other) {
    n = other.n;
    conversionArray = new double[other.conversionArray.length][];
    for (int i = 0; i < other.conversionArray.length; i++) {
      conversionArray[i] = Arrays.copyOf(other.conversionArray[i], other.conversionArray[i].length);
    }
    newDataArray = new CategoricalData[other.newDataArray.length];
    for (int i = 0; i < other.newDataArray.length; i++) {
      newDataArray[i] = other.newDataArray[i].clone();
    }
  }
