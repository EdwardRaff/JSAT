...
/**
 * This transform converts numerical features into categorical ones via a simple histogram. Bins will be created for
 * each numeric feature of equal sizes. Each numeric feature will be converted to the same number of bins.
 *
 * @author Edward Raff
 */
public class NumericalToHistogram implements DataTransform {
  ...
  /**
   * Creates a new transform which will use O(sqrt(n)) bins for each numeric feature, where <i>n</i> is the number of
  ...
  static public class NumericalToHistogramTransformFactory extends DataTransformFactoryParm {
    ...
    /**
     * Sets the number of numeric bins to use. {@link Integer#MAX_VALUE} is used as a special value to indicate that the
     * square root of the number of data points should be used as the number of bins.
     *
     * @param n the number of bins to use, or {@link Integer#MAX_VALUE} to use sqrt(n) bins.
     */
