...
/**
 * Dense sparce transform alters the vectors that store the numerical values. Based on a threshold in (0, 1), vectors
 * will be converted from dense to sparce, sparce to dense, or left alone.
 *
 * @author Edward Raff
 */
public class DenseSparceTransform implements DataTransform {
  ...
  /**
   * Creates a new Dense Sparce Transform. The <tt>factor</tt> gives the maximal percentage of values that may be non
   * zero for a vector to be sparce. Any vector meeting the requirement will be converted to a sparce vector, and others
   * made dense. If the factor is greater than or equal to 1, then all vectors will be made sparce. If less than or
   * equal to 0, then all will be made dense.
   *
