...
public class DenseSparceTransform implements DataTransform {
   ...
   * vectors will be made sparce. If less than or equal to 0, then all will be
   * made dense.
   *
   * @param factor
   *          the fraction of the vectors values that may be non zero to qualify
   *          as sparce
   */
  ...
  public DataPoint transform(final DataPoint dp) {
    ...
    // Else, to sparce
    final SparseVector sv = new SparseVector(orig.length(), nnz);// TODO create
                                                                 // a
                                                                 // constructor
                                                                 // for this
    for (int i = 0; i < orig.length(); i++) {
