...
/**
 * Provides an implementation of the FastICA algorithm for Independent Component Analysis (ICA). ICA is similar to PCA
 * and Whitening, but assumes that the data is generated from a mixture of some <i>C</i> base components where mixing
 * occurs instantaneously (i.e. produced from some matrix transform of the true components). ICA attempts to find the
 * <i>C</i> components from the raw observations. <br>
 * <br>
 * See:
 * <ul>
 * <li>Hyvärinen, A. (1999). <i>Fast and robust fixed-point algorithms for independent component analysis</i>. IEEE
 * Transactions on Neural Networks / a Publication of the IEEE Neural Networks Council, 10(3), 626–34.
 * doi:10.1109/72.761722
 * </li>
 * <li>
 * Hyvärinen, a.,&amp;Oja, E. (2000). <i>Independent component analysis: algorithms and applications</i>. Neural
 * Networks, 13(4-5), 411–430. doi:10.1016/S0893-6080(00)00026-5
 * </li>
 * </ul>
 *
 * @author Edward Raff
...
public class FastICA implements InvertibleTransform {
  ...
  /**
   * The FastICA algorithm requires a function f(x) to be used iteratively in the algorithm, but only makes use of the
   * first and second derivatives of the algorithm.
   */
  public static interface NegEntropyFunc {
     ...
     *
     * @param x the input to the function
     * @param d1 the first derivative of this function (from {@link #deriv1(double) })
  ...
  public enum DefaultNegEntropyFunc implements NegEntropyFunc {
    /**
     * This is function <i>G<sub>1</sub></i> in the paper. This Negative Entropy function is described as a "good
     * general-purpose contrast function" in the original paper, and the default method used.
     */
    LOG_COSH {
    ...
    /**
     * This is function <i>G<sub>2</sub></i> in the paper, and according to the paper may be better than
     * {@link #LOG_COSH} "when the independent components are highly super-Gaussian, or when robustness is very
     * important"
     */
    EXP {
      @Override
    ...
    /**
     * This is the kurtosis-based approximation function <i>G<sub>3</sub>(x) = 1/4*x<sup>4</sup></i>. According to the
     * original paper its use is "is justified on statistical grounds only for estimating sub-Gaussian independent
   ...
   * @param preWhitened {@code true} to assume the data has already been whitened before being given to the transform,
   * {@code false} and the FastICA implementation will perform its own whitening.
   */
