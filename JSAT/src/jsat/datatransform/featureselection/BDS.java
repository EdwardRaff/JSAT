...
/**
 * Bidirectional Search (BDS) is a greedy method of selecting a subset of features to use for prediction. It performs
 * both {@link SFS} and {@link SBS} search at the same time. At each step, a feature is greedily added to one set, and
 * then a feature greedily removed from another set. Once a feature is added / removed in one set, it is unavailable for
 * selection in the other. This can be used to select up to half of the original features.
 *
 * @author Edward Raff
 */
public class BDS implements DataTransform {
   ...
   * @param evaluator the classifier to use in determining accuracy given a feature subset
   ...
   * @param featureCount the number of features to select
   * @param dataSet the data set to perform feature selection on
   * @param evaluator the regressor to use in determining accuracy given a feature subset
   * @param folds the number of cross validation folds to use in selection
   */
