...
/**
 * Performs greedy feature selection based on Mutual Information of the features with respect to the class values. This
 * is an attempt to select features that are discriminative for classification tasks. <br>
 * The method of performing Mutual Information on numeric attributes is controlled by {@link NumericalHandeling}
 *
 * @author Edward Raff
 */
public class MutualInfoFS extends RemoveAttributeTransform {
  ...
  /**
   * The definition for mutual information for continuous attributes requires an integration of an unknown function, as
   * such requires some form of approximation. This controls how the approximation is done
   */
  public enum NumericalHandeling {
    /**
     * Mutual information for numeric attributes is not computed, so no numeric attributes will be removed - and are
     * ignored completely. The number of features to select does not include the numeric attributes in this case.
     */
    NONE,
    /**
     * Numeric attributes are treated as nominal features with binary values. The false value is if the value is zero,
     * and the true value is any non zero value.
     */
    BINARY,
  }

  /**
   * Creates a new Mutual Information feature selection object. Numeric attributes are handled by
   * {@link NumericalHandeling#BINARY}
   *
   * @param dataSet the classification data set to perform feature selection from
   ...
   * @param dataSet the classification data set to perform feature selection from
   * @param featureCount the number of features to select
   * @param numericHandling the way to handle the computation of mutual information for numeric attributes
   */
  ...
  public static class MutualInfoFSFactory extends DataTransformFactoryParm {
    ...
    /**
     * Creates a new MutalInfoFS factory that uses {@link NumericalHandeling#BINARY} handling for numeric attributes.
     *
     * @param featureCount the number of features to select
     */
