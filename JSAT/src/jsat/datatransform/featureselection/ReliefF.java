...
public class ReliefF extends RemoveAttributeTransform {
   ...
   *
   * @param cds
   *          the data set to measure numeric variable importance from
   * @param featureCount
   *          the number of features to keep
   * @param m
   *          the number of learning iterations to perform
   ...
   *          the distance metric to use
   */
  public ReliefF(final ClassificationDataSet cds, final int featureCount, final int m, final int n,
   ...
   *          the distance metric to use
   * @param threadPool
   *          the source of threads to use for the computation
   */
  public ReliefF(final ClassificationDataSet cds, final int featureCount, final int m, final int n,
      final DistanceMetric dm, final ExecutorService threadPool) {
   ...
   */
  public ReliefF(final ClassificationDataSet cds, final int featureCount, final int m, final int n,
      final DistanceMetric dm, final VectorCollectionFactory<Vec> vcf) {
   ...
   */
  public ReliefF(final ClassificationDataSet cds, final int featureCount, final int m, final int n,
      final DistanceMetric dm, final VectorCollectionFactory<Vec> vcf, ExecutorService threadPool) {
    super();
    w = new double[cds.getNumNumericalVars()];
      ...
      threadPool.submit(new Runnable() {
        ...
        public void run() {
          ...
          for (int iter = 0; iter < mm; iter++) {
            ...
            {
              final int searchFor = y == y_k ? n + 1 : n;// +1 so we dont search
                                                         // for ourselves
              List<? extends VecPaired<Vec, Double>> nNearestC = classVC.get(y).search(x_k, searchFor);
              if (searchFor != n) {
                nNearestC = nNearestC.subList(1, searchFor);// chop off the
                                                            // first value which
                                                            // is ourselves
              }
  ...
  protected ReliefF(final ReliefF toCopy) {
    super(toCopy);
    if (toCopy.w != null) {
      w = Arrays.copyOf(toCopy.w, toCopy.w.length);
    }
  }
