...
/**
 * plus-L minus-R Selection (LRS) is a greedy method of selecting a subset of features to use for prediction. Its
 * behavior is dependent upon whether L or R is the larger value. No mater what, L features will be greedily added to
 * the set to decrease the error rate, and R features will be greedily removed while trying to maintain the error rate.
 * <br>
 * If L &gt; R, then L-R features will be selected, the L step running first followed by R performing pruning on the
 * found set. <br>
 * If L &lt; R, then D-R+L features will be selected, where D is the original number of features. First R features will
 * be removed, and then L of the removed features will be added back to the final set. <br>
 * L = R is not allowed.
 *
 * @author Edward Raff
 */
public class LRS implements DataTransform {
   ...
   * @param evaluater the regressor to use in determining accuracy given a feature subset
   * @param folds the number of cross validation folds to use in selection
   */
  ...
  static public class LRSFactory extends DataTransformFactoryParm {
     ...
     * <b>NOTE:</b> setting this and {@link #setFeaturesToRemove(int) } is allowed for the Factory, but is is assumed
     * that it is occurring because you are about to change the value of the other. Attempting to obtain a {@link LRS}
     * transform will result in a runtime exception until one of the values is changed.
     *
     ...
     * <b>NOTE:</b> setting this and {@link #setFeaturesToAdd(int) } is allowed for the Factory, but is is assumed that
     * it is occurring because you are about to change the value of the other. Attempting to obtain a {@link LRS}
     * transform will result in a runtime exception until one of the values is changed.
     *
