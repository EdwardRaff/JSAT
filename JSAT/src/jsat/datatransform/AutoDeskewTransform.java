...
/**
 * This transform applies a shifted Box-Cox transform for several fixed values of &lambda;, and selects the one that
 * provides the greatest reduction in the skewness of the distribution. This is done in an attempt to make the
 * individual features appear more normal. The shifted values are done to preserve zeros and keep sparse inputs sparse.
 * This is done with two passes through the data set, but requires only O(D #&lambda; values) memory.
 * <br><br>
 * The default values of &lambda; are -1, -1/2, 0, 1/2, 1. When using negative &lambda; values all zeros are skipped and
 * left as zeros. &lambda; = 1 is an implicit value that is always included regardless of the input, as it is equivalent
 * to leaving the data unchanged when preserving zero values. The stated default values include the <i>log(x+1)</i> and
 * <i>sqrt(x)</i>
...
public class AutoDeskewTransform implements InPlaceTransform {
   ...
   *
   * @param dataSet the data set to deskew
   * @param ignorZeros {@code true} to ignore zero values when calculating the skewness, {@code false} to include them.
   * @param lambdas the list of lambda values to evaluate
   */
