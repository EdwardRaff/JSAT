...
/**
 * Performing a transform on the whole data set before training a classifier can add bias to the results. For proper
 * evaluation, the transforms must be learned from the training set and not contain any knowledge from the testing set.
 * A DataTransformProcess aids in this by providing a mechanism to contain several different transforms to learn and
 * then apply.
 * <br><br>
 * The Parameters of the Data Transform Process are the parameters from the individual transform factories that make up
 * the whole process. The name "DataTransformProcess" will not be prefixed to the parameter names.
 *
 * @author Edward Raff
 */
public class DataTransformProcess implements DataTransform, Parameterized {
  ...
  /**
   * Creates a new transform process that is empty. Transform factories must be added using 
  ...
  /**
   * Adds a transform to the list of transforms. Transforms are learned and applied in the order in which they are
   * added.
   *
  ...
  /**
   *
   * @return the number of transforms currently chained in this transform process
   */
  ...
  /**
   * Learns the transforms for the given data set. The data set will not be altered. Once finished, <tt>this</tt>
   * DataTransformProcess can be applied to the dataSet to get the transformed data set.
   *
  ...
  /**
   * Learns the transforms for the given data set. The data set is then altered after each transform is learned so the
   * next transform can be learned as well. <br> The results are equivalent to calling 
     * {@link #learnApplyTransforms(jsat.DataSet) } on the data set and then calling {@link DataSet#applyTransform(jsat.datatransform.DataTransform)
   * }
   * with this DataTransformProces.
   *
   * @param dataSet the data set to learn a series of transforms from and alter into the final transformed form
   */
