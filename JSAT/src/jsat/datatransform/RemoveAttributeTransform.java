...
public class RemoveAttributeTransform implements DataTransform {
  ...
  /**
   * Empty constructor that may be used by extending classes. Transforms that extend this will need to call null   {@link #setUp(jsat.DataSet, java.util.Set, java.util.Set)
  ...
  /**
   * Returns an unmodifiable list of the original indices of the numeric attributes that will be kept when this
  ...
  /**
   * Returns an unmodifiable list of the original indices of the nominal attributes that will be kept when this
   * transform is applied.
   *
  ...
  /**
   * A serious of Remove Attribute Transforms may be learned and applied sequentially to a single data set. Instead of
   * keeping all the transforms around indefinitely, a sequential series of Remove Attribute Transforms can be
   * consolidated into a single transform object. <br>
   * This method mutates the this transform by providing it with the transform that would have been applied before this
   * current object. Once complete, this transform can be used two perform both removals in one step.<br><br>
   * Example: <br>
   * An initial set of features <i>A</i> is transformed into <i>A'</i> by transform t<sub>1</sub><br>
   * <i>A'</i> is transformed into <i>A''</i> by transform t<sub>2</sub><br>
   * Instead, you can invoke t<sub>2</sub>.consolidate(t<sub>1</sub>). You can then transform <i>A</i> into <i>A''</i>
   * by using only transform t<sub>2</sub>
   *
   *
   * @param preceding the DataTransform that immediately precedes this one in a sequential list of transforms
   */
