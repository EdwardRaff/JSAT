...
/**
 * Principle Component Analysis is a method that attempts to create a basis of the given space that maintains the
 * variance in the data set while eliminating correlation of the variables.
 * <br>
 * When a full basis is formed, the dimensionality will remain the same, but the data will be transformed to a new
 * space. <br>
 * PCA is particularly useful when a small number of basis can explain most of the variance in the data set that is not
 * related to noise, maintaining information while reducing the dimensionality of the data.
 * <br><br>
 * PCA works only on the numerical attributes of a data set. <br>
 * For PCA to work correctly, a {@link ZeroMeanTransform} should be applied to the data set first. If not done, the
 * first dimension of PCA may contain noise and become uninformative, possibly throwing off the computation of the other
 * PCs
 *
 * @author Edward Raff
...
public class PCA implements DataTransform {
   ...
   * number of PCs computed will be equal to the number of dimensions, meaning no dimensionality reduction has occurred,
   * but a transformation of the dimensions into a new space.
   *
   ...
   *
   * @param dataSet the data set to learn from
   * @param maxPCs the maximum number of Principal Components to let the algorithm learn. The algorithm may stop earlier
   * if all the variance has been explained, or the convergence threshold has been met. Note, the computable maximum
   * number of PCs is limited to the minimum of the number of samples and the number of dimensions.
   */
   ...
   * if all the variance has been explained, or the convergence threshold has been met. Note, the computable maximum
   * number of PCs is limited to the minimum of the number of samples and the number of dimensions.
   * @param threshold a convergence threshold, any small value will work. Smaller values will not produce more accurate
   * results, but may make the algorithm take longer if it would have terminated before <tt>maxPCs</tt> was reached.
   */
