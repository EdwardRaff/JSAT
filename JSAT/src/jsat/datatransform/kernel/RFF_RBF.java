...
public class RFF_RBF implements DataTransform {
  ...
  static public class RFF_RBFTransformFactory extends DataTransformFactoryParm {
     ...
     *
     * @param sigma
     *          the sigma value for the {@link RBFKernel}
     * @param dimensions
     *          the new feature size dimension to project into.
     * @param inMemory
     *          {@code true} if the internal matrix should be stored in memory.
     *          If {@code false}, the memory will be re-computed as needed,
     ...
     *
     * @param sigma
     *          the positive value to use for &sigma;
   ...
   *
   * @param featurSize
   *          the number of numeric features in the original feature space
   * @param sigma
   *          the positive sigma value for the {@link RBFKernel}
   * @param dim
   *          the new feature size dimension to project into.
   * @param rand
   *          the source of randomness to initialize internal state
   * @param inMemory
   *          {@code true} if the internal matrix should be stored in memory. If
   *          {@code false}, the memory will be re-computed as needed,
   *          increasing computation cost but uses no extra memory.
   */
  public RFF_RBF(final int featurSize, final double sigma, final int dim, final Random rand, final boolean inMemory) {
    ...
    if (inMemory) {
      transform = transform.add(0.0);// will copy into a new mutable and add
                                     // nothing
