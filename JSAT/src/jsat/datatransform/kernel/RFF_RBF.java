...
/**
 * An Implementation of Random Fourier Features for the {@link RBFKernel}. It transforms the numerical variables of a
 * feature space to form a new feature space where the dot product between features approximates the RBF Kernel product.
 * <br><br>
 * See: Rahimi, A.,&amp;Recht, B. (2007). <i>Random Features for Large-Scale Kernel Machines</i>. Neural Information
...
public class RFF_RBF implements DataTransform {
   ...
   *
   * @param featurSize the number of numeric features in the original feature space
   ...
   * @param inMemory {@code true} if the internal matrix should be stored in memory. If {@code false}, the memory will
   * be re-computed as needed, increasing computation cost but uses no extra memory.
   */
  ...
  static public class RFF_RBFTransformFactory extends DataTransformFactoryParm {
    ...
    /**
     * Sets the number of dimensions in the new approximate space to use. This will be the number of numeric features in
     * the transformed data, and larger values increase the accuracy of the approximation.
     *
     * @param dimensions
     */
    ...
    /**
     * Guess the distribution to use for the kernel width term {@link #setSigma(double) &sigma;} in the RBF kernel being
     * approximated.
     *
