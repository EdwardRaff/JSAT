...
/**
 * A kernelized implementation of {@link PCA}. Because this works in a different feature space, it will do its own
 * centering in the kernel space.
 * <br><br>
 * KernelPCA is expensive to compute at O(n<sup>3</sup>) work, where <i>n</i> is the number of data points. For this
 * reason, sampling from {@link Nystrom} is used to reduce the data set to a reasonable approximation.
 * <br><br>
 * See: Schölkopf, B., Smola, A.,&amp;Müller, K.-R. (1998). <i>Nonlinear Component Analysis as a Kernel Eigenvalue
 * Problem</i>. Neural Computation, 10(5), 1299–1319. doi:10.1162/089976698300017467
 *
 * @author Edward Raff
...
public class KernelPCA implements DataTransform {
   ...
   * @param dimensions the number of dimensions to project down to. Must be less than than the basis size
   * @param basisSize the number of points from the data set to select. If larger than the number of data points in the
   * data set, the whole data set will be used.
  ...
  static public class KernelPCATransformFactory extends DataTransformFactoryParm {
     ...
     * @param basisSize the number of points from the data set to select. If larger than the number of data points in
     * the data set, the whole data set will be used.
     * @param samplingMethod the sampling method to select the basis vectors
     */
    ...
    /**
     * Sets the basis size for the Kernel PCA to be learned from. Increasing the basis increase the accuracy of the
     * transform, but increased the training time at a cubic rate.
     *
