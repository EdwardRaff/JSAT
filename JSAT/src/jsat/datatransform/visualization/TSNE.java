...
public class TSNE {
  ...
  public <Type extends DataSet> Type transform(DataSet<Type> d) {
    ...
    /**
     * form sec 4.1: "we compute the sparse approximation by finding the floor(3u) nearest neighbors of each of the N
     * input objects (recall that u is the perplexity of the conditional distributions)"
     */
    ...
    /**
     * P_ij does not change at this point, so lets compute these values only once please! j index matches up to the
     * value stored in nearMe
     */
...
//                double cnst = 1*4;
   ...
   * @param i
   * @param z
   * @param workSpace the indicies are the accumulated contribution to the gradient sans multiplicative terms in the
   * first 2 indices.
