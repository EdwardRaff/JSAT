...
/**
 * Provides a generic interface for some loss function on some problem that can be described with a single real
 * prediction value and a single real expected value.
 * <br><br>
...
public interface LossFunc extends Serializable {
  ...
  /**
   * Returns an upper bound on the maximum value of the second derivative. If the second derivative does not exist,
   * {@link Double#NaN} is a valid result. It is also possible for {@code 0} and {@link Double#POSITIVE_INFINITY} to be
   * valid results, and must be checked for.
   *
