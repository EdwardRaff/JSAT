...
/**
 * Specifies a loss function for multi-class problems. A multi-class loss function must support a raw vector of scores
 * for each class, where positive values indicate preference for the class associated with the same index. <br>
 * <br>
 * Calling {@link #process(jsat.linear.Vec, jsat.linear.Vec) } on the raw scores is a mandatory first step, and will
 * transform the raw scores into a usable form for the loss function. <br>
 * <br>
 *
 * @author Edward Raff
 */
public interface LossMC extends LossC {
   ...
   * <br> {@code processed} and {@code derivs} may be the same object, and will simply have all its values altered if
   * so.
   *
  ...
  /**
   * Given the {@link #process(jsat.linear.Vec, jsat.linear.Vec) processed} predictions, returns the classification
   * results for said predictions.
   *
