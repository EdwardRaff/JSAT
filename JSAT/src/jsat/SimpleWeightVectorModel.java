...
public interface SimpleWeightVectorModel {
   ...
   * index. If the model does not support or was not trained with bias weights,
   * {@code 0} will be returned.<br>
   * <br>
  ...
  /**
   * Returns the raw weight vector associated with the given class index. If the
   * given class is an implicit zero vector, a {@link ConstantVector} object may
   * be returned. <br>
   ...
   * classification problems the value may be 1 if only a single weight vector's
   * sign is used to determine the class. For multi-class problems, the weight
   * vector count includes the implicit zero vector (if one is being used).
   *
   * @return the number of weight vectors for which {@link #getRawWeight(int) }
   *         can be called.
   */
