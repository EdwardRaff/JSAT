...
public class RandomSearch extends ModelSearch {
  ...
  public RandomSearch(final RandomSearch toCopy) {
    super(toCopy);
    trials = toCopy.trials;
    searchValues = new ArrayList<Distribution>(toCopy.searchValues.size());
    for (final Distribution d : toCopy.searchValues) {
      searchValues.add(d.clone());
    }
  }
   ...
   *          the distribution to sample from for this parameter
   */
   ...
   *
   * @param trials
   *          the number of models to build and evaluate
   */
  ...
  public void train(final RegressionDataSet dataSet, final ExecutorService threadPool) {
    ...
    for (final Regressor r : paramsToEval) {
      modelService.submit(new Runnable() {
        @Override
        public void run() {
          final RegressionModelEvaluation cme = trainModelsInParallel ? new RegressionModelEvaluation(r, dataSet)
