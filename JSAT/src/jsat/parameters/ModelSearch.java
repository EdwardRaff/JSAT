...
/**
 * This abstract class provides boilerplate for algorithms that search a model's parameter space to find the parameters
 * that provide the best overall performance.
 *
 * @author Edward Raff
 */
abstract public class ModelSearch implements Classifier, Regressor {
  ...
  /**
   * If true, parallelism will be obtained by training the models in parallel. If false, parallelism is obtained from
   * the model itself.
   */
  ...
  /**
   * When set to {@code true} (the default) parallelism is obtained by training as many models in parallel as possible.
   * If {@code false}, parallelsm will be obtained by training the model using the {@link Classifier#trainC(jsat.classifiers.ClassificationDataSet, java.util.concurrent.ExecutorService)
   * } and {@link Regressor#train(jsat.regression.RegressionDataSet, java.util.concurrent.ExecutorService)
   * } methods.<br>
   * <br>
   * When a model supports {@link #setUseWarmStarts(boolean) warms starts}, parallelism obtained by training the models
   * in parallel is intrinsically reduced, as a model can not be warms started until another model has finished. In the
   * case that one of the parameters is annotated as a {@link Parameter.WarmParameter warm paramter} , that parameter
   * will be the one rained sequential, and for every other parameter combination models will be trained in parallel. If
   * there is no warm parameter, the first parameter added will be used for warm training. If there is only one
   * parameter and warm training is occurring, no parallelism will be obtained.
   *
   * @param trainInParallel {@code true} to get parallelism from training many models at the same time, {@code false} to
   * get parallelism from getting the model's implicit parallelism.
   */
  ...
  /**
   *
   * @return {@code true} if parallelism is obtained from training many models at the same time, {@code false} if
   * parallelism is obtained from using the model's implicit parallelism.
   */
  ...
  /**
   * If {@code true} (the default) the model that was found to be best is trained on the whole data set at the end. If
   * {@code false}, the final model will not be trained. This means that this Object will not be usable for predictoin.
   * This should only be set if you know you will not be using this model but only want to get the information about
   * which parameter combination is best.
   *
   * @param trainFinalModel {@code true} to train the final model after grid search, {@code false} to not do that.
   */
  ...
  /**
   *
   * @return {@code true} to train the final model after grid search, {@code false} to not do that.
   */
  ...
  /**
   * Sets whether or not one set of CV folds is created and re used for every parameter combination (the default), or if
   * a difference set of CV folds will be used for every parameter combination.
   *
   * @param reuseSameSplit {@code true} if the same split is re-used for every combination, {@code false} if a new CV
   * set is used for every parameter combination.
   */
  ...
  /**
   *
   * @return {@code true} if the same split is re-used for every combination, {@code false} if a new CV set is used for
  ...
  /**
   * Returns the base classifier that was originally passed in when constructing this GridSearch. If this was not
   * constructed with a classifier, this may return null.
   *
   ...
   *
   * @return the trained classifier after a call to null null null   {@link #train(jsat.regression.RegressionDataSet, 
     * java.util.concurrent.ExecutorService) }, or null if it has not been trained.
   */
  ...
  /**
   * Returns the base regressor that was originally passed in when constructing this GridSearch. If this was not
   * constructed with a regressor, this may return null.
   *
   ...
   *
   * @return the trained regressor after a call to null null null   {@link #train(jsat.regression.RegressionDataSet, 
     * java.util.concurrent.ExecutorService) }, or null if it has not been trained.
   */
   ...
   *
   * @return the classification score that is trying to be optimized via grid search
   */
  ...
  /**
   * Finds the parameter object with the given name, or throws an exception if a parameter with the given name does not
   * exist.
   *
