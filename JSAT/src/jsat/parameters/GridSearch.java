...
/**
 * GridSearch is a simple method for tuning the parameters of a classification or regression algorithm. It naively tries
 * all possible pairs of parameter values given. For this reason, it works best when only a small number of parameters
 * need to be turned. <br>
 * The model it takes must implement the {@link Parameterized} interface. By default, no parameters are selected for
 * optimizations. This is because parameters value ranges are often algorithm specific. As such, the user must specify
 * the parameters and the values to test using the <tt>addParameter</tt>
 * methods.
 *
 * @author Edward Raff
 *
...
public class GridSearch extends ModelSearch {
  ...
  /**
   * The matching list of values we will test. This includes the integer parameters, which will have to be cast back and
   * forth from doubles.
   */
  ...
  /**
   * Creates a new GridSearch to tune the specified parameters of a regression model. The parameters still need to be
   * specified by calling {@link #addParameter(jsat.parameters.DoubleParameter, double[]) }
   *
   * @param baseRegressor the regressor to tune the parameters of
   * @param folds the number of folds of cross-validation to perform to evaluate each combination of parameters
   * @throws FailedToFitException if the base regressor does not implement {@link Parameterized}
   */
  ...
  /**
   * Creates a new GridSearch to tune the specified parameters of a classification model. The parameters still need to
   * be specified by calling {@link #addParameter(jsat.parameters.DoubleParameter, double[]) }
   *
   * @param baseClassifier the classifier to tune the parameters of
   * @param folds the number of folds of cross-validation to perform to evaluate each combination of parameters
   * @throws FailedToFitException if the base classifier does not implement {@link Parameterized}
   */
  ...
  /**
   * This method will automatically populate the search space with parameters based on which Parameter objects return
   * non-null distributions. Each parameter will be tested with 10 different values<br>
   * <br>
   * Note, using this method with Cross Validation has the potential for over-estimating the accuracy of results if the
   * data set is actually used to for parameter guessing.
   *
  ...
  /**
   * Sets whether or not warm starts are used, but only if the model in use supports warm starts. This is set to
   * {@code true} by default.
   *
   * @param useWarmStarts {@code true} if warm starts should be used when possible, {@code false} otherwise.
   */
  ...
  /**
   *
   * @return {@code true} if warm starts will be used when possible. {@code false} if they will not.
   */
  ...
  public void train(final RegressionDataSet dataSet, final ExecutorService threadPool) {
    ...
    /**
     * Use this to keep track of which parameter we are altering. Index correspondence to the parameter, and its value
     * corresponds to which value has been used. Increment and carry counts to iterate over all possible combinations.
     */
    int[] setTo = new int[searchParams.size()];
    /**
     * Each model is set to have different combination of parameters. We then train each model to determine the best
     * one.
     */
    ...
    /**
     * make sure we don't do a warm start if its only supported when trained on the same data but we aren't reuse-ing
     * the same CV splits So we get the truth table
     *
     * a | b | (a&&b)||¬a T | T | T T | F | F F | T | T F | F | T
     *
     * where a = warmFromSameDataOnly and b = reuseSameSplit So we can instead use ¬ a || b
     */
   ...
   *
   * @param setTo the array of length equal to {@link #searchParams} indicating which parameters have already been tried
   * @return a boolean indicating <tt>true</tt> if all combinations have been tried, or <tt>false</tt> if combinations
   * remain to be attempted.
   */
   ...
   *
   * @param setTo the index corresponds to the parameters, and the value which parameter value to use.
   */
