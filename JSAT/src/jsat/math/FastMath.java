...
public class FastMath {
  /*
   * Exponent biast for doubles is 1023 so exponentValue-1023 = unbiased value
   */
  ...
  public static double digamma(final double x) {
     ...
     * log(x+2)-1/(2 (x+2))-1/(12 (x+2)^2) -1/x-1/(x+1), the x+2 and x and x+1
     * are grouped sepratly
     */
   ...
   * The results are generally accurate to an absolute and relative error of 10
   * <sup>-4</sup>, but relative error can get as high as 10<sup>-10</sup>
   *
   * @param x
   *          the input
   * @return the log base 2 of {@code x}
  ...
  public static double pow(final double a, final double b) {
     ...
     * 2^(b * log2(m*2^e)) which simplifies to m^b 2^(b e) when m, e, and b are
     * positive.
     * 
    ...
    // compute m^b and exploit the fact that we know there is no need for the
    // exponent
    final double m = longBitsToDouble(1023L << 52 | mantissa_a);// m in [1, 2]

    final double log2m = 1.790711564253215 + 0.248597253161674 * m
        - 3.495545043418375 / (0.714309275671154 + 1.000000000000000 * m);

    // we end up with 2^(b * log_2(m)) * 2^(b * e), which we can reduce to a
    // single pow2 call
    return pow2(b * log2m + b * e_a);// fun fact, double*int is faster than
                                     // casting an int to a double...
  }

  /**
   * Computes 2<sup>x</sup>.<br>
   * The results are generally accurate to an relative error of 10<sup>-4</sup>,
   * but can be as accurate as 10<sup>-10</sup>
   *
   * @param x
  ...
  public static double pow2(final double x) {
    ...
    final double pow2frac = -4.704682932438695 + 27.543765058113320 / (4.828085122666891 - frac)
        - 0.490129071734273 * frac;
   ...
   *
   * @param x
   *          the integer power to raise 2 too
   * @return 2<sup>x</sup>
   */
