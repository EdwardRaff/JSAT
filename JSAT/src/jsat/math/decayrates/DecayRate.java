...
/**
 * Many algorithms use a learning rate to adjust the step size by which the search space is covered. In practice, it is
 * often useful to reduce this learning rate over time. In this way, large steps can be taken in the beginning when we
 * are far from the solution, and smaller steps when we have gotten closer to the solution and do not want to step too
 * far away.
 *
 * @author Edward Raff
 */
public interface DecayRate extends Serializable {
   ...
   *
   * @param time the current time through the algorithm in the range [0, <tt>maxTime</tt>]
