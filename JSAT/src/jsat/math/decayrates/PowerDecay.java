...
/**
 *
 * Decays an input by power of the amount of time that has occurred, the max time being irrelevant. More specifically as
 * &eta; ({@link #setTau(double) &tau;} + time)<sup>-{@link #setAlpha(double) &alpha;}</sup><br>
 * <br> {@link InverseDecay} is a special case of this decay when &alpha;=1. <br> {@link NoDecay} is a special case of
 * this decay when &alpha; = 0 <br>
 *
 * @author Edward Raff
 */
public class PowerDecay implements DecayRate, Parameterized {
  ...
  /**
   * Controls the scaling via exponentiation, increasing &alpha; increases the rate at which the rate decays. As &alpha;
   * goes to zero, the decay rate goes toward zero (meaning the value returned becomes constant),
   *
   * @param alpha the scaling parameter in [0, &infin;), but should generally be kept in (0, 1).
   */
  ...
  /**
   * Controls the rate early in time, but has a decreasing impact on the rate returned as time goes forward. Larger
   * values of &tau; dampen the initial rates returned, while lower values let the initial rates start higher.
   *
