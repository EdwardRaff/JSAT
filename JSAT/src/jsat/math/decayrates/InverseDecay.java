...
/**
 *
 * Decays an input by the inverse of the amount of time that has occurred, the max time being irrelevant. More
 * specifically as &eta; / ({@link #setAlpha(double) &alpha;}({@link #setTau(double) &tau;} + time))<br>
 * <br>
 * This decay rate can be used to create the same rate used by {@link Pegasos}, by using an initial rate of 1 and
 * setting &tau; = 1 and &alpha; = &lambda;, where &lambda; is the regularization term used by the method calling the
 * decay rate.
 *
 * @author Edward Raff
 */
public class InverseDecay implements DecayRate, Parameterized {
  ...
  /**
   * Controls the scaling of the divisor, increasing &alpha; dampens the whole range of values. Increasing it increases
   * the values. value.
   *
