...
/**
 * This implements the Modified Orthant-Wise Limited memory Quasi-Newton(mOWL-QN) optimizer. This algorithm is an
 * extension of {@link LBFGS}, and solves minimization problems of the form: f(x) + {@link #setLambda(double) &lambda;}
 * ||x||<sub>1</sub>. It requires the function and it's gradient to work.  <br>
 * <br>
 * See:<br>
 * <ul>
 * <li>Gong, P., & Ye, J. (2015). <i>A Modified Orthant-Wise Limited Memory Quasi-Newton Method with Convergence
 * Analysis</i>. In The 32nd International Conference on Machine Learning (Vol. 37).</li>
 * <li>Andrew, G., & Gao, J. (2007). <i>Scalable training of L1 -regularized log-linear models</i>. In Proceedings of
 * the 24th international conference on Machine learning - ICML ’07 (pp. 33–40). New York, New York, USA: ACM Press.
...
public class ModifiedOWLQN implements Optimizer2 {
  ...
  /**
   * This method sets a vector that will contain a separate multiplier for {@link #setLambda(double) lambda} for each
   * dimension of the problem. This allows for each dimension to have a different regularization penalty.<br>
   * <br>
   * If set to {@code null}, all dimensions will simply use &lambda; as their regularization value.
   *
  ...
  /**
   * Sets the epsilon term that helps control when the gradient descent step is taken instead of the normal Quasi-Newton
   * step. Larger values cause more GD steps. You shouldn't need to alter this variable
   *
