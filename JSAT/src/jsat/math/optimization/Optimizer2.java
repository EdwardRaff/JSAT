...
public interface Optimizer2 {
   ...
   * that will minimize the value returned by {@code f(w)}, using <i>w = x
   * <sub>0</sub></i> as an initial starting point.
   *
   * @param tolerance
   *          the value that the gradient norm must be less than to consider
   *          converged
   * @param w
   *          the the location to store the final solution
   * @param x0
   *          the initial guess for the solution. This value will not be
   *          changed, and intermediate matrices will be created as the same
   *          type.
   * @param f
   *          the objective function to minimizer
   * @param fp
   *          the derivative of the objective function, may be {@code null}
   *          depending on the optimizer
   * @param fpp
   *          the Hessian of the objective function, may be {@code null}
   *          depending on the optimizer
   */
