...
public class AdaDelta implements GradientUpdater {
  ...
  public AdaDelta(final AdaDelta toCopy) {
    ...
    if (toCopy.gSqrd != null) {
      gSqrd = toCopy.gSqrd.clone();
      deltaXSqrt = toCopy.deltaXSqrt.clone();
    }
    biasGSqrd = toCopy.biasGSqrd;
    deltaBiasSqrt = toCopy.deltaBiasSqrt;
  }
  ...
  public double update(final Vec x, final Vec grad, final double eta, final double bias, final double biasGrad) {
    ...
    for (final IndexValue iv : grad) {
      ...
      final double newDeltaX_i = -Math.sqrt((deltaX_i + eps) / (gSqrd_i + eps)) * grad_i;// step
                                                                                         // 5
      x.increment(indx, eta * newDeltaX_i);// step 7
      deltaXSqrt.increment(indx, (1 - rho) / rho * newDeltaX_i * newDeltaX_i);// step
                                                                              // 6,
                                                                              // using
                                                                              // (1-rho)/rho
                                                                              // so
