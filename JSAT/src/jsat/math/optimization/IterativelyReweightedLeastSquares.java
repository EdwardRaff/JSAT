...
/**
 * Provides an implementation of the Iteratively Reweighted Least Squares (IRLS)
 * algorithm for solving certain classes of optimization problems. See
 * <a href="http://en.wikipedia.org/wiki/Iteratively_reweighted_least_squares">
 * Wikepida</a> for more information.
 *
 * @author Edward Raff
 */
public class IterativelyReweightedLeastSquares implements Optimizer {
  ...
  private double iterationStep(final Function f, final Function fd, final Vec vars, final List<Vec> inputs,
      final Vec outputs) {
    Vec delta = null;
    ...
    final LUPDecomposition lupDecomp = new LUPDecomposition(hessian.clone());// We
                                                                             // sent
                                                                             // a
                                                                             // clone
                                                                             // of
  ...
  private double iterationStep(final Function f, final Function fd, final Vec vars, final List<Vec> inputs,
      final Vec outputs, final ExecutorService threadpool) {
    Vec delta = null;
    for (int i = 0; i < inputs.size(); i++) {
      final Vec x_i = inputs.get(i);
      final double y = f.f(x_i);
    ...
    final LUPDecomposition lupDecomp = new LUPDecomposition(hessian.clone(), threadpool);// We
                                                                                         // sent
                                                                                         // a
                                                                                         // clone
                                                                                         // of
  ...
  @Override
  public Vec optimize(final double eps, int iterationLimit, final Function f, final Function fd, final Vec vars,
