...
/**
 * An implementation of the Wolfe Line Search algorithm described by Nocedal and Wright in <i>Numerical Optimization</i>
...
public class WolfeNWLineSearch implements LineSearch {
  ...
  /**
   * Creates a new Wolfe line search with {@link #setC1(double) } set to {@code 1e-4} and {@link #setC2(double) } to
   * {@code 0.9}
   */
  ...
  public enum AlphaInit {
     ...
     * &nabla;f(x<sub>prev</sub>)<sup>T</sup>p<sub>prev</sub>/ &nabla;f(x<sub>cur</sub>)<sup>T</sup>p<sub>cur</sub>
     */
    METHOD1,
    /**
     * Initializes the new &alpha; value via 2( f(x<sub>cur</sub>)-f(x<sub>prev</sub>))/&phi;'(0)
     */
    METHOD2
  }
