...
public class WolfeNWLineSearch implements LineSearch {
  ...
  public enum AlphaInit {
    /**
     * Initializes the new &alpha; value via &alpha;<sub>prev</sub> &nabla;f(x
     * <sub>prev</sub>)<sup>T</sup>p<sub>prev</sub>/ &nabla;f(x<sub>cur</sub>)
     * <sup>T</sup>p<sub>cur</sub>
     */
    METHOD1, /**
              * Initializes the new &alpha; value via 2( f(x<sub>cur</sub>)-f(x
              * <sub>prev</sub>))/&phi;'(0)
              */
    METHOD2
  }

  // default values that make setting in the constructor simple (shouldn't
  // actually use)
   ...
   * @param c2
   *          the <i>curvature condition</i> constant
   */
  ...
  public WolfeNWLineSearch clone() {
    ...
    clone.initMethod = initMethod;
    clone.alpha_prev = alpha_prev;
    clone.f_x_prev = f_x_prev;
    clone.gradP_prev = gradP_prev;
    return clone;
  }
  ...
  @Override
  public double lineSearch(final double alpha_max, final Vec x_k, final Vec x_grad, final Vec p_k, final Function f,
      final FunctionVec fp, double f_x, double gradP, final Vec x_alpha_pk, final double[] fxApRet,
      final Vec grad_x_alpha_pk, final ExecutorService ex) {
    if (Double.isNaN(f_x)) {
    ...
    for (int iter = 1; iter <= 10 && valToUse == 0; iter++) {
      ...
      final double phi_curP = ex != null ? fp.f(x_alpha_pk, grad_x_alpha_pk, ex).dot(p_k)
          : fp.f(x_alpha_pk, grad_x_alpha_pk).dot(p_k);// computed early b/c
      ...
      if (phi_cur > phi0 + c1 * alpha_cur * phi0P || phi_cur >= phi_prev && iter > 1) {
        ...
        valToUse = zoom(alpha_prev, alpha_cur, phi_prev, phi_cur, phi_prevP, phi_curP, phi0, phi0P, x_k, x_alpha_pk,
            p_k, f, fp, fxApRet, grad_x_alpha_pk, ex);
        break;
      }
      // Evaluate φ'(αi );
      ...
      if (phi_curP >= 0) {
        ...
        valToUse = zoom(alpha_cur, alpha_prev, phi_cur, phi_prev, phi_curP, phi_prevP, phi0, phi0P, x_k, x_alpha_pk,
            p_k, f, fp, fxApRet, grad_x_alpha_pk, ex);
        break;
      }
      // Choose αi+1 ∈(αi,αmax);
  ...
  /**
   * Sets the constant used for the <i>curvature condition</i> p<sup>T</sup>
   * &nabla;f(x+&alpha; p) &ge; c<sub>2</sub> p<sup>T</sup>&nabla;f(x)
   *
   * @param c2
   *          the <i>curvature condition</i> constant
   */
   ...
   *          the value of grad_x_alpha_pk
   * @param ex
   *          the value of ex
   * @return the double
   */
  private double zoom(double alphaLow, double alphaHi, double phi_alphaLow, double phi_alphaHigh, double phi_alphaLowP,
      double phi_alphaHighP, final double phi0, final double phi0P, final Vec x, final Vec x_alpha_p, final Vec p,
      final Function f, final FunctionVec fp, final double[] fxApRet, final Vec grad_x_alpha_pk,
    ...
    for (int iter = 0; iter < 10; iter++) {
      ...
      // try cubic interp eq (3.59)
      {
        ...
        alpha_j = alphaHi
            - (alphaHi - alphaLow) * (phi_alphaHighP + d2 - d1) / (phi_alphaHighP - phi_alphaLowP + 2 * d2);
      }
      ...
      final double phi_jP = ex != null ? fp.f(x_alpha_p, grad_x_alpha_pk, ex).dot(p)
          : fp.f(x_alpha_p, grad_x_alpha_pk).dot(p);// computed early
