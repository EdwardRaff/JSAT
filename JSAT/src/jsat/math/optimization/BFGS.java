...
/**
 * Implementation of the Broyden–Fletcher–Goldfarb–Shanno (BFGS) algorithm for function minimization. For {@code n}
 * dimensional problems it requires
 * <i>O(n<sup>2</sup>)</i> work per iteration and uses first order information to approximate the Hessian.
 *
 * @author Edward Raff
 */
public class BFGS implements Optimizer2 {
  ...
  /**
   * Creates a new BFGS optimization object that uses a maximum of 250 iterations and a
   * {@link BacktrackingArmijoLineSearch backtracking} line search.
   */
  public BFGS() {
   ...
   *
   * @param maxIterations the maximum number of iterations to allow before stopping
  ...
  /**
   * By default the infinity norm is used to judge convergence. If set to {@code false}, the 2 norm will be used
   * instead.
   *
  ...
  /**
   * Returns whether or not the infinity norm ({@code true}) or 2 norm ({@code false}) is used to determine convergence.
   *
   * @return {@code true} if the infinity norm is in use, {@code false} for the 2 norm
   */
