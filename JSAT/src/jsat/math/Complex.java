...
public class Complex implements Cloneable, Serializable {
   ...
   * The standard complex division performs a set of operations that is suseptible to both overflow and underflow. This
   * method is more numerically stable while still being relatively fast to execute.
   *
   * @param a the real part of the first number
   * @param b the imaginary part of the first number
   * @param c the real part of the second number
  ...
  public static void cDiv(double a, double b, double c, double d, double[] results) {
    /**
     * Douglas M. Priest. Efficient scaling for complex division. ACM Trans. Math. Softw., 30(4):389–401, 2004
     */
    long aa, bb, cc, dd, ss;
    double t;
  ...
  /**
   * Computes the Argument, also called phase, of this complex number. Unless the result is {@link Double#NaN}, which
   * occurs only for complex zero, the result will be in the range [-pi, pi]
   *
   ...
   * @param eps the maximum acceptable difference between values to be considered equal
