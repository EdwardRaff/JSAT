package jsat.math;

import java.io.Serializable;

/**
 * A class for representing a complex value by a real and imaginary double pair.
 *
 * @author Edward Raff
 */
public class Complex implements Cloneable, Serializable {

  private static final long serialVersionUID = -2219274170047061708L;

  /**
   * Performs a complex division operation. <br>
   * The standard complex division performs a set of operations that is
   * suseptible to both overflow and underflow. This method is more numerically
   * stable while still being relatively fast to execute.
   *
   * @param a
   *          the real part of the first number
   * @param b
   *          the imaginary part of the first number
   * @param c
   *          the real part of the second number
   * @param d
   *          the imaginary part of the second number
   * @param results
   *          an array to store the real and imaginary results in. First index
   *          is the real, 2nd is the imaginary.
   */
  public static void cDiv(final double a, final double b, double c, double d, final double[] results) {
    /**
     * Douglas M. Priest. Efficient scaling for complex division. ACM Trans.
     * Math. Softw., 30(4):389–401, 2004
     */
    long aa, bb, cc, dd, ss;
    double t;
    int ha, hb, hc, hd, hz, hw, hs;

    /* extract high-order 32 bits to estimate |z| and |w| */
    aa = Double.doubleToRawLongBits(a);
    bb = Double.doubleToRawLongBits(b);

    ha = (int) (aa >> 32 & 0x7fffffff);
    hb = (int) (bb >> 32 & 0x7fffffff);
    hz = ha > hb ? ha : hb;

    cc = Double.doubleToRawLongBits(c);
    dd = Double.doubleToRawLongBits(d);

    hc = (int) (cc >> 32 & 0x7fffffff);
    hd = (int) (dd >> 32 & 0x7fffffff);
    hw = hc > hd ? hc : hd;

    /* compute the scale factor */
    if (hz < 0x07200000 && hw >= 0x32800000 && hw < 0x47100000) {
      /* |z| < 2^-909 and 2^-215 <= |w| < 2^114 */
      hs = (0x47100000 - hw >> 1 & 0xfff00000) + 0x3ff00000;
    } else {
      hs = (hw >> 2) - hw + 0x6fd7ffff & 0xfff00000;
    }
    ss = (long) hs << 32;

    /* scale c and d, and compute the quotient */
    final double ssd = Double.longBitsToDouble(ss);
    c *= ssd;
    d *= ssd;
    t = 1.0 / (c * c + d * d);
    c *= ssd;
    d *= ssd;
    results[0] = (a * c + b * d) * t;
    results[1] = (b * c - a * d) * t;
  }

  /**
   * Performs a complex multiplication
   *
   * @param a
   *          the real part of the first number
   * @param b
   *          the imaginary part of the first number
   * @param c
   *          the real part of the second number
   * @param d
   *          the imaginary part of the second number
   * @param results
   *          an array to store the real and imaginary results in. First index
   *          is the real, 2nd is the imaginary.
   */
  public static void cMul(final double a, final double b, final double c, final double d, final double[] results) {
    results[0] = a * c - b * d;
    results[1] = b * c + a * d;
  }

  /**
   * Returns the complex number representing sqrt(-1)
   *
   * @return the complex number <i>i</i>
   */
  public static Complex I() {
    return new Complex(0.0, 1.0);
  }

  private double real, imag;

  /**
   * Creates a new Complex number
   *
   * @param real
   *          the real part of the number
   * @param imag
   *          the imaginary part of the number
   */
  public Complex(final double real, final double imag) {
    this.real = real;
    this.imag = imag;
  }

  /**
   * Creates a new complex number containing the resulting addition of this and
   * another
   *
   * @param c
   *          the number to add
   * @return <tt>this</tt>+c
   */
  public Complex add(final Complex c) {
    final Complex ret = new Complex(real, imag);
    ret.mutableAdd(c);
    return ret;
  }

  @Override
  protected Complex clone() {
    return new Complex(real, imag);
  }

  /**
   * Creates a new complex number containing the resulting division of this by
   * another
   *
   * @param c
   *          the number to divide by
   * @return <tt>this</tt>/c
   */
  public Complex divide(final Complex c) {
    final Complex ret = new Complex(real, imag);
    ret.mutableDivide(c);
    return ret;
  }

  @Override
  public boolean equals(final Object obj) {
    return equals(obj, 0.0);
  }

  /**
   * Checks if <i>this</i> is approximately equal to another Complex object
   *
   * @param obj
   *          the object to compare against
   * @param eps
   *          the maximum acceptable difference between values to be considered
   *          equal
   * @return <tt>true</tt> if the objects are approximately equal
   */
  public boolean equals(final Object obj, final double eps) {
    if (obj instanceof Complex) {
      final Complex other = (Complex) obj;
      if (Math.abs(real - other.real) > eps) {
        return false;
      } else if (Math.abs(imag - other.imag) > eps) {
        return false;
      }
      return true;
    }
    return false;
  }

  /**
   * Computes the Argument, also called phase, of this complex number. Unless
   * the result is {@link Double#NaN}, which occurs only for complex zero, the
   * result will be in the range [-pi, pi]
   *
   * @return the argument of this complex number
   */
  public double getArg() {
    return Math.atan2(imag, real);
  }

  /**
   * Returns a new complex number representing the complex conjugate of this one
   *
   * @return the complex conjugate of <tt>this</tt>
   */
  public Complex getConjugate() {
    return new Complex(real, -imag);
  }

  /**
   * Returns the imaginary part of this complex number
   *
   * @return the imaginary part of this complex number
   */
  public double getImag() {
    return imag;
  }

  /**
   * Computes the magnitude of this complex number, which is sqrt(
   * {@link #getReal() Re}<sup>2</sup>+{@link #getImag() Im}<sup>2</sup>)
   *
   * @return the magnitude of this complex number
   */
  public double getMagnitude() {
    return Math.hypot(real, imag);
  }

  /**
   * Returns the real part of this complex number
   *
   * @return the real part of this complex number
   */
  public double getReal() {
    return real;
  }

  @Override
  public int hashCode() {
    int hash = 5;
    hash = 67 * hash + (int) (Double.doubleToLongBits(real) ^ Double.doubleToLongBits(real) >>> 32);
    hash = 67 * hash + (int) (Double.doubleToLongBits(imag) ^ Double.doubleToLongBits(imag) >>> 32);
    return hash;
  }

  /**
   * Creates a new complex number containing the resulting multiplication
   * between this and another
   *
   * @param c
   *          the number to multiply by
   * @return <tt>this</tt>*c
   */
  public Complex multiply(final Complex c) {
    final Complex ret = new Complex(real, imag);
    ret.mutableMultiply(c);
    return ret;
  }

  /**
   * Alters this complex number to contain the result of the addition of another
   *
   * @param c
   *          the complex value to add to this
   */
  public void mutableAdd(final Complex c) {
    mutableAdd(c.real, c.imag);
  }

  /**
   * Alters this complex number as if an addition of another complex number was
   * performed.
   *
   * @param r
   *          the real part of the other number
   * @param i
   *          the imaginary part of the other number
   */
  public void mutableAdd(final double r, final double i) {
    real += r;
    imag += i;
  }

  /**
   * Alters this complex number to contain the result of the division by another
   *
   * @param c
   *          the number to divide by
   */
  public void mutableDivide(final Complex c) {
    mutableDivide(c.real, c.imag);
  }

  /**
   * Alters this complex number as if a division by another complex number was
   * performed.
   *
   * @param c
   *          the real part of the other number
   * @param d
   *          the imaginary part of the other number
   */
  public void mutableDivide(final double c, final double d) {
    final double[] r = new double[2];
    cDiv(real, imag, c, d, r);
    real = r[0];
    imag = r[1];
  }

  /**
   * Alters this complex number to contain the result of the multiplication of
   * another
   *
   * @param c
   *          the number to multiply by
   */
  public void mutableMultiply(final Complex c) {
    mutableMultiply(c.real, c.imag);
  }

  /**
   * Alters this complex number as if a multiplication of another complex number
   * was performed.
   *
   * @param c
   *          the real part of the other number
   * @param d
   *          the imaginary part of the other number
   */
  public void mutableMultiply(final double c, final double d) {
    final double newR = real * c - imag * d;
    final double newI = imag * c + real * d;
    real = newR;
    imag = newI;
  }

  /**
   * Alters this complex number to contain the result of the subtraction of
   * another
   *
   * @param c
   *          the number to subtract
   */
  public void mutableSubtract(final Complex c) {
    mutableSubtract(c.real, c.imag);
  }

  /**
   * Alters this complex number as if a subtraction of another complex number
   * was performed.
   *
   * @param r
   *          the real part of the other number
   * @param i
   *          the imaginary part of the other number
   */
  public void mutableSubtract(final double r, final double i) {
    mutableAdd(-r, -i);
  }

  /**
   * Alters this complex number so that it represents its complex conjugate
   * instead.
   */
  public void mutateConjugate() {
    imag = -imag;
  }

  /**
   * Sets the imaginary value part of this complex number
   *
   * @param imag
   *          the new imaginary value
   */
  public void setImag(final double imag) {
    this.imag = imag;
  }

  /**
   * Sets the real value part of this complex number
   *
   * @param r
   *          the new real value
   */
  public void setReal(final double r) {
    real = r;
  }

  /**
   * Creates a new complex number containing the resulting subtracting another
   * from this one
   *
   * @param c
   *          the number to subtract
   * @return <tt>this</tt>-c
   */
  public Complex subtract(final Complex c) {
    final Complex ret = new Complex(real, imag);
    ret.mutableSubtract(c);
    return ret;
  }

  @Override
  public String toString() {
    if (imag == 0) {
      return Double.toString(real);
    } else if (real == 0) {
      return imag + "i";
    } else {
      return "(" + real + " + " + imag + "i)";
    }
  }

}
