...
/**
 * This class provides static methods for computing accurate approximations to many special functions. <br>
 * <br>
 * All methods should return absolute differences of less than 10<sup>-9</sup>
 * for all reasonable values. Unreasonable values would be those in areas of high change (such as those approaching
...
public class SpecialMath {
  ...
  /**
   * The gamma function is a generalization of the factorial function. This method provides the gamma function for
  ...
  public static double gamma(double z) {
    ...
    /**
     * General case for z > 0, from Numerical Recipes in C (2nd ed. Cambridge University Press, 1992). |error| is <= 2*10^-10 for all z
     * > 0
     *
     * ____ / 6 \ / __ | ===== p | \/ 2 || | \ n | z + 0.5 - (z + 5.5) gamma(z) = ------- |p + > -----| (z + 5.5) e z |
     * 0 / z + n| | ===== | \ n = 1 /
     *
     *
  ...
  /**
   * Computes the natural logarithm of {@link #gamma(double) }. This method is more numerically stable than taking the
   * log of the result of Γ(z).
   *
   * <br>Special Values:
   * <ul>
  ...
  /**
   * Computes the value of the digamma function, &#936;(x), which is the derivative of {@link #lnGamma(double) }. <br>
   * <br>
   ...
   * </ul>
   * <br>
   * This method should be accurate to an absolute difference of 10<sup>-14</sup> for all values that are not near an
   * asymptote.
   *
  ...
  /**
   * The upper polynomial for adjustment to the digamma function approximation in the range [5, 500]. Computationally
   * the absolute difference is less than 1e-14 after 70
   */
  ...
  /**
   * The lower polynomial for adjustment to the digamma function approximation in the range [5, 500]
   */
  ...
  /**
   * The upper polynomial for computing the digamma function, fit against psi(x)/(x-1.46163...)
   *
   * Absolute difference less than 1e-15 for the range [2,7]
   *
   * Idea from Chebyshev Approximations for the Psi Function. Mathematics of Computation, Volume 27, Number 121. By
  ...
  /**
   * The lower polynomial for computing the digamma function, fit against psi(x)/(x-1.46163...)
   *
   * Absolute difference less than 1e-15 for the range [2,7]
   */
  ...
  /**
   * The upper polynomial for approximating the digamma function in the range [7, 70]. The coefficients are in reverse
   * order
   *
   * Absolute difference is less than 1e-14 for the whole range (1e-15 for most of it!)
   */
  ...
  /**
   * The lower polynomial for approximating the digamma function in the range [7, 70]. The coefficients are in reverse
   * order
   */
   ...
   * <ul>
   * <li>For x &lt; -0.5, the values returned will be of the correct magnitude - but are not very accurate</li>
   * <li>For x in [-0.5, 2.5], the values returned will be of reasonable accurate (absolute difference around
   * 10<sup>-7</sup>), unless it is very close to 1</li>
   ...
   * <br> The Bernoulli zeros for odd n will return {@link Double#NEGATIVE_INFINITY} and for any value less than 0 will
   ...
   * <i>B<sub>n</sub></i>. The Bernoulli numbers grow in value rapidly, and so the accuracy of this method decays
   * quickly. n &gt; 20 should have the correct order of magnitude, but may not have many significant figures. {@link #reLnBn(int)
  ...
  /**
   * Computes the regularized incomplete beta function, I<sub>x</sub>(a, b). The result of which is always in the range
   * [0, 1]
   *
  ...
  /**
   * Computes the inverse of the incomplete beta function, I<sub>p</sub><sup>-1</sup>(a,b), such that
   * {@link #betaIncReg(double, double, double) I<sub>x</sub>(a, b) } = <tt>p</tt>. The returned value, x, will always
   * be in the range [0,1]. The input <tt>p</tt>, must also be in the range [0,1].
   *
   ...
   * This method is more numerically stable and accurate than computing it via the direct method, and is always in the
   * range [0,1]. <br><br>
  ...
  public static double gammaQ(double a, double z) {
    ...
    /**
     * On the range of x from 0.5 to 50 a=0.15, |rel error| is ~ 3e-15 for most values of x, with a bad spot of |rel
     * error| ~ 3e-11 when x ~= 5.75 a=0.5, max |rel error| is 3.9e-15 a=1, max |rel error| is 4e-15, rel error groining
     * as x -> infinity a=5, max |rel error| is 7e-13, but only near x ~= 0, most is in the range 3e-15 a=10, max |rel
     * error| is 4e-6, but only near x ~= 0, most is in the range 3e-15
     */
  ...
  public static double gammaIncUp(double a, double z) {
    ...
    /**
     * On the range of x from 0.5 to 50 a=0.15, max |rel error| is ~1.3e-11, but only in a small range x ~= 11.
     * Otherwise rel error ~ 2e-15 a=0.5, max |rel error| is ~3.6e-15, less in all places otherwise a=1, max |rel error|
     * is ~4e-15, rel error grows as x-> infinity a=5, max |rel error| is 7e-13, but only near x ~= 0, most is in the
     * range 2e-15 a=10, max |rel error| is 4.9e-6, but only near x ~= 0, most is in the range 5e-15
     */
  ...
  public static double gammaIncLow(double a, double z) {
    ...
    /**
     * On the range of x from 0.5 to 50 a=0.15, see a=1 a=0.5, see a=1 a=1, max |rel error| is ~1.7e-13, rel error
     * starts at 1e-15 and grows to the max as z increases a=5, max |rel error| is 3e-13, but only near x ~= 0. for x >
     * epsilon error starts at 5e-15 and grows with z up to 1e-13 a=10, max |rel error| is 2e-7, but only near x ~= 0,
     * most is in the range 5e-15, grows to 5e-14 as z-> infinity
     */
