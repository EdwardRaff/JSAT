...
/**
 *
 * This class provides a means of updating summary statistics as each new data point is added. The data points are not
 * stored, and values are updated with an online algorithm.
 * <br>
 * As such, this class has constant memory usage, regardless of how many values are added. But the results may not be as
 * numerically accurate, and can degrade badly given specific data sequences.
 *
 * @author Edward Raff
 */
public class OnLineStatistics implements Serializable, Cloneable {
  ...
  /**
   * Effectively removes a sample with the given value and weight from the total. Removing values that have not been
   * added may yield results that have no meaning
   * <br><br>
  ...
  /**
   * Computes a new set of statistics that is the equivalent of having removed all observations in {@code B} from
   * {@code A}. <br>
   * NOTE: removing statistics is not as numerically stable. The values of the 3rd and 4th moments {@link #getSkewness()
   * } and {@link #getKurtosis() }
   * will be inaccurate for many inputs. The {@link #getMin() min} and {@link #getMax() max} can not be determined in
   * this setting, and will not be altered.
   *
   * @param A the first set of statistics, which must have a larger value for {@link #getSumOfWeights() } than {@code B}
   * @param B the set of statistics to remove from {@code A}.
   * @return a new set of statistics that is the removal of {@code B} from {@code A}
   */
  ...
  /**
   * Removes from this set of statistics the observations that where collected in {@code B}.<br>
   * NOTE: removing statistics is not as numerically stable. The values of the 3rd and 4th moments {@link #getSkewness()
   * } and {@link #getKurtosis() }
   * will be inaccurate for many inputs. The {@link #getMin() min} and {@link #getMax() max} can not be determined in
   * this setting, and will not be altered.
   ...
   * <br><br>
   * NOTE: Adding two statistics is not as numerically stable. If A and B have values of similar size and scale, the
   * values of the 3rd and 4th moments {@link #getSkewness() } and {@link #getKurtosis() } will suffer from catastrophic
   * cancellations, and may not be as accurate.
   *
  ...
  /**
   * Adds to the current statistics all the samples that were collected in {@code B}. <br>
   * NOTE: Adding two statistics is not as numerically stable. If A and B have values of similar size and scale, the
   * values of the 3rd and 4th moments {@link #getSkewness() } and {@link #getKurtosis() } will suffer from catastrophic
   * cancellations, and may not be as accurate.
   *
  ...
  /**
   * Returns the sum of the weights for all data points added to the statistics. If all weights were 1, then this value
   * is the number of data points added.
   *
   * @return the sum of weights for every point currently contained in the statistics.
   */
