...
/**
 * DDM (Drift Detection Method) is a drift detector for binary events, and is meant to detect decreases in the success
 * rate over time. As such it will not inform of any positive drift. <br>
 * <br>
 * This drift detector supports a warning state, and will not begin to store the object history until a warning state
 * begins. If the warning state ends before a detection of drift occurs, the history will be dropped.
 * <br>
 * See: Gama, J., Medas, P., Castillo, G.,&amp;Rodrigues, P. (2004). <i>Learning with Drift Detection</i>. In A. C.
 * Bazzan&amp;S. Labidi (Eds.), Advances in Artificial Intelligence – SBIA 2004 (pp. 286–295). Springer Berlin
...
public class DDM<V> extends BaseDriftDetector<V> {
  ...
  /**
   * Returns the current estimate of the success rate (number of {@code true} inputs) for the model.
   *
   ...
   * ({@code true}) drifts to a new value. This detector begins storing a history of the {@code obj} inputs only once it
   * has entered a warning state. <br>
   * This detector is specifically meant to detect drops in the success rate, and will not cause any warning or drift
   * detections for increases in the success rate.
   *
   * @param trial the result of the trial
   * @param obj the object to associate with the trial
   * @return {@code true} if we are in a warning or drift state, {@code false } if we are not
   */
  ...
  /**
   * Sets the multiplier on the standard deviation that must be exceeded to initiate a warning state. Once in the
   * warning state, DDM will begin to collect a history of the inputs <br>
   * Increasing the warning threshold makes it take longer to start detecting a change, but reduces false positives.
   * <br>
   * If the warning threshold is set above the {@link #setDriftThreshold(double) }, the drift state will not occur until
   * the warning state is reached, and the warning state will be skipped.
   *
   * @param warningThreshold the positive multiplier threshold for starting a warning state
   */
  ...
  /**
   * Sets the multiplier on the standard deviation that must be exceeded to recognize the change as a drift. <br>
   * Increasing the drift threshold makes it take longer to start detecting a change, but reduces false positives.
   *
   * @param driftThreshold the positive multiplier threshold for detecting a drift
   */
