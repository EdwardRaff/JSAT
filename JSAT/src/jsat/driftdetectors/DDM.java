 ...
 * <br>
 * This drift detector supports a warning state, and will not begin to store the
 * object history until a warning state begins. If the warning state ends before
 * a detection of drift occurs, the history will be dropped. <br>
...
public class DDM<V> extends BaseDriftDetector<V> {
   ...
   *          the threshold for starting a warning state
   * @param driftThreshold
   *          the threshold for recognizing a drift
   */
   ...
   * the number of successful trials ({@code true}) drifts to a new value. This
   * detector begins storing a history of the {@code obj} inputs only once it
   ...
   * @param obj
   *          the object to associate with the trial
   * @return {@code true} if we are in a warning or drift state, {@code false }
   *         if we are not
   */
   ...
   * Increasing the drift threshold makes it take longer to start detecting a
   * change, but reduces false positives.
   *
   * @param driftThreshold
   *          the positive multiplier threshold for detecting a drift
   */
   ...
   * Increasing the warning threshold makes it take longer to start detecting a
   * change, but reduces false positives. <br>
   ...
   *
   * @param warningThreshold
   *          the positive multiplier threshold for starting a warning state
   */
