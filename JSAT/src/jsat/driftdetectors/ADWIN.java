...
public class ADWIN<V> extends BaseDriftDetector<V> {
   ...
   *          the desired false positive rate
   * @param maxHistory
   *          the maximum history of objects to keep
   */
  ...
  public boolean addSample(final double value, final V obj) {
    ...
    final double deltaPrime = delta / Math.log(allStats.getSumOfWeights());// will
                                                                           // be
                                                                           // >
                                                                           // 1
                                                                           // in
    ...
    while (testIter.hasNext()) {
      ...
      if (Math.abs(mu_0 - mu_1) > e_cut) // CHANGE! OMG
      {
        ...
        /*
         * we keep going incase there is a more recent start point for the
         * drift, as the change in mean at the front could have been large
         * enough / dramatic enough to make preciding cuts also look like drift
         */
      }
    }
    compress();
  ...
  private void compress() {
    ...
    while (listIter.hasNext()) {
      ...
      if (n == lastSizeSeen) {
        if (++lastSizeCount > M) // compress, can only occur if there is a
                                 // previous
        {
          listIter.previous();
          ...
          // so nowe we must be looking at a new range since we just promoted a
          // window
   ...
   * If {@code false} is passed in to drop the <i>newer</i> values that drifted
   * it is probably that continuing to add new examples will continue to cause
   * detections.
   *
   * @param dropOld
   *          {@code true} to drop the older values out of the window that we
   *          drifted away from, or {@code false} to drop the newer values and
   *          retain the old ones.
   */
  public void driftHandled(final boolean dropOld) {
    /*
     * Iterate through and either drop everything to the left OR the right Track
     * statiscits so that we can update allStats
     */
    final Iterator<OnLineStatistics> testIter = windows.descendingIterator();
    final OnLineStatistics leftStats = new OnLineStatistics();
  ...
  /**
   * Returns the mean value determined for the newer values that we have drifted
   * into. <br>
  ...
  /**
   * Returns the standard deviation for the newer values that we have drifted
   * into. <br>
   * If drifting has not occurred or has already been handled,
   * {@link Double#NaN} will be returned.
   *
  ...
  /**
   * Returns the mean value determined for the older values that we have drifted
   * away from. <br>
   * If drifting has not occurred or has already been handled,
   * {@link Double#NaN} will be returned.
   *
  ...
  /**
   * Returns the standard deviation for the older values that we have drifted
   * away from. <br>
   * If drifting has not occurred or has already been handled,
   * {@link Double#NaN} will be returned.
   *
   ...
   * items that have been added to the ADWIN object since the last drift, and is
   * reduced when drift occurres.
   *
  ...
  /**
   * This parameter controls the trade off of space and accuracy for the sliding
   * window. The larger {@code M} becomes, the more accurate the window will be
   * - but at the cost of execution speed.
   *
   * @param M
   *          the window space constant in [1, &infin;)
   */
