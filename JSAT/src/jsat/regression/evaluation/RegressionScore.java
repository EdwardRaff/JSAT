 ...
 * <br>
 * All regression scores must override the {@link #equals(java.lang.Object)} and
 * {@link #hashCode() } methods. If a score has parameters, different objects
 * with different parameters must not be equal. However, different objects with
 * the same parameters must be equal <i>even if their internal states are
 * different</i>
 *
 * @author Edward Raff
 */
public interface RegressionScore extends Serializable {
   ...
   *
   * @param prediction
   *          the prediction for the data point
   * @param trueValue
   *          the true value for the data point
   * @param weight
   *          the weigh to assign to the data point
   */
   ...
   * already accumulated in the {@code other} object. This does not result in an
   * averaging, but alters the current object to have the same score it would
   * have had if all the results were originally inserted into <i>this </i>
   * object. <br>
   * <br>
   * This method is only required to work if {@code other} if of the same class
   * as {@code this} object.
   *
   * @param other
