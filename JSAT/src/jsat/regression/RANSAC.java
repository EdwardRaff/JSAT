...
/**
 * RANSAC is a randomized meta algorithm that is useful for fitting a model to a data set that has a large amount of
 * outliers that do not represent the true distribution well. <br>
 * RANSAC has the concept of inliers and outliers. An initial number of seed points is specified. This makes the initial
 * inlier set. The algorithm than iterates several times, randomly selecting the specified number of points. It then
 * regresses on all other points, adding all points within a specified absolute error to the set of inliers. The model
 * is then trained again on the larger set, and the training error becomes the measure of the strength of the model. The
...
public class RANSAC implements Regressor, Parameterized {
  ...
  /**
   * True marks that the data point is part of the consensus set. False indicates it is not.
   */
  private boolean[] consensusSet;
  private double modelError;

  /**
   * Creates a new RANSAC training object. Because RANSAC is sensitive to parameter settings, which are data and model
   * dependent, no default values exist for them.
   *
   * @param baseRegressor the model to fit using RANSAC
   * @param iterations the number of iterations of the algorithm to perform
   * @param initialTrainSize the number of points to seed each iteration of training with
   * @param minResultSize the minimum number of inliers to make it into the model to be considered a possible fit.
   * @param maxPointError the maximum allowed absolute difference in the output of the model and the true value for the
   * data point to be added to the inlier set.
   */
  ...
  /**
   * class that does the loop iteration work and returns a reference to itself. The are sortable based on the lowest
   * error
   */
  ...
  /**
   * Each data point not in the initial training set will be tested against. If a data points error is sufficiently
   * small, it will be added to the set of inliers.
   *
   ...
   * small, it will be added to the set of inliers.
   *
   * @param maxPointError the new maximum error a data point may have to be considered an inlier.
   */
  ...
  /**
   * RANSAC requires an initial model to be accurate enough to include a minimum number of inliers before being
   * considered as a potentially good model. This is the number of points that must make it into the inlier set for a
  ...
  /**
   * Once RANSAC is complete, it maintains its trained version of the finalized regressor. A clone of it may be
  ...
  /**
   * Returns an boolean array where the indices correspond to data points in the original training set. <tt>true</tt>
   * indicates that the data point was apart of the final consensus set. <tt>false</tt> indicates that it was not.
   *
   * @return a boolean array indicating which points made it into the consensus set
   */
  ...
  /**
   * Returns the model error, which is the average absolute difference between the model and all points in the set of
   * inliers.
   *
   * @return the error for the learned model. Returns {@link Double#POSITIVE_INFINITY} if the model has not been trained
   * or failed to fit.
   */
