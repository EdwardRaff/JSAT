 ...
 * additional learning when a new example is provided via the null
 * {@link #update(jsat.classifiers.DataPoint, double) } method. <br>
 ...
 * classifier, or {@link #setUp(jsat.classifiers.CategoricalData[], int) } to
 * prepare for online updates. Once one of these is called, it should then be
 * safe to call null {@link #update(jsat.classifiers.DataPoint, double) }
...
public interface UpdateableRegressor extends Regressor {
   ...
   *
   * @param categoricalAttributes
   *          an array containing the categorical attributes that will be in
   *          each data point
   * @param numericAttributes
   *          the number of numeric attributes that will be in each data point
   */
