...
/**
 * Logistic regression is a common method used to fit a probability between binary outputs. It can also be used to
 * perform regression on a real function. For classification tasks, when all variables are independent, the results
 * converge to the same prediction values as {@link NaiveBayes}. When variables have a high degree of correlation,
 * Logistic Regression should produce better results.
 *
 * @author Edward Raff
 */
public class LogisticRegression implements Classifier, Regressor, SingleWeightVectorModel {
  ...
  /**
   * Logistic regression needs values on the range [0, 1]. The shift makes sure that values are on the range [0, x], for
   * some x
   */
  private double shift;
  /**
   * Logistic regression needs values on the range [0, 1]. The scale makes sure that values of the form [0, x] are
   * converted to [0, 1]
   */
  ...
  /**
   * Returns the backing vector that containing the learned coefficients for the logistic regression. Changes to it will
