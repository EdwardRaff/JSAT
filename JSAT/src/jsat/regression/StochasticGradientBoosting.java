...
/**
 * An implementation of Stochastic Gradient Boosting (SGB) for the Squared Error loss. SGB is also known as Gradient
 * Boosting Machine. There is a specialized version of SGB known as TreeBoost, that is not implemented by this method.
 * SGB is a boosting method derived for regression. It uses many weak learners by attempting to estimate the residual
 * error of all previous learners. It can also use an initial strong learner and use the weak learners to refine the
 * initial estimate.
 *
 * <br><br>
 * See papers:<br>
 ...
 * Stochastic gradient boosting</a>. Computational Statistics&amp;Data Analysis, 38(4), 367–378.
 * <br><br>
 * Mohan, A., Chen, Z.,&amp;Weinberger, K. (2011).
 * <a href="http://www1.cse.wustl.edu/~kilian/papers/mohan11a.pdf">Web-search ranking with initialized gradient boosted
 * regression trees</a>. Journal of Machine Learning Research, 14,
 *
 *
 *
 * @author Edward Raff
...
public class StochasticGradientBoosting implements Regressor, Parameterized {
  ...
  /**
   * The default value for the {@link #setTrainingProportion(double) training proportion} is
   * {@value #DEFAULT_TRAINING_PROPORTION}.
   */
  public static final double DEFAULT_TRAINING_PROPORTION = 0.5;

  /**
   * The default value for the {@link #setLearningRate(double) } is {@value #DEFAULT_LEARNING_RATE}
   */
  public static final double DEFAULT_LEARNING_RATE = 0.1;

  /**
   * The proportion of the data set to be used for each iteration of training. The points that make up the iteration are
  ...
  /**
   * Sets the learning rate of the algorithm. The GB version uses a learning rate of 1. SGB uses a learning rate in
   * (0,1) to avoid overfitting. The learning rate is multiplied by the output of each weak learner to reduce its
   * contribution.
   *
   * @param learningRate the multiplier to apply to the weak learners
  ...
  /**
   * The GB version uses the whole data set at each iteration. SGB can use a fraction of the data set at each iteration
   * in order to reduce overfitting and add randomness.
   *
   * @param trainingProportion the fraction of training the data set to use for each iteration of SGB
   * @throws ArithmeticException if the trainingPortion is not a valid fraction in (0, 1]
   */
   ...
   *
   * @return the fraction of the training data set to use for each iteration of SGB
   */
  ...
  public void train(RegressionDataSet dataSet, ExecutorService threadPool) {
    ...
    /**
     * Instead of recomputing previous weak learner's output, keep track of the current total sum to know the current
     * prediction value
     */
  ...
  /**
   * Finds the constant <tt>y</tt> such that the squared error of the Regressor <tt>h</tt> on the set of residuals
   * <tt>backingResidsList</tt>
   * is minimized.
   *
  ...
  /**
   * Returns a function object that approximates the derivative of the squared error of the Regressor as a function of
   * the constant factor multiplied on the Regressor's output.
   *
   * @param backingResidsList the DataPointPair list of residuals
   * @param h the regressor that is having the error of its output minimized
