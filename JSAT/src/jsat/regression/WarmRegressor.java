 ...
 * <br>
 * Just because a regressor fits the type that the warm start interface states doesn't mean that it is a valid
 * classifier to warm start from. <i>Regressors of the same class trained on the same data must <b>always</b> be valid
...
public interface WarmRegressor extends Regressor {
  ...
  /**
   * Trains the regressor and constructs a model for regression using the given data set. If the training method knows
   * how, it will used the
