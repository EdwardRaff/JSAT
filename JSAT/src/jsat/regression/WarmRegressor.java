...
public interface WarmRegressor extends Regressor {
  ...
  /**
   * Trains the regressor and constructs a model for regression using the given
   * data set.
   *
   * @param dataSet
   *          the data set to train on
   * @param warmSolution
   *          the solution to use to warm start this model
   */
  public void train(RegressionDataSet dataSet, Regressor warmSolution);

  /**
   * Trains the regressor and constructs a model for regression using the given
   * data set. If the training method knows how, it will used the
   * <tt>threadPool</tt> to conduct training in parallel. This method will block
   * until the training has completed.
   *
   * @param dataSet
   *          the data set to train on
   * @param warmSolution
  ...
  /**
   * Some models can only be warm started from a solution trained on the exact
   * same data set as the model it is warm starting from. If this is the case
   * {@code true} will be returned. The behavior for training on a different
   * data set when this is defined is undefined. It may cause an error, or it
   * may cause the algorithm to take longer or reach a worse solution. <br>
   * When {@code true}, it is important that the data set be unaltered - this
   * includes mutating the values stored or re-arranging the data points within
   * the data set.
   *
