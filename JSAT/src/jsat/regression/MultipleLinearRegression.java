...
public class MultipleLinearRegression implements Regressor, SingleWeightVectorModel {
  ...
  /**
   * The vector B such that Y = X * B is the least squares solution. Will be
   * stored as Y = X * B + a
   */
  private Vec B;
  /**
  ...
  public void train(final RegressionDataSet dataSet, final ExecutorService threadPool) {
    ...
    // Construct matrix and vector, Y = X * B, we will solve for B or its least
    // squares solution
    ...
    if (useWeights) {
      // The sqrt(weight) vector can be applied to X and Y, and then QR can
      // procede as normal
