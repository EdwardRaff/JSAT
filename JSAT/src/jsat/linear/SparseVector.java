...
/**
 * Provides a vector implementation that is sparse. It does not allocate space for a vector of the specified size, and
 * only stores non zero values. All values not stored are implicitly zero.
 * <br>
 * Operations that change several zero values in a sparse vector to non-zero values may have degraded performance.
 * <br>
 * Sparce vector should never be used unless at least half the values are zero. If more then half the values are
 * non-zero, it will use more memory then an equivalent {@link DenseVector}. The more values that are zero in the
 * vector, the better its performance will be.
 *
 * @author Edward Raff
 */
public class SparseVector extends Vec {
  ...
  /**
   * Creates a new sparse vector of the same length as {@code vals} and sets each value to the values in the list.
   *
  ...
  /**
   * Creates a new sparse vector of the specified length, and pre-allocates enough internal state to hold
   * {@code capacity} non zero values. The vector itself will start out with all zero values.
   *
   * @param length the length of the sparse vector
  ...
  /**
   * Creates a new sparse vector backed by the given arrays. Modifying the arrays will modify the vector, and no
   * validation will be done. This constructor should only be used in performance necessary scenarios<br>
   * To make sure the input values are valid, the {@code indexes } values must be increasing and all values less than
   * {@code length} and greater than {@code -1} up to the first {@code used} indices.<br>
   * All the values stored in {@code values} must be non zero and can not be a special value. <br> {@code used} must be
   * greater than -1 and less than the length of the {@code indexes} and {@code values} arrays. <br>
   ...
   * @param length the length of the sparse vector
   * @param used the number of non zero values in the vector taken from the given input arrays.
   */
  ...
  /**
   * Because sparce vectors do not have most value set, they can have their length increased, and sometimes decreased,
   * without any effort. The length can always be extended. The length can be reduced down to the size of the largest
   * non zero element.
   *
   ...
   * and adjust the vector to add the given value into this location. Should only be called with negative input returned
   * by said method. Should never be called for an index that in fact does already exist in this sparce vector.
   *
   * @param insertLocation the negative insertion index such that -(insertLocation+1) is the address that the value
   * should have
