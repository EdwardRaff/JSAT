...
/**
 * Class for performing the Eigen Value Decomposition of a matrix. The EVD of a real matrix may contain complex results.
 * When this occurs, the EVD is less useful since JSAT only supports real matrices. The
 * {@link SingularValueDecomposition} of a real matrix is always real, and may be more useful.
 * <br><br>
 ...
 * orthogonal. V * V' equals the identity matrix.
 * <br><br>
 ...
 * eigenvectors in the sense that A*V = V*D. The matrix V may be badly conditioned, or even singular, so the validity of
 * the equation A = V*D*inverse(V) depends upon the condition of V. <br>
 * If there are no complex eigen values, which can be checked using {@link #isComplex() }, then D is a normal diagonal
 * matrix.
 *
 * @author Edward Raff
 */
public class EigenValueDecomposition implements Serializable {
  ...
  /**
   * Creates a new new Eigen Value Decomposition. The input matrix will not be altered. If the input is symmetric, a
   * more efficient algorithm will be used.
   *
   ...
   *
   * @param A the square matrix to work on.
   * @param eps the numerical tolerance for differences in value to be considered the same.
   */
   ...
   * @param n the column to alter, and the preceding column will be altered as well
   * @param q first constant
   * @param p second constant
   * @param shift the direction to perform the computation. Either 1 for after the current column, or -1 for before the
   * current column.
   */
  ...
  /**
   * Indicates wether or not the EVD contains complex eigen values. Because EVD works with real matrices, the complex
   * eigen vectors are lost - and the complex eigen values are in the off diagonal spaces of the D matrix.
   *
