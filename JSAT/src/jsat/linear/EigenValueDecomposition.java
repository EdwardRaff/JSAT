 ...
 * and the eigenvector matrix V is orthogonal. V * V' equals the identity
 * matrix. <br>
 * <br>
 ...
 * the eigenvectors in the sense that A*V = V*D. The matrix V may be badly
 * conditioned, or even singular, so the validity of the equation A =
 * V*D*inverse(V) depends upon the condition of V. <br>
...
public class EigenValueDecomposition implements Serializable {
  ...
  /**
   * Updates the columns of the matrix M such that <br>
   * <br>
   * <code><br>
   * for (int i = low; i <= high; i++)<br> {<br>
   * &nbsp;&nbsp; z = M[i][n+shift];<br>
   ...
   * @param n
   *          the column to alter, and the preceding column will be altered as
   *          well
   * @param q
   *          first constant
   * @param p
   *          second constant
   * @param shift
   *          the direction to perform the computation. Either 1 for after the
   *          current column, or -1 for before the current column.
   */
  private static void columnOpTransform(final Matrix M, final int low, final int high, final int n, final double q,
      final double p, final int shift) {
    double z;
  ...
  /**
   * Updates the rows of the matrix M such that <br>
   ...
   * @param n
   *          the row to alter, and the preceding row will be altered as well
   * @param q
   *          the first constant
   * @param p
   *          the second constant
   */
  private static void rowOpTransform(final Matrix M, final int low, final int high, final int n, final double q,
      final double p) {
    double z;
   ...
   *          the square matrix to work on.
   * @param eps
   *          the numerical tolerance for differences in value to be considered
   *          the same.
   */
  public EigenValueDecomposition(final Matrix A, final double eps) {
    ...
    if (Matrix.isSymmetric(A, eps)) {
      // Would give it the transpose, but the input is symmetric. So its the
      // same thing
  ...
  private void backsubtituteFindVectors(final int nn, double z, double s, final double eps, final double norm,
      final double[] cr) {
    double p;
    double q;
    double w;
   ...
   *          first constant
   * @param k
   *          this column and the column after will be altered
   * @param y
   *          second constant
   * @param notlast
   ...
   *          fifth constant
   */
  private void columnOpTransform2(final Matrix M, final int low, final int high, final double x, final int k,
      final double y, final boolean notlast, final double z, final double r, final double q) {
    double p;
    for (int i = low; i <= high; i++) {
    ...
    while (n >= low) {
      ...
      } else if (l == n - 1) // Two roots found
      {
        ...
        while (m >= l) {
          ...
          if (abs(H.get(m, m - 1)) * (abs(q) + abs(r)) < eps
              * (abs(p) * (abs(H.get(m - 1, m - 1)) + abs(z) + abs(H.get(m + 1, m + 1))))) {
            break;
          }
          m--;
        }
  ...
  private void hqr2SolveComplexEigenEquation(final int i, final double p, final double q, final double eps,
      final double norm, final double w, final double z, final double r, final double ra, final double sa,
      final double s, final double[] cr, final int n) {
    double x;
    double y;
    double vr;
    double vi;
  ...
  /**
   * Indicates wether or not the EVD contains complex eigen values. Because EVD
   * works with real matrices, the complex eigen vectors are lost - and the
   * complex eigen values are in the off diagonal spaces of the D matrix.
   *
  ...
  /**
   * Alters the rows of the matrix M according to <code><br>
   * for (int j = low; j <= high; j++) {<br> &nbsp;&nbsp; p = M[k][j] + q * M[k + 1][j];<br>
   * &nbsp;&nbsp; if (notlast)<br>
   * &nbsp;&nbsp; {<br>
   * &nbsp;&nbsp;&nbsp;&nbsp; p = p + r * M[k + 2][j];<br>
   ...
   *          second constant
   * @param notlast
   *          <tt>true<tt> if the 2nd row after <tt>k</tt> should be updated
   * @param z
   *          third constant
   * @param r
   *          fourth constant
   ...
   *          fifth constant
   */
  private void rowOpTransform2(final Matrix M, final int low, final int high, final double x, final int k,
      final double y, final boolean notlast, final double z, final double r, final double q) {
    double p;
    for (int j = low; j <= high; j++) {
   ...
   *
   * @param cmp
   *          the comparator to use to sort the eigen values
   */
