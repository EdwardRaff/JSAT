...
/**
 * A wrapper for a vector that represents the vector multiplied by a scalar constant. This allows for using and altering
 * the value multiplied by a constant factor quickly, especially when the multiplicative factor must be changed often.
 * Mutable operations will alter the underling vector, and all operations will automatically be scaled on a per element
 * basis as needed.
 * <br><br>
 * If a point is reached where the multiplicative constant will be infrequently modified relative to the use of the
 * vector, it may be more efficient to use the original vector scaled appropriately. This can be done by first calling 
 * {@link #embedScale() } and then calling {@link #getBase() } .
 * <br><br>
 * When the multiplicative constant is set to zero, the underlying base vector is {@link Vec#zeroOut() zeroed out} and
 * the constant reset to 1.
 *
 * @author Edward Raff
 */
