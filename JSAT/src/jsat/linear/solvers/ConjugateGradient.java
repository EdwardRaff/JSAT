...
/**
 * Provides an iterative implementation of the COnjugate Gradient Method. <br>
 * <br>
 * The Conjugate method, if using exact arithmetic, produces the exact result
 * after a finite number of iterations that is no more then the number of rows
 * in the matrix. Because of this, no max iteration parameter is given. <br>
 * <br>
 *
 *
 * @author Edward Raff
...
public class ConjugateGradient {
  ...
  /**
   * Uses the Conjugate Gradient method to solve a linear system of equations
   * involving a symmetric positive definite matrix.<br>
   * <br>
   ...
   * <br>
   * <br>
   * NOTE: No checks will be performed to confirm these properties of the given
   * matrix. If a matrix is given that does not meet this requirements, invalid
   * results may be returned.
   *
   * @param eps
   *          the precision of the desired result.
   * @param A
   *          the symmetric positive definite matrix
   * @param x
   *          an initial guess for x, can be all zeros. This vector will be
   *          altered
   * @param b
   *          the target values
   ...
   *          the target values
   * @param Minv
   *          the of a matric M, such that M is a symmetric positive definite
   *          matrix. Is applied as M<sup>-1</sup>( A x - b = 0) to increase
   *          convergence and stability. These increases are soley a property of
   *          M<sup>-1</sup>
   *
   * @return the approximate solution to the equation <i>A x = b</i>
   */
  public static Vec solve(final double eps, final Matrix A, final Vec x, final Vec b, final Matrix Minv) {
    ...
    do {
      ...
      z_k = Minv.multiply(r_k);// TODO implement method so we can reuse z_k
                               // space, instead of creating a new vector
  ...
  /**
   * Uses the Conjugate Gradient method to compute the least squares solution to
   * a system of linear equations.<br>
   * Computes the least squares solution to A x = b. Where A is an m x n matrix
   * and b is a vector of length m and x is a vector of length n
   *
   * <br>
   * <br>
   * NOTE: Unlike
   * {@link #solve(double, jsat.linear.Matrix, jsat.linear.Vec, jsat.linear.Vec) }
   * , the CGNR method does not need any special properties of the matrix.
   * Because of this, slower convergence or numerical error can occur.
   *
   * @param eps
   ...
   *          any m x n matrix
   * @param x
   *          the initial guess for x, can be all zeros. This vector will be
   *          altered
   * @param b
   *          the target values
