...
/**
 * This class provides a base mechanism to create a Matrix 'view' from a list of {@link Vec} objects. The vector objects
 * will be used to back the rows of the matrix, so changes to one are shown in the other. If the matirx is altered using {@link #changeSize(int, int)
 * }, this may no longer be true. <br>
...
public class MatrixOfVecs extends GenericMatrix {
  ...
  /**
   * Creates a new Matrix of Vecs from the given array of Vec objects. All Vec objects contained should be of the same
   * length.
   *
  ...
  /**
   * Creates a new Matrix of Vecs from the given list of Vec objects. All Vec objects contained should be of the same
   * length.
   *
   * @param rows the rows of vecs to make this matrix.
   */
   ...
   * @param sparse if {@code true} {@link SparseVector} objects will be used for the rows. Else {@link DenseVector} will
   * be used.
   */
