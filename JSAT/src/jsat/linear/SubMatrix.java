...
/**
 * This class allows for the selection of an area of a matrix to operate on
 * independently. Mutable matrix operations perform on this sub matrix, will be
 * visible in the base matrix given. This allows for easy implementation of
 * blocking algorithms, but care must be taken to not create and use two sub
 * matrices that overlap and are altered simultaneously.
 *
 * @author Edward Raff
 */
public class SubMatrix extends GenericMatrix {
   ...
   *
   * @param baseMatrix
   *          the base matrix to get a view of
   * @param firstRow
   *          the first row to consider as part of the sub matrix, starting from
   *          0 inclusive
   * @param firstColumn
   *          the first column to consider as part of the sub matrix, starting
   *          from 0 inclusive
   * @param toRow
   *          the last row from the base matrix to consider, exclusive
   * @param toCol
   *          the last column from the base matrix to consider, exclusive
   */
  public SubMatrix(final Matrix baseMatrix, final int firstRow, final int firstColumn, final int toRow,
      final int toCol) {
    ...
    if (firstColumn < 0 || firstRow < 0 || toRow < 0 || toCol < 0) {
      ...
      throw new ArithmeticException(
          "You can not specify a matrix that goes past the row / column boundry of the base matrix");
    ...
    // If we are given a SubMatrix, lets adjust to use it directly instead of
    // accesing through layers
   ...
   * Values of the removed rows / columns will be lost.<br>
   * <br>
   * When a SubMatrix attempts to change size, it may alter the values of the
   * underlying matrix in an unusual way. If the base matrix is large enough to
   * hold the changed size, then the values in the underlying matrix that this
   * SubMatrix is expanding to encompass will be set to zero. If the underlying
   * matrix is not large enough, change size will be called upon it as well.<br>
  ...
  public double get(final int i, final int j) {
    // We MUST do a bounds check, as they might go past us but an index that
    // does exist in the base
    if (i >= rows() || j >= cols()) {
      throw new ArrayIndexOutOfBoundsException(
          "Can not access index [" + i + ", " + j + "] in the matrix of dimension [" + rows() + ", " + cols() + "]");
    }
