...
/**
 * This class allows for the selection of an area of a matrix to operate on independently. Mutable matrix operations
 * perform on this sub matrix, will be visible in the base matrix given. This allows for easy implementation of blocking
 * algorithms, but care must be taken to not create and use two sub matrices that overlap and are altered
 * simultaneously.
 *
 * @author Edward Raff
 */
public class SubMatrix extends GenericMatrix {
   ...
   * bottom/right most row / column removed. Values of the removed rows / columns will be lost.<br>
   * <br>
   * When a SubMatrix attempts to change size, it may alter the values of the underlying matrix in an unusual way. If
   * the base matrix is large enough to hold the changed size, then the values in the underlying matrix that this
   * SubMatrix is expanding to encompass will be set to zero. If the underlying matrix is not large enough, change size
   * will be called upon it as well.<br>
   * When decreasing the number of rows / columns, the SubMatrix will always reduce its view - but never shrink the
   * underling matrix that the view is of.
   * <br>
