...
/**
 * Generic class with some pre-implemented methods for a Matrix object. Throughout the documentation, the object that
 * has its method called on will be denoted as <i>A</i>. So if you have code that looks like
 * <br><br><center> {@code Matrix gramHat = gram.subtract(Matrix.eye(gram.rows()));}
 * </center><br><br>
 * Then {@code gram} would be the matrix <i>A</i> in the documentation.
 * <br>
 * Matrices will use a capital letter, vectors a <b>bold</b> lower case letter, and scalars a normal lower case letter.
 *
 *
 * @author Edward Rafff
 */
public abstract class Matrix implements Cloneable, Serializable {
  ...
  /**
   * Indicates whether or not this matrix can be mutated. If {@code false}, any method that contains "mutate" will not
   * work.
   * <br><br>
   * By default, this returns {@code true}
   *
   * @return {@code true} if the matrix supports being altered, {@code false} other wise.
   */
  ...
  /**
   * Returns an appropriate matrix to use for some operation A <i>op</i> B, where {@code A = this }
   *
  ...
  /**
   * If this matrix is <i>A<sub>m x n</sub></i>, and <i><b>b</b></i> has a length of n, and <i><b>c</b></i> has a length
   * of m, then this will mutate c to store <i><b>c</b> = <b>c</b> + A*<b>b</b>*z</i>
   *
  ...
  /**
   * This method alters the size of a matrix, either adding or subtracting rows from the internal structure of the
   * matrix. Every resize call may cause a new allocation internally, and should not be called for excessive changing of
   * a matrix. All added rows/ columns will have values of zero. If a row / column is removed, it is always the
   * bottom/right most row / column removed. Values of the removed rows / columns will be lost.
   *
   ...
   * @throws ArithmeticException if the dimensions of <i>C</i> do not match the dimensions of <i>this'</i>
   */
  ...
  /**
   * Returns {@code true} if the matrix is square, meaning it has the same number of {@link #rows() rows} and
   * {@link #cols() columns}.
   *
   * @return {@code true} if this matrix is square, {@code false} if it is rectangular.
   */
  ...
  /**
   * Creates a vector that has a copy of the values in column <i>j</i> of this matrix. Altering it will not effect the
   * values in <i>this</i> matrix
   *
  ...
  /**
   * Obtains a vector that is backed by <i>this</i>, at very little memory cost. Mutations to this vector will alter the
   * values stored in the matrix, and vice versa.
   *
  ...
  /**
   * Creates a vector that has a copy of the values in row <i>i</i> of this matrix. Altering it will not effect the
   * values in <i>this</i> matrix.
   *
  ...
  /**
   * Convenience method that will return {@code true} only if the two input matrices have the exact same dimensions.
   *
   * @param A the first matrix
   * @param B the second matrix
   * @return {@code true} if they have the exact same dimensions, {@code false} otherwise.
   */
  ...
  /**
   * Convenience method that will return {@code true} only if the two input matrices have dimensions compatible for
   * multiplying <i>A*B</i>
   *
   * @param A the first matrix
   * @param B the second matrix
   * @return {@code true} if they have dimensions allowing multiplication, {@code false} otherwise.
   */
  ...
  /**
   * Performs the same as {@link #equals(java.lang.Object) }, but allows a leniency in the differences between matrix
   * values. This is useful for when some amount of numerical error is expected
   *
   * @param obj the other matrix
   * @param range the max acceptable difference between two cell values
   * @return {@code true} if the difference between the values of each pair of matrix elements are less than or equal to
   * <i>range</i>
   */
   ...
   * @throws ArithmeticException if the vector dimensions are not compatible with the matrix <i>A</i>
   */
  ...
  /**
   * Creates a new dense matrix filled with random values from {@link Random#nextDouble() }
   *
  ...
  /**
   * Returns a new dense square matrix such that the main diagonal contains the values given in <tt>a</tt>
   *
   ...
   * <i>A = A*diag(<b>b</b>)</i>. This is equivalent to the code    <code>
  ...
  /**
   * Alters the matrix <i>A</i> so that it contains the result of sparse matrix represented by only its diagonal values
   * times <i>A</i> or
   * <i>A = diag(<b>b</b>)*A</i>. This is equivalent to the code    <code>
  ...
  /**
   * Checks to see if the given input is approximately symmetric. Rounding errors may cause the computation of a matrix
   * to come out non symmetric, where |a[i,h] - a[j, i]| &lt; eps. Despite these errors, it may be preferred to treat
   * the matrix as perfectly symmetric regardless.
   *
  ...
  /**
   * Creates a new square matrix that is a pascal matrix. The pascal matrix of size <i>n</i> is <i>n</i> by <i>n</i> and
   * symmetric.
   *
