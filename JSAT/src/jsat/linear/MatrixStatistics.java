...
public class MatrixStatistics {
  ...
  /**
   * Computes the weighted result for the covariance matrix of the given data set. If all weights have the same value,
   * the result will come out equivalent to {@link #covarianceMatrix(jsat.linear.Vec, java.util.List) }
   *
  ...
  public static void covarianceMatrix(Vec mean, List<DataPoint> dataSet, Matrix covariance, double sumOfWeights, double sumOfSquaredWeights) {
     ...
     *
     * n
     * ===== \ > w / i n ===== ===== T i = 1 \ / _\ / _\ ---------------------- > w |x - x| |x - x| 2 / i \ i / \ i / /
     * n \ n ===== |===== | ===== i = 1 |\ | \ 2 | > w | - > w |/ i| / i |===== | ===== \i = 1 / i = 1
     */
    Vec scratch = new DenseVector(mean.length());
  ...
  /**
   * Computes the weighted covariance matrix of the given data set. Superfluous calculations are avoided by having the
   * call provide information.
   *
   * @param mean the mean of the data set
   * @param dataSet the dataset to compute the covariance of
   * @param covariance the zeroed out matrix to store the result into
  ...
  /**
   * Computes the weighted diagonal of the covariance matrix, which is the standard deviations of the columns of all
   * values.
   *
   * @param means the already computed mean of the data set
   * @param diag the zeroed out vector to store the diagonal in. Its contents will be altered
