...
/**
 * A wrapper for a vector that represents the vector added with a scalar value. This allows for using and altering the
 * value added by a constant factor quickly. Mutable operations will alter the underling vector, and all operations will
 * automatically be adjusted on a per element basis as needed. The most significant performance difference is in
 * representing a sparse input sifted by a constant factor. Note, that when using a sparse base vector - the ShiftedVec
 * will always return {@code true} for {@link #isSparse() }, even if shifting makes all values non-zero
 * <br>
 *
 * @author Edward Raff
 */
public class ShiftedVec extends Vec {
  ...
  /**
   * Embeds the current shift scalar into the base vector, modifying it and then setting the new shit to zero. This
   * makes the base vector have the value represented by this ShiftedVec
   */
      ...
      public IndexValue next() {
        ...
        do {
          ...
          if (nextVal.getIndex() == lastIndx + 1) {
            nextVal = null;//done
          } else if (nextBaseVal != null && nextBaseVal.getIndex() == nextVal.getIndex())//there is a base non-zero next
          {
