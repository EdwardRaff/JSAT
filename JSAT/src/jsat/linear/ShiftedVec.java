...
public class ShiftedVec extends Vec {
   ...
   *
   * @param base
   *          the base vector to represent
   * @param shift
   *          the scalar shift to add to the base vector
   */
  ...
  public double dot(final Vec v) {
    if (v instanceof ShiftedVec) {
      ...
      return base.dot(other.base) + other.base.sum() * shift + base.sum() * other.shift
          + length() * shift * other.shift;
    }
  ...
  public Iterator<IndexValue> getNonZeroIterator(final int start) {
    if (!isSparse()) {// dense case, just add the shift and use the base
                      // implemenaton since its going to do the exact same thing
                      // I would
      ...
      public IndexValue next() {
        ...
        do {
          ...
          if (nextVal.getIndex() == lastIndx + 1) {
            nextVal = null;// done
          } else if (nextBaseVal != null && nextBaseVal.getIndex() == nextVal.getIndex()) // there
                                                                                          // is
                                                                                          // a
                                                                                          // base
                                                                                          // non-zero
  ...
  // TODO: In the case of the y also being a ShiftedVec and sparse some
  // significant performance could be saved.
  ...
  public void mutablePairwiseDivide(final Vec b) {
    // this would require multiple different shifts, so we have to fold it back
   ...
   *
   * @param shift
   *          the new value to use for the additive scalar
   */
