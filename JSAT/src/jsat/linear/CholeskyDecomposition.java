...
/**
 * The Cholesky Decomposition factors a symmetric positive definite matrix A into the form A = L L<sup>T</sup>. The
 * Cholesky Decomposition of a matrix is unique.
 *
 * @author Edward Raff
 */
public class CholeskyDecomposition implements Serializable {
  ...
  /**
   * Contains the matrix 'L', but instead of just keeping the lower triangular, we keep it in a symmetric copy so {@link LUPDecomposition#forwardSub(jsat.linear.Matrix, jsat.linear.Vec)
   * }
   ...
   * <tt>A</tt> will be altered to form the decomposition <tt>L</tt>. If A is still needed after this computation a
   * {@link Matrix#clone() clone} of the matrix should be given instead. <br>
   * NOTE: No check for the symmetric positive definite property will occur. The results of passing a matrix that does
   * not meet this properties is undefined.
   *
  ...
  private double computeLJJ(final Matrix A, final int j) {
    /**
     * _________________ / j - 1 / ===== / \ 2 L = / A - > L j j / j j / j k / ===== \/ k = 1
     */
