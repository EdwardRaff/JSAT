...
public class CholeskyDecomposition implements Serializable {
  ...
  /**
   * Contains the matrix 'L', but instead of just keeping the lower triangular,
   * we keep it in a symmetric copy so
   * {@link LUPDecomposition#forwardSub(jsat.linear.Matrix, jsat.linear.Vec) }
   * and backSub can be done without copying.
   */
  private final Matrix L;

  /**
   * Computes the Cholesky Decomposition of the matrix A. The matrix <tt>A</tt>
   * will be altered to form the decomposition <tt>L</tt>. If A is still needed
   * after this computation a {@link Matrix#clone() clone} of the matrix should
   * be given instead. <br>
   * NOTE: No check for the symmetric positive definite property will occur. The
   * results of passing a matrix that does not meet this properties is
   * undefined.
   *
   * @param A
   *          the matrix to create the Cholesky Decomposition of
   */
  ...
  private double computeLJJ(final Matrix A, final int j) {
    /**
     * _________________ / j - 1 / ===== / \ 2 L = / A - > L j j / j j / j k /
     * ===== \/ k = 1
     */
  ...
  /**
   * The Cholesky Decomposition computes the factorization A = L L<sup>T</sup>.
   * This method returns L<sup>T</sup>
   *
  ...
  private void updateRows(final int j, final int start, final int end, final int skip, final Matrix A,
      final double L_jj) {
    /*
     * 
     * / j - 1 \ | ===== | 1 | \ | L = ---- |A - > L L | i j L | i j / i k j k|
     * j j | ===== | \ k = 1 /
     */
