...
/**
 * This class provides a simple utility to represent an immutable vector where all values in the vector must have the
 * same constant value. The standard mutable methods can not alter this vector. However, the length and constant value
 * can be altered by calling the {@link #setLength(int) } and {@link #setConstant(double) } values respectively. The
 * Constant Vector representation uses only constant space.
 * <br><br>
 * This class can be useful in providing a generalized way to handle multiple unique values or a constant value. For
 * example, a separate regularization constant could be used for every feature in a learning problem. Instead of writing
 * code to handle multiple values separately from a single value, a ConstantVector can be used so that the constant
 * value case can be an efficient call to the multiple value version of the code.
 *
 * @author Edward Raff
 */
