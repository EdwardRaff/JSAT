 ...
 * <br>
 * If the base vector has {@code N} non zero values, then this vec will have O(N
 * <sup>2</sup>) non zero values. (N+2)(N+1)/2 non zero values to be exact.
 *
 * @author Edward Raff
 */
public class Poly2Vec extends Vec {
  ...
  public double get(final int index) {
    if (index == 0) {
      ...
      throw new IndexOutOfBoundsException(
          "Vector is of length " + length() + ", but index " + index + " was requested");
    }
    ...
    final int y = (x * x + x) / 2 + index - base.length() - 1 - base.length() * x;// the
                                                                                  // first
                                                                                  // term
                                                                                  // is
                                                                                  // safe
  ...
  public Iterator<IndexValue> getNonZeroIterator(int start) {
    ...
    boolean stage1Good = true;// fail occurs when the last index (or more) in
                              // the base vector is zero
    if (start == 0) {
      startStage = 0;
    ...
    } else// where do we start?
    {
      ...
      start--;// lazy ness so we can update first thing in each iteration (we
              // dont actually want to change the first value in the looping
      do {
        start++;
        ...
        final int y = (x * x + x) / 2 + start - base.length() - 1 - base.length() * x;// the
                                                                                      // first
                                                                                      // term
                                                                                      // is
                                                                                      // safe
        ...
        /*
         * If the x coefficeint is zero, we will jump to the next non zero x.
         * This means y must change as well, so we will check if that has
         * happened by grabbing another iterator to get the value. If this has
         * happened, we know that y should be set to x's value
         */
        final int nextXIndex = candidateOuterIter.hasNext() ? base.getNonZeroIterator(x).next().getIndex() : -1;
        if (candidateOuterIter.hasNext() && nextXIndex > x) {// x is at a zero,
                                                             // so we need to
                                                             // inner iter to go
                                                             // back to the
                                                             // "begining"
          candidateInerIter = base.getNonZeroIterator(nextXIndex);// next
                                                                  // variable
                                                                  // starts at
                                                                  // val^2
        } else {
    ...
    return new Iterator<IndexValue>() {
      int stage = startStage;// 0 is for bias, 1 is for stanrdard values, 2 is
                             // for combinations, 3 is for empty
      ...
      public IndexValue next() {
        if (stage == 0) {
          stage++;
          outerIter = base.getNonZeroIterator();// we know its non empty b/c of
                                                // first case
          return new IndexValue(0, 1.0);
        } else if (stage == 1) // outerIter must always have a next item if
                               // stage = 1
        {
  ...
  /*
   * Some math needed for this class to make sense. Given an input we want to
   * poly 2 form plus a bias term. So for (x + y + z) we want (1 + x+ y + z +
   ...
   * give me the index of the coeff that contains their product. Let x start
   * from 0 and let x^2 also start from zero, so we map from one space to the
   * other.
   * 
