...
/**
 * This class is used to create an implicit representation of the degree 2 polynomial of an input vector, with an
 * implicit bias term added so that the original vector values are present in the implicit vector. This means no extra
 * memory will be allocated, and all values accessed will be re-computed as needed. This works with sparse vectors, and
 * work bet with algorithms that iterate over the nonzero values once.
 * <br><br>
 * Any change in the base vector will change the values in this vector. Because changing one value in the base effects
 * multiple values in this one, altering this vector directly is not allowed.
 * <br><br>
 * If the base vector has {@code N} non zero values, then this vec will have O(N<sup>2</sup>) non zero values.
 * (N+2)(N+1)/2 non zero values to be exact.
 *
 * @author Edward Raff
 */
public class Poly2Vec extends Vec {
  ...
  /**
   * This maps values pas the original coefficients (and bias term) shifted to start from zero, to the appropriate value
   * for the fist coefficient.
   * <br>
