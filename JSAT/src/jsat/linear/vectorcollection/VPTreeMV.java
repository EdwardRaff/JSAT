...
/**
 * The VPTreeMV is an extension of the VPTree, the MV meaning "of Minimum Variance". This extension alters the splitting
 * method of nodes, giving up the O(log n) bound on query time. This is done to reduce the variance in the distance to
 * the parent node of each split, which can result in lopsided distributions of values for each split. At the same time,
 * this lopsidedness may better reflect the locality of points in the data set. This can result in a decrease in query
 * time for some data sets, with minimal impact on construction time.
 *
 * @author Edward Raff
 */
