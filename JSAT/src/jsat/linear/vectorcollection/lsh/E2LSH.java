 ...
 * {@link ManhattanDistance L<sub>1</sub>} and {@link EuclideanDistance L
 * <sub>2</sub> } distance metrics. This is essentially a vector collection that
 * can only perform a radius search for a pre-defined radius. In addition, the
 * results are only approximate - not all of the correct points may be returned,
 * and it is possible no points will be returned when the truth is that some
 * data points do exist. <br>
 * <br>
 ...
 * See:<br>
 * <ul>
 * <li>Datar, M., Immorlica, N., Indyk, P.,&amp;Mirrokni, V. S. (2004).
 * <i> Locality-sensitive hashing scheme based on p-stable distributions</i>.
...
public class E2LSH<V extends Vec> {
   ...
   * @param c
   *          the approximation constant &gt; > 1
   * @return the exact P2 value to use
   */
  private static double getP2L1(final double w, final double c) {
   ...
   * @return the exact P2 value to use
   */
  private static double getP2L2(final double w, final double c) {
   ...
   *
   * @param vecs
   *          the set of vector to place into the LSH
   * @param radius
   *          the searchR radius for vectors
   * @param eps
   ...
   *          computed.
   * @param delta
   *          (1-delta) will be the desired minimum probability of correctly
   *          selecting the correct nearest neighbor if there is only 1-NN
   *          within a distance of {@code radius}. It will be used to determine
   *          some number {@link #getL() } hash tables to reach the desired
   *          probability. 0.10 is a good value.
   * @param dm
   *          the distance metric to use, must be {@link EuclideanDistance} or
   *          {@link ManhattanDistance}.
   */
  public E2LSH(final List<V> vecs, final double radius, final double eps, final int w, final int k, final double delta,
   ...
   *          {@link ManhattanDistance}.
   * @param distCache
   *          the distance acceleration cache to use, if {@code null}, and it is
   *          supported, one will not be built. This is provided to a void
   *          redundant calculation when initializing multiple LSH tables using
   *          the same data set.
   */
  public E2LSH(final List<V> vecs, final double radius, final double eps, final int w, final int k, final double delta,
      final DistanceMetric dm, final List<Double> distCache) {
    this.vecs = vecs;
    setRadius(radius);
    this.delta = delta;
    setEps(eps);
   ...
   */
  private void createTablesAndHashes(final Random rand) {
    final int D = vecs.get(0).length();
    h = new Vec[L][k];
    b = new double[L][k];

    for (int l = 0; l < L; l++) {
      for (int i = 0; i < k; i++) {
        final DenseVector dv = new DenseVector(D);
        for (int j = 0; j < D; j++) {
    ...
    for (int l = 0; l < L; l++) {
      ...
      for (int id = 0; id < vecs.size(); id++) {
        final int hash = hash(l, vecs.get(id));
  ...
  private int hash(final int l, final Vec v) {
   ...
   *
   * @param q
   *          the query point to search near
   ...
   */
  public List<? extends VecPaired<Vec, Double>> searchR(final Vec q) {
   ...
   *          the query point to search near
   * @param approx
   *          whether or not to return results in the approximate query range
   * @return a list of vectors paired with their true distance from the query
   *         point that are within the desired radius of the query point
   */
  public List<? extends VecPaired<Vec, Double>> searchR(final Vec q, final boolean approx) {
    final List<VecPairedComparable<Vec, Double>> toRet = new ArrayList<VecPairedComparable<Vec, Double>>();

    final Set<Integer> candidates = new IntSet();
    for (int l = 0; l < L; l++) {
      final int hash = hash(l, q);
      final List<Integer> list = tables.get(l).get(hash);
    ...
    for (final int id : candidates) {
      final double trueDist = dm.dist(id, q, q_qi, vecs, distCache);
      if (trueDist <= R) {
  ...
  private void setEps(final double eps) {
    this.eps = eps;
    this.c = eps + 1;
  }
