...
/**
 * This is an implementation of Locality Sensitive Hashing for the {@link ManhattanDistance L<sub>1</sub>} and
 * {@link EuclideanDistance L<sub>2</sub> } distance metrics. This is essentially a vector collection that can only
 * perform a radius search for a pre-defined radius. In addition, the results are only approximate - not all of the
 * correct points may be returned, and it is possible no points will be returned when the truth is that some data points
 * do exist.
 * <br><br>
 * Searching is done using the {@link #searchR(jsat.linear.Vec, boolean) }
 * methods. While the set of points returned is approximate, the distance values are exact. This is because no
 * approximate distance is available, so the distances must be computed to remove violators.
 * <br><br>
 * LSH may be useful if any of the following apply to your problem<br>
 * <ul>
 * <li>Only need to do a radius searches of a small number of fixed size increments</li>
 * <li>You need only the first few nearest neighbors, and can compute a threshold for the NN</li>
 * <li>Approximate neighbor results do not heavily impact the results of your algorithm</li>
 ...
 * Locality-sensitive hashing scheme based on p-stable distributions</i>. Proceedings of the twentieth annual symposium
 * on Computational geometry - SCG ’04 (pp. 253–262). New York, New York, USA: ACM Press. doi:10.1145/997817.997857</li>
 * <li> Andoni, Alex (2005).
...
public class E2LSH<V extends Vec> {
  ...
  private double c;
  /**
   * 1-delta is the probability of correctly selecting the neighbors within radius R
   */
   ...
   * @param eps the approximation error, where vectors as fast as R(1+eps) are likely to be returned. Must be positive.
   * @param w the projection radius. If given a value &lt;= 0, a default value of 4 will be used.
   * @param k the number of hash functions to conjoin into the final hash per vector. If a value &lt;= 0 is given, a
   * default value will be computed.
   * @param delta (1-delta) will be the desired minimum probability of correctly selecting the correct nearest neighbor
   * if there is only 1-NN within a distance of {@code radius}. It will be used to determine some number {@link #getL()
   * } hash tables to reach the desired probability. 0.10 is a good value.
   * @param dm the distance metric to use, must be {@link EuclideanDistance} or {@link ManhattanDistance}.
   * @param distCache the distance acceleration cache to use, if {@code null}, and it is supported, one will not be
   * built. This is provided to a void redundant calculation when initializing multiple LSH tables using the same data
   * set.
   */
...
//        L = (int) ceil(pow(vecs.size(), log(1/p1)/log(1/p2)));
  ...
  /**
   * Performs a search for points within the set {@link #getRadius() radius} of the query point.
   *
   * @param q the query point to search near
   * @return a list of vectors paired with their true distance from the query point that are within the desired radius
   * of the query point
   */
  ...
  /**
   * Returns the exact value value that should be used with the euclidean distance for the P2 probability .
   *
  ...
  /**
   * Returns the exact value value that should be used with the manhattan distance for the P2 probability .
   *
   * @param w the projection distance
   * @param c the approximation constant &gt; > 1
   * @return the exact P2 value to use
  ...
  /**
   * Creates and initializes the tables of hash functions for {@link #h} and {@link #b}
   *
  ...
  /**
   * Sets the distance metric and {@link #p1} and {@link #p2}. Must be called after {@link #setEps(double) } and
   * {@link #w} are set.
   *
