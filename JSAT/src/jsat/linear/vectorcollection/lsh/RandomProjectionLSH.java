 ...
 * correct. For this reason the results are also approximate. <br>
 * <br>
 * See:<br>
 * <ul>
 ...
 * Theory of computing - STOC ’02 (pp. 380–388). New York, New York, USA: ACM
...
public class RandomProjectionLSH<V extends Vec> implements VectorCollection<V> {
  ...
  public static class RandomProjectionLSHFactory<V extends Vec> implements VectorCollectionFactory<V> {
     ...
     *
     * @param intsToUse
     *          the number of integers to use for the encoding
     * @param inMemory
     *          {@code true} to construct the full matrix in memory, or
     *          {@code false} to construct the needed values on demand. This
     *          reduces
     */
    ...
    /**
     * Creates a new Random Projection LSH factory that uses a pool of normally
     ...
     *          the number of integers to use for the encoding
     * @param poolSize
     *          the number of normally distributed random variables to store.
     *          Matrix values will be pulled on demand from an index in the pool
     *          of values.
     */
  ...
  /*
   * Implemtation note: store an integer for the bits, but if we use a small 64
   * bit encoding, we waste anotehr 32-64 bits on object overhead, so store it
   * all in one big array isntead of individual ones. Haming distance is the
   * number of bit differences, and sinx XOR results in a 1 bit only if the bits
   * are not the same, we can use the bit count of the XORed value to count the
   * hamming distance.
   */
   ...
   *
   * @param vecs
   *          the list of vectors to form a collection for
   * @param ints
   *          the number of integers to use for the encoding
   * @param inMemory
   *          {@code true} to construct the full matrix in memory, or
   *          {@code false} to construct the needed values on demand. This
   *          reduces memory use at increased CPU usage.
   */
   ...
   * @param slot
   *          the index into the array to start placing the bit values
   * @param projected
   *          a vector full of zeros of the same length as
  ...
  public List<? extends VecPaired<V, Double>> search(final Vec query, final int neighbors) {
    final BoundedSortedList<VecPairedComparable<V, Double>> toRet = new BoundedSortedList<VecPairedComparable<V, Double>>(
        neighbors);
