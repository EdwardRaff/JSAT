...
/**
 * An implementation of Locality Sensitive Hashing for the {@link CosineDistance} using random projections. This forms a
 * vector collection that performs a linear search of the data set, but does so in a more efficient manner by comparing
 * hamming distances in a byte array. However, the reported distances are only approximations - and may not be correct.
 * For this reason the results are also approximate.
 * <br><br>
 * See:<br>
 * <ul>
 * <li>Charikar, M. S. (2002). <i>Similarity estimation techniques from rounding algorithms</i>. Proceedings of the
 * thiry-fourth annual ACM symposium on Theory of computing - STOC ’02 (pp. 380–388). New York, New York, USA: ACM
 * Press. doi:10.1145/509907.509965</li>
 * <li>Durme, B. Van,&amp;Lall, A. (2010). <i>Online Generation of Locality Sensitive Hash Signatures</i>. Proceedings
 * of the ACL 2010 Conference Short Papers (pp. 231–235). Stroudsburg, PA, USA.</li>
 * </ul>
 *
 * @author Edward Raff
 */
public class RandomProjectionLSH<V extends Vec> implements VectorCollection<V> {
   ...
   * @param inMemory {@code true} to construct the full matrix in memory, or {@code false} to construct the needed
   * values on demand. This reduces memory use at increased CPU usage.
   */
  ...
  /**
   * Creates a new Random Projection LSH object that uses a pool of normally distributed values to approximate a full
   * matrix with considerably less memory storage.
   *
   * @param vecs the list of vectors to form a collection for
   * @param ints the number of integers to use for the encoding
   * @param poolSize the number of normally distributed random variables to store. Matrix values will be pulled on
   * demand from an index in the pool of values.
   */
   ...
   * @param projected a vector full of zeros of the same length as {@link #getSignatureBitLength() } to use as a temp
   * space.
   */
  ...
  public static class RandomProjectionLSHFactory<V extends Vec> implements VectorCollectionFactory<V> {
    ...
    /**
     * Creates a new Random Projection LSH factory that uses a pool of normally distributed values to approximate a full
     * matrix with considerably less memory storage.
     *
     * @param intsToUse the number of integers to use for the encoding
     * @param poolSize the number of normally distributed random variables to store. Matrix values will be pulled on
