...
/**
 * Provides an implementation of Vantage Point Trees, as described in "Data Structures and Algorithms for Nearest
 * Neighbor Search in General Metric Spaces" by Peter N. Yianilos
 * <br>
 * VPTrees are more expensive to create, requiring O(n log n) distance computations. However, they work well for high
 * dimensional data sets, and provide O( log n ) query time for {@link #search(jsat.linear.Vec, int) }
 * <br>
 * Note: In the original paper, the VP-tree is detailed, and then enhanced to the VPs-tree, and the VPsb-tree, which
 * each add additional optimizations. This implementation is equivalent to the VPsb-tree presented in the original
 * paper.
 *
 * @author Edward Raff
 */
public class VPTree<V extends Vec> implements VectorCollection<V> {
  ...
  /**
   * Computes the distances to the vantage point, Sorts the list by distance to the vantage point, finds the splitting
   * index, and sets up the parent node.
   *
   * @param S the list
   ...
   * @return the index that should be used to split on [0, index] belonging to the left, and (index, S.size() )
  ...
  /**
   * Returns the maximum leaf node size. Leaf nodes are used to reduce inefficiency of splitting small lists. If a
   * sublist will fit into a leaf node, a leaf node will be created instead of splitting. This is the maximum number of
   * points that may be used to construct a leaf node.
   *
  ...
  /**
   * Sets the maximum leaf node size. Leaf nodes are used to reduce inefficiency of splitting small lists. If a sublist
   * will fit into a leaf node, a leaf node will be created instead of splitting. This is the maximum number of points
   * that may be used to construct a leaf node. <br>
  ...
  private abstract class TreeNode implements Cloneable, Serializable {
     ...
     * @param x the distance between this node's parent vantage point to the query vector. Though not all nodes will use
     * this value, the leaf nodes will - so it should always be given. Initial calls from the root node may choose to us
     * zero.
