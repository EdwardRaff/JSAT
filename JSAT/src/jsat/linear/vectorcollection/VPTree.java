 ...
 * Structures and Algorithms for Nearest Neighbor Search in General Metric
 ...
 * provide O( log n ) query time for {@link #search(jsat.linear.Vec, int) } <br>
 * Note: In the original paper, the VP-tree is detailed, and then enhanced to
 * the VPs-tree, and the VPsb-tree, which each add additional optimizations.
 * This implementation is equivalent to the VPsb-tree presented in the original
 * paper.
 *
 * @author Edward Raff
 */
public class VPTree<V extends Vec> implements VectorCollection<V> {
  ...
  private abstract class TreeNode implements Cloneable, Serializable {
     ...
     * @param x
     *          the distance between this node's parent vantage point to the
     *          query vector. Though not all nodes will use this value, the leaf
     *          nodes will - so it should always be given. Initial calls from
     *          the root node may choose to us zero.
     * @param qi
     *          the value of qi
     */
     ...
     *          the query vector
     * @param range
     *          the maximal distance a point can be from the query point to be
     *          added to the return list
     * @param list
     *          the storage location on the data points within the range of the
     *          query vector
     * @param x
     *          the distance between this node's parent vantage point to the
     *          query vector. Though not all nodes will use this value, the leaf
  ...
  private class VPLeaf extends TreeNode {
    ...
    public VPLeaf(final int[] points, final double[] bounds) {
      this.bounds = Arrays.copyOf(bounds, bounds.length);
      this.points = new int[points.length];
      System.arraycopy(points, 0, this.points, 0, points.length);
    }

    public VPLeaf(final List<Pair<Double, Integer>> points) {
    ...
    @Override
    public void searchKNN(final Vec query, final int k, final BoundedSortedList<ProbailityMatch<V>> list,
        final double x, final List<Double> qi) {
      double dist = -1;

      // The zero check, for the case that the leaf is the ONLY node, x will be
      // passed as 0.0 <= Max value will be true
    ...
    @Override
    public void searchRange(final Vec query, final double range, final List<VecPaired<V, Double>> list, final double x,
        final List<Double> qi) {
  ...
  private class VPNode extends TreeNode {
    ...
    public VPNode(final int p) {
      this.p = p;
    }

    @Override
    public TreeNode clone() {
      final VPNode clone = new VPNode(p);
    ...
    private boolean searchInLeft(final double x, final double tau) {
      if (left == null) {
        return false;
      }
    ...
    private boolean searchInRight(final double x, final double tau) {
      if (right == null) {
        return false;
      }
    ...
    @Override
    public void searchKNN(final Vec query, final int k, final BoundedSortedList<ProbailityMatch<V>> list, double x,
        final List<Double> qi) {
      ...
      final double middle = (this.left_high + this.right_low) * 0.5;
    ...
    @Override
    public void searchRange(final Vec query, final double range, final List<VecPaired<V, Double>> list, double x,
        final List<Double> qi) {
  ...
  public static class VPTreeFactory<V extends Vec> implements VectorCollectionFactory<V> {
    ...
    public VPTreeFactory(final VPSelection vpSelectionMethod) {
  ...
  public VPTree(final List<V> list, final DistanceMetric dm) {
    this(list, dm, VPSelection.Random);
  }

  public VPTree(final List<V> list, final DistanceMetric dm, final VPSelection vpSelection) {
    this(list, dm, vpSelection, new Random(), 80, 40);
  }

  public VPTree(final List<V> list, final DistanceMetric dm, final VPSelection vpSelection, final Random rand,
      final int sampleSize, final int searchIterations) {
    this(list, dm, vpSelection, rand, sampleSize, searchIterations, null);
  }

  public VPTree(final List<V> list, final DistanceMetric dm, final VPSelection vpSelection, final Random rand,
      final int sampleSize, final int searchIterations, final ExecutorService threadpool) {
    this.dm = dm;
    if (!dm.isSubadditive()) {
    ...
    final List<Pair<Double, Integer>> tmpList = new SimpleList<Pair<Double, Integer>>(list.size());
   ...
   *          the object ot copy
   */
  protected VPTree(final VPTree<V> toClone) {
  ...
  /**
   * Returns the maximum leaf node size. Leaf nodes are used to reduce
   * inefficiency of splitting small lists. If a sublist will fit into a leaf
   * node, a leaf node will be created instead of splitting. This is the maximum
   * number of points that may be used to construct a leaf node.
   *
  ...
  private TreeNode makeVPTree(final List<Pair<Double, Integer>> S) {
    if (S.isEmpty()) {
      return null;
    } else if (S.size() <= maxLeafSize) {
      final VPLeaf leaf = new VPLeaf(S);
      return leaf;
    }

    final VPNode node = new VPNode(selectVantagePoint(S));

    final int splitIndex = sortSplitSet(S, node);

    /*
     * Re use the list and let it get altered. We must compute the right side
     * first. If we altered the left side, the median would move left, and the
     * right side would get thrown off or require aditonal book keeping.
     */
  ...
  private TreeNode makeVPTree(final List<Pair<Double, Integer>> S, final ExecutorService threadpool,
  ...
  public List<? extends VecPaired<V, Double>> search(final Vec query, final double range) {
    ...
    final List<VecPairedComparable<V, Double>> returnList = new ArrayList<VecPairedComparable<V, Double>>();
  ...
  public List<? extends VecPaired<V, Double>> search(final Vec query, final int neighbors) {
    final BoundedSortedList<ProbailityMatch<V>> boundedList = new BoundedSortedList<ProbailityMatch<V>>(neighbors,
        neighbors);
  ...
  /**
   * Determines what point from the data set will become a vantage point, and
   * removes it from the list
   *
   * @param S
   *          the set to select a vantage point from
   * @return the index of thevantage point removed from the set
   */
  private int selectVantagePoint(final List<Pair<Double, Integer>> S) {
    final int vpIndex = selectVantagePointIndex(S);

    return S.remove(vpIndex).getSecondItem();
  }

  private int selectVantagePointIndex(final List<Pair<Double, Integer>> S) {
    int vpIndex;
    if (vpSelection == VPSelection.Random) {
      vpIndex = rand.nextInt(S.size());
    } else// Sampling
    {
      final List<Integer> samples = new IntList(sampleSize);
      ...
      final double[] distances = new double[sampleSize];
      ...
      for (int i = 0; i < Math.min(searchIterations, S.size()); i++) {
        ...
        final int candIndx = searchIterations <= S.size() ? i : rand.nextInt(S.size());
        final int candV = S.get(candIndx).getSecondItem();
        ...
        final double median = distances[distances.length / 2];
        double spread = 0;
  ...
  /**
   * Sets the maximum leaf node size. Leaf nodes are used to reduce inefficiency
   * of splitting small lists. If a sublist will fit into a leaf node, a leaf
   * node will be created instead of splitting. This is the maximum number of
   * points that may be used to construct a leaf node. <br>
   * The minimum leaf size is 5 for implementation reasons. If a value less than
   * 5 is given, 5 will be used isntead.
   *
   * @param maxLeafSize
   *          the new maximum leaf node size.
   */
  public void setMaxLeafSize(final int maxLeafSize) {
  ...
  /**
   * Computes the distances to the vantage point, Sorts the list by distance to
   * the vantage point, finds the splitting index, and sets up the parent node.
   *
   * @param S
   *          the list
   * @param node
  ...
  private int sortSplitSet(final List<Pair<Double, Integer>> S, final VPNode node) {
    for (final Pair<Double, Integer> S1 : S) {
      S1.setFirstItem(dm.dist(node.p, S1.getSecondItem(), allVecs, distCache)); // Each
                                                                                // point
                                                                                // gets
                                                                                // its
                                                                                // distance
    ...
    final int splitIndex = splitListIndex(S);
   ...
   * @return the index that should be used to split on [0, index] belonging to
   *         the left, and (index, S.size() ) belonging to the right.
   */
