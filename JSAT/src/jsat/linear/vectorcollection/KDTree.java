...
/**
 * Standard KDTree implementation. KDTrees are fast to create with no distance computations needed. Though KDTrees can
 * be constructed in O(n) time, this implementation is O(n log n). KDTrees can be very fast for low dimensional data
 * queries, but degrade as the dimensions increases. For very high dimensions or pathologically bad data,
 * O(n<sup>2</sup>) performance worse then {@link VectorArray} can occur.
 * <br>
 * <br>
 * Note: KD trees are only usable with Distance Metrics based off of the pNorm between two vectors. The valid distance
 * metrics are
...
public class KDTree<V extends Vec> implements VectorCollection<V> {
   ...
   * These control the method used to determine the pivot at each step.
   */
  public enum PivotSelection {
    /**
     * The next pivot will be selected by iteratively going through each possible pivot. This method has no additional
     * overhead.
     */
    Incremental,
    /**
     * The next pivot will be selected by determining which pivot index contains the most variance. This method requires
     * an additional O(n d) work per step. Where n is the number of data points being split, and d is the dimension of
     * the data set.
     */
    Variance
  }
   ...
   * @param threadpool the source of threads to use when constructing. Null is permitted, in which case a serial
