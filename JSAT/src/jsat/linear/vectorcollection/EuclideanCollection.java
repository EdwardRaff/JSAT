 ...
 * d(x, y)<sup>2</sup> = || x - y||<sup>2</sup> = &lt;x, x&gt; + &lt;y, y&gt; -
 * 2 &lt;x, y&gt; <br>
...
public class EuclideanCollection<V extends Vec> implements VectorCollection<V> {
   ...
   *
   * @param source
   *          the set of vectors to form the collection
   */
  ...
  public EuclideanCollection(final List<V> source, final ExecutorService threadpool) {
    ...
    for (int id = 0; id < SystemInfo.LogicalCores; id++) {
      final int S = start;
      final int E = id == SystemInfo.LogicalCores - 1 ? dotCache.length
          : start + dotCache.length / SystemInfo.LogicalCores;
      start = E;
  ...
  public List<? extends VecPaired<V, Double>> search(final Vec query, final double range) {
    ...
    // get the sacled and shifted value of range for fast comparison for d(x,
    // y)^2
