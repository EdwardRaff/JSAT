...
/**
 * Provides a specialized collections that only supports the {@link EuclideanDistance} by brute force search. It does
 * this more efficiently by decomposing the squared distance into<br>
 * d(x, y)<sup>2</sup> = || x - y||<sup>2</sup> = &lt;x, x&gt; + &lt;y, y&gt; - 2 &lt;x, y&gt;
 * <br>
 * The shortest distances can then be obtained using only 1 dot product per element, as the input and collection values
 * are cached. This avoids redundant calculations present in the naive approach, and can result in drastic performance
 * increases when using sparse vectors or a mix of dense and sparse vectors.
 *
 * @author Edward Raff
 */
