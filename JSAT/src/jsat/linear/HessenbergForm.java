...
public class HessenbergForm implements Serializable {
  ...
  public static void hess(final Matrix A) {
    hess(A, null);
  }
   ...
   *
   * @param A
   *          the matrix to transform into upper Hessenberg form
   */
  public static void hess(final Matrix A, final ExecutorService threadpool) {
    if (!A.isSquare()) {
     ...
     */
    final DenseVector columnUpdateTmp = new DenseVector(m);
    final double[] vk = new double[m];
    /**
     * Space used for updating the sub matrix at step i
     */
    final double[] subMatrixUpdateTmp = new double[m];
    ...
    for (int i = 0; i < m - 2; i++) {
      ...
      vk[i + 1] = sigh;
      s += sigh * sigh;
      sigh = sigh > 0 ? 1 : -1;// Sign dosnt change the squaring, so we do it
                               // first
      ...
      final double s1 = -sigh * Math.sqrt(s);
      // Now re use s to quickly get the norm of vk, since it will be almost the
      // same vector
      s -= vk[i + 1] * vk[i + 1];
      vk[i + 1] -= s1;
      s += vk[i + 1] * vk[i + 1];
      final double s1Inv = 1.0 / Math.sqrt(s);// Re use to store the norm of vk.
                                              // Do the inverse to multiply
                                              // quickly instead of divide
      for (int j = i + 1; j < m; j++) {
        vk[j] *= s1Inv;
      }

      // Update sub sub matrix A[i+1:m, i:m]
      // NOTE: The first column that will be altered can be done ourslves, since
      // we know the value set (s1) and that all below it will ber zero
      Matrix subA = new SubMatrix(A, i + 1, i, m, m);
      final DenseVector vVec = new DenseVector(vk, i + 1, m);
      final Vec tmpV = new DenseVector(subMatrixUpdateTmp, i, m);
      tmpV.zeroOut();
