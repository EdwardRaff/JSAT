...
/**
 * Vec is a object representing the math concept of a vector. A vector could be either sparse or dense, where sparse
 * vectors have a high number of zero values that are not explicitly stored.
 * <br><br>
 * This abstract class provides a large number of pre-implemented methods. Some of which are implemented only for a
 * dense vector, or may not be completely efficient for the underlying implementation. Methods that should be considered
 * for overloading by an implementation will be indicated in the documentation.
 *
 * @author Edward Raff
 */
public abstract class Vec implements Cloneable, Iterable<IndexValue>, Serializable {
  ...
  /**
   * Indicates whether or not this vector can be mutated. If {@code false}, any method that contains "mutate" will not
   * work.
   * <br><br>
   * By default, this returns {@code true}
   *
   * @return {@code true} if the vector supports being altered, {@code false} other wise.
   */
  ...
  /**
   * Returns a suitable vector that can be altered for some function of the form a <i>op</i> b, where {@code a = this}
   *
   ...
   * @throws IndexOutOfBoundsException if the index given is greater than or equal to its {@link #length() }
   */
  ...
  /**
   * Returns a new vector that is the result of multiplying each value in {@code this} by its corresponding value in
   * {@code b}
   *
   * @param b the vector to pairwise multiply by
   * @return the result of the pairwise multiplication of {@code b} onto the values of {@code this}
   */
  ...
  /**
   * Returns a new vector that is the result of dividing each value in {@code this} by the value in the same index in
   * {@code b}
   *
  ...
  /**
   * Mutates {@code this} by multiplying each value by the value in {@code b} that has the same index.
   * <br><br>
   * This method should be overloaded for a serious implementation.
   *
   * @param b the vector to pairwise multiply by
  ...
  /**
   * Mutates {@code this} by dividing each value by the value in {@code b} that has the same index
   * <br><br>
   * This method should be overloaded for a serious implementation.
   *
   * @param b the vector to pairwise divide by
  ...
  /**
   * Computes the variance of the values in this vector, which is {@link #standardDeviation() }<sup>2</sup>
   *
   * @return the variance
   */
  ...
  /**
   * Indicates whether or not this vector is optimized for sparce computation, meaning that most values in the vector
   * are zero - and considered implicit. Only non-zero values are stored.
   *
  ...
  /**
   * Returns a new vector that is the result of normalizing this vector by the L<sub>2</sub> norm
   *
  ...
  /**
   * Applies the given function to each and every value in the vector. The function takes 2 arguments, an arbitrary
   * value, and then an index. The index passed to the function is the index in the array that the value came from.
   * <br><br>
   * <b><i>NOTE:</i></b> Because negative values are invalid indexes. The given function should return 0.0 when given a
   * negative index, if and only if, f(0,index) = 0 for any valid index. If f(0, index) != 0 for even one value of
   * index, it should return any non zero value when given a negative index.
   * <br><br>
   * IE: f(value_i, i) = x
   * <br><br>
   * This method should be overloaded for a serious implementation.
  ...
  /**
   * Returns an iterator that will go over the non zero values in the given vector. The iterator does not support the {@link Iterator#remove()
   * }
   * method.
   *
   ...
   * This method should be overloaded for a serious implementation.
   *
   * @param start the first index (inclusive) to start returning non-zero values from
   * @return an iterator for the non zero index value pairs
   */
  ...
  /**
   * Provides a hashcode for Vectors. All vector implementations should return the same result for cases when {@link #equals(java.lang.Object)
   * } returns true. Below is the code used for this class<br>
   * <p>
   * <code>
   * int result = 1;<br>
   * <br>
