 ...
 * though may not need all the properties to hold. <br>
 * <br>
 ...
 * automatically use the acceleration capabilities of supported distance
 * metrics.
 *
 * @author Edward Raff
 */
public interface DistanceMetric extends Cloneable, Serializable {
   ...
   * with that of another vector not from the original list. <br>
   * If the cache input is {@code null}, then
   * {@link #dist(jsat.linear.Vec, jsat.linear.Vec) } will be called directly.
   *
   * @param a
  ...
  /**
   * Computes the distance between one vector in the original list of vectors
   * with that of another vector not from the original list, but had information
   * generated by {@link #getQueryInfo(jsat.linear.Vec) }. <br>
   * If the cache input is {@code null}, then
   * {@link #dist(jsat.linear.Vec, jsat.linear.Vec) } will be called directly.
   *
   * @param a
  ...
  /**
   * Computes the distance between 2 vectors. The smaller the value, the closer,
   * and there for, more similar, the vectors are. 0 indicates the vectors are
   * the same.
   *
   * @param a
   *          the first vector
  ...
  /**
   * Returns a cache of double values associated with the given list of vectors
   * in the given order. This can be used by the distance metric to increase
   * runtime at the cost of memory. This is an optional method. <br>
   * If this metric does not support acceleration, {@code null} will be
   * returned.
   *
   * @param vecs
   *          the list of vectors to build an acceleration cache for
   ...
   *          the list of vectors to build an acceleration cache for
   * @param threadpool
   *          source of threads for parallel computation of result. This may be
   *          {@code null}, which means the
   *          {@link #getAccelerationCache(java.util.List) singled threaded}
   *          version may be used.
   * @return the list of double for the cache
   */
   ...
   * large number of distance computations are going to be done against points
   * in the original set for a point that is outside the original space. <br>
   * <br>
   * If this metric does not support acceleration, {@code null} will be
   * returned.
   *
  ...
  /**
   * Returns true if this distance metric obeys the rule that, for any x, y, and
   * z &isin; S <br>
  ...
  /**
   * All metrics must return values greater than or equal to 0. The upper bound
   * on the value returned is different for different metrics. This method
   * returns the theoretical maximal value that could be returned by this
   * distance metric. That means {@link Double#POSITIVE_INFINITY } is a valid
   * return value.
   *
   * @return the maximal distance for any two points in that could exist by this
   *         distance metric.
   */
   ...
   * {@link #dist(int, jsat.linear.Vec, java.util.List, java.util.List) } and
   * {@link #dist(int, int, java.util.List, java.util.List) } to perform
