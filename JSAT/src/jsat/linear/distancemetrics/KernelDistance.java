...
public class KernelDistance implements DistanceMetric {
  ...
  /**
   * Creates a distane metric from the given kernel. For the metric to be valid,
   * the kernel must be positive definite. This means that <br>
   * <br>
   * &forall; c<sub>i</sub> &isin; &real; , x<sub>i</sub> &isin; &real;
   * <sup>d</sup> <br>
   * &sum;<sub>i, j = 1</sub><sup>m</sup> c<sub>i</sub> c<sub>j</sub> K(x
   * <sub>i</sub>, x<sub>j</sub>) &ge; 0
   *
   * @param kf
   */
   ...
   * <br>
   * Special Notes:<br>
   * The use of {@link RBFKernel} or {@link PolynomialKernel} of degree 1 in the
   * {@link NearestNeighbour} classifier will degenerate into the normal nearest
   * neighbor algorithm.
   *
   * @param a
   *          the first vector
