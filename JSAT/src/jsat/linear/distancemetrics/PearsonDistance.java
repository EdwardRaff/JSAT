...
public class PearsonDistance implements DistanceMetric {
  ...
  /**
   * Creates a new standard Pearson Distance that does not ignore zero values and anti-correlated values are considered
   * far away.
   */
   ...
   *
   * @param bothNonZero {@code true} if non zero values should be treated as "missing" or "no vote", and will not
   * contribute. But this will not change the mean value used. {@code false} produces the standard Pearson value.
   * @param absoluteDistance {@code true} to use the absolute correlation, meaning correlated and anti-correlated values
   ...
   * <br>
   * In cases where {@code bothNonZero} is {@code true}, and the vectors have no overlapping non zero values, 0 will be
   * returned.
   *
