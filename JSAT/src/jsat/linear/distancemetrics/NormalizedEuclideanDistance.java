...
/**
 * Implementation of the Normalized Euclidean Distance Metric. The normalized version divides each variable by its
 * standard deviation, and then continues as the normal {@link EuclideanDistance}. <br>
 * The same results can be achieved by first applying {@link UnitVarianceTransform} to a data set before using the L2
 * norm.<br>
 * It is equivalent to the {@link MahalanobisDistance} if only the diagonal values were used.
 *
 *
 * @author Edward Raff
 */
