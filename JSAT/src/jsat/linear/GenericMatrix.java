 ...
 * major form. Only a small portion must be implemented by the extending class
 *
 * @author Edward Raff
 */
public abstract class GenericMatrix extends Matrix {
  ...
  /**
   * Step size if the computation accesses 2*NB2^2 * dataTypeSize data, so that
   * the data being worked on fits into the L2 cache
   */
  ...
  public Matrix[] lup(final ExecutorService threadPool) {
    ...
    try {
          ...
          bigIndecies.add(threadPool.submit(new Callable<Integer>() {
            ...
            public Integer call() throws Exception {
              ...
              for (int i = kk + 1 + threadID; i < UU.rows(); i += LogicalCores) {
                ...
                // We perform the first iteration of the loop outside, as we
                // want to cache its value for searching later
      ...
      if (rows() > cols()) // Clean up!
      {
        // We need to change U to a square nxn matrix in this case, we can
        // safely drop the last 2 rows!
        final Matrix newU = getMatrixOfSameType(cols(), cols());
        for (int i = 0; i < cols(); i++) {
          for (int j = 0; j < cols(); j++) {
            newU.set(i, j, U.get(i, j));
  ...
  public void multiply(final Matrix b, final Matrix C) {
    if (!canMultiply(this, b)) {
      throw new ArithmeticException(
          "Matrix dimensions do not agree: [" + rows() + ", " + cols() + "] * [" + b.rows() + ", " + b.cols() + "]");
     ...
     * , see Data structures in Java for matrix computations CONCURRENCY AND
     * COMPUTATION: PRACTICE AND EXPERIENCE Concurrency Computat.: Pract. Exper.
     * 2004; 16:799–815 (DOI: 10.1002/cpe.793)
     */
    ...
    if (b.isSparse()) {
      for (int i = 0; i < rows(); i++) {
        ...
        for (final IndexValue iv : b) {
          dot += get(i, iv.getIndex()) * iv.getValue();
        }
        c.increment(i, dot * z);
      }
    } else {
      ...
      for (int ID = 0; ID < LogicalCores; ID++) {
        ...
        threadPool.submit(new Runnable() {
          ...
          private void parallelQRUpdateQ() {
            ...
            // order, which represents Q in row major order, which is what we
            // want!
          ...
          private void parallelQRUpdateR() {
            // First run of loop removed, as it will be setting zeros. More
            // accurate to just set them ourselves
            if (kk < N && threadID == 0) {
  ...
  private void qrUpdateR(final int k, final int N, final Matrix A, final double[] vk, final double TwoOverBeta,
      final int M) {
  ...
  private void qrUpdateRInitalLoop(final int k, final Matrix A, final double[] vk, final double TwoOverBeta,
      final int M) {
    double y = 0;// y = vk dot A_j
    for (int i = k; i < A.cols(); i++) {
      y += vk[i] * A.get(k, i);
