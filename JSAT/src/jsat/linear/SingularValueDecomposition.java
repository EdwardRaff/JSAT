...
/**
 * The Singular Value Decomposition (SVD) of a matrix A<sub>m,n </sub> = U<sub>m,n </sub> &Sigma;<sub>n,n </sub>
 * V<sup>T</sup><sub>n,n </sub>, where S is the diagonal matrix of the singular values sorted in descending order and
 * are all non negative.
 * <br> The SVD of a matrix has many practical uses, but is expensive to compute.
 * <br><br>
 * Implementation adapted from the Public Domain work of <a href="http://math.nist.gov/javanumerics/jama/"> JAMA: A Java
 * Matrix Package</a>
 * <br>
 * <b>NOTE:</b> The current implementation has been revised and is now passing all test cases. However, it is still
 * being tested. Use with awareness that it used to be bugged. Note left at revision 597
 *
 * @author Edward Raff
 */
public class SingularValueDecomposition implements Cloneable, Serializable {
  ...
  /**
   * Creates a new SVD of the matrix {@code A} such that A = U &Sigma; V<sup>T</sup>. The matrix {@code  A} will be
   * modified and used as temp space when computing the SVD.
   *
  ...
  /**
   * Returns the diagonal matrix S such that the SVD product results in the original matrix. The diagonal contains the
   * singular values.
   *
  ...
  /**
   * Returns the condition number of the matrix. The condition number is a positive measure of the numerical instability
   * of the matrix. The larger the value, the less stable the matrix. For singular matrices, the result is
  ...
  /**
   * Returns the Moore–Penrose pseudo inverse of the matrix. The pseudo inverse for a matrix is unique. If a matrix is
   * non singular, the pseudo inverse is the inverse.
   *
  ...
  /**
   * Computes the pseudo determinant of the matrix, which corresponds to absolute value of the determinant of the full
   * rank square sub matrix that contains all non zero singular values.
   *
  ...
  /**
   * Computes the pseudo determinant of the matrix, which corresponds to absolute value of the determinant of the full
   * rank square sub matrix that contains all non singular values &gt; <tt>tol</tt>.
   *
   * @param tol the cut of for singular values
   * @return the pseudo determinant
   */
   ...
   * <br>
   * When A is not full rank, this results in a more numerically stable approximation that minimizes the least squares
   * error.
   *
