...
/**
 * The Singular Value Decomposition (SVD) of a matrix A<sub>m,n </sub> = U
 * <sub>m,n </sub> &Sigma;<sub>n,n </sub> V<sup>T</sup><sub>n,n </sub>, where S
 * is the diagonal matrix of the singular values sorted in descending order and
 * are all non negative. <br>
 * The SVD of a matrix has many practical uses, but is expensive to compute.
 * <br>
 * <br>
 * Implementation adapted from the Public Domain work of
 * <a href="http://math.nist.gov/javanumerics/jama/"> JAMA: A Java Matrix
 * Package</a> <br>
 * <b>NOTE:</b> The current implementation has been revised and is now passing
 * all test cases. However, it is still being tested. Use with awareness that it
 * used to be bugged. Note left at revision 597
 *
 * @author Edward Raff
 */
public class SingularValueDecomposition implements Cloneable, Serializable {
  ...
  private Matrix U, V;
  /**
   * Stores the diagonal values of the S matrix, and contains the bidiagonal
   * values of A during initial steps.
   */
  private final double[] s;

  /**
   * Creates a new SVD of the matrix {@code A} such that A = U &Sigma; V
   * <sup>T</sup>. The matrix {@code  A} will be modified and used as temp space
   * when computing the SVD.
   *
   * @param A
   *          the matrix to create the SVD of
   */
   ...
   *          the matrix to create the SVD of
   * @param maxIterations
   *          the maximum number of iterations to perform per singular value
   *          till convergence.
   */
  public SingularValueDecomposition(final Matrix A, final int maxIterations) {
    // By doing this we get to keep the colum major algo and get row major
    // performance
   ...
   * @param V
   *          the V matrix of an SVD
   * @param s
   *          the singular sorted by magnitude of an SVD
   */
  ...
  private void bidiagonalize(final int nct, final int nrt, final int m, final Matrix A, final int n, final double[] e,
      final double[] work) {
  ...
  private void case3QRStep(final int p, final double[] e, final int k, final int n, final int m) {
    ...
    final double scale = max(max(max(max(abs(s[p - 1]), abs(s[p - 2])), abs(e[p - 2])), abs(s[k])), abs(e[k]));
  ...
  /**
   * Returns the condition number of the matrix. The condition number is a
   * positive measure of the numerical instability of the matrix. The larger the
   * value, the less stable the matrix. For singular matrices, the result is
  ...
  /**
   * Returns an array containing the inverse singular values. Near zero values
  ...
  /**
   * Returns an array containing the inverse singular values. Values that are
   * &lt;= <tt>tol</tt> are converted to zero.
   *
   * @param tol
   *          the cut of for singular values
   * @return an array containing the inverse singular values
   */
  ...
  /**
   * Computes the pseudo determinant of the matrix, which corresponds to
   * absolute value of the determinant of the full rank square sub matrix that
   * contains all non zero singular values.
   *
   ...
   * Computes the pseudo determinant of the matrix, which corresponds to
   * absolute value of the determinant of the full rank square sub matrix that
   * contains all non singular values &gt; <tt>tol</tt>.
   *
   * @param tol
   *          the cut of for singular values
   * @return the pseudo determinant
  ...
  /**
   * Returns the Moore–Penrose pseudo inverse of the matrix. The pseudo inverse
   * for a matrix is unique. If a matrix is non singular, the pseudo inverse is
   * the inverse.
   *
   ...
   *
   * @param tol
   *          the tolerance for singular values to ignore
   * @return the pseudo inverse of the matrix
   */
  ...
  /**
   * Returns the numerical rank of the matrix. Values &lt;= than <tt>tol</tt>
   * will be ignored.
   *
   * @param tol
   *          the cut of for singular values
  ...
  /**
   * Returns the diagonal matrix S such that the SVD product results in the
   * original matrix. The diagonal contains the singular values.
   *
  ...
  /**
   * Returns a copy of the sorted array of the singular values, include the near
   * zero ones.
   *
   * @return a copy of the sorted array of the singular values, including the
   *         near zero ones.
   */
  ...
  /**
   * Indicates whether or not the input matrix was of full rank, full rank
   * matrices are more numerically stable.
   *
    ...
    while (p > 0 && iter < maxIterations) {
      ...
      if (k == p - 2) {
        ...
        for (ks = p - 1; ks >= k; ks--) {
          ...
          final double t = (ks != p ? abs(e[ks]) : 0.) + (ks != k + 1 ? abs(e[ks - 1]) : 0.);
          if (abs(s[ks]) <= eps * t) {
            s[ks] = 0.0;
            break;
          }
  ...
  /**
   * Solves the linear system of equations for A x = B by using the equation<br>
   * <code>
   * x = A<sup>-1</sup> B = V S<sup>-1</sup> U<sup>T</sup> B </code> <br>
   * When A is not full rank, this results in a more numerically stable
   * approximation that minimizes the least squares error.
   *
   * @param B
  ...
  private void superDiagonalCreation(final double[] e, final int k, final int n, final int m, final double[] work,
      final Matrix A) {
  ...
  private void UVCase12Update(final Matrix UV, final int m, final double cs, final int j, final double sn,
      final int k) {
    double t;
    for (int i = 0; i < m; i++) {
