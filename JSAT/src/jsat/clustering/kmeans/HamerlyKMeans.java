 ...
 * <br>
 * See: Hamerly, G. (2010). <i>Making k-means even faster</i>. SIAM International Conference on Data Mining (SDM) (pp.
 * 130–140). Retrieved from
...
public class HamerlyKMeans extends KMeans {
  ...
  protected double cluster(final DataSet dataSet, List<Double> accelCache, final int k, final List<Vec> means, final int[] assignment, final boolean exactTotal, ExecutorService threadpool, boolean returnError) {
    ...
    /**
     * lower bound on the distance between x(i) and its second closest center – that is, the closest center to x(i) that
     * is not c(a(i)) <br>
     * denoted l(i)
     */
