 ...
 * <a href=
 * "http://72.32.205.185/proceedings/datamining/2010/dm10_012_hamerlyg.pdf">here
 * </a>
 *
 * @author Edward Raff
 */
public class HamerlyKMeans extends KMeans {
   ...
   *          the method of initial seed selection
   * @param rand
   *          the source of randomnes to use
   */
  public HamerlyKMeans(final DistanceMetric dm, final SeedSelectionMethods.SeedSelection seedSelection,
      final Random rand) {
  ...
  protected double cluster(final DataSet dataSet, final List<Double> accelCache, final int k, final List<Vec> means,
    ...
    final List<Double> distAccel;// used like htis b/c we want it final for
                                 // convinence, but input may be null
    if (accelCache == null) {
    ...
    /**
     * upper bound on the distance between x(i) and its assigned center c(a(i))
     * <br>
     * denoted u(i)
     */
    final double[] u = new double[N];
    /**
     * lower bound on the distance between x(i) and its second closest center –
     * that is, the closest center to x(i) that is not c(a(i)) <br>
     * denoted l(i)
     */
  ...
  private void Initialize(final DataSet d, final AtomicLongArray q, final List<Vec> means, final Vec[] tmp,
      final Vec[] cP, final double[] u, final double[] l, final int[] a, final ExecutorService threadpool,
      final ThreadLocal<Vec[]> localDeltas, final List<Vec> X, final List<Double> distAccel,
      final List<List<Double>> meanQI) {
   ...
   *          data set
   * @param i
   *          the index to do the work for
   * @param s
   *          the centroid centroid nearest distance
   * @param assignment
   *          the array assignments are stored in
   * @param u
   *          the "u" array of the algo
   * @param l
   *          the "l" array of the algo
   * @param q
   *          the "q" array of the algo (cluster counts)
   * @param deltas
   *          the location to store the computed delta if one occurs
   * @return 0 if no changes in assignment were made, 1 if a change in
   *         assignment was made
   */
  private int mainLoopWork(final DataSet dataSet, final int i, final double[] s, final int[] assignment,
      final double[] u, final double[] l, final AtomicLongArray q, final Vec[] deltas, final List<Vec> X,
      final List<Double> distAccel, final List<Vec> means, final List<List<Double>> meanQI) {
    final int a_i = assignment[i];
    final double m = Math.max(s[a_i] / 2, l[i]);
    if (u[i] > m) // first bound test
    {
      final Vec x = X.get(i);
      u[i] = dm.dist(i, means.get(a_i), meanQI.get(a_i), X, distAccel);// tighten
                                                                       // upper
                                                                       // bound
  ...
  private void moveCenters(final List<Vec> means, final Vec[] tmpSpace, final Vec[] cP, final AtomicLongArray q,
      final double[] p, final List<List<Double>> meanQI) {
    for (int j = 0; j < means.size(); j++) {
   ...
   */
  private int PointAllCtrs(final Vec x, final int i, final List<Vec> means, final int[] a, final double[] u,
      final double[] l, final List<Vec> X, final List<Double> distAccel, final List<List<Double>> meanQI) {
  ...
  private void updateS(final double[] s, final List<Vec> means, final ExecutorService threadpool,
      final List<List<Double>> meanQIs) {
    final int tasks = means.size();
    final CountDownLatch latch = new CountDownLatch(tasks);
    Arrays.fill(s, Double.MAX_VALUE);
    // TODO temp object for puting all the query info into a cache, should
    // probably be cleaned up - or change original code to have one massive list
    // and then use sub lits to get the QIs individualy
    ...
    for (int j = 0; j < means.size(); j++) {
      if (threadpool == null) {
        ...
        if (otherIndx < s.length) {// if index i is our min, we may be their min
                                   // too
