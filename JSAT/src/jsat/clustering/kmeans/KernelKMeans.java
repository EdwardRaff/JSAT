...
/**
 * Base class for various Kernel K Means implementations. Because the Kernelized version is more computationally
 * expensive, only the clustering methods where the number of clusters is specified apriori are supported. <br>
 * <br>
 * KernelKMeans keeps a reference to the data passed in for clustering so that queries can be conveniently answered,
 * such as getting {@link #findClosestCluster(jsat.linear.Vec) the closest cluster} or finding the
 * {@link #meanToMeanDistance(int, int) distance between means}
 *
 * @author Edward Raff
 */
public abstract class KernelKMeans extends KClustererBase implements Parameterized {
  ...
  /**
   * The normalizing constant for each mean. General this would be 1/owned[k]<sup>2</sup>
   */
  ...
  /**
   * A temporary space for updating ownership designations for each datapoint. When done, this will store the final
   * designations for each point
   */
  ...
  /**
   * Computes the kernel sum of data point {@code i} against all the points in cluster group {@code clusterID}.
   *
  ...
  /**
   * Computes the kernel sum of the given data point against all the points in cluster group {@code clusterID}.
   *
  ...
  /**
   * Updates the normalizing constants for each mean. Should be called after every change in ownership
   */
  ...
  /**
   * Accumulates the updates to the means and ownership into the provided arrays. This does not update
   * {@link #meanSqrdNorms}, and is meant to accumulate the change. To apply the changes pass the same arrays to {@link #applyMeanUpdates(double[], int[])
   * }
   *
   * @param i the index of the data point to try and update the means based on its movement
   * @param designations the old assignments for ownership of each data point to one of the means
   ...
   * @param k1SqrdNorm the <i>normalized</i> squared norm for the mean indicated by {@code k1}. (ie:
   * {@link #meanSqrdNorms} multiplied by {@link #normConsts}
   * @return
   */
