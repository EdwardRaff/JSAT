...
/**
 * This class provides a method of performing {@link KMeans} clustering when the value of {@code K} is not known. It
 * works by recursively splitting means up to some specified maximum. value. <br>
 * <br>
 * When the value of {@code K} is specified, the implementation will simply call the regular KMeans object it was
 * constructed with. <br>
 * <br>
 * Note, that specifying a minimum value of {@code K=1} has a tendency to not be split by the algorithm, returning the
 * naive result of 1 cluster. It is better to use at least {@code K=2} as the default minimum, which is what the
 * implementation will start from when no range of {@code K} is given. <br>
 * <br>
 * See: Pelleg, D.,&amp;Moore, A. (2000). <i>X-means: Extending K-means with Efficient Estimation of the Number of
 * Clusters</i>. In ICML (pp. 727–734). San Francisco, CA, USA: Morgan Kaufmann Publishers Inc. Retrieved from
...
public class XMeans extends KMeans {
  ...
  /**
   * Each new cluster will be tested for improvement according to the BIC metric. If this is set to {@code true} then an
   * optimization is done that once a center fails be improved by splitting, it will never be tested again. This is a
   * safe assumption when {@link #setIterativeRefine(boolean) } is set to {@code false}, but otherwise may not quite be
   * true. <br>
   * <br>
   * When {@code trustH0} is {@code true} , X-Means will make at most O(k) runs of k-means for the final value of k
   * chosen. When {@code false} (the default option), at most O(k<sup>2</sup>) runs of k-means will occur.
   *
   * @param stopAfterFail {@code true} if a centroid shouldn't be re-tested once it fails to split.
   */
  ...
  /**
   *
   * @return {@code true} if clusters that fail to split wont be re-tested. {@code false} if they will.
   */
   ...
   *
   * @param minClusterSize the minimum number of data points that must be present in a cluster to consider splitting it
   */
  ...
  /**
   *
   * @return the minimum number of data points that must be present in a cluster to consider splitting it
   */
  ...
  /**
   * Sets whether or not the set of all cluster centers should be refined at every iteration. By default this is
   * {@code true} and part of how the X-Means algorithm is described. Setting this to {@code false} can result in large
   * speedups at the potential cost of quality.
   *
   * @param refineCenters {@code true} to refine the cluster centers at every step, {@code false} to skip this step of
   * the algorithm.
   */
  ...
  /**
   *
   * @return {@code true} if the cluster centers are refined at every step, {@code false} if skipping this step of the
   * algorithm.
   */
