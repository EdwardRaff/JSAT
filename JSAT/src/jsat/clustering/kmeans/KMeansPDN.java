...
public class KMeansPDN extends KMeans {
  ...
  /**
   * Creates a new clustered that uses the specified object to perform
   * clustering for all {@code k}.
   *
   * @param kmeans
   *          the k-means object to use for clustering
   */
  ...
  public KMeansPDN(final KMeansPDN toCopy) {
    ...
    if (toCopy.fKs != null) {
      fKs = Arrays.copyOf(toCopy.fKs, toCopy.fKs.length);
    }
  }
  ...
  public int[] cluster(final DataSet dataSet, final int lowK, final int highK, final ExecutorService threadpool,
    ...
    double minFk = lowK == 1 ? 1.0 : Double.POSITIVE_INFINITY;// If our low k is
                                                              // > 1, force the
                                                              // check later to
                                                              // kick in at the
                                                              // first candidate
    ...
    for (int k = 2; k < highK; k++) {
      ...
      final double S_k = cluster(dataSet, accelCache, k, curMeans, designations, true, threadpool, true);// TODO
                                                                                                         // could
                                                                                                         // add
                                                                                                         // a
                                                                                                         // flag
    ...
    // contract is we return designations with the data in it if we can, so copy
    // the values back
   ...
   * The value at index {@code i} is the score for cluster {@code i+1}. Smaller
   * values indicate better clusterings.
   *
