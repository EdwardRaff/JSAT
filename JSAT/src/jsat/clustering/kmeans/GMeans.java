 ...
 * constructed with. <br>
 * <br>
 * See: Hamerly, G.,&amp;Elkan, C. (2003). <i>Learning the K in K-Means</i>. In seventeenth annual conference on neural
 * information processing systems (NIPS) (pp. 281–288). Retrieved from
...
public class GMeans extends KMeans {
  ...
  /**
   * Each new cluster will be tested for normality, with the null hypothesis H0 being that the cluster is normal. If
   * this is set to {@code true} then an optimization is done that once a center fails to reject the null hypothesis, it
   * will never be tested again. This is a safe assumption when {@link #setIterativeRefine(boolean) } is set to
   * {@code false}, but otherwise may not quite be true. <br>
   * <br>
   * When {@code trustH0} is {@code true} (the default option), G-Means will make at most O(k) runs of k-means for the
   * final value of k chosen. When {@code false}, at most O(k<sup>2</sup>) runs of k-means will occur.
   *
   * @param trustH0 {@code true} if a centroid shouldn't be re-tested once it fails to split.
   */
  ...
  /**
   *
   * @return {@code true} if cluster that fail to split wont be re-tested. {@code false} if they will.
   */
   ...
   * {@code true} and part of how the GMeans algorithm is described. Setting this to {@code false} can result in large
