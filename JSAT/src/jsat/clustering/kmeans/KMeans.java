...
/**
 * Base class for the numerous implementations of k-means that exist. This base class provides an slow heuristic
 * approach to the selection of k.
 *
 * @author Edward Raff
 */
public abstract class KMeans extends KClustererBase implements Parameterized {
  ...
  /**
   * This is the default seed selection method used in ElkanKMeans. When used with the {@link EuclideanDistance}, it
   * selects seeds that are log optimal with a high probability.
   */
  ...
  /**
   * Indicates whether or not the distance between a datapoint and its nearest centroid should be saved after
   * clustering. This only applies when the error of the model is requested
   */
  ...
  /**
   * If set to {@code true} the computed means will be stored after clustering is completed, and can then be retrieved
   * using {@link #getMeans() }.
   *
   * @param storeMeans {@code true} if the means should be stored for later, {@code false} to discard them once
  ...
  /**
   * Sets the method of seed selection to use for this algorithm. {@link SeedSelection#KPP} is recommended for this
   ...
   * @param accelCache acceleration cache to use, or {@code null}. If {@code null}, the kmeans code will attempt to
   * create one
   * @param k the number of clusters
   * @param means the initial points to use as the means. Its length is the number of means that will be searched for.
   * These means will be altered, and should contain deep copies of the points they were drawn from. May be empty, in
   * which case the list will be filled with some selected means
   ...
   * will be used. This also impacts the value stored in {@link #nearestCentroidDist}
   * @param threadpool the source of threads for parallel computation. If
   * <tt>null</tt>, single threaded execution will occur
   * @param returnError {@code true} is the sum of squared distances should be returned. {@code false} means any value
   * can be returned. {@link #saveCentroidDistance} only applies if this is {@code true}
   * @return the double
   */
