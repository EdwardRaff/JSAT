...
public class DBSCAN extends ClustererBase {
  ...
  private class ClusterWorker implements Runnable {
     ...
     *
     * @param vc
     *          the accelerate structure to search for data points. Must support
     *          concurent method calls
     * @param range
     *          the range to search <tt>tt</tt> with
     * @param resultQ
     *          The que to place this worker object into on completion
     */
    public ClusterWorker(final VectorCollection<VecPaired<Vec, Integer>> vc, final double range,
        final BlockingQueue<List<? extends VecPaired<VecPaired<Vec, Integer>, Double>>> resultQ,
        final BlockingQueue<Vec> sourceQ) {
      this.vc = vc;
      this.range = range;
  ...
  private class StatsWorker implements Callable<OnLineStatistics> {
    ...
    public StatsWorker(final BlockingQueue<DataPoint> queue, final VectorCollection<VecPaired<Vec, Integer>> vc,
        final int minPts) {
      this.queue = queue;
      this.vc = vc;
      this.minPts = minPts;
  ...
  /**
   * Used by {@link #cluster(DataSet, double, int, VectorCollection,int[]) } to
   * mark that a data point as not yet been visited. <br>
   * Clusters that have been visited have a value >= 0, that indicates their
   * cluster. Or have the value {@link #NOISE}
   */
  private static final int UNCLASSIFIED = -1;
  /**
   * Used by
   * {@link #expandCluster(int[], DataSet, int, int, double, int, VectorCollection) }
   * to mark that a data point has been visited, but was considered noise.
   */
  private static final int NOISE = -2;
  /**
   * Factory used to create a vector space of the inputs. The paired Integer is
   * the vector's index in the original dataset
   */
  ...
  public DBSCAN(final DBSCAN toCopy) {
    vecFactory = toCopy.vecFactory.clone();
    dm = toCopy.dm.clone();
    stndDevs = toCopy.stndDevs;
  }
  ...
  public int[] cluster(final DataSet dataSet, final double eps, final int minPts, final ExecutorService threadpool,
  ...
  private int[] cluster(final DataSet dataSet, final double eps, final int minPts,
      final VectorCollection<VecPaired<Vec, Integer>> vc, final ExecutorService threadpool, int[] pointCats) {
    if (pointCats == null) {
  ...
  private int[] cluster(final DataSet dataSet, final double eps, final int minPts,
      final VectorCollection<VecPaired<Vec, Integer>> vc, int[] pointCats) {
    if (pointCats == null) {
      pointCats = new int[dataSet.getSampleSize()];
    }
    Arrays.fill(pointCats, UNCLASSIFIED);
  ...
  public int[] cluster(final DataSet dataSet, final int minPts, final ExecutorService threadpool,
      final int[] designations) {
   ...
   *         noise
   */
  private boolean expandCluster(final int[] pointCats, final DataSet dataSet, final int point, final int clId,
      final double eps, final int minPts, final VectorCollection<VecPaired<Vec, Integer>> vc) {
    ...
    final Queue<VecPaired<VecPaired<Vec, Integer>, Double>> workQue = new ArrayDeque<VecPaired<VecPaired<Vec, Integer>, Double>>(
        seeds);
