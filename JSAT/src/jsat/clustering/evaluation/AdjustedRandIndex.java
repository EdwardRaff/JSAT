...
/**
 * Adjusted Rand Index (ARI) is a measure to evaluate a cluster based on the true class labels for the data set. The ARI
 * normally returns a value in [-1, 1], where 0 indicates the clustering appears random, and 1 indicate the clusters
 * perfectly match the class labels, and negative values indicate a clustering that is worse than random. To match the
 * {@link ClusterEvaluation} interface, the value returned by evaluate will be 1.0-Adjusted Rand Index so the best value
 * becomes 0.0 and the worse value becomes 2.0.
 * <br>
 * <b>NOTE:</b> Because the ARI needs to know the true class labels, only {@link #evaluate(int[], jsat.DataSet) } will
 * work, since it provides the data set as an argument. The dataset given must be an instance of
