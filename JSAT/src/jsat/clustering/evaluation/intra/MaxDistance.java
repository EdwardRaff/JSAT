...
/**
 * Evaluates a cluster's validity by returning the maximum distance between any
      ...
      for (int j = i + 1; j < dataSet.getSampleSize(); j++) {
        if (designations[i] == clusterID) {
          maxDistance = Math.max(
              dm.dist(dataSet.getDataPoint(i).getNumericalValues(), dataSet.getDataPoint(j).getNumericalValues()),
              maxDistance);
        }
      }
    }
  ...
  public double evaluate(final List<DataPoint> dataPoints) {
    ...
    for (int i = 0; i < dataPoints.size(); i++) {
      for (int j = i + 1; j < dataPoints.size(); j++) {
        maxDistance = Math.max(dm.dist(dataPoints.get(i).getNumericalValues(), dataPoints.get(j).getNumericalValues()),
            maxDistance);
      }
    }
