...
/**
 * An implementation of Gaussian Mixture models that learns the specified number
 * of Gaussians using Expectation Maximization algorithm.
 *
 * @author Edward Raff
 */
public class EMGaussianMixture extends ElkanKMeans implements MultivariateDistribution {
  ...
  /**
   * Copy constructor. The new Gaussian Mixture can be altered without effecting
   * <tt>gm</tt>
   *
   * @param gm
   *          the Guassian Mixture to duplicate
   */
  public EMGaussianMixture(final EMGaussianMixture gm) {
    if (gm.gaussians != null && !gm.gaussians.isEmpty()) {
      gaussians = new ArrayList<NormalM>(gm.gaussians.size());
      for (final NormalM gaussian : gm.gaussians) {
        gaussians.add(gaussian.clone());
      }
    }
    if (gm.a_k != null) {
      a_k = Arrays.copyOf(gm.a_k, gm.a_k.length);
    }
    MaxIterLimit = gm.MaxIterLimit;
  ...
  protected double clusterCompute(final int K, final DataSet dataSet, final int[] assignment, final List<Vec> means,
      final List<Matrix> covs, final ExecutorService execServ) {
    ...
    while (true) {
      ...
      try {
        ...
        if (logDifference < tolerance) {
          break;// We accept this as converged. Probablities could be refined,
                // but no one should be changing class anymore
        } else {
  ...
  private double eStep(final int N, final List<DataPoint> dataPoints, final int K, final double[][] p_ik,
      final ExecutorService execServ) throws InterruptedException, ExecutionException {
    double logLike = 0;
    /*
     * p / | p p\ a P|x | mean , Sigma | k \ i | k k/ p =
     * ------------------------------ i k K ===== \ p / | p p\ > a P|x | mean ,
     * Sigma | / k \ i | k k/ ===== k = 1
     */
     ...
     * 
     * N / K \ ===== |===== | \ |\ | L(x, Theat) = > log| > a P/x | mean , Sigma
     * \| / |/ k \ i | k k/| ===== |===== | i = 1 \k = 1 /
     */
    if (execServ == null) {
      for (int i = 0; i < N; i++) {
  ...
  private void mStep(final List<Vec> means, final int N, final List<DataPoint> dataPoints, final int K,
      final double[][] p_ik, final List<Matrix> covs, final ExecutorService execServ) throws InterruptedException {
    /**
     * Dimensions
     */
    ...
    } else// Parllalle version is limited in scalability to the number of
          // clusters k, as we are updating the k's so we can not distribute row
          // wise, but must give each therad its own k
    {
    ...
    // We can now dived all the means by their sums, which are stored in a_k,
    // and then normalized a_k after
    for (int k = 0; k < a_k.length; k++) {
      means.get(k).mutableDivide(a_k[k]);
    }

    // We hold off on nomralized a_k, becase we will use its values to update
    // the covariances
