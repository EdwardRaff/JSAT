...
public abstract class AbstractClusterDissimilarity implements ClusterDissimilarity {
   ...
   * points in the data set. The main diagonal will contain all zeros, since the
   * distance between a point and itself is always zero. This main diagonal is
   * not stored, and is implicit <br>
   * To save space, the matrix is staggered, and is of a size such that all
   * elements to the left of the main diagonal are not present. <br>
   * To compute the index into the returned array for the index [i][j], the
   * values should be switched such that i &ge; j, and accessed as [i][j-i-1]
   *
   * @param dataSet
   *          the data set to create distance matrix for
   * @param cd
   *          the cluster dissimilarity measure to use
  ...
  /**
   * A convenience method. If the <i>distanceMatrix</i> was created with null
   * null
   ...
   *          the distance matrix to query from
   * @param i
   *          the first index
   * @param j
   *          the second index
   * @return the correct value from the distance matrix from the index given as
   *         if the distance matrix was of full form
   */
   ...
   *          the second index
   * @param dist
   *          the new distance value to store in the matrix
   */
