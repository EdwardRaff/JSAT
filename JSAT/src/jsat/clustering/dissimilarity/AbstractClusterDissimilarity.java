...
public abstract class AbstractClusterDissimilarity implements ClusterDissimilarity {
  ...
  /**
   * A convenience method. If the <i>distanceMatrix</i> was created with null null   {@link #createDistanceMatrix(jsat.DataSet, jsat.clustering.dissimilarity.ClusterDissimilarity)
     * }, then this method will return the appropriate value for the desired index.
   *
   * @param distanceMatrix the distance matrix to query from
   * @param i the first index
   * @param j the second index
   * @return the correct value from the distance matrix from the index given as if the distance matrix was of full form
   */
  ...
  /**
   * A convenience method. If the <i>distanceMatrix</i> was created with null null   {@link #createDistanceMatrix(jsat.DataSet, jsat.clustering.dissimilarity.ClusterDissimilarity)
     * }, then this method will set the appropriate value for the desired index.
   *
   * @param distanceMatrix the distance matrix to query from
   * @param i the first index
   * @param j the second index
  ...
  /**
   * Creates an upper triangular matrix containing the distance between all points in the data set. The main diagonal
   * will contain all zeros, since the distance between a point and itself is always zero. This main diagonal is not
   * stored, and is implicit <br> To save space, the matrix is staggered, and is of a size such that all elements to the
   * left of the main diagonal are not present. <br> To compute the index into the returned array for the index [i][j],
   * the values should be switched such that i &ge; j, and accessed as [i][j-i-1]
   *
