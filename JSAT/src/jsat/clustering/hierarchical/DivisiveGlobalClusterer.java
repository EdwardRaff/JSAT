...
/**
 * DivisiveGlobalClusterer is a hierarchical clustering method that works by splitting the data set into sub trees from
 ...
 * <br>
 * Like many HAC methods, DivisiveGlobalClusterer will store the merge order of the clusters so that the clustering
 * results for many <i>k</i> can be obtained. It is limited to the range of clusters successfully computed before.
 * <br><br>
 * Specifically, DivisiveGlobalClusterer greedily chooses the cluster to split based on an evaluation of all resulting
 * clusters after a split. Because of this global search of the world, DivisiveLocalClusterer has can make a good
 * estimate of the number of clusters in the data set. The quality of this result is dependent on the accuracy of the
 * {@link ClusterEvaluation} used. This quality comes at the cost of execution speed, as more and more large evaluations
 * of the whole dataset are needed at each iteration. If execution speed is more important,
 * {@link DivisiveLocalClusterer} should be used instead, which requires only a fixed number of evaluations per
 * iteration.
 *
 * @author Edward Raff
 */
...
//                            baseClusterer.cluster(subDataSet, 2, threadpool, subDesignation[z]);
  ...
  /**
   * Returns the clustering results for a specific <i>k</i> number of clusters for a previously computed data set. If
   * the data set did not compute up to the value <i>k</i>  <tt>null</tt> will be returned.
   *
   * @param targetK the number of clusters to get the result for.
   * @return an array containing the assignments for each cluster in the original data set.
