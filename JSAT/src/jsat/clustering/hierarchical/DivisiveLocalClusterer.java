 ...
 * where the exact number of clusters are recommended. <br>
...
public class DivisiveLocalClusterer extends KClustererBase {
  ...
  public int[] cluster(final DataSet dataSet, final int clusters, final ExecutorService threadpool,
     ...
     * index value. Negative value indicates not set
     */
    ...
    clusterToSplit.add(0);// We must start out spliting the one cluster of
                          // everyone!
    ...
    /*
     * TODO it could be updated to use the split value to do a search range and
     * stop when a large jump occurs. This will perform poorl and underestimate
     * the number of clusters, becase it will decend one path as splitting
     * clusters improves untill it gets to the correct cluster size. Popping
     * back up will then casue an increase, which will cause the early
     * termination.
     */
    for (int k = 1; k < clusters; k++) {
      ...
      computeSubClusterSplit(subDesignation, useSplit, dpSubC1, dataSet, designations, originalPositions,
          splitEvaluation, clusterToSplit, threadpool);

      computeSubClusterSplit(subDesignation, newClusterID, dpSubC2, dataSet, designations, originalPositions,
          splitEvaluation, clusterToSplit, threadpool);
    }
  ...
  /**
   * Takes the data set and computes the clustering of a sub cluster, and stores
   * its information, and places the result in the queue
   *
   * @param subDesignation
   *          the array of arrays to store the designation array for the sub
   *          clustering
   * @param originalCluster
   *          the originalCluster that we want to store the split information of
   * @param listOfDataPointsInCluster
   *          the list of all data points that belong to
   *          <tt>originalCluster</tt>
   * @param fullDataSet
   *          the full original data set
   * @param fullDesignations
   *          the designation array for the full original data set
   * @param originalPositions
   *          the array of arrays to store the map from and index in
   *          <tt>listOfDataPointsInCluster</tt> to its index in the full data
   *          set.
   * @param splitEvaluation
   *          the array to store the cluster evaluation of the data set
   * @param clusterToSplit
   *          the priority queue that stores the cluster id and sorts based on
   *          how good the sub splits were.
   */
  private void computeSubClusterSplit(final int[][] subDesignation, final int originalCluster,
      final List<DataPoint> listOfDataPointsInCluster, final DataSet fullDataSet, final int[] fullDesignations,
      final int[][] originalPositions, final double[] splitEvaluation, final PriorityQueue<Integer> clusterToSplit,
