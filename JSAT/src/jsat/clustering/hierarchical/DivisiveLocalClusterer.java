...
/**
 * DivisiveLocalClusterer is a hierarchical clustering method that works by splitting the data set into sub trees from
 * the top down. Unlike many top-up methods, such as {@link SimpleHAC}, top-down methods require another clustering
 * method to perform the splitting at each iteration. If the base method is not deterministic, then the top-down method
 * will not be deterministic.
 * <br><br>
 * Specifically, DivisiveLocalClusterer greedily chooses the cluster to split based on an evaluation of only the cluster
 * being split. Because of this local search of the world, DivisiveLocalClusterer has poor performance in determining
 * the number of clusters in the data set. As such, only the methods where the exact number of clusters are recommended.
 * <br>
 * This greedy strategy can also lead to drilling down clusters into small parts, and works best when only a small
 * number of clusters are needed.
 *
 * @author Edward Raff
 */
public class DivisiveLocalClusterer extends KClustererBase {
  ...
  public int[] cluster(DataSet dataSet, int clusters, ExecutorService threadpool, int[] designations) {
    ...
    /**
     * For each current cluster, we store the clustering results if we attempt to split it into two.
     * <br>
    ...
    /**
     * Stores the dissimilarity of the splitting of the cluster with the same index value. Negative value indicates not
     * set
     */
  ...
  /**
   * Takes the data set and computes the clustering of a sub cluster, and stores its information, and places the result
   * in the queue
   *
   * @param subDesignation the array of arrays to store the designation array for the sub clustering
   * @param originalCluster the originalCluster that we want to store the split information of
   * @param listOfDataPointsInCluster the list of all data points that belong to <tt>originalCluster</tt>
   * @param fullDataSet the full original data set
   * @param fullDesignations the designation array for the full original data set
   * @param originalPositions the array of arrays to store the map from and index in <tt>listOfDataPointsInCluster</tt>
   * to its index in the full data set.
   * @param splitEvaluation the array to store the cluster evaluation of the data set
   * @param clusterToSplit the priority queue that stores the cluster id and sorts based on how good the sub splits
   * were.
   */
