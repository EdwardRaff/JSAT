...
public class PriorityHAC extends KClustererBase {
  ...
  /**
   * Stores the merge list, each merge is in a pair of 2 values. The final merge
   * list should contain the last merged pairs at the front of the array (index
   * 0, 1), and the first merges at the end of the array. The left value in each
   * pair is the index of the data point that the clusters were merged under,
   * while the right value is the index that was merged in and treated as no
   * longer its own cluster.
   */
  private int[] merges;
  ...
  public PriorityHAC(final PriorityHAC toCopy) {
    distMeasure = toCopy.distMeasure.clone();
    if (toCopy.merges != null) {
      merges = Arrays.copyOf(toCopy.merges, toCopy.merges.length);
    }
    curDataSet = toCopy.curDataSet.shallowClone();
  }

  public PriorityHAC(final UpdatableClusterDissimilarity dissMeasure) {
    distMeasure = dissMeasure;
  }

  /**
   * Goes through the <tt>merge</tt> array in order from last merge to first,
   * and sets the cluster assignment for each data point based on the merge
   * list.
   *
   * @param designations
   *          the array to store the designations in, or null to have a new one
   *          created automatically.
   * @param clusters
   *          the number of clusters to assume
   * @return the array storing the designations. A new one will be created and
   *         returned if <tt>designations</tt> was null.
   */
    ...
    for (int i = 0; i < merges.length; i++) {
      if (designations[merges[i]] == -1) // it has not been assigned
      {
        if (curCluster < clusters) {// It will be a top level cluster
          ...
          designations[merges[i]] = designations[merges[i - 1]];// The new
                                                                // cluster is
                                                                // always in an
                                                                // odd index, so
                                                                // its parrent
  ...
  public int[] cluster(final DataSet dataSet, final int lowK, final int highK, int[] designations) {
    ...
    /**
     * Keeps track of the current cluster size for the data point. If zero, the
     * data point has been merged and is no longer a candidate for future
     * consideration. If non zero, it indicates the number of data points in its
     * implicit cluster. All points start out in their own implicit cluster.
     */
    ...
    /*
     * Keep track of the average dist when merging, stop when it becomes
     * abnormaly large
     */
    ...
    // We will choose the cluster size as the most abnormal jump in
    // dissimilarity from a merge
    int clusterSize = lowK;
    double maxStndDevs = Double.MIN_VALUE;

    // We now know the dissimilarity matrix & Qs we can begin merging
    // We will perform all merges, and store them - and then return a clustering
    // level from the merge history
    for (int k = 0; k < I.length - 1; k++) {
      ...
      // Keep track of the changes in cluster size, and mark if this one was
      // abnormall large
      distChange.add(dk1);

      if (I.length - k >= lowK && I.length - k <= highK) // IN the cluster
                                                         // window?
      {
      ...
      // We have now found the smalles pair in O(n), first we will update the Qs
      // and matrix. k1 will be the new merged cluster
   ...
   *
   * @param designations
   *          the array to store the assignments in
   * @param clusters
   *          the number of clusters desired
   * @return the original array passed in, or <tt>null</tt> if no data set has
   *         been clustered.
   * @see #hasStoredClustering()
   */
  ...
  /**
   * The PriorityHAC stores its merging order, so that multiple clusterings can
   * of different sizes can be obtained without having to recluster the data
   * set. This is possible in part because HAC is deterministic. <br>
   * This returns <tt>true</tt> if there is currently a data set and its merge
   * order stored.
   *
   * @return <tt>true</tt> if you can call for more clusterings, <tt>false</tt>
   *         if no data set has been clustered.
   */
  ...
  /**
   * Reverses the merge array so that the front contains the last merges instead
   * of the first. This is done so that creating new clusters is accessed in
   * order which is cache friendly. <br>
   * This method must be called once before using
   * {@link #assignClusterDesignations(int[], int) }
   */
  ...
  private void updateDistanceTableAndQueues(final List<IntPriorityQueue> P, final int[] I, final int k1, final int k2,
