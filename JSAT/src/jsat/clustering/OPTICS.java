...
/**
 * An Implementation of the OPTICS algorithm, which is a generalization of {@link DBSCAN}. OPTICS creates an ordering of
 * the ports, and then clusters can be extracted from this ordering in numerous different ways.
 * <br>
 * NOTE: The original clustering method proposed in the paper is fairly complicated, and its implementation is not yet
 * complete. Though it does perform some amount of clustering, it may not return the expected results.
 * <br><br>
 * See original paper<br>
 * Ankerst, M., Breunig, M., Kriegel, H.-P.,&amp;Sander, J. (1999).
 * <a href="http://dl.acm.org/citation.cfm?id=304187"><i>OPTICS: ordering points to identify the clustering
 * structure</i></a>. Proceedings of the 1999 ACM SIGMOD international conference on Management of data (Vol. 28, pp.
 * 49–60). Philadelphia, Pennsylvania: ACM.
 *
 * @author Edward Raff
 */
public class OPTICS extends ClustererBase implements Parameterized {
  ...
  /**
   * Stores the reachability distance of each point in the order they were first observed in the data set. After
   * clustering is finished, it is altered to be in the reachability order used in clustering
   */
  ...
  /**
   * The objects contained in OrderSeeds are sorted by their reachability-distance to the closest core object from which
   * they have been directly density reachable.
   *
   * The paired double is their distance, the paired integer the the vector's index in the data set
   *
  ...
  public enum ExtractionMethod {
    ...
    XI_STEEP_ORIGINAL,
    /**
     * Forms clusters in the reachability plot by drawing a line across it, and using the separations to mark clusters
     */
    THRESHHOLD,
    /**
     * Forms clusters in the reachability plot by drawing a line across it, and using the separations to mark clusters.
     * It then de-noises points by checking their nearest neighbors for consensus
     */
    THRESHHOLD_FIXUP
  }

  /**
   * Creates a new OPTICS cluster object. Because the radius of OPTICS is not sensitive, it is estimated from the data
   * and set to a sufficiently large value. The {@link EuclideanDistance} will be used as the metric.
   */
  public OPTICS() {
  ...
  /**
   * Creates a new OPTICS cluster object. Because the radius of OPTICS is not sensitive, it is estimated from the data
   * and set to a sufficiently large value.
   *
  ...
  /**
   * Returns the xi value used in {@link ExtractionMethod#XI_STEEP_ORIGINAL} to produce cluster results.
   *
  ...
  /**
   * Sets the minimum number of points needed to compute the core distance. Higher values tend to smooth out the
   * reachability plot.
   *
  ...
  /**
   * Sets the {@link VectorCollectionFactory} used to produce acceleration structures for the OPTICS computation.
   *
...
//                r_dists[objIndex] = new_r_dist;
  ...
  /**
   * Returns a copy of the reachability array in correct reachability order. Some values that are not density
   * reachability could have a value of {@link Double#POSITIVE_INFINITY}.
   *
