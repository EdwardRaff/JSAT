...
/**
 * This class provides methods for sampling a data set for a set of initial points to act as the seeds for a clustering
 * algorithm.
 *
 * @author Edward Raff
 */
public class SeedSelectionMethods {
  ...
  static public enum SeedSelection {
     ...
     * The seed values will be probabilistically selected from the data set. <br>
     * The solution is O(log(k)) competitive with the optimal k clustering when using {@link EuclideanDistance}.
     * <br><br>
     * See k-means++: The Advantages of Careful Seeding
     */
    KPP,
    /**
     * The first seed is chosen randomly, and then all others are chosen to be the farthest away from all other seeds
     */
    FARTHEST_FIRST,
    /**
     * Selects the seeds in one pass by selecting points as evenly distributed quantiles for the distance of each point
     * from the mean of the whole data set. This makes the seed selection deterministic
     * <br><br>
     * See: J. A. Hartigan and M. A. Wong, "A k-means clustering algorithm", Applied Statistics, vol. 28, pp. 100–108,
     * 1979.
     */
    MEAN_QUANTILES
  };
  ...
  /**
   * Selects seeds from a data set to use for a clustering algorithm. The indices of the chosen points will be placed in
   * the <tt>indices</tt> array.
   *
   * @param d the data set to perform select from
   * @param indices a storage place to note the indices that were chosen as seed. The length of the array indicates how
   * many seeds to select.
   * @param dm the distance metric to used when selecting points
   * @param rand a source of randomness
   * @param selectionMethod The method of seed selection to use.
   */
