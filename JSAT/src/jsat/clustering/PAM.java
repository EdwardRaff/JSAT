...
public class PAM extends KClustererBase {
  ...
  /**
   * If set to {@code true} the computed medoids will be stored after clustering is completed, and can then be retrieved
   * using {@link #getMedoids() }.
   *
   * @param storeMedoids {@code true} if the medoids should be stored for later, {@code false} to discard them once
   ...
   * @param doInit {@code true} if the initialization procedure of training the distance metric, initiating its cache,
   * and selecting he seeds, should be done.
   * @param medioids the array to store the indices that get chosen as the medoids. The length of the array indicates
   * how many medoids should be obtained.
   * @param assignments an array of the same length as <tt>data</tt>, each value indicating what cluster that point
   * belongs to.
