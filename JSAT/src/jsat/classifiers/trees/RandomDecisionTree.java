...
public class RandomDecisionTree extends DecisionTree {
   ...
   */
  public RandomDecisionTree(final int numFeatures, final int maxDepth, final int minSamples,
      final TreePruner.PruningMethod pruningMethod, final double testProportion) {
  ...
  public RandomDecisionTree(final RandomDecisionTree toCopy) {
    super(toCopy);
    numFeatures = toCopy.numFeatures;
  }
  ...
  protected Node makeNodeC(final List<DataPointPair<Integer>> dataPoints, final Set<Integer> options, final int depth,
    ...
    final int featureCount = dataPoints.get(0).getDataPoint().numCategoricalValues()
        + dataPoints.get(0).getDataPoint().numNumericalValues();
    fillWithRandomFeatures(options, featureCount);
    return super.makeNodeC(dataPoints, options, depth, threadPool, mcdl); // To
                                                                          // change
                                                                          // body
                                                                          // of
                                                                          // generated
  ...
  protected Node makeNodeR(final List<DataPointPair<Double>> dataPoints, final Set<Integer> options, final int depth,
    ...
    final int featureCount = dataPoints.get(0).getDataPoint().numCategoricalValues()
        + dataPoints.get(0).getDataPoint().numNumericalValues();
    fillWithRandomFeatures(options, featureCount);
    return super.makeNodeR(dataPoints, options, depth, threadPool, mcdl); // To
                                                                          // change
                                                                          // body
                                                                          // of
                                                                          // generated
  ...
  /**
   * Sets the number of random features to and use at each node of the decision
   * tree
   *
   * @param numFeatures
