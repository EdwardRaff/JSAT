...
/**
 * Random Forest is an extension of {@link Bagging} that is applied only to {@link DecisionTree DecisionTrees}. It works
 * in a similar manner, but also only uses a random sub set of the features for each tree trained. This provides
 * increased performance in accuracy of predictions, and reduced training time over just Bagging.
 *
 * @author Edward Raff
 * @see Bagging
 */
public class RandomForest implements Classifier, Regressor, Parameterized {
  ...
  /**
   * Setting the number of features to use. Default value is -1, indicating the heuristic of sqrt(N) or N/3 should be
   * used for classification and regression respectively. This value should be set away from -1 before training work
   * begins, and set back if it was not set explicitly by the used
   */
  ...
  /**
   * RandomForest performs Bagging. Bagging samples from the training set with replacement, and draws a
   * sampleWithReplacement at least as large as the training set. This controls how many extra samples are taken. If
   * negative, fewer samples will be taken. Using negative values is not recommended.
   *
  ...
  /**
   * Instead of using a heuristic, the exact number of features to sample is provided. If equal to or larger then the
   * number of features in one of the training data sets, RandomForest degrades to {@link Bagging} performed on
  ...
  /**
   * Tells the class to automatically select the number of features to use. For classification problems, this is the
   * square root of the number of features. For regression, the number of features divided by 3 is used.
   */
  ...
  /**
   * Returns true if heuristics are currently in use for the number of features, or false if the number has been
   * specified.
   *
   * @return true if heuristics are currently in use for the number of features, or false if the number has been
   * specified.
   */
  ...
  /**
   * Returns the number of rounds of boosting that will be done, which is also the number of base learners that will be
   * trained
   *
   * @return the number of rounds of boosting that will be done, which is also the number of base learners that will be
   * trained
   */
  ...
  /**
   * If {@link #isUseOutOfBagError() } is false, then this method will return 0 after training. Otherwise, it will
   * return the out of bag error estimate after training has completed. For classification problems, this is the 0/1
  ...
  /**
   * Does the actual set up and training. {@link #predicting } and {@link #forest} should be set up appropriately first.
   * Everything else is handled by this and {@link LearningWorker}
   *
