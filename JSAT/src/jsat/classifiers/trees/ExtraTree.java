...
/**
 * The ExtraTree is an Extremely Randomized Tree. Splits are chosen at random, and the features that are selected are
 * also chosen at random for each new node in the tree. <br>
 * If set to randomly select one feature for each node, it becomes a <i>Totally Randomized Tree</i><br>
 * See: <br>
...
public class ExtraTree implements Classifier, Regressor, TreeLearner, Parameterized {
  ...
  /**
   * Sets the stopping size for tree growth. When a node has less than or equal to <tt>stopSize</tt> data points to
   * train from, it terminates and produces a leaf node.
   *
  ...
  /**
   * The ExtraTree will select the best of a random subset of features at each level, this sets the number of random
   * features to select. If set larger than the number of features in the training set, all features will be eligible
   * for selection at every level.
   *
  ...
  /**
   * The normal implementation of ExtraTree always produces binary splits, including for categorical features. If set to
   * <tt>false</tt> categorical features will expand out for each value in the category. This reduces the randomness of
   * the tree.
   *
   * @param binaryCategoricalSplitting whether or not to use the original splitting algorithm, or to fully expand
   * nominal features
   */
  ...
  /**
   * Adds the lists from the list of lists to the stack of reusable lists. The lists will not be removed from
   * <tt>aSplit</tt>, despite being added to
   * <tt>resuableLists</tt>
   *
   * @param <T> the content type of the list
   * @param reusableLists available pre allocated lists for reuse
   * @param aSplit the lists of lists to add all elements of to the reusable lists
   */
  ...
  private static abstract class NodeBase extends TreeNodeVisitor {
    ...
    public boolean isLeaf() {
      ...
      for (TreeNodeVisitor children1 : children) {
        if (children1 != null) {
          return false;
        }
      }
