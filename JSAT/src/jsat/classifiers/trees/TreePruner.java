...
public class TreePruner {
  ...
  public static enum PruningMethod {
     ...
     */
    NONE, /**
           * The tree will be pruned in a bottom up fashion, removing leaf nodes
           * if the removal does not reduce the accuracy on the testing set
           */
    REDUCED_ERROR, /**
                    * Bottom-Up pessimistic pruning using Error based Pruning
                    * from the C4.5 algorithm. If the node visitor supports
                    * {@link TreeNodeVisitor#setPath(int, jsat.classifiers.trees.TreeNodeVisitor) }
                    * it will perform sub tree replacement for the maximal sub
                    * tree. <br>
                    * The default Confidence (CF) is 0.25, as used in the C4.5
                    * algorithm.<br>
                    * <br>
                    * NOTE: For the one case where the root would be pruned by
                    * taking the sub tree with the most nodes, this
                    * implementation will not perform that step. However, this
                    * is incredibly rare - and otherwise performs the same.
                    */
    ERROR_BASED
  }
   ...
   * @param testSet
   *          the test set of data points to use for pruning
   */
  public static void prune(final TreeNodeVisitor root, final PruningMethod method,
      final ClassificationDataSet testSet) {
   ...
   */
  public static void prune(final TreeNodeVisitor root, final PruningMethod method,
      final List<DataPointPair<Integer>> testSet) {
   ...
   * @param pathFollowed
   *          the path from the parent node to the current node
   * @param current
   *          the current node to evaluate
   * @param testSet
   *          the set of points to estimate error from
   * @param alpha
   *          the Confidence
   * @return expected upperbound on errors
   */
  private static double pruneErrorBased(final TreeNodeVisitor parent, final int pathFollowed,
      final TreeNodeVisitor current, final List<DataPointPair<Integer>> testSet, final double alpha) {
    // TODO this does a lot of redundant computation. Re-write this code to keep
    // track of where datapoints came from to avoid redudancy.
    ...
    final List<List<DataPointPair<Integer>>> splitSet = new ArrayList<List<DataPointPair<Integer>>>(
    ...
    // Find child wich gets the most of the test set as the candidate for
     ...
     * Instead, just compute exact using inverse beta Upper Bound = 1.0 -
     * BetaInv(alpha, n-k, k+1)
     */
    ...
    // MaxChildTreeScore is not the min, or it was not supported - so we do not
    // compare against it any more
   ...
   * @param pathFollowed
   *          the path from the parent that lead to the current node
   * @param current
   *          the current node being considered
   * @param testSet
   *          the set of testing points to apply to this node
   * @return the number of nodes pruned from the tree
   */
  private static int pruneReduceError(final TreeNodeVisitor parent, final int pathFollowed,
      final TreeNodeVisitor current, final List<DataPointPair<Integer>> testSet) {
    if (current == null) {
      return 0;
    }
    ...
    if (!current.isLeaf()) {
      // Each child should only be given testing points that would decend down
      // that path
      ...
      // TODO if splits = 2, reorder the original array and use subList to
      // return memory efficent references
    ...
    // If we pruned all our children, we may have become a leaf! Should we prune
    // ourselves?
    if (current.isLeaf() && parent != null) // Compare this nodes accuracy vs
                                            // its parrent
    {
      ...
      if (parrentCorrect >= childCorrect) // We use >= b/c if they are the same,
                                          // we assume smaller trees are better
      {
