...
/**
 * Provides post-pruning algorithms for any decision tree that can be altered using the {@link TreeNodeVisitor}. Pruning
 * is done with a held out testing set
 * <br>
...
public class TreePruner {
  ...
  public static enum PruningMethod {
    ...
    NONE,
    /**
     * The tree will be pruned in a bottom up fashion, removing leaf nodes if the removal does not reduce the accuracy
     * on the testing set
     */
    REDUCED_ERROR,
    /**
     * Bottom-Up pessimistic pruning using Error based Pruning from the C4.5 algorithm. If the node visitor supports 
     ...
     * <br>
     * NOTE: For the one case where the root would be pruned by taking the sub tree with the most nodes, this
     * implementation will not perform that step. However, this is incredibly rare - and otherwise performs the same.
     */
    ERROR_BASED
  };
