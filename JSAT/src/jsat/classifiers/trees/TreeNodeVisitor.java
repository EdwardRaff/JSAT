...
/**
 * Provides an abstracted mechanism for traversing and predicting from nodes in a tree meant for a supervised learning
 * problem. <i>Paths</i> and <i>children
 * </i> are used interchangeably, every node has one path to one child<br>
 * Paths to children nodes can be disabled, but not removed. This is done so that the implementation does not have to
 * worry about changes in the indices to children nodes, which would be complicated to implement. Once a path is
 * disabled, it can not be re-enabled.<br>
 * If all paths to any children have been disabled, {@link #childrenCount() }
 * may choose to return 0, otherwise - it must return the original number of paths to children nodes.
 *
 * The implementation for a given tree should override {@link #localClassify(jsat.classifiers.DataPoint) } and 
 * {@link #localRegress(jsat.classifiers.DataPoint) } if the operations are supported.
 *
 * @author Edward Raff
 * @see TreeLearner
 * @see TreePruner
...
public abstract class TreeNodeVisitor implements Serializable, Cloneable {
  ...
  /**
   * Returns the number of children this node of the tree has, and may return a non zero value even if the node is a
   * leaf
   *
  ...
  /**
   * Disables the selected path to the specified child from the current node. All child indices will not be effected by
   * this operation.
   *
  ...
  /**
   * Returns true if the path to the specified child is disabled, meaning it can not be traveled to. It will also return
   * true for an invalid child path, since a non existent node can not be reached.
   *
   * @param child the child index to check the path for
   * @return <tt>true</tt> if the path is unavailable, <tt>false</tt> if the path is good.
   */
   ...
   * @throws UnsupportedOperationException if the tree node does not support or was not trained for classification
   */
  ...
  /**
   * Returns the path down the tree the given data point would have taken, or -1 if this node was a leaf node
   *
