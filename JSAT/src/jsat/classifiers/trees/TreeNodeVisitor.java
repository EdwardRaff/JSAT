 ...
 * a tree meant for a supervised learning problem. <i>Paths</i> and
 * <i>children </i> are used interchangeably, every node has one path to one
 * child<br>
...
public abstract class TreeNodeVisitor implements Serializable, Cloneable {
  ...
  /**
   * Returns the number of children this node of the tree has, and may return a
   * non zero value even if the node is a leaf
   *
   ...
   *
   * @param child
   *          the index of the child to disable the path too
   */
  abstract public void disablePath(int child);

  /**
   * Returns the node for the specific child, or null if the child index was not
   * valid
   *
   * @param child
  ...
  /**
   * Returns the path down the tree the given data point would have taken, or -1
   * if this node was a leaf node
   *
   * @param dp
  ...
  /**
   * Returns true if the path to the specified child is disabled, meaning it can
   * not be traveled to. It will also return true for an invalid child path,
   * since a non existent node can not be reached.
   *
   * @param child
   ...
   *
   * @param dp
   *          the data point to localClassify
   * @return the classification result
   * @throws UnsupportedOperationException
   *           if the tree node does not support or was not trained for
   *           classification
   */
  ...
  /**
   * Returns the regression result that would have been obtained if the current
   * node was a leaf node.
   *
   * @param dp
   *          the data point to regress
   * @return the classification result
   * @throws UnsupportedOperationException
   *           if the tree node does not support or was not trained for
   *           classification
   ...
   * This method, if supported, will set the path so that the child is set to
   * the given value. <br>
   * The implementation may choose to throw an exception if the NodeVisitor is
   * not of the same implementing class.
   *
   * @param child
   *          the child path
   * @param node
   *          the node to make the child
   */
