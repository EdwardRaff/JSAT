...
/**
 * Creates a decision tree from {@link DecisionStump DecisionStumps}. How this tree performs is controlled by pruning
 * method selected, and the methods used in the stump.
 *
 * @author Edward Raff
 */
public class DecisionTree implements Classifier, Regressor, Parameterized, TreeLearner {
  ...
  /**
   * Base decision stump used to clone so that we can keep certain features inside the stump instead of duplicating them
   * here.
   */
  ...
  /**
   * Creates a Decision Tree that uses {@link PruningMethod#REDUCED_ERROR} pruning on a held out 10% of the data.
   */
  public DecisionTree() {
  ...
  /**
   * Returns a Decision Tree with settings initialized so that its behavior is approximately that of the C4.5 decision
   * tree algorithm when used on classification data. The exact behavior not identical, and certain base cases may not
   * behave in the exact same manner. However, it uses all of the same general algorithms. <br><br>
   ...
   *
   * @param handling the method of numeric attribute handling to use during classification
   */
   ...
   * Performs exactly the same as 
     * {@link #trainC(jsat.classifiers.ClassificationDataSet, java.util.concurrent.ExecutorService) }, but the user can
   * specify a subset of the features to be considered.
   *
