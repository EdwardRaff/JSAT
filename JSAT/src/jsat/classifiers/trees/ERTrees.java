...
/**
 * Extra Randomized Trees (ERTrees) is an ensemble method built on top of {@link ExtraTree}. The randomness of the trees
 * provides incredibly high variance, yet a low bias. The sum of many randomized trees proves to be a powerful and fast
 * learner. <br>
 * The default settings are those suggested in the paper. However, the default stop size suggested (especially for
 * classification) is often too small. You may want to consider increasing it if the accuracy is too low. <br>
 * See: <br>
...
public class ERTrees extends ExtraTree {
   ...
   * node. If <tt>true</tt> the value of selectionCount will be modified during training, using sqrt(n) features for
   * classification and all features for regression. Otherwise, whatever value set before hand will be used.
   *
   * @param useDefaultSelectionCount whether or not to use the heuristic version
   */
  ...
  /**
   * Sets whether or not to us the default heuristic for the number of points to force a new node to be a leaf. If
   * <tt>true</tt> the value for stopSize will be altered during training, set to 2 for classification and 5 for
