...
/**
 * ImpurityScore provides a measure of the impurity of a set of data points respective to their class labels. The
 * impurity score is maximized when the classes are evenly distributed, and minimized when all points belong to one
 * class. <br>
 * The gain in purity can be computed using the static <i>gain</i> methods of the class. However, not all impurity
 * measures can be used for arbitrary data and splits. Some may only support binary splits, and some may only support
...
public class ImpurityScore implements Cloneable {
  ...
  public enum ImpurityMeasure {
    ...
    /**
     * Normalized Mutual Information. The {@link #getScore() } value will be the same as {@link #INFORMATION_GAIN},
     * however - the gain returned is considerably different - and is a normalization of the mutual information between
     * the split and the class label by the class and split entropy.
     */
    NMI,
    GINI,
  ...
  /**
   * Computes the current impurity score for the points that have been added. A higher score is worse, a score of zero
   * indicates a perfectly pure set of points (all one class).
   *
