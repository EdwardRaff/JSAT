...
/**
 * This class is a 1-rule. It creates one rule that is used to classify all inputs, making it a decision tree with only
 * one node. It can be used as a weak learner for ensemble learners, or as the nodes in a true decision tree.
 * <br><br>
 * Categorical values are handled similarly under all circumstances. <br>
 * During classification, numeric attributes are separated based on most likely probability into their classes. <br>
 * During regression, numeric attributes are done with only binary splits, finding the split that minimizes the total
 * squared error sum.
 *
 * @author Edward Raff
 */
public class DecisionStump implements Classifier, Regressor, Parameterized {
  ...
  /**
   * Only used during regression. Contains the averages for each branch in the first and 2nd index. 3rd index contains
   * the split value. If no split could be done, the length is zero and it contains only the return value
   */
  ...
  /**
   * The minimum number of points that must be inside the split result for a split to occur.
   */
  ...
  public static enum NumericHandlingC {
    /**
     * Numeric attributes may be split into an arbitrary number of branches based on the approximated intersections of
     * the PDF.
     */
    PDF_INTERSECTIONS,
    /**
     * Numeric attributes are split into a binary branch based on a linear search for the split that produces the
  ...
  /**
   * Unlike categorical values, when a continuous attribute is selected to split on, not all values of the attribute
   * become the same. It can be useful to split on the same attribute multiple times. If set true, continuous attributes
   * will be removed from the options list. Else, they will be left in the options list.
   *
  ...
  /**
   * Sets the method of attribute selection used when numeric attributes are encountered during classification.
   *
   * @param numericHandlingC the method of numeric attribute handling to use during classification
   */
  ...
  /**
   * Returns the method of attribute selection used when numeric attributes are encountered during classification.
   *
   * @return the method of numeric attribute handling to use during classification
   */
   ...
   * The min result split size parameter specifies the minimum allowable number of points to end up in one of the splits
   * for it to be admisible for consideration.
   *
  ...
  /**
   * Sets the DecisionStump's predicting information. This will be set automatically by calling {@link #trainC(jsat.classifiers.ClassificationDataSet)
   * } or {@link #trainC(jsat.classifiers.ClassificationDataSet, java.util.concurrent.ExecutorService) }, but it must be
   * called before using {@link #trainC(java.util.List, java.util.Set) }.
   *
  ...
  /**
   * From the score for the original set that is being split, this computes the gain as the improvement in
   * classification from the original split.
   *
  ...
  /**
   * This method finds a value that is the overlap of the two distributions, representing a separation point. This
   * method works in 3 steps. It first determines if the two distributions have no overlap, and will return the value
   * in-between the distributions. <br>
   * If there is overlap, it attempts to find the point between the means that marks the overlap <br>
   * If this fails, it attempts to find an overlapping point by starting at the least probable value appearing at either
   * end of the real numbers. <br>
   * <br>
   * This method may fail on some pairs of distributions, especially if the standard deviations are significantly
   * different from each other and have similar means.
   *
   ...
   * @return an double, indicating the separating point, and an integer indicating which class is most likely when on
   * the left. 0 indicates <tt>dist1</tt>, and 1 indicates <tt>dist2</tt>
   ...
   * Else, 2 lists are returned. Each are the same length, and their values are matched up. The list of doubles is in
   * sorted order. The last element is always positive Infinity. For index i, the double value at index i indicates that
   * for all values between the double indices for i and (i-1), is most likely to belong to the class indicated from the
   * integer list for index i.
   *
  ...
  /**
   * Determines which split path this data point would follow from this decision stump. Works for both classification
   * and regression.
   *
  ...
  /**
   * Returns the number of paths that this decision stump leads to. The stump may not ever direct a data point on some
   * of the paths. A result of 1 path means that all data points will be given the same decision, and is generated when
   * the entropy of a set is 0.0.
   * <br><br>
   ...
   * @param options the set of attributes that this classifier may choose from. The attribute it does choose will be
   * removed from the set.
   * @return the a list of lists, containing all the datapoints that would have followed each path. Useful for training
   * a decision tree
   */
   ...
   * @param finalGain array used to reference a double that can be returned. If this method determined the gain in order
   * to find the split, it sets the value at index zero to the gain it computed. May be null, in which case it is
   * ignored.
   * @return A pair of lists of the same size. The list of doubles containing the split boundaries, and the integers
   * containing the path number. Multiple splits could go down the same path.
   */
