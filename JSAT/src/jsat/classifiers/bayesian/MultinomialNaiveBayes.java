...
public class MultinomialNaiveBayes extends BaseUpdateableClassifier implements Parameterized {
   ...
   */
  public MultinomialNaiveBayes(final double smoothing) {
   ...
   *          the one to copy
   */
  protected MultinomialNaiveBayes(final MultinomialNaiveBayes other) {
    this(other.smoothing);
    if (other.apriori != null) {
      apriori = new double[other.apriori.length][][];
      wordCounts = new double[other.wordCounts.length][];
      totalWords = Arrays.copyOf(other.totalWords, other.totalWords.length);
      priors = Arrays.copyOf(other.priors, other.priors.length);
      priorSum = other.priorSum;

      for (int c = 0; c < other.apriori.length; c++) {
        apriori[c] = new double[other.apriori[c].length][];
        for (int j = 0; j < other.apriori[c].length; j++) {
          apriori[c][j] = Arrays.copyOf(other.apriori[c][j], other.apriori[c][j].length);
        }
        wordCounts[c] = Arrays.copyOf(other.wordCounts[c], other.wordCounts[c].length);
      }

      priorSum = other.priorSum;
      priors = Arrays.copyOf(other.priors, other.priors.length);
    }
    finalizeAfterTraining = other.finalizeAfterTraining;
  ...
  public CategoricalResults classify(final DataPoint data) {
    ...
    if (finalized) {
      for (int c = 0; c < priors.length; c++) {
        ...
        final double[] counts = wordCounts[c];
      ...
      for (int c = 0; c < priors.length; c++) {
        ...
        final double[] counts = wordCounts[c];
        final double logTotalCounts = Math.log(totalWords[c] + smoothing * counts.length);
        ...
        for (int j = 0; j < apriori[c].length; j++) {
          ...
          final double p = apriori[c][j][data.getCategoricalValue(j)] + smoothing;
   ...
   * further, causing {@link #update(jsat.classifiers.DataPoint, int) } to throw
  ...
  public void finalizeModel() {
    ...
    for (int c = 0; c < priors.length; c++) {
      final double logProb = Math.log((priors[c] + smoothing) / priorSumSmooth);
      priors[c] = logProb;
      ...
      for (final double[] item : apriori[c]) {
        double sum = 0;
        for (final double element : item) {
          sum += element + smoothing;
        }
   ...
   * {@link #trainC(jsat.classifiers.ClassificationDataSet) }. This prevents the
   * model from being updated in an online fashion for an reduction in
   * classification time.
   *
   * @param finalizeAfterTraining
   *          {@code true} to finalize after a call to train, {@code false} to
   *          keep the model updatable.
   */
  public void setFinalizeAfterTraining(final boolean finalizeAfterTraining) {
   ...
   *
   * @param smoothing
   *          the positive smoothing constant
   */
