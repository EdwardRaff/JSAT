...
/**
 *
 * Provides an implementation of the Naive Bayes classifier that assumes numeric features come from some continuous
 * probability distribution. By default this implementation restricts itself to only the {@link Normal Gaussian}
 * distribution, and becomes Gaussian Naive Bayes. Other distributions are supported, and a
 * {@link KernelDensityEstimator} can be used as well.
 * <br><br>
 * By default, this implementation assumes that the input vectors are sparse and the distribution will only be estimated
 * by the non-zero values, and features that are zero will be ignored during prediction time. This should be turned off
 * when using dense data by calling {@link #setSparceInput(boolean) }
 * <br><br>
...
public class NaiveBayes implements Classifier, Parameterized {
  ...
  /**
   * There are multiple ways of handling numerical attributes. These provide the different ways that NaiveBayes can deal
   * with them.
   */
  public enum NumericalHandeling {
    ...
    /**
     * The best fitting {@link ContinuousDistribution} is selected by 
         * {@link DistributionSearch#getBestDistribution(jsat.linear.Vec, double) }, and provides a cut off value to use the
     * {@link KernelDensityEstimator} instead
     */
    BEST_FIT_KDE {
...
//            }
  ...
  /**
   * Sets the method used by this instance for handling numerical attributes. This has no effect on an already trained
   * classifier, but will change the result if trained again.
   *
  ...
  /**
   * Tells the Naive Bayes classifier to assume the importance of sparseness in the numerical values. This means that
   * values of zero will be ignored in computation and classification.<br>
   * This allows faster, more efficient computation of results if the data points are indeed sparce. This will also
   * produce different results. This value should not be changed after training and before classification.
   *
   * @param sparceInput <tt>true</tt> to assume sparseness in the data, <tt>false</tt> to ignore it and assume zeros are
   * meaningful values.
   * @see #isSparceInput()
   */
