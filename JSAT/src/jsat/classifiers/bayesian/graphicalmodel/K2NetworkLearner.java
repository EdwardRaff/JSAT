 ...
 * {@link #learnNetwork(jsat.classifiers.ClassificationDataSet) } directly. <br>
 * <br>
 ...
 * not provide satisfactory results for classification. <br>
 * <br>
 * See: <i>A bayesian method for the induction of probabilistic networks from
 * data</i>. Gregory F. Cooper and Edward Herskovits.
 *
 * @author Edward Raff
 */
public class K2NetworkLearner extends DiscreteBayesNetwork {
  ...
  public double f(final int i, final Set<Integer> pi, final ClassificationDataSet D) {
    ...
    // We do this by incrementing the values array and breaking out when we get
    // to an invalid state
    while (true) {
      term2 = Nijk = 0.0;
      ...
      int pos = 0;
      values[pos]++;
      values[pi.size()] = 0;// We set this to zero. If its value is non zero
                            // after this loop,
      // then all values have been iterated past their max value, they have
      // rolled
  ...
  /**
   * Returns the maximum number of parents allowed when learning a network
   * structure, or zero if any number of parents are valid.
   *
  ...
  /**
   * Queries the data set for the number of instances that have each possible
   * combination of values. <tt>classes</tT> and <tt>values</tt> should have the
   * same length. Each value in classes corresponds to the target value
   * specified in <tt>values</tt>. We will return the number of data points that
   * satisfy all class value pairs
   *
   *
   * @param classes
   ...
   * @return the number of times the value constraints are satisfied in the data
   *         set
   */
  private double query(final int[] classes, final int[] values, final ClassificationDataSet D) {
    ...
    for (int i = 0; i < D.getSampleSize(); i++) {
      ...
      // Use j to break early (set value) or indicate success (j ==
      // classes.length)
      int j;
  ...
  /**
   * Sets the maximum number of parents to allow a node when learning the
   * network structure. If a non zero value is supplied, nodes will be allowed
   * any number of parents.
   *
   * @param maxParents
   *          sets the maximum number of parents a node may learn
   */
