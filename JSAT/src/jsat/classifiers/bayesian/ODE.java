 ...
 * features. <br>
 * This class is primarily for use by {@link AODE} <br>
 * <br>
...
public class ODE extends BaseUpdateableClassifier {
   ...
   *
   * @param dependent
   *          the categorical feature to be dependent of
   */
  ...
  protected ODE(final ODE toClone) {
    ...
    predTargets = toClone.predTargets;
    depTargets = toClone.depTargets;
    if (toClone.counts != null) {
      counts = new double[toClone.counts.length][][][];
      for (int i = 0; i < counts.length; i++) {
        counts[i] = new double[i][][];
        for (int j = 0; j < counts[i].length; j++) {
          counts[i][j] = new double[toClone.counts[i][j].length][];
          for (int z = 0; z < counts[i][j].length; z++) {
            counts[i][j][z] = Arrays.copyOf(toClone.counts[i][j][z], toClone.counts[i][j][z].length);
          }
        }
      }

      priors = new double[toClone.priors.length][];
      for (int i = 0; i < priors.length; i++) {
        priors[i] = Arrays.copyOf(toClone.priors[i], toClone.priors[i].length);
      }
      priorSum = toClone.priorSum;
    }
  }
   ...
   *
   * @param catVals
   *          the catigorical values for a data point
   * @param c
   *          the target value to get the probability of
