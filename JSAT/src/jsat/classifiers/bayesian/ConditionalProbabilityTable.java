...
/**
 * The conditional probability table (CPT) is a classifier for categorical attributes. It builds the whole conditional
 * probability table for a data set. The size of the CPT grows exponentially with the number of dimensions and options,
...
public class ConditionalProbabilityTable implements Classifier {
  ...
  /**
   * Maps the index values of the training set to the index values used by the CPT. A value of '-1' indicates that the
   * feature is not in the CPT
   */
  ...
  /**
   * Converts a data point pair into a coordinate. The paired value contains the value for the predicting index. Though
   * this value will not be used if the predicting class of the original data set was not used to make the table.
   *
   ...
   * @throws ArithmeticException if the <tt>cord</tt> array does not match the {@link #getDimensionSize() dimension} of
   * the CPT
   */
   ...
   * @param categoriesToUse the attributes to use in training. Each value corresponds to the categorical index in
   * <tt>dataSet</tt>, and adding the value {@link DataSet#getNumCategoricalVars() }, which is not a valid index,
   * indicates to used the {@link ClassificationDataSet#getPredicting()  predicting class} of the data set in the CPT.
   */
  ...
  /**
   * Queries the CPT for the probability that the class value of <tt>targetClas</tt> would occur with the given
   * DataPointPair.
   *
   * @param targetClass the index in the original data set of the class that we want the probability of
   * @param dataPoint the data point of values paired with the value of the predicting attribute in the original
   * training set
  ...
  /**
   * Queries the CPT for the probability of the target class occurring with the specified value given the class values
   ...
   * @param cord the coordinate array that corresponds the the class values for the CPT, where the coordinate of the
   * <tt>targetClass</tt> may contain any value.
   * @return the probability in [0, 1] of the <tt>targetClass</tt> occurring with the <tt>targetValue</tt> given the
   * information in <tt>cord</tt>
