...
/**
 * This class represents the probabilities for each possible result
 * classification.
 *
 * @author Edward Raff
 */
public class CategoricalResults implements Cloneable {
  ...
  /**
   * Creates a new Categorical Result using the given array. It will use only a
   * reference to the given array, and will assume the values are already
   * normalized and sum to one.
   *
   * @param probabilities
   *          the array of probabilities for each outcome
   */
   ...
   *
   * @param numCategories
   *          the number of options to support.
   */
   ...
   *
   * @param c
   *          the constant to divide all probabilities by
   */
   ...
   *          the category
   * @param prob
   *          the value to increment by, may be greater then one.
   * @throws IndexOutOfBoundsException
   *           if a non existent category is specified
   * @throws ArithmeticException
   *           if the value set is negative or not a number
   */
  public void incProb(final int cat, final double prob) {
    if (cat > probabilities.length) {
      throw new IndexOutOfBoundsException(
          "There are only " + probabilities.length + " posibilties, " + cat + " is invalid");
  ...
  /**
   * Returns the category that is the most likely according to the current
   * probability values
   *
  ...
  /**
   * Adjusts the probabilities by dividing each value by the total sum, so that
