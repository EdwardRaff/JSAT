...
/**
 * A One VS One classifier extends binary decision classifiers into multi-class decision classifiers. This is done by
 * creating a binary-classification problem for every possible pair of classes, and then classifying by taking the
 * result of all possible combinations and choosing the class that got the most results. This allows for a soft decision
 * result, however, it often proves to be a meaningless soft boundary.
 *
 * @author Edward Raff
 */
public class OneVSOne implements Classifier, Parameterized {
  ...
  /**
   * Uper-diagonal matrix of classifiers sans the first index since a classifier vs itself is useless. First index is
   * the first dimensions is the first class, 2nd index + the value of the first + 1 is the opponent class
   */
   ...
   *
   * @param baseClassifier the binary classifier to extend
   * @param concurrentTrain <tt>true</tt> to have training of individual classifiers occur in parallel, <tt>false</tt>
   * to have them use their native parallel training method.
   */
  ...
  /**
   *
   * @return <tt>true</tt> if training of individual classifiers occur in parallel, <tt>false</tt> they use their native
