...
/**
 * This interface defines the contract for applying a regularization scheme to the weight and bias values of a laying in
 * a neural network.
 *
 * @author Edward Raff
 */
public interface WeightRegularizer extends Serializable {
  ...
  /**
   * Applies regularization to one matrix, where the rows of the matrix correspond tot he weights associated to one
   * neuron's input. The vector of bias terms must then have the same length as the number of rows in the given matrix.
   *
  ...
  /**
   * Applies the regularization to one row of the weight matrix, where the row corresponds to the weights into one
   * neuron.
   *
