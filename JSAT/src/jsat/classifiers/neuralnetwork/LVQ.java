...
/**
 * Learning Vector Quantization (LVQ) is an algorithm that extends {@link SOM} to take advantage of label information to
 * perform classification. It creates a number of representatives, or learning vectors, for each class. The LVs are then
 * updated iteratively to push away from the wrong class and pull closer to the correct class. LVQ is equivalent to a
 * type of 2 layer neural network.
 *
 * @author Edward Raff
 */
public class LVQ implements Classifier, Parameterized {
  ...
  /**
   * The default eps distance factor between the two wining vectors {@value #DEFAULT_EPS}
   */
  public static final double DEFAULT_EPS = 0.3;
  /**
   * The default scaling factor for the {@link LVQVersion#LVQ3} case is {@value #DEFAULT_MSCALE}
   */
  ...
  /**
   * The default number of representatives per class is {@value #DEFAULT_REPS_PER_CLASS}
   */
  public static final int DEFAULT_REPS_PER_CLASS = 3;
  /**
   * The default stopping distance for convergence is {@value #DEFAULT_STOPPING_DIST}
   */
  ...
  /**
   * Records the number of times each neuron won and was off the correct class during training. Neurons that end with a
   * count of zero wins will be ignored
   */
  protected int[] wins;
   ...
   * @param representativesPerClass the number of representatives to create for each class
   */
  ...
  /**
   * When using {@link LVQVersion#LVQ3}, a 3rd case exists where up to two learning vectors can be updated at the same
   * time if they have the same class. To avoid over fitting, an additional regularizing weight is placed upon the
   * learning rate for their update. THis sets the additional multiplied. It is suggested to use a value in the range of
   * [0.1, 0.5]
   *
  ...
  /**
   * Sets the epsilon multiplier that controls the maximum distance two learning vectors can be from each other in order
   * to be updated at the same time. If they are too far apart, only one can be updated. It is recommended to use a
   * value in the range [0.1, 0.3]
   *
   * @param eps the scale factor of the maximum distance for two learning vectors to be updated at the same time
   */
   ...
   *
   * @return the scale of the allowable distance between learning vectors when updating
   */
  ...
  /**
   * Sets the learning rate of the algorithm. It should be set in accordance with {@link #setLearningDecay(jsat.math.decayrates.DecayRate)
   * }.
   *
  ...
  /**
   * Sets the number of representatives to create for each class. It is possible to have an unbalanced number of
   * representatives per class, but that is not currently supported. Increasing the number of representatives per class
  ...
  /**
   * The algorithm terminates early if the learning vectors are only moving small distances. The stopping distance is
   * the minimum distance that one of the learning vectors must move for the algorithm to continue.
   *
  ...
  public enum LVQVersion {
    ...
    LVQ1,
    /**
     * Two vectors will be updated if they are close enough together. The closest was the wrong class but the 2nd closet
     * was the correct class.
     */
    LVQ2,
    /**
     * Two vectors will be updated if they are close enough together and do not belong to the same class if one of them
     * was the correct class to a training vector.
     */
    LVQ21,
    /**
     * Two vectors will be updated if they are close enough together and are of the same class as the training vector.
     */
    LVQ3
  }
      ...
      for (int i = 0; i < dataSet.getSampleSize(); i++) {
        ...
        if (lvqVersion.ordinal() >= LVQVersion.LVQ2.ordinal()
          ...
          weights[minDistIndx2].mutableSubtract(alpha, tmp);
                ...
                && weightClass[minDistIndx] == weightClass[minDistIndx2]
                && min(minDist / minDist2, minDist2 / minDist) > (1 - eps) * (1 + eps)) {//Update both vectors in the same direction
          x.copyTo(tmp);
          tmp.mutableSubtract(weights[minDistIndx]);
