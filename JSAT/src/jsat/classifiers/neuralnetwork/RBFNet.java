...
/**
 * This provides a highly configurable implementation of a Radial Basis Function Neural Network. A RBF network is a type
 * of neural network that contains one hidden layer, and is related to the {@link LVQ} algorithm. In a classical RBF
 * Network, the distance between two data points is generally the {@link EuclideanDistance} or
 * {@link MahalanobisDistance}. This implementation allows the specification of any arbitrary distance metric. <br>
 * <br>
 * Another restriction on classical RBF Nets is that a weighted sum of the output of the hidden units be used to make
 * the final decision. Instead this implementation allows the specification of an arbitrary Classifier or Regressor to
 * estimate the outputs based on the hidden unit activations. Whether or not the predictor supports Classification,
 * Regression, and what classification features it supports - will determine what the RBF Network supports. This allows
 * for models technically more complicated and powerful than the standard RBF network. <br>
 * <br>
 * The initial phases of a RBF Network is to learn the neuron locations and activations. This part can also be seen as
 * learning a data transformation. As such, the RBF Network can be used as a DataTransform itself. <br>
 * The last phase of the network is to learn the model based on the data point activations. <br>
 * <br>
...
public class RBFNet implements Classifier, Regressor, DataTransform, Parameterized {
  ...
  /**
   * Creates a new RBF Network suitable for binary classification or regression. One of the other constructors should be
   * used if you need classification for multi-class or if you need probability outputs. <br>
   * <br>
   * This will use {@link Phase1Learner#K_MEANS} for neuron selection and
   * {@link Phase2Learner#NEAREST_OTHER_CENTROID_AVERAGE} for activation tuning. The {@link EuclideanDistance} will be
   * use as the metric.
   *
   * @param numCentroids the number of centroids or neurons to use in the network's hidden layer
   */
  ...
  /**
   * Creates a new RBF Network for classification tasks. If the classifier can also perform regression, then the network
   ...
   * @param alpha a parameter that may have an effect on the neuron activation learning method.
   * @param p a parameter that may have an effect on the neuron activation learning method
   * @param dm the distance metric to use
   * @param baseClassifier the base classifier to learn on top of the hidden layer activations.
   */
  ...
  /**
   * Creates a new RBF Network for regression tasks. If the regressor can also perform classification, then the network
   * will be able to perform both.
   *
   * @param numCentroids the number of centroids or neurons to use in the network's hidden layer
   * @param cl the method to learn the neuron locations
   ...
   * @param p a parameter that may have an effect on the neuron activation learning method
   * @param dm the distance metric to use
   * @param baseRegressor the base regressor to learn on op of the hidden layer activations.
   */
  ...
  /**
   * The first phase of learning a RBF Neural Network is to determine the neuron locations. This enum controls which
   * method is used.
   */
  ...
  /**
   * The second phase of learning a RBF Neural Network is to determine how the neurons are activated to produce the
   * output of the hidden layer. This enum control which method is used.
   */
  public static enum Phase2Learner {
    /**
     * This method sets the bandwidth for each neuron based on the distances to the neuron from each data point that is
     * closest to said neuron. If &mu; is the average distance to the neuron, and &sigma; the standard deviation, then
     * the bandwidth <i>b</i> of the <i>j</i>'th neuron is seto to <i>b<sub>j</sub> = &mu;<sub>j</sub> +
    ...
    /**
     * This bandwidth estimator only works for classification problems. Each neuron is assigned a class based on the
     * majority class labels of the data points closes to said neuron. The bandwidth is then estimated as
     * {@link #setAlpha(double) &alpha;} times the distance from the neuron to the closest neuron with a different class
     * label.<br>
     * <br>
     * For this method &alpha; values between (0, 1) usually work best, 0.25 is a good starting value. The value of
     * &alpha; can go past 1.
     */
    CLOSEST_OPPOSITE_CENTROID {
      ...
      protected double[] estimateBandwidths(final double alpha, int p, DataSet data, final List<Vec> centroids, final List<Double> centroidDistCache, final DistanceMetric dm, ExecutorService threadpool) {
        ...
        /**
         * An array of arrays. Each centroid gets its own atomic array, where each value indicates how many objects of
         * class is stored.
         */
    ...
    /**
     * This method sets the bandwidth for each neuron based on the average distance of the {@link #setP(int) p} nearest
     * neurons. The number of standard deviations to add to the activation is controlled by
  ...
  /**
   * Sets the alpha parameter. This value is used for certain {@link Phase2Learner} learners as a parameter. A good
   * default value for most methods is often 1 or 3. However the parameter must always be a non-negative value.
   *
   * @param alpha a non negative value that controls the width of the learned bandwidths.
   */
  ...
  /**
   * Sets the nearest neighbor parameter. This value is used for certain {@link Phase2Learner} learners as a parameter.
   * It is used to control the number of neighbors taken into account in learning the parameter value. It must always be
   * a positive value. 3 is usually a good value for this parameter.
   *
   * @param p the positive integer used that controls the width of the learned bandwidths
   */
  ...
  /**
   * Sets the number of centroids to learn for this model. Increasing the number of centroids increases the complexity
   * of the model as well as training and evaluation time. The centroids serve as the hidden units in the network.
   * <br><br>
   * The centroids learned are controlled via the
   * {@link #setPhase1Learner(jsat.classifiers.neuralnetwork.RBFNet.Phase1Learner)} method
   *
  ...
  /**
   * Sets the method used for learning the bandwidths for each centroid in the network. Depending on the method used, {@link #setAlpha(double)
   * } or {@link #setP(int)} may impact the learned bandwidths.
   *
   ...
   * Normalizing the outputs can increase the generalization ability of the network. By default this is set to
   * {@code true}
   *
   * @param normalize {@code true} to normalize the neuron outputs, {@code false} to use the raw activation values.
   */
