...
public class BackPropagationNet implements Classifier, Regressor, Parameterized {
   ...
   * used to predict from inputs and train the network by propagating the errors
  ...
  public static abstract class ActivationFunction implements Function {
     ...
     * function. The derivative must be calculated using only the output of the
     * response when given the original input. Meaning: given an input {@code x}
     * , the value of f'(x) must be computable as g(f(x))
     *
    ...
    /**
     * Computes the response of the response of this activation function on the
     * given input value
     *
     * @param x
     *          the input value
  ...
  public enum WeightInitialization {
    UNIFORM {
      @Override
      public double getWeight(final int inputSize, final int layerSize, final double eta, final Random rand) {
    ...
    TANH_NORMALIZED_INITIALIZATION {
      @Override
      public double getWeight(final int inputSize, final int layerSize, final double eta, final Random rand) {
        final double cnst = sqrt(6.0 / (inputSize + layerSize));
     ...
     *
     * @param inputSize
     *          also referred to as the fan<sub>in</sub>
     * @param layerSize
     *          also referred to as the fan<sub>out</sub>
     * @param eta
  ...
  /**
   * The logit activation function. This function goes from [0, 1]. It has more
   * difficultly learning than symmetric activation functions, often requiring
   * considerably more layers and neurons than other activation functions.
   */
  public static final ActivationFunction logitActiv = new ActivationFunction() {
    ...
    @Override
    public double response(final double x) {
      return 1 / (1 + exp(-x));
    }
   ...
   * of [-1, 1]. It works well for classification problems, and is very fast to
   * compute. It sometimes requires more neurons to learn more complicated
   ...
   *          the one to copy
   */
  protected BackPropagationNet(final BackPropagationNet toClone) {
    this(Arrays.copyOf(toClone.npl, toClone.npl.length));
    inputSize = toClone.inputSize;
    outputSize = toClone.outputSize;
    f = toClone.f;
    momentum = toClone.momentum;
    weightDecay = toClone.weightDecay;
    epochs = toClone.epochs;
    initialLearningRate = toClone.initialLearningRate;
    learningRateDecay = toClone.learningRateDecay;
    weightInitialization = toClone.weightInitialization;
    targetBump = toClone.targetBump;
    targetMax = toClone.targetMax;
    targetMin = toClone.targetMin;
    targetMultiplier = toClone.targetMultiplier;
    batchSize = toClone.batchSize;

    if (toClone.Ws != null) {
      Ws = new ArrayList<Matrix>(toClone.Ws);
      for (int i = 0; i < Ws.size(); i++) {
        Ws.set(i, Ws.get(i).clone());
      }
    }

    if (toClone.bs != null) {
      bs = new ArrayList<Vec>(toClone.bs);
      for (int i = 0; i < bs.size(); i++) {
        bs.set(i, bs.get(i).clone());
      }
    }
  }
   ...
   *
   * @param npl
   *          the array of hidden layer information. The length indicates how
   *          many hidden layers, and the value of each index indicates how many
  ...
  public BackPropagationNet(final int... npl) {
    ...
    params.add(new ObjectParameter<ActivationFunction>() {
      ...
      @Override
      public boolean setObject(final ActivationFunction obj) {
  ...
  public CategoricalResults classify(final DataPoint data) {
    final CategoricalResults cr = new CategoricalResults(outputSize);
    final Vec x = feedForward(data.getNumericalValues());
  ...
  /**
   * Computes the delta between the networks output for a same and its true
   * value
   *
   * @param dataSet
   *          the data set we are learning from
   * @param idx
   *          the index into the data set for the current data point
   * @param delta_out
   *          the place to store the delta, may already be initialized with
   *          random noise
   * @param a_i
   *          the activation of the final output layer for the data point
   * @param d_i
   *          the derivative of the activation of the final output layer
   * @return the error that occurred in predicting this data point
   */
  private double computeOutputDelta(final DataSet dataSet, final int idx, final Vec delta_out, final Vec a_i,
      final Vec d_i) {
    double error = 0;
   ...
   *
   * @param inputthe
   *          input vector to feed in
   * @return the output vector for the given input at the final layer
   */
  private Vec feedForward(final Vec input) {
    Vec x = input;
    for (int i = 0; i < Ws.size(); i++) {
      final Matrix W_i = Ws.get(i);
      final Vec b_i = bs.get(i);

      final Vec a_i = W_i.multiply(x);
      a_i.mutableAdd(b_i);
   ...
   *
   * @param input
   *          the input to feed forward though the network
   * @param activations
   *          the list of allocated vectors to store the activation outputs for
   *          each layer
   * @param derivatives
   *          the list of allocated vectors to store the derivatives of the
   *          activations
   */
  private void feedForward(final Vec input, final List<Vec> activations, final List<Vec> derivatives) {
    Vec x = input;
    for (int i = 0; i < Ws.size(); i++) {
      final Matrix W_i = Ws.get(i);
      final Vec b_i = bs.get(i);

      final Vec a_i = activations.get(i);
      a_i.zeroOut();
      ...
      final Vec d_i = derivatives.get(i);
      a_i.copyTo(d_i);
  ...
  private void initializeWeights(final Matrix W, final Random rand) {
  ...
  private void initializeWeights(final Vec b, final int inputSize, final Random rand) {
   ...
   *
   * @param batchSize
   *          the number of training instances to use on each update
   */
   ...
   *
   * @param momentum
   *          the momentum to apply to training
   */
   ...
   */
  private void setUp(final Random rand) {
  ...
  /**
   * Sets the weight decay used for each update. The weight decay must be in the
   * range [0, 1). Weight decay values must often be very small, often 1e-8 or
   * less.
   *
   * @param weightDecay
   *          the weight decay to apply when training
   */
  public void setWeightDecay(final double weightDecay) {
   ...
   */
  public void setWeightInitialization(final WeightInitialization weightInitialization) {
  ...
  public void train(final RegressionDataSet dataSet) {
    ...
    for (int i = 0; i < dataSet.getSampleSize(); i++) {
      final double val = dataSet.getTargetValue(i);
   ...
   */
  private void trainNN(final DataSet dataSet) {
    // batchSize

    final List<List<Vec>> activations = new ArrayList<List<Vec>>(batchSize);
    final List<List<Vec>> derivatives = new ArrayList<List<Vec>>(batchSize);
    final List<List<Vec>> deltas = new ArrayList<List<Vec>>(batchSize);

    final List<Matrix> updates = new ArrayList<Matrix>(Ws.size());

    final List<Vec> cur_x = new ArrayList<Vec>(batchSize);
    final List<Vec> prev_x = new ArrayList<Vec>(batchSize);
    ...
    final IntList iterOrder = new IntList(dataSet.getSampleSize());
    ...
    for (int epoch = 0; epoch < epochs; epoch++) {
      ...
      for (int iter = 0; iter < dataSet.getSampleSize(); iter += batchSize) {
        ...
        for (int bi = 0; bi < batchSize; bi++) {
          ...
          final Vec x = dataSet.getDataPoint(idx).getNumericalValues();
          cur_x.add(x);
          feedForward(x, activations.get(bi), derivatives.get(bi));

          // Compution of Deltas
          final Vec delta_out = deltas.get(bi).get(npl.length);

          final Vec a_i = activations.get(bi).get(npl.length);
          final Vec d_i = derivatives.get(bi).get(npl.length);
        ...
        for (int bi = 0; bi < batchSize; bi++) {
          for (int i = Ws.size() - 2; i >= 0; i--) {
            final Vec delta = deltas.get(bi).get(i);
            delta.zeroOut();
            final Matrix W = Ws.get(i + 1);
          ...
          for (int i = 1; i < Ws.size(); i++) {
            final Matrix W = Ws.get(i);
            final Vec b = bs.get(i);
            W.mutableSubtract(eta * weightDecay, W);

            if (momentum != 0) {
              final Matrix update = updates.get(i);
          ...
          // input layer
          final Matrix W = Ws.get(0);
          W.mutableSubtract(eta * weightDecay, W);
          final Vec b = bs.get(0);

          if (momentum != 0) {
            final Matrix update = updates.get(0);
            update.mutableMultiply(momentum);
