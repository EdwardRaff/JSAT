...
/**
 * An implementation of a Feed Forward Neural Network (NN) trained by Back Propagation. NNs are powerful classifiers and
 * regressors, but can suffer from slow training time and overfitting. <br>
 *
 * @author Edward Raff
 */
public class BackPropagationNet implements Classifier, Regressor, Parameterized {
  ...
  /**
   * Length of the array determines how many layers of hidden units. Value at each index determines how many neurons are
  ...
  /**
   * Target min and max and scaling multiplier for regression problems to make the target into a range that the
   ...
   *
   * @param npl the array of hidden layer information. The length indicates how many hidden layers, and the value of
   * each index indicates how many neurons to place in each hidden layer
   */
  ...
  /**
   * Sets the weight decay used for each update. The weight decay must be in the range [0, 1). Weight decay values must
   * often be very small, often 1e-8 or less.
   *
  ...
  /**
   * The neural network needs an activation function for the neurons that is used to predict from inputs and train the
   * network by propagating the errors back through the network.
   */
  public static abstract class ActivationFunction implements Function {
    ...
    /**
     * Returns the function object for the derivative of this activation function. The derivative must be calculated
     * using only the output of the response when given the original input. Meaning: given an input {@code x}, the value
     * of f'(x) must be computable as g(f(x))
     *
  ...
  /**
   * The logit activation function. This function goes from [0, 1]. It has more difficultly learning than symmetric
   * activation functions, often requiring considerably more layers and neurons than other activation functions.
   */
  ...
  /**
   * The tanh activation function. This function is symmetric in the range of [-1, 1]. It works well for many problems
   * in general.
   */
  ...
  /**
   * The softsign activation function. This function is symmetric in the range of [-1, 1]. It works well for
   * classification problems, and is very fast to compute. It sometimes requires more neurons to learn more complicated
   ...
   * @param activations the list of allocated vectors to store the activation outputs for each layer
   * @param derivatives the list of allocated vectors to store the derivatives of the activations
   */
