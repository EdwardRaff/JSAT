...
/**
 * LVQ with Locally Learned Classifier (LVQ-LLC) is an adaption of the LVQ
 * algorithm I have come up with. Given a classification data set, LVQ develops
 * and moves several prototype vectors throughout the space, trying to place
 * them as good representatives. Classification is then done Nearest Neighbor
 * style among the prototypes. <br>
 ...
 * near the prototype using the {@link #getEpsilonDistance() } parameter that is
 * used to update two prototypes at the same time. Classification can then be
 * done by getting the Classifier for the nearest prototype, or averaging the
 * results of the two closest prototypes if the point is near a boundary. <br>
 ...
 * <li>Approximate decision boundaries can be more complicated than voronoi
 * diagrams</li>
...
public class LVQLLC extends LVQ {
  ...
  /**
   * Creates a new LVQ-LLC instance that uses {@link MultivariateNormals} as the
   * local classifier.
   *
   * @param dm
   ...
   * @param iterations
   *          the number of iterations to perform
   * @param localClasifier
   *          the classifier to use as a local classifier for each prototype
   */
   ...
   */
  public LVQLLC(final DistanceMetric dm, final int iterations, final Classifier localClasifier,
      final double learningRate, final int representativesPerClass) {
   ...
   */
  public LVQLLC(final DistanceMetric dm, final int iterations, final Classifier localClasifier,
      final double learningRate, final int representativesPerClass, final LVQVersion lvqVersion,
  ...
  protected LVQLLC(final LVQLLC toCopy) {
    ...
    if (toCopy.localClassifier != null) {
      localClassifier = toCopy.localClassifier.clone();
    }
    if (toCopy.localClassifeirs != null) {
      localClassifeirs = new Classifier[toCopy.localClassifeirs.length];
      for (int i = 0; i < localClassifeirs.length; i++) {
        localClassifeirs[i] = toCopy.localClassifeirs[i].clone();
      }
    }
  }
  ...
  public CategoricalResults classify(final DataPoint data) {
    final List<? extends VecPaired<VecPaired<Vec, Integer>, Double>> nns = vc.search(data.getNumericalValues(), 2);
   ...
   *          the local classifier to use for each prototype
   */
  ...
  public void trainC(final ClassificationDataSet dataSet, final ExecutorService threadPool) {
    ...
    final List<List<DataPointPair<Integer>>> listOfLocalPoints = new ArrayList<List<DataPointPair<Integer>>>(
        weights.length);
    ...
    for (int i = 0; i < weights.length; i++) {
      ...
      final ClassificationDataSet localSet = new ClassificationDataSet(listOfLocalPoints.get(i),
