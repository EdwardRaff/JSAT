...
/**
 * LVQ with Locally Learned Classifier (LVQ-LLC) is an adaption of the LVQ algorithm I have come up with. Given a
 * classification data set, LVQ develops and moves several prototype vectors throughout the space, trying to place them
 * as good representatives. Classification is then done Nearest Neighbor style among the prototypes. <br>
 * LVQ-LLC trains a local classifier on all of the data points that belong to the prototype, and the data points that
 * lie across the border but are still near the prototype using the {@link #getEpsilonDistance() } parameter that is
 * used to update two prototypes at the same time. Classification can then be done by getting the Classifier for the
 * nearest prototype, or averaging the results of the two closest prototypes if the point is near a boundary. <br>
 * This is done because, given a complex decision boundary and a sufficient number of prototypes, each prototype's
 * domain will be a smaller subset of the problem and will hopefully resemble a simpler decision problem that can be
...
public class LVQLLC extends LVQ {
  ...
  /**
   * Creates a new LVQ-LLC instance that uses {@link MultivariateNormals} as the local classifier.
   *
  ...
  /**
   * Each prototype will create a classifier that is local to itself, and trained on the points that belong to the
   * prototype and those near the border of the prototype. This sets the classifier that will be used
   *
