...
public class SGDNetworkTrainer implements Serializable {
   ...
   *
   * @param X
   *          the matrix to dropout values from
   * @param randThresh
   *          the threshold that a random integer must be less than to get
   *          dropped out
   * @param rand
   *          the source of randomness
   * @param ex
   *          the source of threads for parlallel computation, or {@code null}
   */
  ...
  /**
   * This list contains the neuron weight connection matrix for each layer after
   * the input layer
   */
  ...
  public SGDNetworkTrainer(final SGDNetworkTrainer toCopy) {
    layerSizes = Arrays.copyOf(toCopy.layerSizes, toCopy.layerSizes.length);
    eta = toCopy.eta;
    weightInit = toCopy.weightInit.clone();
    biasInit = toCopy.biasInit.clone();
    regularizer = toCopy.regularizer.clone();
    updater = toCopy.updater.clone();
    setDropoutInput(toCopy.getDropoutInput());
    setDropoutHidden(toCopy.getDropoutHidden());
    if (toCopy.W != null) {
    ...
    if (toCopy.W_deltas != null) {
      W_deltas = new ArrayList<Matrix>();
      for (final Matrix w : toCopy.W_deltas) {
        W_deltas.add(w.clone());
      }
      B_deltas = new ArrayList<Vec>();
      for (final Vec b : toCopy.B_deltas) {
        B_deltas.add(b.clone());
      }
    }
    if (toCopy.W_updaters != null) {
      W_updaters = new ArrayList<List<GradientUpdater>>();
      for (final List<GradientUpdater> updaters : toCopy.W_updaters) {
        ...
        W_updaters.add(copyUpdaters);
        for (final GradientUpdater item : updaters) {
          copyUpdaters.add(item.clone());
        }
      }
      B_updaters = new ArrayList<GradientUpdater>(toCopy.B_updaters);
      for (final GradientUpdater item : toCopy.B_updaters) {
        B_updaters.add(item.clone());
      }
    }
    layersActivation = new ArrayList<ActivationLayer>(toCopy.layersActivation.size());
    for (final ActivationLayer activation : toCopy.layersActivation) {
      layersActivation.add(activation.clone());
    }
  }

  private void accumulateUpdates(final Matrix X, final Matrix[] activationsM, final Matrix[] deltasM,
      final ExecutorService ex, final List<Vec> x) {
  ...
  private double backpropagateError(final Matrix[] deltasM, final Matrix[] activationsM, final List<Vec> x,
      final List<Vec> y, double errorMade, final ExecutorService ex, final Matrix[] unactivatedM) {
  ...
  private void feedforward(final Matrix X, final Matrix[] activationsM, final Matrix[] unactivatedM,
      final ExecutorService ex, final Random rand) {
    // feed forward
    for (int l = 0; l < layersActivation.size(); l++) {
      final Matrix a_lprev = l == 0 ? X : activationsM[l - 1];
   ...
   *
   * @param x
   *          the input vector to feed forward through the network
  ...
  /**
   * This method assumes that the neural network structure is already in place,
   * and prepares only the structure needed to perform updates. <br>
   ...
   *
   * @param p
   *          the probability in [0, 1) of dropping a value in the hidden layer
   */
  ...
  /**
   * Sets the array indicating the total number of layers in the network and the
   * sizes of each layer. The length of the array is the number of layers and
   * the value at each index is the size of that layer.
   *
   * @param layerSizes
   ...
   *
   * @param regularizer
   *          the method of regularizing the network
   */
  ...
  public double updateMiniBatch(final List<Vec> x, final List<Vec> y, final ExecutorService ex) {
    ...
    for (int i = 0; i < layersActivation.size(); i++) {
      // TODO isntead of making a whole new matrix every time, use a submatrix
      // when bigger and enlarge when too small
