...
/**
 * Multinomial Logistic Regression is an extension of {@link LogisticRegression}
 * for classification when there are more then two target classes. The results
 * of this extension can differ greatly from applying {@link OneVSAll} to
 * Logistic Regression.
 *
 * @author Edward Raff
 */
public class MultinomialLogisticRegression implements Classifier {
  ...
  public CategoricalResults classify(final DataPoint data) {
     ...
     *
     * exp/X B \ \ i k/ P/y = k\ = -------------------- \ i / K ===== \ 1 + >
     * exp/X B \ / \ i j/ ===== j = 1
     *
     * and for class = 0
     *
     *
     * 1 P/y = 0\ = -------------------- \ i / K ===== \ 1 + > exp/X B \ / \ i
     * j/ ===== j = 1
     */
  ...
  public MultinomialLogisticRegression clone() {
    ...
    if (classCoefficents != null) {
      clone.classCoefficents = new Vec[classCoefficents.length];
      for (int i = 0; i < classCoefficents.length; i++) {
        clone.classCoefficents[i] = classCoefficents[i].clone();
      }
    }
