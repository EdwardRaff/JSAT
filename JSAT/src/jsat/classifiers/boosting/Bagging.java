 ...
 * <br>
 * Bagging is an ensemble learner, it takes a weak learner and trains several to create a better over result. Bagging is
 * particularly useful when the base classifier has some amount of predictive power, but is hindered by variance in the
 * output (small change in input causes large change in output), or variances in input (handles noisy data badly or is
 * has a brittle learning algorithm). It is common to perform bagging on {@link DecisionTree Decision Trees}, because
 * they meet these strengths and weaknesses.  <br>
 * Bagging produces little to no improvement when using learners that have low variance and robust learning methods.
 * {@link NearestNeighbour} is an example of a particularly bad method to bag.
 * <br>
...
public class Bagging implements Classifier, Regressor, Parameterized {
  ...
  /**
   * The number of extra samples to take when bagging in each round used by default in the constructor:
   * {@value #DEFAULT_EXTRA_SAMPLES}
   */
  public static final int DEFAULT_EXTRA_SAMPLES = 0;
  /**
   * The default behavior for parallel training, as specified by {@link #setSimultaniousTraining(boolean) } is
  ...
  /**
   * Bagging samples from the training set with replacement, and draws a sampleWithReplacement at least as large as the
   * training set. This controls how many extra samples are taken. If negative, fewer samples will be taken. Using
   * negative values is not recommended.
   *
  ...
  /**
   * Bagging produces multiple base learners. These can all be trained at the same time, using more memory, or
   * sequentially using the base learner's parallel training method. If set to true, the base learners will be trained
   * simultaneously.
   *
  ...
  /**
   * Creates a new data set from the given sample counts. Points sampled multiple times will have multiple entries in
   * the data set.
   *
   ...
   * set with their weight multiplied by the number of times it was sampled.
   *
   * @param dataSet the data set that was sampled from
   * @param sampledCounts the sampling values obtained from 
     * {@link #sampleWithReplacement(int[], int, java.util.Random) }
  ...
  /**
   * Performs the sampling based on the number of data points, storing the counts in an array to be constructed from
   * XXXX
   *
   * @param sampleCounts an array to keep count of how many times each data point was sampled. The array will be filled
   * with zeros before sampling starts
