 ...
 * Additive Modeling using a Multi-Class Exponential loss function), presented
 * in <i>Multi-class AdaBoost</i> by Ji Zhu, Saharon Rosset, Hui Zou,&amp;Trevor
 * Hasstie <br>
 * <br>
 * This algorithm reduces to {@link AdaBoostM1 } for binary classification
 * problems. Its often performs better for <i>k</i> class classification
 * problems, and has a weaker requirement of besting 1/<i>k</i> accuracy for any
 * k instead of 1/2.
 *
 * @author Edward Raff
 */
public class SAMME implements Classifier, Parameterized {
  ...
  public SAMME clone() {
    ...
    if (hypWeights != null) {
      clone.hypWeights = new DoubleList(hypWeights);
    }
    if (hypoths != null) {
      clone.hypoths = new ArrayList<Classifier>(hypoths.size());
      for (int i = 0; i < hypoths.size(); i++) {
        clone.hypoths.add(hypoths.get(i).clone());
      }
    }
    if (predicting != null) {
  ...
  public void trainC(final ClassificationDataSet dataSet, final ExecutorService threadPool) {
    ...
    for (int t = 0; t < maxIterations; t++) {
      ...
      error /= sumOfWeights;
      if (error >= 1.0 - 1.0 / K || error == 0.0) {/// Diference, we only need
                                                   /// to be better then random
                                                   /// guessing classes
        return;
      }
