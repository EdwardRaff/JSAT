...
/**
 * An implementation of the original 2 class LogitBoost algorithm. While there
 * is a multi-class description in the original paper, its implementation is
 * congruent with the result of using LogitBoost with {@link OneVSAll}
 * classifier. <br>
 * LogitBoost differs from its predecessors in that it boosts {@link Regressor
 * regression} models to create a powerful classifier. <br>
 * <br>
 * Paper: <b>Special Invited Paper Additive Logistic Regression: A Statistical
 * View of Boosting</b>, By Jerome Friedman, Trevor Hastie and Robert
 * Tibshirani. <i>The Annals of Statistics</i> 2000, Vol. 28, No. 2, 337–407
 *
 * @author Edward Raff
 */
public class LogitBoost implements Classifier, Parameterized {
   ...
   *
   * @param baseLearner
   *          the weak learner to build an ensemble out of.
   * @param M
   *          the maximum number of iterations.
   */
  ...
  protected double P(final DataPoint x) {
    /**
     * F(x) e p(x) = --------------- F(x) - F(x) e + e
     */
    final double fx = F(x);
    final double efx = Math.exp(fx);
    final double enfx = Math.exp(-fx);
    if (Double.isInfinite(efx) && efx > 0 && enfx < 1e-15) {// Well classified
                                                            // point could
                                                            // return a Infinity
                                                            // which turns into
                                                            // NaN
  ...
  /**
   * Sets the penalty bound for miss-classification of results. This also
   * provides numerical stability to the algorithm. The results are not
   * sensitive to this value. The recommended value range is in [2, 4]
   *
   * @param zMax
   *          the penalty bound
   * @throws ArithmeticException
   *           if the value is not in (0, {@link Double#MAX_VALUE}]
   */
  ...
  public void trainC(final ClassificationDataSet dataSet) {
    if (dataSet.getClassSize() != 2) {
      throw new FailedToFitException(
          "LogitBoost only supports binary decision tasks, not " + dataSet.getClassSize() + " class problems");
    }
    /**
     * The data points paired with what we will use to store the target
     * regression values.
     */
