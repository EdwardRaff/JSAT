...
/**
 * An implementation of the original 2 class LogitBoost algorithm. While there is a multi-class description in the
 * original paper, its implementation is congruent with the result of using LogitBoost with {@link OneVSAll} classifier.
 * <br>
 * LogitBoost differs from its predecessors in that it boosts {@link Regressor regression} models to create a powerful
 * classifier.
 * <br><br>
 * Paper: <b>Special Invited Paper Additive Logistic Regression: A Statistical View of Boosting</b>, By Jerome Friedman,
...
public class LogitBoost implements Classifier, Parameterized {
  ...
  /**
   * Sets the maximum number of iterations of boosting that can occur, giving the maximum number of base learners that
   * may be trained
   *
  ...
  /**
   * Sets the penalty bound for miss-classification of results. This also provides numerical stability to the algorithm.
   * The results are not sensitive to this value. The recommended value range is in [2, 4]
   *
