...
/**
 * Emphasis Boost is a generalization of the Real AdaBoost algorithm, expanding the update term and providing the {@link #setLambda(double) &lambda;
 * } term to control the trade off. With &lambda; = 1/2, it becomes equivalent to Real AdaBoost. If the weak learner
 * does not support confidence outputs (non-hard decisions), then it further becomes equivalent to Discrete Ada Boost.
 * <br>
 * Emphasis Boost only supports binary classification problems, the learner used should support weighted predictions.
 * <br><br>
 * NOTE: In the face of extreme outliers, it is possible for numerical instability to occur. This implementation
 * attempts to reset weights when numerical issues occur.
 * <br><br>
 * See: <br>
 * Gómez-Verdejo, V., Ortega-Moral, M., Arenas-García, J.,&amp;Figueiras-Vidal, A. R. (2006). <i>Boosting by weighting
 * critical and erroneous samples</i>. Neurocomputing, 69(7-9), 679–685. doi:10.1016/j.neucom.2005.12.011
 *
 * @author Edward Raff
 */
public class EmphasisBoost implements Classifier, Parameterized, BinaryScoreClassifier {
  ...
  /**
   * &lambda; controls the trade off between weighting the errors based on their distance to the margin and the
   * quadratic error of the output. The three extreme values are: <br>
   * <ul>
   * <li> &lambda; = 0 , in this case all the weight is placed on points based on their distance to the margin of the
   * classification boundary. </li>
   * <li>&lambda; = 1/2, in this case weight is balanced between the margin distance and the quadratic error. This is
   * equivalent to Real Ada Boost
   * </li>
   * <li>&lambda; = 1, in this case the weight is placed purely based on the quadratic error of the output</li>
   * </ul>
   * <br><br> According to the original paper, values in the range [0.3, 0.4] often perform well.
   *
   * @param lambda the trade off parameter in [0, 1]
   */
