...
/**
 * This provides an implementation of the Stacking ensemble method meant for Updatable models. Stacking learns several
 * base classifiers and a top level classifier learns to predict the target based on the outputs of all the ensambled
 * models. Historically a linear model (such as {@link LinearBatch}) is used, which translates to learning a weighted
 * vote of the classifier outputs. However any classifier may be used so long as it supports the desired target type.
 * <br>
 * <br>
...
public class UpdatableStacking implements UpdateableClassifier, UpdateableRegressor {
  ...
  /**
   * Gets the predicted vector wrapped in a new DataPoint from a data point assuming we are doing classification
   *
