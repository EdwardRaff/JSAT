...
public class ArcX4 implements Classifier, Parameterized {
  ...
  private class Tester implements Runnable {
    ...
    public Tester(final ClassificationDataSet cds, final int[] errors, final int start, final int end,
        final Classifier hypoth, final CountDownLatch latch) {
      this.cds = cds;
      this.errors = errors;
      this.start = start;
      this.end = end;
  ...
  public ArcX4 clone() {
    ...
    if (predicing != null) {
      clone.predicing = predicing.clone();
    }
    if (hypoths != null) {
      clone.hypoths = new Classifier[hypoths.length];
      for (int i = 0; i < clone.hypoths.length; i++) {
        clone.hypoths[i] = hypoths[i].clone();
      }
    }
   ...
   *
   * @param coef
   *          the multiplicative factor on the errors in weight construction
   */
  ...
  /**
   * Weights are updated as 1+coef*errors<sup>expo</sup>. This sets the exponent
   * used to update the errors
   *
   * @param expo
   *          the exponent to use
   */
  ...
  public void trainC(final ClassificationDataSet dataSet, final ExecutorService threadPool) {
    // Create a low memory clone that only has different dataPoint Objects to
    // save space
