 ...
 * the aggregating model does. <br>
 * See: Wolpert, D. H. (1992). Stacked generalization. Neural Networks, 5,
 * 241–259.
 *
 * @author Edward Raff
 */
public class Stacking implements Classifier, Regressor {
  ...
  /**
   * Creates a new Stacking classifier that uses {@value #DEFAULT_FOLDS} folds
   * of cross validation
   *
   * @param aggregatingClassifier
   *          the classifier used to merge the results of all the input
   *          classifiers
   * @param baseClassifiers
   ...
   *          classifiers
   * @param baseClassifiers
   *          the list of base classifiers to ensemble
   */
   ...
   *
   * @param folds
   *          the number of cross validation folds for learning the base model
   * @param aggregatingClassifier
   *          the classifier used to merge the results of all the input
   *          classifiers
   * @param baseClassifiers
  ...
  public Stacking(final int folds, final Classifier aggregatingClassifier, final List<Classifier> baseClassifiers) {
    if (baseClassifiers.size() < 2) {
      throw new IllegalArgumentException(
          "base classifiers must contain at least 2 elements, not " + baseClassifiers.size());
    }
    setFolds(folds);
    ...
    if (allRegressors) {
      ...
      baseRegressors = (List) baseClassifiers;// ugly type easure
                                              // exploitation...
    }
  }
   ...
   * @param folds
   *          the number of cross validation folds for learning the base model
   * @param aggregatingRegressor
   *          the regressor used to merge the results of all the input
   *          classifiers
   * @param baseRegressors
   *          the list of base regressors to ensemble
   */
  public Stacking(final int folds, final Regressor aggregatingRegressor, final List<Regressor> baseRegressors) {
    ...
    if (allClassifiers) {
      ...
      baseClassifiers = (List) baseRegressors;// ugly type easure
                                              // exploitation...
    }
  }
   ...
   *          classifiers
   * @param baseRegressors
   *          the array of base regressors to ensemble
   */
  ...
  /**
   * Creates a new Stacking regressor that uses {@value #DEFAULT_FOLDS} folds of
   * cross validation
   *
   * @param aggregatingRegressor
   *          the regressor used to merge the results of all the input
  ...
  public Stacking(final Stacking toCopy) {
    folds = toCopy.folds;
    weightsPerModel = toCopy.weightsPerModel;
    if (toCopy.aggregatingClassifier != null) {
      aggregatingClassifier = toCopy.aggregatingClassifier.clone();
      baseClassifiers = new ArrayList<Classifier>(toCopy.baseClassifiers.size());
      for (final Classifier bc : toCopy.baseClassifiers) {
        baseClassifiers.add(bc.clone());
      }

      if (toCopy.aggregatingRegressor == toCopy.aggregatingClassifier) // supports
                                                                       // both
      {
        aggregatingRegressor = (Regressor) aggregatingClassifier;
        baseRegressors = (List) baseClassifiers;// ugly type easure
    ...
    {
      aggregatingRegressor = toCopy.aggregatingRegressor.clone();
      baseRegressors = new ArrayList<Regressor>(toCopy.baseRegressors.size());
      for (final Regressor br : toCopy.baseRegressors) {
        baseRegressors.add(br.clone());
      }
    }
  }
   ...
   * Note that the number of folds may be 1, and will run significantly faster
   * since models do not need to be re-trained. However it will be more prone to
   * overfitting.
   *
   * @param folds
  ...
  public void train(final RegressionDataSet dataSet, final ExecutorService threadPool) {
    ...
    for (final RegressionDataSet rds : dataFolds) {
      for (int i = 0; i < rds.getSampleSize(); i++) {
        metaSet.addDataPoint(new DataPoint(new DenseVector(weightsPerModel * models), rds.getDataPoint(i).getWeight()),
    ...
    for (int c = 0; c < baseRegressors.size(); c++) {
      ...
      for (int f = 0; f < dataFolds.size(); f++) {
        ...
        for (int i = 0; i < test.getSampleSize(); i++)// evaluate and mark each
                                                      // point in the held out
                                                      // fold.
        {
    ...
    // train the final classifiers, unless folds=1. In that case they are
    // already trained
    if (folds != 1) {
      for (final Regressor reg : baseRegressors) {
        if (threadPool == null) {
  ...
  public void trainC(final ClassificationDataSet dataSet, final ExecutorService threadPool) {
    ...
    final ClassificationDataSet metaSet = new ClassificationDataSet(weightsPerModel * models, new CategoricalData[0],
