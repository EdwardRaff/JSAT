...
/**
 * This provides an implementation of the Stacking ensemble method. Stacking learns several base classifiers and a top
 * level classifier learns to predict the target based on the outputs of all the ensambled models. Historically a linear
 * model (such as {@link LinearBatch}) is used, which translates to learning a weighted vote of the classifier outputs.
 * However any classifier may be used so long as it supports the desired target type. <br>
 * <br>
 * Note, that Stacking tends to work best when the base classifiers produce reasonable probability estimates. <br>
 * Stacking supports {@link #supportsWeightedData() weighted data instances} if the aggregating model does.
 * <br>
...
public class Stacking implements Classifier, Regressor {
  ...
  /**
   * Sets the number of folds of cross validation to use when creating the new set of weights that will be feed into the
   * aggregating model. <br>
   * Note that the number of folds may be 1, and will run significantly faster since models do not need to be
   * re-trained. However it will be more prone to overfitting.
   *
