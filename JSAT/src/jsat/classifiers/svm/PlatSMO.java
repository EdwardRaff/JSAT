...
/**
 * An implementation of SVMs using Platt's Sequential Minimum Optimization (SMO) for both Classification and Regression
 * problems. <br>
 * <br>
 * This algorithm can be warm started for classification problems by any algorithm implementing the
 * {@link BinaryScoreClassifier} interface. For regression any algorithm can be used as a warms start. For best results,
 * warm starts should be from algorithms that will have a similar solution to PlatSMO.
 * <br><br>
 * See:<br>
 * <ul>
 * <li>Platt, J. C. (1998). <i>Sequential Minimal Optimization: A Fast Algorithm for Training Support Vector
 * Machines</i>. Advances in kernel methods (pp. 185 – 208). Retrieved from
 * <a href="http://www.bradblock.com/Sequential_Minimal_Optimization_A_Fast_Algorithm_for_Training_Support_Vector_Machine.pdf">here</a></li>
 * <li>Keerthi, S. S., Shevade, S. K., Bhattacharyya, C.,&amp;Murthy, K. R. K. (2001). <i>Improvements to Platt’s SMO
 * Algorithm for SVM Classifier Design
 * </i>. Neural Computation, 13(3), 637–649. doi:10.1162/089976601300014493</li>
 * <li>Smola, A. J.,&amp;Schölkopf, B. (2004). <i>A tutorial on support vector regression</i>. Statistics and Computing,
 * 14(3), 199–222. doi:10.1023/B:STCO.0000035301.49549.88</li>
 * <li>Shevade, S. K., Keerthi, S. S., Bhattacharyya, C.,&amp;Murthy, K. K. (1999) . <i>Improvements to the SMO
 * algorithm for SVM regression</i>. Control D ivision, Dept. of Mechanical Engineering CD-99–16. Control Division,
 * Dept. of Mechanical Engineering. doi:10.1109/72.870050</li>
 * <li>Shevade, S. K., Keerthi, S. S., Bhattacharyya, C.,&amp;Murthy, K. K. (2000) . <i>Improvements to the SMO
 * algorithm for SVM regression</i>. IEEE transactions on neural networks / a publication of the IEEE Neural Networks
...
public class PlatSMO extends SupportVectorLearner implements BinaryScoreClassifier, WarmRegressor, Parameterized, WarmClassifier {
  ...
  private boolean[] I0;
  /**
   * Indicates if the value that is currently in I0 is also in I0_a. If its not in I0, the value in this array is false
   */
  private boolean[] I0_a;
  /**
   * Indicates if the value that is currently in I0 is also in I0_b. If its not in I0, the value in this array is false
   */
  ...
  /**
   * Creates a new SVM object with a {@link LinearKernel} that uses no cache mode.
   *
   */
  public PlatSMO() {
  ...
  private void trainC_warm_and_normal(ClassificationDataSet dataSet, Classifier warmSolution) {
    ...
    if (iter >= maxIterations) {//1 extra pass to get a better guess on bUp & bLow since we quit early
      for (int i = 0; i < N; i++) {
        numChanged += examineExample(i);
      }
    }
...
//        sparsify();
  ...
  /**
   * Sets the complexity parameter of SVM. The larger the C value the harder the margin SVM will attempt to find. Lower
   * values of C allow for more misclassification errors.
   *
  ...
  /**
   * Sets the maximum number of iterations to perform of the training loop. This is important for cases with a C value
   * that is to large for a non linear problem, which can result in SVM failing to converge.
   *
  ...
  /**
   * Sets where or not modification one or two should be used when training. Modification two is more aggressive, but
   * often results in less kernel evaluations.
   *
   * @param modificationOne {@code true} to us modificaiotn one, {@code false} to use modification two.
   */
  ...
  /**
   * Sets the epsilon for the epsilon insensitive loss when performing regression. This variable has no impact during
   * classification problems. For regression problems, any predicated value that is within the epsilon of the target
   * will be treated as "correct". Increasing epsilon usually decreases the number of support vectors, but may reduce
   * the accuracy of the model
   *
   * @param epsilon the positive value for the acceptable error when doing regression
   */
...
//                err = signum(err)*min(abs(err), 1);
  ...
  /**
   * Guess the distribution to use for the regularization term {@link #setC(double) C} in a SVM.
   *
