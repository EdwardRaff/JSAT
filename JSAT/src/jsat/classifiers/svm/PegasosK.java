 ...
 * with noisy or overlapping class distributions. <br>
 * <br>
 * See: Shalev-Shwartz, S., Singer, Y.,&amp;Srebro, N. (2007). <i>Pegasos :
 * Primal Estimated sub-GrAdient SOlver for SVM</i>. 24th international
 * conference on Machine learning (pp. 807–814). New York, NY: ACM.
...
public class PegasosK extends SupportVectorLearner implements BinaryScoreClassifier, Parameterized {
   ...
   *          the amount of regularization to apply, normally a very small
   *          positive value
   * @param iterations
   *          the number of update iterations to perform
   * @param kernel
   *          the kernel to use
   */
   ...
   *          the kernel to use
   * @param cacheMode
   *          what type of kernel caching to use
   */
  public PegasosK(final double regularization, final int iterations, final KernelTrick kernel,
  ...
  public PegasosK clone() {
    ...
    if (vecs != null) {
      ...
      clone.alphas = new double[alphas.length];
      for (int i = 0; i < vecs.size(); i++) {
        clone.vecs.set(i, vecs.get(i).clone());
        clone.alphas[i] = alphas[i];
      }
    }
  ...
  public void trainC(final ClassificationDataSet dataSet, final ExecutorService threadPool) {
    ...
    try {
      ...
      final int blockSize = m / SystemInfo.LogicalCores + (m % SystemInfo.LogicalCores == 0 ? 0 : 1);// 1
                                                                                                     // extra
                                                                                                     // if
                                                                                                     // we
                                                                                                     // want
