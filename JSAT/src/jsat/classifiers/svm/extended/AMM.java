...
/**
 * This is the batch variant of the Adaptive Multi-Hyperplane Machine (AMM) algorithm. It is related to linear SVMs
 ...
 * While the AMM trained in a batch setting can continue to be updated in an online fashion, the accuracy may reduce if
 * done. This is because only the batch variant will reach a local optima.<br>
 * For this version the {@link #setEpochs(int) } method controls the total number of iterations of the learning
 * algorithm. A small value in [5, 20] should be sufficient.
 * <br>
 * See:
 * <ul>
...
public class AMM extends OnlineAMM {
  ...
  /**
   * Each iteration of the batch AMM algorithm requires at least one epoch over the training set. This control how many
   * epochs make up each iteration of training.
   *
   * @param subEpochs the number passes through the training set done on each iteration of training
   */
