 ...
 * guarantees. However it still produces good results. <br>
 * See:
 * <ul>
...
public class OnlineAMM extends BaseUpdateableClassifier implements Parameterized {
  ...
  /*
   * b/c of the batch learner we use a map, so that we dont have to think about
   * how to handle re-assignment of data points to weight vectors. Also allows
   * us to handle removed cases by checking if our owner is in the map. Use
   * nextID to make sure we give every new vec a unique ID with respect to the
   * class label
   */
  ...
  public OnlineAMM(final OnlineAMM toCopy) {
    if (toCopy.weightMatrix != null) {
      weightMatrix = new ArrayList<Map<Integer, Vec>>(toCopy.weightMatrix.size());
      for (final Map<Integer, Vec> oldW : toCopy.weightMatrix) {
        ...
        weightMatrix.add(newW);
      }
      nextID = Arrays.copyOf(toCopy.nextID, toCopy.nextID.length);
    }
    time = toCopy.time;
    lambda = toCopy.lambda;
    k = toCopy.k;
    c = toCopy.c;
    classBudget = toCopy.classBudget;
    setEpochs(toCopy.getEpochs());
  }
   ...
   * Changes to the scaling of feature vectors may require a change in the value
   * of C <br>
   * <b>NOTE:</b> This parameter <i>is not the same</i> as the standard C
   * parameter associated with SVMs.
   *
   * @param c
   *          the positive pruning constant to use in (0, &infin;)
   */
  ...
  /**
   * When given bad parameters there is the possibility for unbounded growth in
   * the number of hyperplanes used. By setting this value to a reasonable
   ...
   * suggests trying values 10<sup>-2</sup>, 10<sup>-3</sup>, ..., 10
   * <sup>-6</sup>, 10<sup>-7</sup>.
   *
   * @param lambda
   *          the positive regularization parameter in (0, &infin;)
   */
   ...
   * @param y_t
   *          the true label of the data point
   * @param z_t
   *          the hyperplane of the true class with the maximum response, or
   *          {@link Integer#MIN_VALUE} if it should be calculated
   * @return the index of the hyperplane of the true class with that maximum
   *         response
   */
  protected int update(final DataPoint dataPoint, final int y_t, int z_t) {
    ...
    /*
     * Note, we use the same code for both online and batch AMM. If the input is
     * the miniumum value, we are doing a normal online update. If not, we use
     * the given input & need to find out the response for the specified z_t
     * instead
     * 
     * Not clear in the paper, how do we handle the case when z_t was assigned
     * to someone removed? Lets just treat it as unknown. Should be rare, so we
     * wont worry about values changing and being reassigned. Especially since
     * weight vectors near the front should be stable (they survided longer than
     * those formerly infront of them after all).
     */
    double z_t_val;
    if (z_t == Integer.MIN_VALUE || z_t > nextID[y_t]) // z_t is not known, so
                                                       // we will "update" it
                                                       // ourselves
    {
      z_t_val = 0.0;// infinte implicit zero weight vectors, so max is always at
                    // least 0
    ...
    } else// z_t is given, we just need z_t_val
    {
      if (!weightMatrix.get(y_t).containsKey(z_t)) {
        ...
        return update(dataPoint, y_t, Integer.MIN_VALUE);// restart and have a
    ...
    int i_t = y_t > 0 ? 0 : 1;// j_t may be implicit, but i_t needs to belong to
                              // someone in the event of a tie. So just give it
                              // to the first class that isn't y_t
    double i_t_val = 0.0;
    int j_t = -1;
    ...
    for (int i = 0; i < weightMatrix.size(); i++) {
      ...
      // Also must check for implicit weight vectors needing an update (making
      // them non-implicit)
