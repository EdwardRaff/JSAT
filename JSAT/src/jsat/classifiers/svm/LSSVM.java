 ...
 * @author Edward Raff
 */
public class LSSVM extends SupportVectorLearner
  ...
  private class TakeStepLoop implements Callable<PairedReturn<Integer, Integer>> {
    ...
    public TakeStepLoop(final int from, final int to, final int i1, final int i2, final double alph1,
        final double alph2) {
      this.from = from;
      this.to = to;
      this.i1 = i1;
  ...
  public static Distribution guessC(final DataSet d) {
    return PlatSMO.guessC(d);// LS-SVM isn't technically the same algo, but
                             // still a good search
  }
   ...
   *          the kernel method to use
   * @param cacheMode
   *          the caching scheme to use for kernel evaluations
   */
  ...
  public LSSVM(final LSSVM toCopy) {
    ...
    if (toCopy.fcache != null) {
      fcache = Arrays.copyOf(toCopy.fcache, toCopy.fcache.length);
    }
  }
  ...
  private double computeDualityGap(final boolean fast, final ExecutorService ex, final int P)
    ...
    if (fast) {
      ...
      for (int id = 0; id < P; id++) {
        bParts.add(
            ex.submit(new BiasGapCallable(getStartBlock(alphas.length, id, P), getEndBlock(alphas.length, id, P))));
      }
    ...
    for (int id = 0; id < P; id++) {
      gapParts.add(
          ex.submit(new DualityGapCallable(getStartBlock(alphas.length, id, P), getEndBlock(alphas.length, id, P))));
    }
  ...
  private void initializeVariables(final double[] targets, final LSSVM warmSolution, final DataSet data) {
    ...
    if (warmSolution != null) {
      if (warmSolution.alphas.length != alphas.length) {
        throw new FailedToFitException(
            "Warm LS-SVM solution could not have been trained on the sama data, different number of alpha values present");
      }
      final double C_ratio = C / warmSolution.C;
      for (int i = 0; i < targets.length; i++) {
        alphas[i] = warmSolution.alphas[i];
        fcache[i] = warmSolution.fcache[i] - (C_ratio - 1) * warmSolution.alphas[i] / C;
  ...
  /**
   * Sets the regularization constant when training. Lower values correspond to
   * higher amounts of regularization.
   *
   * @param C
  ...
  private boolean takeStep(final int i1, final int i2, final ExecutorService ex, final int P)
      throws InterruptedException, ExecutionException {
    ...
    // Update Fcache[i] for all i in I using (4.10)
    // Compute (i_low, b_low) and (i_up, b_up) using (3.4)
    final List<Future<PairedReturn<Integer, Integer>>> futures = new ArrayList<Future<PairedReturn<Integer, Integer>>>(
        P);
    for (int id = 0; id < P; id++) {
  ...
  public void train(final RegressionDataSet dataSet, final Regressor warmSolution, final ExecutorService threadPool) {
    if (warmSolution != null && !(warmSolution instanceof LSSVM)) {
      throw new FailedToFitException(
          "Warm solution must be an implementation of LS-SVM, not " + warmSolution.getClass());
    }
