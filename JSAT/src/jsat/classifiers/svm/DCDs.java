 ...
 * Retrieved from
 * <a href="http://ntu.csie.org/~cjlin/papers/linear-svr.pdf"> here</a>
 * </ul>
 *
 * @author Edward Raff
 * @see DCD
...
public class DCDs
   ...
   * @param gP
   *          the g'<sub>p</sub>(beta_i) value
   * @param U
   *          the upper bound value obtained from {@link #getU(double) }
   ...
   * @param useL1
   *          whether or not to use L1 or L2 form
   */
   ...
   *          the maximum number of training iterations
   * @param tolerance
   *          the tolerance value for early stopping
   * @param C
  ...
  public DCDs clone() {
    ...
    if (alpha != null) {
      clone.alpha = Arrays.copyOf(alpha, alpha.length);
    }
   ...
   * 1, and values larger than 4 are not normally needed according to the
   * original paper.
   *
   * @param C
   *          the penalty parameter in (0, Inf)
   */
   ...
   * when performing regression. Errors in the output that less than {@code eps}
   * during training are treated as correct. <br>
   ...
   *
   * @param tolerance
   *          the tolerance value to use to stop early
   */
   ...
   *
   * @param useBias
   *          {@code true} to add an implicit bias term to inputs, {@code false}
   *          to use the input data as provided.
   */
   ...
   *
   * @param useL1
   *          <tt>true</tt> to use the L<sup>1</sup> form, <tt>false</tt> to use
   *          the L<sup>2</sup> form.
   */
  ...
  public void train(final RegressionDataSet dataSet, final Regressor warmSolution) {
    ...
    if (warmSolution != null) {
      if (warmSolution instanceof DCDs) {
        ...
        if (alpha != null && other.alpha.length != alpha.length) {
          throw new FailedToFitException("Warm solution could not have been trained on the same data set");
        }

        final double C_mul = C / other.C;
        other.w.copyTo(w);
        w.mutableMultiply(C);
        bias = other.bias * C_mul;
        System.arraycopy(other.alpha, 0, alpha, 0, alpha.length);
    ...
    /*
     * From profling Shufling & RNG generation takes a suprising amount of time
     * on some data sets, so use one of our fast ones
     */
  ...
  public void trainC(final ClassificationDataSet dataSet, final Classifier warmSolution) {
    ...
    if (warmSolution != null) {
      // TODO the below code works OK for warm starting classification problems,
      // but we also need code that works well for warm starting the regression
      // problems to meet the API contract. Having more difficulty with that
      // one.
      ...
      // throw new FailedToFitException("Can not warm start from given solution,
      // it has more than 1 weight vector");
      //
      // Vec w_warm = swvm.getRawWeight(0);
      // double b_warm = useBias ? swvm.getBias(0) : 0;
      // //we can't just copy the values in b/c we need the solution to always
      // be a linear combination of the training data
    ...
    for (int t = 0; t < maxIterations; t++) {
      ...
      while (iter.hasNext())// 2.
      {
        final int i = iter.next();
        // a
        final double G = y[i] * (w.dot(vecs[i]) + bias) - 1 + D[i] * alpha[i];// bias
                                                                              // will
                                                                              // be
                                                                              // zero
                                                                              // if
      ...
      if (M - m < tolerance) // 3.
      {
        ...
        noShrinking = M <= 0 || m >= 0; // technically less agressive then the
                                        // original paper
      }
    }
