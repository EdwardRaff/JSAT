...
/**
 * Implements Dual Coordinate Descent with shrinking (DCDs) training algorithms for a Linear L<sup>1</sup> or
 * L<sup>2</sup> Support Vector Machine for binary classification and regression. NOTE: While this implementation makes
 * use of the dual formulation only the linear kernel is ever used. The algorithm also uses the primal representation
 * and uses the explicit formulation of <i>w</i> in training and classification. As such, the support vectors found are
 * not necessary once training is complete - and will be discarded.<br>
 * <br>
 ...
 * <ul>
 * <li>
 * Hsieh, C.-J., Chang, K.-W., Lin, C.-J., Keerthi, S. S., &amp; Sundararajan, S. (2008). <i>A Dual Coordinate Descent
 * Method for Large-scale Linear SVM</i>. Proceedings of the 25th international conference on Machine learning - ICML
 * ’08 (pp. 408–415). New York, New York, USA: ACM Press. doi:10.1145/1390156.1390208
 * </li>
 * <li>
 * Ho, C.-H., &amp; Lin, C.-J. (2012). <i>Large-scale Linear Support Vector Regression</i>. Journal of Machine Learning
 * Research, 13, 3323–3348. Retrieved from <a href="http://ntu.csie.org/~cjlin/papers/linear-svr.pdf">
 * here</a>
 * </ul>
 *
 * @author Edward Raff
...
public class DCDs implements BinaryScoreClassifier, Regressor, Parameterized, SingleWeightVectorModel, WarmClassifier, WarmRegressor {
  ...
  /**
   * Sets the penalty parameter for misclassifications. The recommended value is 1, and values larger than 4 are not
   * normally needed according to the original paper.
   *
  ...
  /**
   * Sets the {@code eps} used in the epsilon insensitive loss function used when performing regression. Errors in the
   * output that less than {@code eps} during training are treated as correct.
   * <br>
  ...
  /**
   * Sets the tolerance for the stopping condition when training, a small value near zero allows training to stop early
   * when little to no additional convergence is possible.
   *
   ...
   *
   * @param useBias {@code true} to add an implicit bias term to inputs, {@code false} to use the input data as
   * provided.
   */
  ...
  /**
   * Returns {@code true} if an implicit bias term is in use, or {@code false} if not.
   *
   * @return {@code true} if an implicit bias term is in use, or {@code false} if not.
   */
...
//            }
    ...
    for (int t = 0; t < maxIterations; t++) {
      ...
      if (M - m < tolerance)//3.
      {
        ...
        noShrinking = M <= 0 || m >= 0; //technically less agressive then the original paper
      }
    }
