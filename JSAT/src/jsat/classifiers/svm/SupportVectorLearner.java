...
/**
 * Base class for support vector style learners. This means that the learner performs batch training on a fixed set of
 * training points using a {@link KernelTrick kernel} to project the data into a different space. The final set of
 * vectors used may or may not be sparse. It does not necessarily have to be a Support Vector machine.
 * <br><br>
...
public abstract class SupportVectorLearner {
  ...
  /**
   * The array of vectors. In the training phase, this should be the set of all training vectors. After training, this
   * should contain only the set of support vectors.
   */
  protected List<Vec> vecs;
  /**
   * The array of coefficients associated with each support vector. This should be instantiated directly when training.
   * When the set of alphas and support vectors is finalized, {@link #setAlphas(double[]) } should be called with a
   * reference to itself or the array where the final alphas are stored. This will initialized any accelerating
   * structures so that {@link #kEvalSum(jsat.linear.Vec) } can be called.
   */
  ...
  /**
   * Holds an available row for inserting into the cache, null if not available. All values already set to Nan
   */
  private double[] availableRow;
  private int cacheConst = 500;

  /**
   * Sets the final set of alphas, and indicates that the final accelerating structures (if available) should be
   * constructed for performing kernel evaluations against unseen vectors.
   * <br>
   * This may be called multiple times in an online scenario, but calls will involve a re-construction of the whole
   * cache.
   *
  ...
  /**
   * Determines how the final kernel values are cached. The total number of raw kernel evaluations can be tracked using
   * {@link #evalCount}<br> {@link #setCacheMode(jsat.classifiers.svm.SupportVectorLearner.CacheMode) }
   * should be called before training begins by the training algorithm as described in the method documentation.
   */
  public enum CacheMode {
    ...
    NONE,
    /**
     * The entire kernel matrix will be created and cached ahead of time. This is the best option if your data set is
     * small and the kernel cache can fit into memory.
     */
    FULL,
    /**
     * Only the most recently used rows of the kernel matrix will be cached (LRU). When a call to {@link #k(int, int) }
     * occurs, the first value will be taken to be the row of the matrix. <br>
     * Because the kernel matrix is symmetric, if a cache miss occurs - the column value will be checked for its
     * existence. If the row is present, it will be used instead. If both rows are not present, then a new row is
     * inserted for the first index, and another row evicted if necessary.
     * <br>
     * The {@link #cacheEvictions} indicates how many times a row has been evicted from the cache.
     * <br>
  ...
  /**
   * This constructor is meant manly for Serialization to work. It uses a linear kernel and no caching.
   */
...
//            this.availableRow = Arrays.copyOf(toCopy.availableRow, toCopy.availableRow.length);
  ...
  /**
   * Sets the cache value, which may be interpreted differently by different caching schemes. <br>
   * This is currently only used for {@link CacheMode#ROWS}, where the value indicates how many rows will be cached.
   *
  ...
  /**
   * Sets the {@link #setCacheValue(int) cache value} to one that will use the specified amount of memory. If the amount
   * of memory specified is great enough, this method will automatically set the null null   {@link #setCacheMode(jsat.classifiers.svm.SupportVectorLearner.CacheMode)
   ...
   * This is called called by the implementing class to initialize and clear the caches. Calling this with the current
   * cache mode will initialize the caches. Once training is complete, call again with {@code null} to deinitialize the
   * caches.
   *
   * @param cacheMode
   */
   ...
   * for each support vector and associated alpha value currently stored in the support vector machine. It is not
   * necessary to call {@link #setAlphas(double[]) } before calling this, but kernel evaluations may be slower if this
   * is not done.
   *
  ...
  /**
   * Performs a kernel evaluation of the product between two vectors directly. This is the slowest way to do a kernel
   * evaluation, and should be avoided unless there is a specific reason to do so.
   * <br>
  ...
  /**
   * Performs a kernel evaluation of the a'th and b'th vectors in the {@link #vecs} array.
   *
