 ...
 * have to be a Support Vector machine. <br>
 * <br>
...
public abstract class SupportVectorLearner {
  ...
  /**
   * Determines how the final kernel values are cached. The total number of raw
   * kernel evaluations can be tracked using {@link #evalCount}<br>
  ...
  public enum CacheMode {
    ...
    NONE, /**
           ...
           */
    FULL, /**
           * Only the most recently used rows of the kernel matrix will be
           * cached (LRU). When a call to {@link #k(int, int) } occurs, the
           * first value will be taken to be the row of the matrix. <br>
           * Because the kernel matrix is symmetric, if a cache miss occurs -
           * the column value will be checked for its existence. If the row is
           * present, it will be used instead. If both rows are not present,
           * then a new row is inserted for the first index, and another row
           * evicted if necessary. <br>
  ...
  // Implementation note, NaN is used to indicate a cache value that has not
  // been computed yet.
  @ParameterHolder
  private KernelTrick kernel;
  /**
   * The array of vectors. In the training phase, this should be the set of all
   * training vectors. After training, this should contain only the set of
   * support vectors.
   */
  protected List<Vec> vecs;
  /**
   * The array of coefficients associated with each support vector. This should
   * be instantiated directly when training. When the set of alphas and support
   * vectors is finalized, {@link #setAlphas(double[]) } should be called with a
   * reference to itself or the array where the final alphas are stored. This
   * will initialized any accelerating structures so that
  ...
  /**
   * Holds an available row for inserting into the cache, null if not available.
   * All values already set to Nan
   */
  ...
  /**
   * This constructor is meant manly for Serialization to work. It uses a linear
   * kernel and no caching.
   */
   ...
   * @param cacheMode
   *          the kernel caching method to use
   */
  ...
  public SupportVectorLearner(final SupportVectorLearner toCopy) {
    ...
    cacheMode = toCopy.cacheMode;
    if (toCopy.accelCache != null) {
      accelCache = new DoubleList(toCopy.accelCache);
    }
    if (toCopy.fullCache != null) {
      fullCache = new double[toCopy.fullCache.length][];
      for (int i = 0; i < toCopy.fullCache.length; i++) {
        fullCache[i] = Arrays.copyOf(toCopy.fullCache[i], toCopy.fullCache[i].length);
      }
    }
    if (toCopy.partialCache != null) // TODO handling this better needs to be
                                     // done
    {
      setCacheMode(cacheMode);
      // if(toCopy.availableRow != null)
      // this.availableRow = Arrays.copyOf(toCopy.availableRow,
      // toCopy.availableRow.length);
    }

    cacheConst = toCopy.cacheConst;
   ...
   *
   * @param a
   *          the first vector index
   * @param b
   *          the second vector index
   ...
   * for each support vector and associated alpha value currently stored in the
   * support vector machine. It is not necessary to call
   * {@link #setAlphas(double[]) } before calling this, but kernel evaluations
   * may be slower if this is not done.
   *
   * @param y
   *          the vector to perform the kernel product sum against
   ...
   * This is called called by the implementing class to initialize and clear the
   * caches. Calling this with the current cache mode will initialize the
   ...
   * enough, this method will automatically set the null null
   * {@link #setCacheMode(jsat.classifiers.svm.SupportVectorLearner.CacheMode)
   ...
   * @param bytes
   *          the number of bytes of memory to make the cache
   */
  public void setCacheSize(final long N, long bytes) {
    ...
    {
      // guessing 2 work overhead for object header + one pointer reference to
      // the array, asusming 64 bit
   ...
   *
   * @param cacheValue
   *          the cache value to be used
   */
  public void setCacheValue(final int cacheValue) {
    cacheConst = cacheValue;
  }
