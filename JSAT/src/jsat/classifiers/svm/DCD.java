...
/**
 * Implements Dual Coordinate Descent (DCD) training algorithms for a Linear L<sup>1</sup> or L<sup>2</sup> Support
 * Vector Machine for binary classification and regression. NOTE: While this implementation makes use of the dual
 * formulation only the linear kernel is ever used. The algorithm also uses the primal representation and uses the
 * explicit formulation of <i>w</i> in training and classification. As such, the support vectors found are not necessary
 * once training is complete - and will be discarded.
 * <br><br>
 * See:
 * <ul>
 * <li>
...
public class DCD implements BinaryScoreClassifier, Regressor, Parameterized, SingleWeightVectorModel {
  ...
  /**
   * By default, Algorithm 1 is used. Algorithm 2 is an "online" version that updates the dual form by only one data
   * point at a time. This controls which version is used.
   *
