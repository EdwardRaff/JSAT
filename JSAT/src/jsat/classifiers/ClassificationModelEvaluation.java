...
/**
 * Provides a mechanism to quickly perform an evaluation of a model on a data set. This can be done with cross
 * validation or with a testing set.
 *
 * @author Edward Raff
 */
public class ClassificationModelEvaluation {
  ...
  /**
   * Constructs a new object that can perform evaluations on the model. The model will not be trained until evaluation
   * time.
   *
   ...
   * @param classifier the model to train and evaluate
   * @param dataSet the training data set.
   * @param threadpool the source of threads for parallel training. If set to null, training will be done using the 
  ...
  /**
   * Performs an evaluation of the classifier using the training data set. The evaluation is done by performing cross
   * validation.
   *
  ...
  /**
   * Performs an evaluation of the classifier using the training data set, where the folds of the training data set are
   ...
   * do.<br>
   * <br>
   * Performs an evaluation of the classifier using the training data set, where the folds of the training data set, and
   ...
   * based on the data set and avoid all possible excessive memory movement. (For example, {@link DataSet#getNumericColumns()
   ...
   * } or {@link #evaluateTestSet(jsat.classifiers.ClassificationDataSet) }. The statistics for the given score are
   * reset on every call, and the mean / standard deviation comes from multiple folds in cross validation. <br>
   * <br>
   * The score statistics can be obtained from {@link #getScoreStats(ClassificationScore) }
   ...
   * {@code null} will be returned. The object passed in does not need to be the exact same object passed to 
     * {@link #addScorer(ClassificationScore) }, it only needs to be equal to the object.
   *
  ...
  /**
   * Indicates whether or not the predictions made during evaluation should be stored with the expected value for
   * retrieval later.
   *
   * @param keepPredictions <tt>true</tt> if space should be allocated to store the predictions made
   */
  ...
  /**
   * If {@link #keepPredictions(boolean) } was set, this method will return the array storing the predictions made by
   * the classifier during evaluation. These results may not be in the same order as the data set they came from, but
   * the order is paired with {@link #getTruths() }
   *
  ...
  /**
   * If {@link #keepPredictions(boolean) } was set, this method will return the array storing the target classes that
   * should have been predicted during evaluation. These results may not be in the same order as the data set they came
   * from, but the order is paired with {@link #getPredictions()}
   *
  ...
  /**
   * If {@link #keepPredictions(boolean) } was set, this method will return the array storing the weights for each of
   * the points that were classified
   *
  ...
  /**
   * Computes the weighted error rate of the classifier. If all weights of the data points tested were equal, then the
   * value returned is also the percent of data points that the classifier erred on.
   *
  ...
  /**
   * Returns the object that keeps track of the error on individual evaluations. If cross-validation was used, it is the
   * statistics for the errors of each fold. If not, it is for each time {@link #evaluateTestSet(jsat.classifiers.ClassificationDataSet)
   * } was called.
   *
