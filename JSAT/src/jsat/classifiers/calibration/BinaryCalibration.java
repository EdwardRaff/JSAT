...
public abstract class BinaryCalibration implements Classifier, Parameterized {
  ...
  public static enum CalibrationMode {
    /**
     * The naive methods trains the classifier on the whole data set, and then
     * produces the scores for each training point. This may cause over fitting.
     */
    NAIVE, /**
            * The model will be trained by cross validation, using the specified
            * number of {@link #setCalibrationFolds(int) }. The default is 3
            * folds, where the classifier will be trained on the folds not in
            * the set, and then produce scores for the unobserved test points in
            * the held out fold.
            */
    CV, /**
   ...
   *
   * @param label
   *          the set of labels, where {@code true} indicates the positive class
   *          label, and {@code false} indicates the negative class label.
   * @param scores
   *          the score associated with each label from the learning algorithm.
   * @param len
   *          the number of values (from zero) of the label and scores array to
   *          use. This value may be less than the actual array size
   */
  ...
  /**
   * If the calibration mode is set to {@link CalibrationMode#CV}, this controls
   * how many folds of cross validation will be used. The default is 3.
   *
   * @param folds
   ...
   *
   * @param holdOut
   *          the portion in (0, 1) to hold out
   */
   ...
   *
   * @param mode
   *          the calibration mode to use during training.
   */
  ...
  public void trainC(final ClassificationDataSet dataSet, final ExecutorService threadPool) {
    final double[] deci = new double[dataSet.getSampleSize()];// array of SVM
                                                              // decision values
    final boolean[] label = new boolean[deci.length];// array of booleans: is
                                                     // the example labeled +1?
    ...
    if (mode == CalibrationMode.CV) {
      ...
      final ClassificationDataSet train = new ClassificationDataSet(wholeSet.subList(0, splitMark),
          dataSet.getPredicting());
      final ClassificationDataSet test = new ClassificationDataSet(wholeSet.subList(splitMark, wholeSet.size()),
          dataSet.getPredicting());
