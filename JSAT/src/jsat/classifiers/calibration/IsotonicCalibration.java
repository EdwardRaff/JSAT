...
/**
 * Isotonic Calibration is non-parametric, and only assumes that the underlying distribution from negative to positive
 * examples is strictly a non-decreasing function. It will then attempt to model the distribution. This may over-fit for
 * small data sizes, and imposes an additional <i>O(log n)</i> search look up when performing classification, where n is
 * &lt;= the number of data points in the data set.
 * <br><br>
 * Isotonic Calibration inherently creates non-adjacent bins of varying size. Smooth transitions in output probability
 * are created by simple linear interpolation between bin values.
 * <br><br>
 * See: Niculescu-Mizil, A.,&amp;Caruana, R. (2005). <i>Predicting Good Probabilities with Supervised Learning</i>.
