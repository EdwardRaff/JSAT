...
/**
 * This base class provides shared functionality and variables used by two different training algorithms for
 * L<sub>1</sub> regularized linear models. Both squared and log loss are supported, making the results equivalent to
 ...
 * rescaling, but rescaling may destroy sparsity. If the data set is sparse and all values are zero or positive use the
 * default [0,1] rescaling to perform efficient rescaling that will not destroy sparsity. <br>
 * <br>
...
public abstract class StochasticSTLinearL1 implements Classifier, Regressor, Parameterized, SingleWeightVectorModel {
  ...
  /**
   * Sets the regularization constant used for learning. The regularization must be positive, and the learning rate is
   * proportional to the regularization value. This means regularizations very near zero will take a long time to
   * converge.
   *
  ...
  /**
   * Sets the loss function to use. This should not be altered after training unless the leaner is going to be trained
   * again.
   *
  ...
  /**
   * Sets whether or not scaling should be applied on th feature values of the training vectors. Scaling should be used
   * intelligently, scaling can destroy sparsity in the data set. If scaling is not applied, and a value is not in the
   * range [-1, 1], a {@link FailedToFitException} could occur.
  ...
  /**
   * Computes {@link #w}.{@link Vec#dot(jsat.linear.Vec) }<tt>x</tt> and does so by rescaling <tt>x</tt> as needed
   * automatically and efficiently, even if <tt>x</tt> is sparse.
   *
