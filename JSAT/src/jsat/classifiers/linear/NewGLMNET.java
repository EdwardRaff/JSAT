...
/**
 * NewGLMNET is a batch method for solving Elastic Net regularized Logistic Regression problems of the form <br>
 * 0.5 * (1-&alpha;) ||w||<sub>2</sub> + &alpha; * ||w||<sub>1</sub> + C
 *
 * <big>&sum;</big><sup>N</sup><sub>i=1</sub> &#8467; (w<sup>T</sup> x<sub>i</sub> + b, y<sub>i</sub>).
 * <br>
 * <br>
 * For &alpha; = 1, this becomes pure Lasso / L<sub>1</sub> regularized Logistic Regression. For &alpha; = 0, this
 * becomes pure Ridge/ L<sub>2</sub> regularized Logistic Regression, however better solvers such as
 * {@link LogisticRegressionDCD} are faster if using &alpha; = 0. <br>
 * The default behavior is to use &alpha;=1, and includes the bias term. Including the bias term can take longer to
 * train, but can also increase sparsity for some problems. <br>
 * <br>
 * This algorithm can be warm started from any classifier implementing the {@link SingleWeightVectorModel} interface.
 * <br>
 * <br>
 * See:
 * <ul>
 * <li>Yuan, G., Ho, C.-H.,&amp;Lin, C. (2012). <i>An improved GLMNET for L1-regularized logistic regression</i>.
 * Journal of Machine Learning Research, 13, 1999–2030. doi:10.1145/2020408.2020421</li>
 * <li>King, R., Morgan, B. J. T., Gimenez, O., Brooks, S. P., Crc, H.,&amp;Raton, B. (2010). <i>Regularization Paths
 * for Generalized Linear Models via Coordinate Descent</i>. Journal of Statistical Software, 36(1), 1–22.</li>
 * <li>Zou, H.,&amp;Hastie, T. (2005). <i>Regularization and variable selection via the elastic net</i>. Journal of the
 * Royal Statistical Society, Series B, 67(2), 301–320. doi:10.1111/j.1467-9868.2005.00503.x</li>
 * </ul>
 *
 * @author Edward Raff
 */
public class NewGLMNET implements WarmClassifier, Parameterized, SingleWeightVectorModel {
  ...
  /**
   * The default number of outer iterations of the training algorithm is {@value #DEFAULT_MAX_OUTER_ITER} .
   */
  ...
  /**
   * Using &alpha; = 1 corresponds to pure L<sub>1</sub> regularization, and &alpha; = 0 corresponds to pure
   * L<sub>2</sub> regularization. Any value in-between is then an Elastic Net regularization.
   *
   * @param alpha the value in [0, 1] for determining the regularization penalty's interpolation between pure
   * L<sub>2</sub> and L<sub>1</sub>
   * regularization.
   */
  ...
  /**
   * Sets the maximum number of training iterations for the algorithm, specifically the outer loop as mentioned in the
   * original paper. {@value #DEFAULT_MAX_OUTER_ITER} is the default value used, and may need to be increased for more
   * difficult problems.
   *
  ...
  /**
   * Sets the tolerance parameter for convergence. Smaller values will be more exact, but larger values will converge
   * faster. The default value is fairly exact at {@value #DEFAULT_EPS}, increasing it by an order of magnitude can
  ...
  /**
   * Controls whether or not an un-regularized bias term is added to the model. Using a bias term can increase runtime,
   * especially in sparse data sets, as each data point will have work done for the implicit bias term. However the bias
   * term is usually needed for small dimension problems, and can improve the sparsity of the solution for higher
   * dimensional problems.
   *
   * @param useBias {@code true} if an un-regularized bias term should be used or {@code false} to not use any bias
   * term.
   */
  ...
  /**
   *
   * @return {@code true} if an un-regularized bias term will be used or {@code false} to not use any bias term.
   */
  ...
  private void train(ClassificationDataSet dataSet, Vec w_init, double b_init, boolean useInit) {
    ...
    /**
     * Stores the value H<sup>k</sup><sub>j,j</sub> computer at the start of each iteration for the bias term
     */
    double H_bias = 0;
    /**
    ...
    /**
     * sum of all x_j values in the negative class. Used for ∇_j L in trick from LIBLINEAR eq(44)
     */
...
//        }
    ...
    for (int k = 0; k < maxOuterIters; k++)//For k = 1, 2, 3, . . .
    {
      ...
      /**
       * Sometimes we see the |z| be very small over and over, so we stop if we see it too many times in a row (which
       * means we really aren't making much progress)
       */
      int smallZInARow = 0;
...
//            System.out.println("Iter "+ k + " has New Obj Val: " + objVal);
  ...
  /**
   * When we perform a warm start, we want to train to the same point that we would have if we had not done a warm
   * start. But our stopping point is based on the initial relative error. To get around that, this method computes what
  ...
  /**
   * Guess the distribution to use for the trade off term term {@link #setAlpha(double) (double) &alpha;} in Elastic Net
   * regularization.
   *
  ...
  /**
   * Guess the distribution to use for the regularization term {@link #setC(double) C} in Logistic Regression.
   *
   * @param d the data set to get the guess for
