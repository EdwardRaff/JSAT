 ...
 * C <big>&Sigma;</big> log(1+exp(-y<sub>i</sub>w<sup>T</sup>x<sub>i</sub>)) + w
 * <sup>T</sup>w/2 <br>
 * <br>
 * See:<br>
...
public class LogisticRegressionDCD implements Classifier, Parameterized, SingleWeightVectorModel {
   ...
   *
   * @param C
   *          the regularization tradeoff term
   */
   ...
   *          the regularization tradeoff term
   * @param maxIterations
   *          the maximum number of iterations through the data set
   */
  ...
  /**
   * Sets the regularization trade-off term. larger values reduce the amount of
   * regularization, and smaller values increase the regularization.
   *
   * @param C
   *          the positive regularization tradeoff value
   */
  ...
  public void trainC(final ClassificationDataSet dataSet) {
    if (dataSet.getClassSize() != 2) {
      throw new FailedToFitException("Logistic Regression is a binary classifier, can can not handle "
          + dataSet.getClassSize() + " class problems");
    }
     ...
     * coefficients. But we expect a few alphas to quickly go to larger values.
     */
