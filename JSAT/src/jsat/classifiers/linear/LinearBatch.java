...
/**
 * LinearBatch learns either a classification or regression problem depending on the
 ...
 * ||w||<sub>2</sub><sup>2</sup>, and is trained using a batch optimization method. <br>
 * <br>
 * LinearBatch can be warm started from any model implementing the {@link SimpleWeightVectorModel} interface.
 * <br>
 * <br>
...
public class LinearBatch implements Classifier, Regressor, Parameterized, SimpleWeightVectorModel, WarmClassifier, WarmRegressor {
  ...
  /**
   * Sets the method of batch optimization that will be used. {@code null} is valid for this value, in which case the
   * implementation will attempt to select a reasonable optimizer automatically. <br>
   * <br>
   * NOTE: the current implementation requires the optimizer to work based off only the function value and its
   * derivative.
   *
  ...
  /**
   * Sets the convergence tolerance to user for training. Smaller values reach a more accuracy solution but may take
   * longer to complete.<br>
   * While zero is a valid tolerance value, it is not usually useful in practice. Values in [10<sup>-4</sup>,
   * 10<sup>-2</sup>] are usually more practical.
   *
  ...
  /**
   * Function for using the single weight vector loss functions related to {@link LossC} and {@link LossR}.
   */
