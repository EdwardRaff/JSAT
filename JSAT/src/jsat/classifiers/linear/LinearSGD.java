...
/**
 * LinearSGD learns either a classification or regression problem depending on the
 * {@link #setLoss(jsat.lossfunctions.LossFunc) loss function &#x2113;(w,x)} used. The solution attempts to minimize
 * <big>&sum;</big><sub>i</sub> &#x2113;(w,x<sub>i</sub>) + {@link #setLambda0(double) &lambda;<sub>0</sub>}/2
 * ||w||<sub>2</sub><sup>2</sup> + {@link #setLambda1(double) &lambda;<sub>1</sub>} ||w||<sub>1</sub>, and is trained by
 * Stochastic Gradient Descent. <br>
 * <br>
 * <br>
 * NOTE: To support L<sub>1</sub> regularization with sparse results and online learning at the same time, the
 * normalization of the regularization penalty by the number of data points is not present in the implementation at this
 * time. Setting {@link #setLambda1(double) &lambda;<sub>1</sub>} to the desired value divided by the number of unique
 * data points in the whole set will result in the correct regularization penalty being applied.
 *
 * See:
 * <ul>
 * <li>Tsuruoka, Y., Tsujii, J.,&amp;Ananiadou, S. (2009). <i>Stochastic gradient descent training for L1-regularized
 * log-linear models with cumulative penalty</i>. Proceedings of the Joint Conference of the 47th Annual Meeting of the
 * ACL and the 4th International Joint Conference on Natural Language Processing of the AFNLP, 1, 477.
 * doi:10.3115/1687878.1687946</li>
 * </ul>
 *
 * @author Edward Raff
 */
public class LinearSGD extends BaseUpdateableClassifier implements UpdateableRegressor, Parameterized, SimpleWeightVectorModel {
   ...
   *
   * @param gradientUpdater the method to use for updating the weight vectors from the gradient
   */
  ...
  /**
   * Sets the rate at which {@link #setEta(double) &eta;} is decayed at each update.
   *
  ...
  /**
   * Sets the initial learning rate &eta; to use. It should generally be in (0, 1), but any positive value is
   * acceptable.
   *
  ...
  /**
   * Sets the loss function used for the model. The loss function controls whether or not regression, binary
   * classification, or multi-class classification is supported.
   *
   * @param loss the loss function to use
   */
  ...
  /**
   * Guess the distribution to use for the regularization term {@link #setLambda0(double) &lambda;<sub>0</sub>} .
   *
  ...
  /**
   * Guess the distribution to use for the regularization term {@link #setLambda0(double) &lambda;<sub>1</sub>} .
   *
   * @param d the data set to get the guess for
