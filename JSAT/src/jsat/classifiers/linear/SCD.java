...
/**
 * Implementation of Stochastic Coordinate Descent for L1 regularized classification and regression. Which one is
 * supported is controlled by the {@link LossFunc} used. To be used the loss function must be twice differentiable with
 * a finite maximal second derivative value. {@link LogisticLoss} for classification and {@link SquaredLoss} for
 * regression are the ones used in the original paper.
 * <br><br>
 * Because the SCD needs column major data for efficient implementation, a second copy of data will be created in memory
 * during training.
 * <br><br>
 * See: Shalev-Shwartz, S.,&amp;Tewari, A. (2009). <i>Stochastic Methods for L<sub>1</sub>-regularized Loss
 * Minimization</i>. In 26th International Conference on Machine Learning (Vol. 12, pp. 929–936). Retrieved from
