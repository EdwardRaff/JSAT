...
/**
 * Provides a kernelized version of the {@link ALMA2} algorithm. It is important to note that the number of "support
 * vectors" ALMA may learn is unbounded.
 * <br>
 * The averaged output of all previous hyperplanes is supported at almost no overhead, and can be turned on by setting {@link #setAveraged(boolean)
 * }. This information is always collected, and the output can be changed once already learned.
 * <br><br>
...
public class ALMA2K extends BaseUpdateableClassifier implements BinaryScoreClassifier, Parameterized {
  ...
  /**
   * ALMA2K supports taking the averaged output of all previous hypothesis weighted by the number of successful uses of
   * the hypothesis during training. This effectively reduces the variance of the classifier. It has no impact on the
   * training / update phase, only the classification results are impacted.
   * <br><br>
   * Unlike most algorithms, this can be changed at any time without issue - even after the algorithm has been trained
   * the type of output (averaged or last) can be switched on the fly.
   *
   * @param averaged {@code true} to use the averaged out, {@code false} to only use the last hypothesis
   */
  ...
  /**
   * Computes the output of the summations of the input vector with the current weight vector as a recursive linear
   * combination of all previous support vectors and their associated score values. <br>
   * See Remark 5 in the original paper.
   *
   * @param x the input vector to compute the score value
   * @return the score for the input indicating which side of the hyperplane it is on
   */
