 ...
 * Once the maximum number of support vectors is reached, old vectors are dropped either in a uniform random fashion, or
 * weighted by the kernel function and the current coefficient for the vector. The later is the default method and is
 * referred to as BOGD++.<br>
 * <br>
 * See: Zhao, P., Wang, J., Wu, P., Jin, R.,&amp;Hoi, S. C. H. (2012). <i>Fast Bounded Online Gradient Descent
 * Algorithms for Scalable Kernel-Based Online Learning</i>. In Proceedings of the 29th International Conference on
 * Machine Learning (pp. 169–176). Learning; Machine Learning. Retrieved from
...
public class BOGD extends BaseUpdateableClassifier implements BinaryScoreClassifier, Parameterized {
  ...
  /**
   * Sets the regularization parameter used for training. The original paper suggests values in the range
   * 2<sup>x</sup>/T<sup>2</sup> for <i>x</i>
   * &isin; {-3, -2, -1, 0, 1, 2, 3} where <i>T</i> is the number of data instances that will be trained on
   *
  ...
  /**
   * Sets the learning rate to use for training. The original paper suggests values in the range 2<sup>x</sup> for
   * <i>x</i>
   * &isin; {-3, -2, -1, 0, 1, 2, 3}
   *
  ...
  /**
   * Sets the maximum allowed value for any support vector allowed. The original paper suggests values in the range
   * 2<sup>x</sup> for <i>x</i>
   * &isin; {0, 1, 2, 3, 4}
   *
  ...
  /**
   * Sets whether or not support vectors should be removed by uniform sampling or not. The default is {@code false},
   * which corresponds to BOGD++.
   *
   * @param uniformSampling {@code true} to use uniform sampling, {@code false} otherwise.
   */
  ...
  /**
   * Returns {@code true } is uniform sampling is in use, or {@code false} if the BOGD++ sampling procedure is in use
   *
   * @return {@code true } is uniform sampling is in use, or {@code false} if the BOGD++ sampling procedure is in use
   */
