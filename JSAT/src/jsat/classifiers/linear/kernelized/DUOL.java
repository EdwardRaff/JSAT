...
/**
 * Provides an implementation of Double Update Online Learning (DUOL) algorithm. It is a kernelized extension of
 * {@link PassiveAggressive} (PA-I) where one previously learned support vector may be updated upon each addition to the
 * support vector set. The SV set is unbounded in size. The objective function is not identical because of the dual
 * updates.
 * <br><br>
 * Using a larger {@link #setC(double) C} value for DUOL has theoretical improvements, as it increases the number of
 * "strong" dual updates. The default value is set to 10 as suggested in the paper. See:<br>
 * <ul>
 * <li>
 * Zhao, P., Hoi, S. C. H.,&amp;Jin, R. (2011). <i>Double Updating Online Learning</i>. Journal of Machine Learning
 * Research, 12, 1587–1615. Retrieved from
 * <a href="http://www.cse.msu.edu/~rongjin/publications/zhao11a.pdf"> here</a>
 * </li>
 * <li>
 * Zhao, P., Hoi, S. C. H.,&amp;Jin, R. (2009). <i>DUOL: A Double Updating Approach for Online Learning</i>. In Y.
 * Bengio, D. Schuurmans, J. Lafferty, C. K. I. Williams,&amp;A. Culotta (Eds.), Advances in Neural Information
 * Processing Systems 22 (pp. 2259–2267).
 * </li>
 * </ul>
 *
 * @author Edward Raff
...
public class DUOL extends BaseUpdateableClassifier implements BinaryScoreClassifier, Parameterized {
  ...
  /**
   * Signed weights for each support vector. Original paper uses the notation of gamma in the original paper, but this
   * is weird and easily confused with the sign values y_i. The sign class values can be obtained from the signed alphas
   * using {@link Math#signum(double) }
   */
  ...
  /**
   * Sets the aggressiveness parameter. Increasing the value of this parameter increases the aggressiveness of the
  ...
  /**
   * Sets the "conflict" parameter, which controls how often double updates are performed. Smaller (near zero) values
   * tend to produce more double updates, with values near 1 producing few double updates. The value must be in the
   * range [0, 1]
   *
