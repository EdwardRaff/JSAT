 ...
 * to many algorithms.
 * <br><br>
 * It is possible to obtain a more confident model and a slightly larger model by using several epochs. Instead of using
 * this class, the {@link CSKLRBatch batch version} of this algorithm should be used instead.
 * <br><br>
 * See paper: <br>
 * Zhang, L., Jin, R., Chen, C., Bu, J.,&amp;He, X. (2012). <i>Efficient Online Learning for Large-Scale Sparse Kernel
 * Logistic Regression</i>. Twenty-Sixth AAAI Conference on Artificial Intelligence (pp. 1219–1225). Retrieved from
...
public class CSKLR extends BaseUpdateableClassifier implements Parameterized {
  ...
  /**
   * Controls when updates are performed on the model. Depending on which update model is used, the acceptable values
   * and behaviors of {@link #setEta(double) } and {@link #setGamma(double) } may change.
   * <br><br>
   * The "auxiliary" modes perform updates with a probability of
   * <i>log(1+e<sup>-z</sup>) / a(z)</i>, where <i>a(z)</i> is the auxiliary function and <i>z</i> is the raw margin
   * value times the class label.
   */
  public enum UpdateMode {

    /**
     * NC stands for Non-Conservative, this mode will perform a model update on every new input, creating a very dense
     * model. The {@link #setEta(double) learning rate} may take on any positive value, and {@link #setGamma(double) }
     * is not used.
     */
    NC {
      @Override
    ...
    /**
     * Performs model updates probabilistically based on their distance from the margin of the classifier. In this case, {@link #setEta(double)
     * }
     * should be less than 2, or the model will become dense. {@link #setGamma(double) } is not used.
     */
    MARGIN {
      @Override
      protected double pt(double y, double score, double preScore, double eta, double gamma) {
     ...
     * <i>a(z) = log(&gamma; + e<sup>-z</sup>)</i>. {@link #setGamma(double) gamma} should be in the range (1, Infinity)
     * where larger values increase the sparsity of the model
     * <br><br>
     * This is the main auxiliary method used by the authors. They use values for &gamma; in the range of
     * <i>1+10<sup>&plusmn; x</sup></i> &forall; x &isin; {0, 1, 2, 3, 4}
     */
    AUXILIARY_1 {
      @Override
      protected double pt(double y, double score, double preScore, double eta, double gamma) {
    ...
    /**
     * Performs model updates based on a "auxiliary" function
     * <i>a(z) = log(1 + &gamma; e<sup>-z</sup>)</i>. {@link #setGamma(double) gamma} should be in the range (1,
     * Infinity) where larger values increase the sparsity of the model
     */
    AUXILIARY_2 {
      @Override
      protected double pt(double y, double score, double preScore, double eta, double gamma) {
    ...
    /**
     * Performs model updates based on a "auxiliary" function
     * <i>a(z) = max(loss(z), loss(&gamma;)</i>. {@link #setGamma(double) gamma} should be in the range (0, Infinity)
  ...
  /**
   * Sets the gamma value to use. This value, depending on which {@link UpdateMode} is used, controls the sparsity of
   * the model.
   *
