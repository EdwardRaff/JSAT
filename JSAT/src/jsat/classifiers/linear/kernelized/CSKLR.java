 ...
 * number of features, but that often increases the size of the model. <br>
 * <br>
 ...
 * variables. They behave different compared to many algorithms. <br>
 * <br>
 * It is possible to obtain a more confident model and a slightly larger model
 * by using several epochs. Instead of using this class, the {@link CSKLRBatch
 * batch version} of this algorithm should be used instead. <br>
 * <br>
 * See paper: <br>
 ...
 * <a href=
 * "http://www.aaai.org/ocs/index.php/AAAI/AAAI12/paper/viewPDFInterstitial/5003/5544">
 * here</a>
 *
 * @author Edward Raff
 */
public class CSKLR extends BaseUpdateableClassifier implements Parameterized {
  ...
  /**
   * Controls when updates are performed on the model. Depending on which update
   * model is used, the acceptable values and behaviors of
   * {@link #setEta(double) } and {@link #setGamma(double) } may change. <br>
   * <br>
   * The "auxiliary" modes perform updates with a probability of <i>log(1+e
   * <sup>-z</sup>) / a(z)</i>, where <i>a(z)</i> is the auxiliary function and
   * <i>z</i> is the raw margin value times the class label.
   */
  public enum UpdateMode {

    /**
     * NC stands for Non-Conservative, this mode will perform a model update on
     * every new input, creating a very dense model. The {@link #setEta(double)
     * learning rate} may take on any positive value, and
    ...
    NC {
      ...
      @Override
      protected double pt(final double y, final double score, final double preScore, final double eta,
          final double gamma) {
        return 1;
      }
    },
    /**
     * Performs model updates probabilistically based on their distance from the
     * margin of the classifier. In this case, {@link #setEta(double) } should
     * be less than 2, or the model will become dense.
     * {@link #setGamma(double) } is not used.
     */
    MARGIN {
      @Override
    ...
    /**
     * Performs model updates based on a "auxiliary" function <i>a(z) =
     * log(&gamma; + e<sup>-z</sup>)</i>. {@link #setGamma(double) gamma} should
     * be in the range (1, Infinity) where larger values increase the sparsity
     * of the model <br>
     * <br>
     * This is the main auxiliary method used by the authors. They use values
     * for &gamma; in the range of <i>1+10<sup>&plusmn; x</sup></i> &forall; x
     * &isin; {0, 1, 2, 3, 4}
     */
    AUXILIARY_1 {
      @Override
    ...
    /**
     * Performs model updates based on a "auxiliary" function <i>a(z) = log(1 +
     * &gamma; e<sup>-z</sup>)</i>. {@link #setGamma(double) gamma} should be in
     * the range (1, Infinity) where larger values increase the sparsity of the
     * model
     */
    AUXILIARY_2 {
      @Override
    ...
    /**
     * Performs model updates based on a "auxiliary" function <i>a(z) =
     * max(loss(z), loss(&gamma;)</i>. {@link #setGamma(double) gamma} should be
     * in the range (0, Infinity) where smaller values increase the sparsity of
     * the model
     */
    AUXILIARY_3 {
      @Override
    ...
    /**
     * Get the gradient value that should be applied based on the input variable
     * from the current model
     *
     * @param y
     *          the sign of the input point
     * @param score
     *          the logistic regression score for the input
     * @param preScore
     *          the raw margin before the final
     * @param gamma
     *          the gamma variable
   ...
   *
   * @param y
   *          the sign of the desired class (-1 or 1)
   * @param pre
   *          the raw coefficient score
  ...
  protected CSKLR(final CSKLR toClone) {
    if (toClone.alpha != null) {
      alpha = new DoubleList(toClone.alpha);
    }
    if (toClone.vecs != null) {
      vecs = new ArrayList<Vec>(toClone.vecs);
    }
    curNorm = toClone.curNorm;
    mode = toClone.mode;
    R = toClone.R;
    eta = toClone.eta;
    setKernel(toClone.k.clone());
    if (toClone.accelCache != null) {
      accelCache = new DoubleList(toClone.accelCache);
    }
    gamma = toClone.gamma;
    rand = new XORWOW();
    setEpochs(toClone.getEpochs());
  }
   ...
   * @param R
   *          the maximal norm of the surface
   * @param mode
   *          the mode to use
   */
   ...
   *
   * @param gamma
   *          the gamma parameter, which is at least always positive
   */
  ...
  /**
   * Sets what update mode should be used. The update mode controls the sparsity
   * of the mode, and the behavior of {@link #setGamma(double) }
   *
   * @param mode
   ...
   * maximal norm is too small (less than 5), it may be difficult to get strong
   * confidence results from the algorithm. <br>
