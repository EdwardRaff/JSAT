...
/**
 * Kernel SGD is the kernelized counterpart to {@link LinearSGD}, and learns nonlinear functions via the kernel trick.
 * The implementation is built upon {@link KernelPoint} and {@link KernelPoints} to support budgeted learning. Following
 * the LinearSGD implementation, whether or not this algorithm supports regression, binary-classification, or
 * multi-class classification is controlled by the {@link #setLoss(jsat.lossfunctions.LossFunc) loss function} used.
 * <br>
 * <br>
 * The learning rate decay is not configurable for this implementation, and is decayed at a rate of null {@link #setEta(double) &eta;} /
 * ({@link #setLambda(double) &lambda;} * (t + 2 / &lambda;)) , where {@code t} is the time step.
 *
 * @author Edward Raff
 */
public class KernelSGD implements UpdateableClassifier, UpdateableRegressor, Parameterized {
  ...
  /**
   * Sets the maximum budget size, or number of support vectors, to allow during training. Increasing the budget size
   * will increase the accuracy of the model, but will also increase the computational cost
   *
  ...
  /**
   * Sets the base learning rate to start from. Because of the decay rate in use a good value for &eta; is 1.0.
   *
  ...
  /**
   * Guess the distribution to use for the regularization term {@link #setLambda(double) &lambda;} .
   *
