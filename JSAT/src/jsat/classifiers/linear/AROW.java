...
/**
 * An implementation of Adaptive Regularization of Weight Vectors (AROW), which uses second order information to learn a
 * large margin binary classifier. As such, updates can occur on correctly classified instances if they are not far
 ...
 * <br><br>
 * See: Crammer, K., Kulesza, A.,&amp;Dredze, M. (2013). <i>Adaptive regularization of weight vectors</i>. Machine
 * Learning, 91(2), 155–187. doi:10.1007/s10994-013-5327-x
 *
 * @author Edward Raff
 */
public class AROW extends BaseUpdateableClassifier implements BinaryScoreClassifier, Parameterized, SingleWeightVectorModel {
  ...
  /**
   * Using the full covariance matrix requires <i>O(d<sup>2</sup>)</i> work on mistakes, where <i>d</i> is the dimension
  ...
  /**
   * Sets the r parameter of AROW, which controls the regularization. Larger values reduce the change in the model on
   * each update.
   *
  ...
  /**
   * Guess the distribution to use for the regularization term {@link #setR(double) r} .
   *
