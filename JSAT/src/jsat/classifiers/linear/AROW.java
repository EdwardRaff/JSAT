...
public class AROW extends BaseUpdateableClassifier
   ...
   * @param diagonalOnly
   *          whether or not to use only the diagonal of the covariance
   * @see #setR(double)
   ...
   *
   * @param r
   *          the regularization parameter in (0, Inf)
   */
  ...
  public void update(final DataPoint dataPoint, final int targetClass) {
    ...
    if (diagonalOnly) {
      /*
       * diagonal is pairwise products as well: S += S x x' S S x == x' S b/c
       * symmetry S += Sx Sx so just square the values and then add
       */
      for (final IndexValue iv : x_t) {
