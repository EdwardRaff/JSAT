...
/**
 * This is a Stochastic implementation of Multinomial Logistic Regression. It supports regularization from several
 * different priors, and performs prior updates in a lazy fashion to avoid destroying the sparsity of training inputs.
 * <br>
 * Algorithm is based on the technical report:<br>
 * Carpenter, B. (2008). <i>Lazy Sparse Stochastic Gradient Descent for Regularized Mutlinomial Logistic Regression</i>.
 * Retrieved from http://lingpipe-blog.com/lingpipe-white-papers/
 *
 * @author Edward Raff
 */
public class StochasticMultinomialLogisticRegression implements Classifier, Parameterized, SimpleWeightVectorModel {
  ...
  /**
   * Creates a new Stochastic Multinomial Logistic Regression that uses a {@link Prior#GAUSSIAN} prior with a
   * regularization scale of 1e-6.
   *
   * @param initialLearningRate the initial learning rate to use
   * @param epochs the maximum number of training epochs to go through
   */
  ...
  /**
   * Creates a new Stochastic Multinomial Logistic Regression that uses a {@link Prior#GAUSSIAN} prior with a
   * regularization scale of 1e-6. It will do at most 50 epochs with a learning rate of 0.1
   */
  ...
  public enum Prior {
    ...
    /**
     * This is the Elastic Net prior, and it uses the extra {@link #setAlpha(double) alpha} parameter. This prior is a
     * mix of both {@link #LAPLACE} and {@link #GAUSSIAN}. Alpha should be in the range [0,1]. Alpha weight will be
     * applied to the Laplace prior, and (1-alpha) weight will be applied to the Gaussian prior. The extreme values of
     * this collapse into the Laplace and Gaussian priors.
     */
    ELASTIC {
    ...
    /**
     * This is a prior from the Cauchy (student-t) distribution, and it uses the extra {@link #setAlpha(double) alpha}
     * parameter. Alpha should be in the range (0, Infty).
     */
    CAUCHY {
    ...
    /**
     * This is the Uniform prior. The uniform prior is equivalent to no regularization.
     */
    UNIFORM {
      @Override
      protected double gradientError(double b_i, double s_i) {
  ...
  /**
   * Sets whether or not to learn the bias term for a model. If no bias term is in use, the model learned must pass
   * through the origin of the world. The use of the bias term is very important for low dimensional problems, but less
   * so for many higher dimensional problems.
   *
   * @param useBias {@code true} if the bias term should be used, {@code false} otherwise
   */
  ...
  /**
   * Sets the extra parameter alpha. This is used for some priors that take an extra parameter. This is
   * {@link Prior#CAUCHY} and {@link Prior#ELASTIC}. If these two priors are not in use, the value is ignored.
   *
  ...
  /**
   * Sets whether or not the clip changes in coefficient values caused by regularization so that they can not make the
   * coefficients go from positive to negative or negative to positive. If clipping is on, the value will go to zero
   * instead. If off, the value will be allowed to change signs. <br>
  ...
  /**
   * Sets the initial learning rate to use for the first epoch. The learning rate will decay according to the
   * {@link #setLearningRateDecay(jsat.math.decayrates.DecayRate) decay rate} in use.
   *
   * @param initialLearningRate the initial learning rate to use
   */
  ...
  /**
   * Sets the coefficient applied to the regularization penalty at each update. This is usual set to a small value less
   * than 1. If set to zero, it effectively turns off the use of regularization.
   *
  ...
  /**
   * Sets whether or not to perform implicit standardization of the feature values when performing regularization by the
   * prior. If set on, the input data will be adjusted to have zero mean and unit variance. This is done without
   * destroying sparsity. If there is not regularization, this parameter has no impact.
   *
   * @param standardized {@code true} if the input will be standardized, {@code false} if ti will be left as is.
   */
  ...
  /**
   * Sets the amount of data points used to form each gradient update. Increasing the batch size can help convergence.
   * By default, a mini batch size of 1 is used.
   *
   * @param miniBatchSize the number of data points used to perform each update
   */
  ...
  /**
   * Returns the raw coefficient vector used without the bias term. For a multinomial Logistic model, there are C-1
   * coefficient vectors. C is the number of output classes. Altering the returned vector will alter the model. The i'th
   * index of the vector corresponds to the weight therm for the i'th index in an input.
   *
