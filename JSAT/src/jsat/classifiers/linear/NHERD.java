 ...
 * of the covariance are available. <br>
 ...
 * Pre-proceeding of NIPS (pp. 451–459). Retrieved from <a href=
 * "http://webee.technion.ac.il/Sites/People/koby/publications/gaussian_mob_nips10.pdf">
 * here</a>
 *
 * @author Edward Raff
 */
public class NHERD extends BaseUpdateableClassifier
  ...
  public static enum CovMode {
    ...
    DROP, /**
           ...
           */
    PROJECT, /**
              * Creates the diagonal by solving the derivative with respect to
              * the specific objective function of NHERD
              */
    EXACT
  }
   ...
   * @param covMode
   *          how to form the covariance matrix
   * @see #setC(double)
   ...
   * covariance matrix, rank-1 updates mean updates to the model take <i>O(d
   * <sup>2</sup>)</i> time, where <i>d</i> is the dimension of the input.
   * Runtime can be reduced by using only the diagonal of the matrix to perform
   * updates in <i>O(s)</i> time, where <i>s &le; d</i> is the number of
  ...
  public void update(final DataPoint dataPoint, final int targetClass) {
    ...
    case DROP:
      ...
      break;
    case PROJECT:
      for (final IndexValue iv : x_t)// only the nonzero values in x_t will
