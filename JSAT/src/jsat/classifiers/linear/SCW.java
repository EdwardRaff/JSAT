...
/**
 * Provides an Implementation of Confidence-Weighted (CW) learning and Soft Confidence-Weighted (SCW), both of which are
 * binary linear classifiers inspired by {@link PassiveAggressive}. The SCW mode handles noisy and nonlinearly separable
 * datasets better. <br>
 * NOTE: Unlike other online second order methods, when using the full covariance matrix, all new inputs cost
 * O(d<sup>2</sup>) time to process, even if update is needed.
 * <br>
 ...
 * <ul>
 * <li>
 * Crammer, K., Fern, M.,&amp;Pereira, O. (2008). <i>Exact Convex Confidence-Weighted Learning</i>. In Advances in
 * Neural Information Processing Systems 22 (pp. 345–352). Retrieved from
 * <a href="http://citeseerx.ist.psu.edu/viewdoc/summary?doi=10.1.1.169.3364">
 * here</a>
 * </li>
 * <li>
 * Wang, J., Zhao, P.,&amp;Hoi, S. C. H. (2012). <i>Exact Soft Confidence-Weighted Learning</i>. ICML. Learning.
 * Retrieved from
...
public class SCW extends BaseUpdateableClassifier implements BinaryScoreClassifier, Parameterized, SingleWeightVectorModel {
  ...
  private Vec sigmaV;
  /**
   * Temp vector used to store Sigma * x_t. Make sure the vector is zeroed out before returning from update
   */
  ...
  /**
   * More than one escape point, makes sure to zero out {@link #Sigma_xt} using the input incase of sparseness
   *
   * @param x_t
   */
   ...
   * @param diagonalOnly whether or not to use only the diagonal of the covariance matrix
   * @see #setEta(double)
  ...
  /**
   * SCW uses a probabilistic version of the margin and attempts to make a correction so that the confidence with
   * correct label would be of a certain threshold, which is set by eta. So the threshold must be in [0.5, 1.0]. Values
   * in the range [0.8, 0.9] often work well on a wide range of problems
   *
  ...
  /**
   * Set the aggressiveness parameter. Increasing the value of this parameter increases the aggressiveness of the
   * algorithm. It must be a positive value. This parameter essentially performs a type of regularization on the updates
   * <br>
   * The aggressiveness parameter is only used by {@link Mode#SCWI} and {@link Mode#SCWII}
   *
  ...
  /**
   * Using the full covariance matrix requires <i>O(d<sup>2</sup>)</i> work on updates, where <i>d</i> is the dimension
   * of the data. Runtime can be reduced by using only the diagonal of the matrix to perform updates in <i>O(s)</i>
   * time, where <i>s &le; d</i> is the number of non-zero values in the input
   *
  ...
  /**
   * Guess the distribution to use for the regularization term {@link #setC(double) C} .
   *
  ...
  /**
   * Guess the distribution to use for the regularization term {@link #setEta(double) &eta; } .
   *
   * @param d the data set to get the guess for
   * @return the guess for the C parameter
   */
