...
/**
 * Decision Directed Acyclic Graph (DDAG) classifier. DDAG extends binary
 * decision classifiers into multi-class decision classifiers. Unlike
...
public class DDAG extends OneVSOne {
  ...
  /**
   * Creates a new DDAG classifier to extend a binary classifier to handle
  ...
  public CategoricalResults classify(final DataPoint data) {
    ...
    // Use a priority que so that we always pick the two lowest value class
    // labels, makes indexing into the oneVsOne array simple
    ...
    int c1, c2;
    // We will now loop through and repeatedly pick two combinations, and
    // eliminate the loser, until there is one winer
