...
/**
 * Locally Weighted Learner (LW) is the combined generalized implementation of Locally Weighted Regression (LWR) and
 * Locally Weighted Naive Bayes (LWNB). The concept is simple, prediction begins in a normal NN style. Instead of
 * returning the prediction result as the average / majority of the found neighbors, a classifier is trained to
 * represent the local area that is represented by the neighbors. The prediction result is then based on this locally
 * trained model. <br>
 * Because NN search is already slow, and increasing the search size increases the search time, it is recommended to use
 * moderate sized values of <i>k</i>
 * with simple models such as {@link NaiveBayesUpdateable NaiveBayes} and
 * {@link MultipleLinearRegression LinearRegression}. <br>
 * If the learning algorithm used does not support weighted data points, it will be as if the
 * {@link UniformKF uniform kernel fucntion} was used, regardless of whatever kernel function was set. <br>
 * <br>See:<br>
 * <ul>
 * <li>Atkeson, C., Moore, A.,&amp;Schaal, S. (1997).
 * <a href="http://www.springerlink.com/index/G8280541763Q0223.pdf">Locally Weighted Learning</a>. Artificial
 * intelligence review, 11–73.</li>
 * <li>Frank, E., Hall, M.,&amp;Pfahringer, B. (2003).
 * <a href="http://dl.acm.org/citation.cfm?id=2100614">Locally Weighted Naive Bayes</a>. Proceedings of the Conference
 * on Uncertainty in Artificial Intelligence (pp. 249–256). Morgan Kaufmann.</li>
 * </ul>
 *
 * @author Edward Raff
 */
