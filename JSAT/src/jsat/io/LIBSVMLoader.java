...
/**
 * Loads a LIBSVM data file into a {@link DataSet}. LIVSM files do not indicate whether or not the target variable is
 * supposed to be numerical or categorical, so two different loading methods are provided. For a LIBSVM file to be
 * loaded correctly, it must match the LIBSVM spec without extensions.
 * <br><br>
 * Each line should begin with a numeric value. This is either a regression target or a class label. <br>
 * Then, for each non zero value in the data set, a space should precede an integer value index starting from 1 followed
 * by a colon ":" followed by a numeric feature value. <br> The single space at the beginning should be the only space.
 * There should be no double spaces in the file.
 * <br><br>
 * LIBSVM files do not explicitly specify the length of data vectors. This can be problematic if loading a testing and
 * training data set, if the data sets do not include the same highest index as a non-zero value, the data sets will
 * have incompatible vector lengths. To resolve this issue, use the loading methods that include the optional
 * {@code vectorLength} parameter to specify the length before hand.
 *
 * @author Edward Raff
 */
public class LIBSVMLoader {
   ...
   *
   * @param file the file to load
   * @param sparseRatio the fraction of non zero values to qualify a data point as sparse
   * @return a regression data set
   * @throws FileNotFoundException if the file was not found
   * @throws IOException if an error occurred reading the input stream
   */
   ...
   * @param file the file to load
   * @param sparseRatio the fraction of non zero values to qualify a data point as sparse
   * @param vectorLength the pre-determined length of each vector. If given a negative value, the largest non-zero index
   * observed in the data will be used as the length.
   * @return a regression data set
   * @throws FileNotFoundException if the file was not found
   * @throws IOException if an error occurred reading the input stream
   */
   ...
   * @param sparseRatio
   * @param vectorLength
   * @param classification {@code true} to treat as classification, {@code false} to treat as regression
   * @return
   * @throws IOException
   */
   ...
   * @param os the output stream to write to. The stream will not be closed or flushed by this method
   */
