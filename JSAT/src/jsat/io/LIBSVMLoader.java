...
public class LIBSVMLoader {
  ...
  /**
   * Loads a new classification data set from a LIBSVM file, assuming the label
   * is a nominal target value
   *
   * @param file
   *          the file to load
   ...
   *          the file to load
   * @param sparseRatio
   *          the fraction of non zero values to qualify a data point as sparse
   * @return a classification data set
   * @throws FileNotFoundException
   *           if the file was not found
   * @throws IOException
   *           if an error occurred reading the input stream
   */
  public static ClassificationDataSet loadC(final File file, final double sparseRatio)
   ...
   *          the fraction of non zero values to qualify a data point as sparse
   * @param vectorLength
   *          the pre-determined length of each vector. If given a negative
   *          value, the largest non-zero index observed in the data will be
   *          used as the length.
   * @return a classification data set
   * @throws FileNotFoundException
   *           if the file was not found
   ...
   *           if an error occurred reading the input stream
   */
  public static ClassificationDataSet loadC(final File file, final double sparseRatio, final int vectorLength)
      throws FileNotFoundException, IOException {
   ...
   *
   * @param isr
   *          the input stream for the file to load
   * @param sparseRatio
   *          the fraction of non zero values to qualify a data point as sparse
   * @return a classification data set
   * @throws IOException
   ...
   *           if an error occurred reading the input stream
   */
  public static ClassificationDataSet loadC(final Reader reader, final double sparseRatio, final int vectorLength)
      throws IOException {
   ...
   *          {@code true} to treat as classification, {@code false} to treat as
   *          regression
   * @return
   * @throws IOException
   */
  private static DataSet loadG(final Reader reader, final double sparseRatio, final int vectorLength,
      final boolean classification) throws IOException {
    ...
    while (true) {
      ...
      while (charBuffer.length() - position <= 1)// make sure we have chars to
                                                 // handle
      {
      ...
      if (charBuffer.length() - position == 0) // EOF, no more chars
      {
        ...
        } else if (state == STATE.WHITESPACE_AFTER_LABEL) // last line was
                                                          // empty, but we have
                                                          // already eaten the
                                                          // label
        {
          sparceVecs.add(new SparseVector(maxLen, 0));
        } else if (state == STATE.FEATURE_VALUE || state == STATE.WHITESPACE_AFTER_FEATURE) // line
                                                                                            // ended
                                                                                            // after
                                                                                            // a
                                                                                            // value
        ...
        // we may have ended on a line, and have a sparse vec to add before
        // returning
        break;
      }
      ...
      case WHITESPACE_AFTER_LABEL:
        if (Character.isDigit(ch)) // move to next state
        {
          ...
          if (ch == '\n' || ch == '\r') {
            tempVec.zeroOut();
            sparceVecs.add(new SparseVector(maxLen, 0));/// no features again,
                                                        /// add zero vec
            state = STATE.NEWLINE;
          } else {
            // normal whie space
    ...
    if (classification) {
      ...
      // Give categories a unique ordering to avoid loading issues based on the
      // order categories are presented
   ...
   *           if an error occurred reading the input stream
   */
  public static RegressionDataSet loadR(final File file, final double sparseRatio)
      throws FileNotFoundException, IOException {
   ...
   *           if an error occurred reading the input stream
   */
  public static RegressionDataSet loadR(final File file, final double sparseRatio, final int vectorLength)
      throws FileNotFoundException, IOException {
   ...
   *
   * @param reader
   *          the reader for the file to load
   * @param sparseRatio
   *          the fraction of non zero values to qualify a data point as sparse
   * @param vectorLength
   *          the pre-determined length of each vector. If given a negative
   ...
   * @throws IOException
   */
  public static RegressionDataSet loadR(final Reader reader, final double sparseRatio, final int vectorLength)
      throws IOException {
   ...
   *
   * @param data
   *          the data set to write to a file
   * @param os
  ...
  public static void write(final ClassificationDataSet data, final OutputStream os) {
    ...
    for (int i = 0; i < data.getSampleSize(); i++) {
      ...
      for (final IndexValue iv : vals) {
        writer.write(iv.getIndex() + 1 + ":" + iv.getValue() + " ");// +1 b/c 1
                                                                    // based
                                                                    // indexing
      }
      writer.write("\n");
